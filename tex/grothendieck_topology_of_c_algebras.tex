...
Let	$\varphi: A \hookto \mathbf{LC}\left(\widetilde A\right)$ be a {hereditary full} morphism. 	For any hereditary subalgebra $B \in \mathbf{Hered}/A$ denote by
\be
\overline \varphi \left( A\right)  \bydef C^*\text{-norm completion of } \varphi\left(A \right)^*\widetilde{A} \varphi\left(A \right) 
\ee
...
\ee
Suppose that $\sX$ is a closed subset of $\sY$.
One can define a locale $\mathfrak{Closed}\left( \sX\right)$ such that its elements are closed subsets of $\sX$ and having following operations
...
\begin{split}
	H^p\left(X, \mathscr F \right)\bydef \bydef R^r\Ga\left(X, \F \right)\begin{cases}
