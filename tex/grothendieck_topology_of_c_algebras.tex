...
Let us define a sheaf on $\mathscr S$ on $\sX$ 
\bean
\mathscr S\left(\sU \right)  \bydef  \text{the family of all open subsets of }\sU;\\
\rho_{\sU \sV}: \mathscr S\left(\sU \right) \to \mathscr S\left(\sV \right), \quad \mathcal W \mapsto \mathcal W \cap \sV.
\eean
Note that if $ \mathcal W= \supp C$ then $ \mathcal W\cap \sV =\supp C\cap ~_\sV A _\sV$. 
Let  $\mathrm{Sp\acute{e}}\left(\mathscr S  \right)$ be the	\'espace etal\'e  (cf. Exercise \ref{sheaf_etale_exer}) of the sheaf $\mathscr S$. The empty set $\emptyset$ yields a global  section $s^\emptyset \in \mathscr S\left(\sX \right)$, this section gives an open subset $\mathrm{Sp\acute{e}}\left(\mathscr S  \right)_\emptyset \subset \mathrm{Sp\acute{e}}\left(\mathscr S  \right)$. Denote by 
\be
\mathfrak X\left(\sX \right)  \bydef  \mathrm{Sp\acute{e}}\left(\mathscr S  \right)\setminus\mathrm{Sp\acute{e}}\left(\mathscr S  \right)_\emptyset.
\ee
...
$$
P'_A:\mathrm{Sp\acute{e}}\left(\mathscr P^A  \right) \to \mathrm{Sp\acute{e}}\left(\mathscr S  \right).
$$
\begin{exercise}
...
\begin{remark}\label{local_prod_rem}
	From the Theorem \ref{local_prod_thm} and !!! there are following homomorphisms
	\bean
	\check{H}^\bullet!!!
	\eean 
\end{remark}
...
for all $x \in \sX$. A generated by $a$ hereditary subalgebra $A^a$ (cf. Definition \ref{hered_generated_defn}) is commutative, so it defines a global section $s^a\in \mathscr{P}^0 \left(\sX\right)$, such that $s^a_x \in \mathrm{Sp\acute{e}}\left(\mathscr P^\dl  \right)'$ for all $x \in \sX$.
...
\ee
	...
	trivial bundle $\pi_\dl: \F^\dl\to \sX$ with a fibre $\K(\H)$ and a structure group $\Aut \left( \K(\H)\right)$ such that $A$ is $C_0\left( \sX\right)$ -isomorphic to the space of sections $\Ga_0\left(\sX, \F \right)$. 
	...
	$$
	is a presheaf of $*$-algebras, denote by $\mathscr F^\delta$ the associated sheaf (cf. Definitions \ref{associated_sheaf_defn} and \ref{sheaf_prdf}).
	...
	of sheaves. 
\begin{lemma}
A sheaf $\mathscr G^\delta$ associated with a presheaf
$$
\sU \mapsto \left\{\left.s \in \mathscr F^\delta\left(\sU \right)\sqcup\{0\} \right| \forall x \in \sU\quad  \pi_x\left(s \right) \text { is a rank-one projector} \right\}. 
$$

\end{lemma}	
	
	Consider a sheaf $\mathscr G^\delta$ associated with a presheaf
	$$
	\sU \mapsto \left\{\left.s \in \mathscr F^\delta\left(\sU \right)\sqcup\{0\} \right| \forall x \in \sU\quad  \pi_x\left(s \right) \text { is a rank-one projector} \right\}. 
	$$
...
If $\sU$ is such that there there is a 	
