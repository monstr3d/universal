...
\begin{lemma}\label{locale_open_lem}
	Any hereditary full injective *-homomorphism $f: A \hookto \widetilde A$ yields an open map of locales  (cf. Definition \ref{open_locale_mor_defn}). Moreover the map is a surjection (cf. Definition \ref{loc_emp_sup_defn})
\end{lemma}
\begin{proof}
		Let us define 
...
\check{H}^\bullet\left(A \right) \xrightarrow{\check{H}^\bullet\check{H}^\bullet} \check{H}^\bullet\left(A'' \right),\\
...
\eean
where $\smile$ is a cup product (cf. the equation \eqref{cup_prod_eqn} and the Remark \ref{ring_homo_rem}). From \eqref{cup_eq_eqn} it follows that the above pairing yields a following homomorphism
\be
...
\ee
of $\check{H}^\bullet\left(A \right)$-$\check{H}^\bullet\left(A \right)$-bimodules.
...
\end{proof}
\begin{remark}
From the  Theorem \ref{grot_type_I_thm} and Lemmas \ref{foli_t1_lem}, \ref{blowing_foli_lem} it follows that there is the Hereditary $C^*_r\left(M,\sF, \sigma\right)$-space $\mathfrak{Space}\left(C^*_r\left(M,\sF, \sigma\right) \right)$ (cf. Definition \ref{gelfand_space_defn}) and natural surjective local homeomorphism
\bean
