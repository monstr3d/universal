...
If $A = CT\left(\sX, \dl \right)$ (cf. Notation \ref{ct_not})  then we define a presheaf $\mathscr{P}'^\dl $ on $\sX$ such that if $\sU$ and $\sV$ are open subsets of $\sX$ then
\bean
...
\begin{proof}
	...
	\eean 
	However from $	B^a_x \in \mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B$ it turns out that there is a representative $B'$ of $B^a_x$ such that $B' \in A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}$, so $B^a_x \in \mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_{ A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}}$. From this contradiction it follows that
	$$
	B = A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}.
	$$
\end{proof} 
...
\be
\begin{split}
	j : \sX \to \mathrm{Sp\acute{e}}\left(\mathscr P^0l  \right)';\\
	x \mapsto s^a_x
\end{split}
\ee 
...
\eean
	where  $X^r$, $\check{X}^r$ are Abelian group and the notations \eqref{a_const_shef_coh_eqn}, \eqref{const_shef_coh_eqn} are  used.
\end{theorem}
\begin{proof}
From the equation \eqref{hom_f_eqn} it follows that for all $r> 0$ there are following homomorphisms:
\bean
H^r\left( j\right) 	: H^r\left(CT\left( \sX,0\right), F \right)\to H^r\left(\sX, F \right),\\
H^r\left(p^0\right) 	: H^r\left(\sX, F \right)\to H^r\left(CT\left( \sX,0\right), F\right).
\eean 
...
$$
If $X^r \bydef \ker H^r\left( j\right)$ then from the above equations it follows that $	H^r\left(CT\left( \sX,0\right), F \right)	\cong H^r\left(\sX, F  \right)	\oplus X^r$. Similarly an application of the equation \eqref{cech_hom_eqn}  yields the following
$$
	\check{H}^r\left(CT\left( \sX,0\right), F \right)	\cong \check{H}^r\left(\sX, F  \right)	\oplus \check{X}^r.
$$

\end{proof}
If  $A = CT\left( \sX,0\right)$ is a  stable separable  continuous trace $C^*$-algebra with spectrum $\sX$ then from the Proposition \ref{ctr_bundle_prop}
...
\end{split}
\eean
One can prove that the map $\psi$ is continuous. From the equations \eqref{hom_f_eqn} and \eqref{cech_hom_eqn} for all $r \ge 0$ and an Abelian group $F$ there are homomorphisms
\bean
...
\eean
\begin{empt}
 If $\sU$ is an open subset then there is an equivalence relation $\sim_\sU$ on $\Ga_0\left( \sX, \sF_\dl\right)$ such that 
$$
...
\ee
From the above equation it follows that there is a continuous map
$$
\psi : \mathrm{Sp\acute{e}}\left(\mathscr G^{\delta + \rho} \right) \to \mathrm{Sp\acute{e}}\left(\mathscr G^\delta \right) \times \mathrm{Sp\acute{e}}\left(\mathscr G^\rho \right),
$$
and taking into account \eqref{cech_hom_eqn}, \eqref{kunnet_eqn} one has a homomorphism.
\be\label{hh_p_eqn}
\bigoplus_{p + q = m}\check{H}^q\left(\mathrm{Sp\acute{e}}\left(\mathscr G^\delta  \right),\Z\right) \otimes_\Z \check{H}^p\left(\mathrm{Sp\acute{e}}\left(\mathscr G^\rho  \right),\Z\right)\to \check{H}^m\left(\mathrm{Sp\acute{e}}\left(\mathscr G^{\delta+\rho}  \right),\Z\right) 
\ee
where $\times$ means a product of topological space.
\end{empt}

\begin{theorem} If $CT\left( \sX, \delta\right)$ is given by the Notation \ref{ct_not} $C^*$-algebra then for all $m \ge 0$ one has a following homomorphism
\bea
\bigoplus_{p + q = m}\check{H}^q\left(CT\left( \sX, \delta\right) , \Z \right) \otimes_\Z \check{H}^q\left(CT\left( \sX, \rho\right) , \Z \right) \to \check{H}^m\left(CT\left( \sX, {\delta + \rho}\right) , \Z \right).
\eea
...
$$
\forall x \in \sU\quad  \pi_x\left(s \right)\in \K\quad \text {is a rank-one projector}
$$
...
Secondly we define a map $\psi: \mathrm{Sp\acute{e}}\left(\mathscr P^\delta  \right)'\to \mathrm{Sp\acute{e}}\left(\mathscr G^\dl \right)$. Any $z\mathrm{Sp\acute{e}}\left(\mathscr P^\delta  \right)'$ can be represented by a commutative $C^*$-subalgebra $C \subset CT\left( \sX, \delta\right)$ such that $\pi_{p_\dl\left(z \right) }\left(C  \right) \neq \{0\}$. There is a positive element $a \in C_+$ such that $\pi_{p_\dl\left(z \right) }\left(C  \right) \neq 0$. We leave to the reader a prove of following statement: there is $f \in C_0\left(\sX\right)_+$ and an open neighborhood of $p_\dl\left(z \right)$ such that for all $x\in \sX$ an operator $\pi_x\left(fa \right)$ is a rank-one projector. The element $fa$ represents a section $r \in \mathscr G^\delta\left( \sU\right)$.  The map $\psi$ we define by the following way
$$
z\mapsto r_x.
$$
...
\end{itemize}
So there is a homeomorphism $\mathrm{Sp\acute{e}}\left(\mathscr P^\delta  \right)'\to \mathrm{Sp\acute{e}}\left(\mathscr G^\dl \right)$. This theorem follows from the Corollary \ref{ctr_top_cor} and the equation \eqref{hh_p_eqn}.

\end{proof}
\section{Resume}
\paragraph*{}
The Grothendieck topology can give integer theories od cohomology. For example there are 
...
\eean	
	...
	$$
K_*\left( CT\left(\sX, 0 \right)\right)\cong K^*\left( \sX\right),	
	$$
	\item The equation 
	$$
	\bigoplus_{p + q = m}\check{H}^q\left(CT\left( \sX, \delta\right) , \Z \right) \otimes_\Z \check{H}^q\left(CT\left( \sX, \rho\right) , \Z \right) \to \check{H}^m\left(CT\left( \sX, {\delta + \rho}\right) , \Z \right)
...
$$
K_*\left( CT\left(\sX, \dl \right)\right)\otimes_\Z K_*\left( CT\left(\sX, \rho \right)\right) \to K_*\left( CT\left(\sX, \dl + \rho\right)\right).	
	$$
\end{itemize}
...
\begin{notation}\label{const_shef_coh_not}
	If  $A$ is an Abelian group and $A_X$ is a {constant presheaf} (cf. Definition \ref{constant_presheaf_defn}) then we use the following notation
	\be\label{const_shef_coh_eqn}
	\forall n\ge 0\quad H^n\left(X, A \right) \bydef H^n\left(X, \mathfrak{Ass} \left( A_X\right)  \right). 
	\ee
...
\end{proposition}
\begin{notation}\label{ct_not}
	If $\sX$ is a locally compact Hausdorff space then we denote by $CT\left(\sX, \dl \right)$ the stable
	continuous-trace algebra  with DixmierÐDouady class $\delta \in \check{H}^3\left(\sX \right)$. 
\end{notation}
From the Proposition \ref{ctr_d_prop} it follows that
