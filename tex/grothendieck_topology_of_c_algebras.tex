...
there are natural equivalences of categories of presheaves and sheaves
	...
	\mathbf{PreSh}\left(\mathbf{T}_{CT\left(\sX, \dl \right)} \right) \cong 	\mathbf{PreSh}\left(\mathbf{T}_{\mathrm{Sp\acute{e}}\left(\mathscr P^{\dl} \right)'} \right),\\
		...
		\end{split}
	\eean
	where the notation of the Appendix \ref{appendix_etale_sec} is used. For any group $F$ and any $r > 0$ one has  natural isomorphisms
\bean
H^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^\delta  \right)'\right)\cong \check{H}^r\left(CT\left( \sX,\dl\right), F \right)
\eean
\be\label{chech_hom_eqn}
...
\begin{lemma}\label{blowing_grothendiek_lem} 	Let $A$ be a continuous trace $C^*$-algebra (cf. Definition \ref{continuous_trace_c_alt_defn}), and let $F$ be an Abelian group. For all $r \ge 0$ there are isomorphisms
	\bean
...
\end{lemma}
\begin{proof}
From $p_0\circ j = \Id_\sX$ it follows that $j^*p_0^* \mathfrak{Ass}\left(  F_\sX\right)= F_\sX$, and $j^r\circ p_0^r = \Id_{H^r\left(\sX, \mathfrak{Ass}\left(  F_\sX\right) \right)}$. 
	From the equations  \eqref{blowing_p_j_eqn} and \eqref{cech_hom_eqn} and equation  it follows that for all $r> 0$ there are following homomorphisms:
	\bean
	...
	$$
So if $\check{X}^r \bydef \ker \check{H}^r\left( j\right)$ then $\check{H}^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^0  \right)', F \right)\cong \check{H}^r\left(\sX, F  \right)	\oplus \check{X}^r$. Now this Lemma follows from the isomorphism \eqref{chech_hom_eqn}.
...
is analogical to the following one	
$$
...
%\begin{rem}\label{geom_mor_defn}\cite{johnstone:topos}
	% 1.16 
...
\ee
This is no longer exact (in general). The complex \eqref{complex_eqn}   define the $r^{\text{th}}$ \textit{cohomology
group}  
\be\label{etale_coh_eqn}
H^r\left(X,\ \mathscr F \right)\cong\begin{cases}
	\ker \dl^0 & r = 0\\
 \ker~ \dl_{r} / \im~ \dl_{r - 1}  & r > 0
\end{cases}.
\ee
this group does not depend on choice of injective resolution up to isomorphism.
...
%	Let $f : \sX \to \sY$ be an injective continuous map, and let $F$ be a set. If both $F_\sX$ and $F_\sY$ are constant presheaves  (cf. Definition \ref{constant_presheaf_defn}) on $\sX$ and $\sY$ then there is a natural isomorphism of sheaves
%	\bean\label{dir_image_eqn}
%	\mathfrak{Ass}\left( F_\sX\right)  = f^*\mathfrak{Ass}\left( F_\sY\right)
%	\eean
%	 where 
%	$\mathfrak{Ass}$  means the associated sheaf functor (cf. Definition \ref{site_sheaf_ass_defn}) and 	
%	$f^*$ means the inverse image (cf. Definition \ref{sheaf_inv_im_defn})
%\end{exercise}
