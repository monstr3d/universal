...
From the Theorem \ref{hered_cov_defn} it follows that there is a locale of frame (cf. Definition \ref{locale_defn}) such that
\be\label{locale_eqn}
\begin{split}
...
The explained in  \ref{grothendieck_ca_empt} Grothendieck topology $J_A$  is said to be an  \textit{Grothendieck} $A$-\textit{topology}. A corresponding  site (cf. Definition \ref{grotendieck_topolgy_defn}) $\mathbf{T}_A\bydef \left(\mathbf{Hered}/A, J_A\right)$ is said to be \textit{arising} from $A$ (cf. Definition \ref{top_gro_definition}). A Grothendieck topos $\mathfrak{Topos}(A)\bydef \mathbf{Sh}\left(\mathbf{T}_A \right)$ is said to be the \textit{Hereditary} $A$-\textit{topos}.
\end{definition}
\begin{definition}\label{a_locale_defn}
	From the Theorem \ref{locale_thm} it follows that the {Hereditary} $A$-{topos} $\mathscr{E}_A$ is localic. It corresponds to a locale $\mathfrak{Locale}\left(A \right)$, we say that  $\mathfrak{Locale}\left(A \right)$ is the \textit{Hereditary}  $A$-\textit{locale}, so one 
has
...
\end{remark}
\begin{remark}\label{hom_ring_rem}
...
\bean
\mathfrak{Frm}\left(  A\right)\to \mathfrak{Frm}\left( \widetilde A\right)\\
...
Two $A$-{representatives} $\left( j', y',  C_0\left( \sY'\right)\right)$ and  $\left( j'', y'',  C_0\left( \sY''\right)\right)$ are \textit{hereditary equivalent} if there are open neighborhoods $\sU'$ and $\sU''$ of $x'$ and $x''$ respectively such that
\bean
\text{Generated by } j'\left(C_0\left( 
\sU'\right) \right) \text{hereditary $C^*$-algebra }=\\
=\text{Generated by } j''\left(C_0\left( 
\sU''\right) \right) \text{hereditary $C^*$-algebra }
\eean
Equivalence classes of representatives a said to be   $A$-\textit{points}. An $A$ point represented by  $\left( j,y,  C_0\left( \sY\right)\right)$   will be denoted by $\left[\left( j, y,  C_0\left( \sY\right)\right)\right]_h$.  We denote by $\mathfrak{Points}\left(A \right)$ a set of Hereditary $A$-points.  If $B\subset A$ is a hereditary subalgebra then we say that $\left[\left( j, y,  C_0\left( \sY\right)\right)\right]$ is in 
$B$ if there is a representative $\left( j, y,  C_0\left( \sY\right)\right)$ of $\left[\left( j, y,  C_0\left( \sY\right)\right)\right]_h$ such that $j( C_0\left( \sY\right)\subset B$. We write $\left[\left( j, y,  C_0\left( \sY\right)\right)\right]_h \in B$, We denote by
$$
\mathfrak{Points}\left(B \right)\bydef \left\{\left[\left( j, y,  C_0\left( \sY\right)\right)\right] \in \mathfrak{Points}\left(A \right)| \left[\left( j, y,  C_0\left( \sY\right)\right)\right]_h \in B  \right\}
$$
\end{definition}\begin{lemma}\label{type_I_lem}!!!
...
\section{Locales of continuous trace $C^*$-algebras}
\paragraph{}
...
that for any two distinct elements $U, V \in  E \mathcal O(X)$, there exists a point
