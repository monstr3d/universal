...
\end{tikzcd}
	...
	For any $\mathbf{Hered}/A$-object $B$ a distinguished set of families of morphisms $\left\{B_\iota \to B\right\}_{\iota\in I}$, called a \textit{covering} of $B$ if 
	\begin{enumerate}
		\item [(i)] a set $I$ is finite, 
		\item[(ii)] $\bigcap_{\iota \in I} B_\iota = B$.
	\end{enumerate}
	\begin{theorem}\label{ca_site_thm}
	The described above situation yields a Grothendieck pretopology (cf. Definition \ref{pretopology_defn}).
	\end{theorem}
	\begin{proof}
	One needs check conditions (a) - (c) of the Definition \ref{pretopology_defn},\\
	(a) If $B$ and $C$ are $\mathbf{Hered}/A$-objects such that there is a morphism $C \to B$ then $B \subset C$. Moreover $B\times_A C= C$. If $\left\{B_\iota \to B\right\}_{\iota\in I}$ is a covering of $B$  then $B = \bigcap_{\iota \in I}B_\iota$. Moreover $B\times_A C= C$ 
	From 	
	$$
	C = 	B \cup C = \left( \bigcap_{\iota \in I}B_\iota \right) \cup C = \bigcap_{\iota \in I}\left( B_\iota\cup C\right) 
	$$
	it follows that the family $\left\{ B_\iota\cup C\right\}_{\iota \in I}$ is a covering of $C$.\\
	(b) If $B = \bigcap_{\iota \in I}B_\iota$ and $B_\iota =  \bigcap_{j \in J_\iota}B_{\iota,j}$ for all $\iota \in I$ then $B = \bigcap_{\iota, j} B_{\iota,j}$.\\
	(c) Evident.
	\end{proof}
...
\mathfrak{Site}\left(A \right)\bydef \mathbf{T}_A.
	...
	\ee	
	which is a category of sheaves over the site is said to be the \textit{Gelfand} $A$-\textit{topos}. 
\end{definition}
...
\mathbf{PreAb}\left(A\right)\bydef	\mathbf{Ab}\left(\mathbf{T}_{A} \right) \cong 	\mathbf{Ab}\left(\mathbf{T}_{\mathfrak{Site}\left(A \right)}\right),\\
...
\end{empt}
\begin{empt}
There is an alternative description of $\mathfrak{Site}\left(\K \right)$. Instead to Hilbert spaces we set their orthogonal complements.   A $\mathbf{Hered}/\K$ morphism $\H' \to \H''$ corresponds to an inclusion $\H' \subset \H''$. The fiber product $\H' \times_{\{0\}} \H''$ corresponds to $\H' \cap  \H''$. A covering of $\H'$ is a finite set $\left\{\H_1, ..., \H_n\right\}$ of subspaces  such that $\H'= \sum_{j = 1}^n\H_j$. So one has a pretopology denoted by $J_\K$, and the Gelfand $\K$-site $\mathfrak{Site}\left(\K \right) = \left(\mathbf{Hered}/\K, J_\K \right)$. 
\end{empt}
\begin{empt}
Suppose that $d$ is finite and $d \bydef n - 1$. Then one has a projective space $\C P^d = P\left(\H \right)$. The site $\left(\mathbf{Hered}/\K, J_\K \right)$, can be described by following way. Elements of $J_\K$ are linear subspaces of  $\C P^d$ the fiber product corresponds to intersection of spaces, a covering of subspace $P \H' \subset \C P^d$ is a finite set $\left\{P\H_1, ..., P\H_n\right\}$ of subspaces of $\C P^d$ such that $P\H'$ is a minimal linear subspace which contains a union $P\H_1\cup ...\cup P\H_n$. Three is a site a given by \eqref{top_closed_site_eqn} site
$
\left(\mathfrak{Closed}\left( \C P^d \right), J_{\C P^d}\right)
$. Any $\mathbf{Hered}/\K$-object corresponds to a closed subspace of $\C P^d$, so it is a $\mathfrak{Closed}\left( \C P^d \right)$-object. Let $\mathscr U_\K$ be a family of the given by the equation \eqref{compact_zar_open_eqn} open sets of $\C P^d$ and let $\mathscr U_{\mathfrak{Closed}\left( \C P^d \right)}$ be a family of all subsets of $\mathfrak{Closed}\left( \C P^d \right)$-objects. One has
$$
s \in C^r\left( \mathscr U_\K, F_{\mathbf{Hered}/\K}\right)\quad \Rightarrow \quad s \in C^r\left( \mathscr U_{\mathfrak{Closed}\left( \C P^d \right)}, F_{\mathfrak{Closed}\left( \C P^d \right)}\right)
$$
where the notation \eqref{cech_eqn} is used. Moreover for any $c \in C^r\left( \mathscr U_\K, F_{\mathbf{Hered}/\K}\right)$ a following condition hold:
\be\label{compact_diff_eqn}
d^r_{\mathscr U_\K} c = 0 \quad \Rightarrow \quad d^r_{ \mathscr U_{\C P^d}} c = 0
\ee
where $d^r_{\mathscr U_\K}$ (resp. $d^r_{ \mathscr U_{\C P^d}}$) are given by \eqref{cech_differential_eqn} differentials which are related to the families $\mathscr U_\K$ and  $\mathscr U_{\C P^d}$ respectively. From the equation \eqref{compact_diff_eqn} it follows that for any $r > 0$ there is an  homomorphism
$$
H^r\left( \K, F\right) \hookto H^r\left( \mathfrak{Closed}\left( \C P^d \right), F\right).
$$
Let us fix $f \in F$.
If $P_\H$ is a subspace of is a linear subspace of $\C P^d$ such that codimension of $P\H$ equals to 1 then if $P \H_1$, $~P \H_1$, $~P \H_3$ are such that $P\H_1 \neq  P\H_2,~~ P\H_1 \neq  P\H_3,~~ P\H_2 \neq  P\H_3$ then $P\H \neq  P\H_1 \cap P\H_2 \cap P\H_3$. From this circumstance it follows that if  chain $s^{P\H} \in C^1\left(\mathscr U_\K, F \right)$ is given by
\be\label{compact_cycle_eqn}
\begin{split}
 s^{P\H}_{P\H, \C P^d} = f,\\
\forall \left( P\H_1, P\H_2 \right) \neq \left(P\H, \C P^d \right)   \quad s^{P\H}_{P\H_1, P\H_2}= 0 
\end{split}
\ee
(cf. Equation \eqref{cech_eqn}) then $d^1 s = 0$ (cf. \ref{cech_differential_eqn}), i.e. $s$ is a cocycle. If both $P\H'$, $P\H''$ have codimension 1 and $P\H'\neq P\H''$ then a family $\mathscr U' = \left\{P\H', P\H''\right\}$ is a covering of $\mathbf{Hered}/\K$. If  $s^0 \in C^0\left(\mathscr U_\K, F \right)$ is given by 
\be\label{compact_cycle_0_eqn}
\begin{split}
\quad s^0_{P\H'\cap  P\H''} = f,\\
	\forall P\H \neq P\H'\cap  P\H''   \quad s^0_{P\H}= 0 
\end{split}
\ee
then
$$
d^0 s^0 =  s^{P\H'}- s^{P\H''}
$$
If follows that cohomology class of $s^{P\H''}$ does not depend on choice of $P\H''$. If $P\H^r$ is a subspace  of codimension  $r$ then there is a family $\mathscr U^r = \left\{P \H_1, ..., P\H_r\right\}$ such that $P\H^r= P \H_1\cap ...\cap P\H_r$. There is chain $s^{P\H^r} \in C^r\left(\mathscr U^r, F \right)$
\be\label{compact_cycle_r_eqn}
\begin{split}
s^{P\H^r}_{P\H_1, ..., P\H_r,  \C P^d} = f,\\
\forall \left(L_0, ..., L_{r}\right) \neq \left(P\H_1, ..., P\H_r,  \C P^d \right)  \in \mathscr U^r \quad s^{P\H^r}_{L_0, ..., L_{r}}= 0 
\end{split}
\ee
One has
$$
...
Suppose that $F = \Z$, $f = 1$ and consider  chain $s$ as an element of 
\paragraph{title}

it cannot be be represented by 
$$
P\H \neq  P\H_1 \cap P\H_2 \cap P\H_3; \quad  P\H_1 \neq  P\H_2,\quad P\H_1 \neq  P\H_3,\quad P\H_2 \neq  P\H_3,
$$


there is only three intersections  
\bean
\C P^d\cap \C P^d \cap P\H = C P^d\cap  P\H \cap P\H =  P\H \cap P\H \cap P\H = P\H
\eean  


, then $P_\H = \C P^d \cap \C P^d$. For any $f \in F$ we define a $1$ chain $c \in C^1\left(\mathscr U_\K, F \right)$ such that
$$
\exists f \in F \quad  \quad s_{P\H, P\H} = f.
$$ 
(cf. Equation \eqref{cech_eqn}).


If $P\H$ is a linear subspace of $\C P^d$ such that codimension of $P\H$ equals to $k$ then there are subspaces
\end{empt}
\paragraph{title}
Define
\be\label{cech_eqn!}
...
\paragraph{title}
\begin{empt}\label{compact_alt_constr}

There is an alternative description of $\mathfrak{Site}\left(\K \right)$. Let $p \in \N$ or $p = \infty$. If $p = \infty$ we suppose that $\C^p \bydef \ell^2\left(\N \right)$. There is a complex projective space  $\C P^d$ where $d \bydef n-1$. Now
\begin{enumerate}
	\item[(a)] $\mathbf{Hered}/\K$-objects are subsets of $\C P^d$ given by
	\be\label{compact_zar_open_eqn}
	U = \C P^d \setminus \bigcup_{j = 1}^n L_j
\ee
	where $L_j$ is a complex linear projective subspace,  
	\item[(b)]  $\mathbf{Hered}/\K$-morphisms $\sV \to \sU$ are inclusions $\sV \subset \sU$.
\end{enumerate}
A pullback $\sU' \times_{\C P^d} \sU''$ is an intersection $\sU' \cap \sU''$.
A covering of $\mathbf{Hered}/\K$-object $\sU$ is a finite family $\left\{\sU_1, ..., \sU_n\right\}$ of $\mathbf{Hered}/\K$-objects, such that $\sU = \sU_1 \cup...\cup \sU_n$. 
\end{empt}
\begin{empt}
Suppose that $d$ is finite, and consider $\C P^d$ as an complex manifold with classical  topology $J_{\C P^d}$. On the other there is a category  $\mathbf{Cat}\left( \C P^d\right) $  of open with respect $J_{\C P^d}$  subsets of $\C P^d$ and their inclusions. Since any given by \eqref{compact_zar_open_eqn} set is open one has a functor
\be\label{compac_to_cp_eqn}
\mathbf{Hered}/\K \xrightarrow\phi \mathbf{Cat}\left( \C P^d\right).
\ee
There is a site $\left(\mathbf{Cat}\left( \C P^d\right), J_{\C P^d}\right)$ and  one can prove that the functor \ref{compac_to_cp_eqn} is  a morphism of sites (cf. Definition \ref{morphism_of_sites_defn}). From the Theorem \ref{site_mor_thm} it follows that there is a geometric morphism
\be\label{compac_geom_eqn}
 \mathbf{Sh}\left(\mathbf{Cat}\left( \C P^d\right), J_{\C P^d} \right)\xrightarrow{f} \mathbf{Sh}\left(\mathbf{Hered}/\K, J_\K \right)
\ee
Let $F$ be a  Abelian group, and let both $F_{\mathbf{Hered}/\K} \in \mathbf{Ab}^{\left(\mathbf{Hered}/\K \right)^{\text{op} }}$ and $F_{\mathbf{Cat}\left( \C P^d\right)} \in \mathbf{Ab}^{\left(\C P^d \right)^{\text{op} }}$ be constant presheaves (cf. Definition \ref{constant_presheaf_defn}) of Abelian groups. 
If $\mathbf{Sh}\left(\C P^d, J_{\mathrm{Zar}} \right)\xrightarrow{f_*} \mathbf{Sh}\left(\mathbf{Hered}/\K, J_\K \right)$ is the direct image (cf. Definition \ref{geometric_morphism_defn}) then from \eqref{geom_dir_im_eqn}
it follows that $f_*F_{\mathbf{Cat}\left( \C P^d\right)}\left( U\right)=  F_{\mathbf{Cat}\left( \C P^d\right)}\left( \phi \left( U\right) \right)\cong F$ for all $\forall U \in \mathbf{Hered}/\K$, and taking into account \eqref{compact_f_eqn} one has
\be\label{compact_f_d_eqn}
\begin{split}
	f_*F_{\mathbf{Cat}\left( \C P^d\right)}= F_{\mathbf{Hered}/\K}
\end{split}
\ee
Let $m < d$ and let $\left\{L_0, ..., L_m\right\}$ be a family of different $d - 1$ dimensional linear. 

It is known that if $sU$ is an open with respect to Zariski topology subset of connected complex manifold then $\sU$ is connected. From this fact it follows that both  $F_{\mathbf{Hered}/\K}$ and $F_{\mathbf{Cat}\left( \C P^d\right)}$ are sheaves, i.e. $F_{\mathbf{Hered}/\K}\in \mathbf{Ab}\left(\mathbf{Hered}/\K, J_\K \right)$ and $F_{\mathbf{Cat}\left( \C P^d\right)}\in \mathbf{Ab}\left(\C P^d, J_{\mathrm{Zar}}\right) $ and
\be\label{compact_f_eqn}
\begin{split}
\forall U \in \mathbf{Hered}/\K \quad F_{\mathbf{Hered}/\K}\left( U\right) \cong F,\\ 
\forall \sU \in \C P^d \quad F_{\mathbf{Cat}\left( \C P^d\right)}\left( \sU\right) \cong F.
\end{split}
\ee
There is a specialization of \eqref{geom_ep_im_eqn} given by 
$$
A_\phi \bydef \mathbf{Hered}/\K \xrightarrow{\phi}\mathbf{Cat}\left( \C P^d\right)   \xrightarrow{\mathbf{y}} \mathbf{Sets}^{\mathbf{Cat}\left( \C P^d\right)^{\mathrm{op}}} \xrightarrow{\mathfrak{Ass}}\mathbf{Sh}\left(\C P^d, J_{\mathrm{Zar}} \right)
$$
For any $U \in \mathbf{Hered}/\K$ the one has  $\phi\left( U\right)  =  U $ we $U$ is regarded as a $\mathbf{Cat}\left( \C P^d\right)$-object. 
One has 
$$
\forall \sU \in \mathbf{Cat}\left( \C P^d\right)\quad  \mathbf y \circ \phi \left(U\right)\left(\sU \right) = \Hom_{\mathbf{Cat}\left( \C P^d\right)}\left(\sU, U \right) = \begin{cases}
	\sU \to U & \sU \subset U\\
	\emptyset & \sU \not\subset U.
\end{cases}
$$
From the above equation it turns out that for any $U \in \mathbf{Hered}/\K$ $\mathbf y \circ \phi \left(U\right)$ is sheaf so $A_\phi$ is given by
$$
\forall \sU \in \mathbf{Cat}\left( \C P^d\right)\quad  A_\phi \left(U\right)\left(\sU \right) = \Hom_{\mathbf{Cat}\left( \C P^d\right)}\left(\sU, U \right) = \begin{cases}
	\sU \to U & \sU \subset U\\
	\emptyset & \sU \not\subset U.
\end{cases}
$$
There is a following specialization of \eqref{coprod_topos_eqn}
\be\label{compact_topos_eqn!}
\coprod_{\substack{u: U'\to U\\r \in F_{\mathbf{Hered}/\K}(U)}}\mathbf{y}U' \substack{ \xrightarrow{\th}\\\xrightarrow[\tau]{}} \coprod_{\substack{U \in \mathbf{Hered}/\K\\r \in F_{\mathbf{Hered}/\K}(U)}}\mathbf{y}U\xrightarrow{\eps} F_{\mathbf{Hered}/\K};
\ee
If we write elements of $\coprod_{u,r} \mathbf{y}U'$ as triples $(r, u, v)$ with
$r \in  F_{\mathbf{Hered}/\K}(U)$, $~u: U' \to U$ and $v \in  \mathbf{y}(U')(V)= \mathbf{C}\left( V, U'\right)$  for some object
$V$ in $\mathbf{Hered}/\K$, and similarly write elements of $\coprod_{U,r}$ as pairs $(r, w)$ with $w\in \mathbf{Hered}/\K(V, U)$, then $\th$ and $\tau$ in \eqref{coprod_topos_eqn} are the maps given by
\bean
...
$$
\forall \sU \in \C P^d \quad \left( \mathbf y \circ \phi \left( F_{\mathbf{Hered}/\K}\right)\right) \left(\sU \right)  
$$

The inverse image $ \mathbf{Sh}\left(\mathbf{Hered}/\K, J_\K \right)\xrightarrow{f^*}\mathbf{Sh}\left(\C P^d, J_{\mathrm{Zar}} \right)$ is given by \eqref{geom_inv_im_eqn}, so one has
$$
f^*F_{\mathbf{Hered}/\K} = F_{\mathbf{Hered}/\K}\otimes_{\mathbf{Hered}/\K} A_\phi.
$$
On the other hand from the equation \eqref{coprod_topos_a_eqn} it follows that 
\be\label{compact_topos_a_eqn}
\coprod_{\substack{u: U'\to U\\r \in F_{\mathbf{Hered}/\K}(C)}}A_\phi\left(U' \right)\left(\sU \right)   \substack{ \xrightarrow{\th}\\\xrightarrow[\tau]{}} \coprod_{\substack{U \in \mathbf{Hered}/\K\\r \in F_{\mathbf{Hered}/\K}(U)}}A_\phi\left(U \right)\left(\sU \right) \xrightarrow{\eps} \mathbf{Hered}/\K\otimes_{\mathbf{C}}A_\phi\left(\sU \right) ;
\ee
...
\section{Topoi of liminaly $C^*$-algebras with Hausdorff spectra}
