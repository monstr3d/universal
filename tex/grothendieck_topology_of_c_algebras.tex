...
\begin{empt}\label{hered_repr_p_empt}
	If $\rho: A\hookto B\left(\H \right)$ is a faithful nondegenerate representation.
...
\begin{remark}\label{hered_dense_rem}
	If $\rho\left( \left\{u_\la \right\}_{\la \in \La} \right) \subset \rho\left(B \right)$ then from $p \bydef s$-$\lim_{\la\in\La} \rho\left( u_\la\right)$ it follows that $p\H$ is a norm completion of $\rho\left(B\right)\H$.
\end{remark}
...
\begin{proof}
	...
	$$
	\check{H}^r\left( j\right)\circ \check{H}^r\left( p_0\right)= \Id_{ \check{H}^r\left(\sX, F \right)}.
	$$
...
is analogical to the $K$-theoretic one
	$$
...
\begin{empt}\label{chech_hom_empt}
...
\end{split}
\ee
If both $\sX \xrightarrow{f}\sY$ and $\sY \xrightarrow{g}\sZ$ are continuous maps then from this construction one has
...
\begin{definition}\label{irred_defn}\cite{pedersen:ca_aut}
	A a nonzero representation  $\pi: A \to B\left(\H \right)$ of $C^*$-algebra $A$ said to be \textit{irreducible} if there are no non-trivial $A$-subspaces for $\pi$.
\end{definition}
