...
\end{definition}
\begin{exercise}\label{ccr_exer}
Prove that any $CCR$ (cf. Definition \ref{ccr_defn})), commutative generated $C^*$-algebra is generated by its  commutative hereditary $C^*$-subalgebras.
\end{exercise}
Denote by
...
\begin{exercise}
	...
	\end{enumerate}
\end{exercise}
If $s \in \mathfrak{Ass}\left( \mathscr{P}^A\right) \left( \sU\right)$ then we denote by
\bean
...
\end{empt}
 ...
 If $A$ is a $CCR$, {commutative generated} $C^*$-algebra with  {Hausdorff blowing-up} $C_0\left( \sX\right) \hookto M\left(A \right)$ then $\mathscr X^A$ is the Grothendieck $A$-space $\mathfrak{Grot}\left(A \right)$ (cf. Definition \ref{grothendieck_ca_t_defn}).
\end{lemma}
\begin{proof}
...
\begin{definition}\label{local_prod_defn}
 ...
 we say that $A$ is a \textit{local product} of $A'$ and $A''$ if for any open set $\sU \subset \sX$ and any $x \in \sU$ there is an open neighborhood $\sV$ of $x$ such that $\sV \subset \sU$  and  there is a bijective map
...
\varphi_{\sV} :		\mathfrak{Supp}\left( A'\right)_{\sV}	\times \mathfrak{Supp}\left( A''\right)_{\sV}\cong \mathfrak{Supp}\left( A\right)_{\sV}
	\ee
	(cf. Equation \eqref{supp_set_eqn}). Moreover we require that for any open subset $\mathcal W \subset \sV$ there is a natural isomorphism
	\bea\nonumber
\varphi_{\mathcal W} :		\mathfrak{Supp}\left( A'\right)_{\mathcal W}	\times \mathfrak{Supp}\left( A''\right)_{\mathcal W}\cong \mathfrak{Supp}\left( A\right)_{\mathcal W},\\
\label{inter_c_eqn}
C' \in  \mathfrak{Supp}\left( A'\right)_{\sV}, \quad C' \in  \mathfrak{Supp}\left( A'\right)_{\sV}~ \Rightarrow ~  \varphi_{\mathcal W}\left(C'|_{\mathcal W}, C''|_{\mathcal W} \right)=\varphi_\sV \left( C', C''\right)|_{\mathcal W} 
	\eea
\end{definition}
...
\begin{proof}
	Any element of 
$\mathfrak{Grot}\left(A' \right)\times_{\mathfrak X\left(\sX \right)}\mathfrak{Grot}\left(A'' \right)=\mathscr X^{A'} \times_{\mathfrak X\left(\sX \right)}	 \mathscr X^{A''}$ corresponds to a pair $C'_x \in  \mathscr X^{A'}$, $~C''_x \in  \mathscr X^{A''}$ such that $P_{A'}\left( C'_x\right)= P_{A''}\left( C''_x\right)$ (cf. Equation \eqref{pa_eqn}). Suppose that both $C'_x$ and $C''_x$ are represented by  hereditary commutative $C^*$-algebras $C' \in \mathscr C^{A'}$ and $C'' \in \mathscr C^{A''}$ such that $\supp C' = \supp C''= \sU$. From the Definition \ref{local_prod_defn} it follows that there is a family $\left\{\sV_\iota\right\}_{\iota\in I}$ such  that $\sU = \cup_{\iota \in I}\sV_\iota$ and for all $\iota \in I$ there is a  given by \eqref{local_prod_eqn} bijective map $\varphi_{\sV_\iota} :		\mathfrak{Supp}\left( A'\right)_{\sV_\iota}	\times \mathfrak{Supp}\left( A''\right)_{\sV_\iota}\cong \mathfrak{Supp}\left( A\right)_{\sV_\iota}$ If $C_\iota \bydef \varphi_{\sV_\iota}\left(C'|_{\sV_\iota}, C''|_{\sV_\iota} \right)$ then $C_\iota$ yields a section $s_\iota \in \mathscr P^A\left(\sV_\iota \right)$. From the condition \eqref{inter_c_eqn} it follows that for all $\iota', \iota'' \in I\times I$ one has  $s_{\iota'}|_{\sV_{\iota''}}=s_{\iota''}|_{\sV_{\iota'}}$. From the sheaf condition (cf. equation \ref{etale_sheaf_eqn}) it follows that there is a section $s \in \mathfrak{Ass}\left( \mathscr P^A \right)\left(\sU \right)$ such that $s|_{\sV_\iota}= s_\iota$ for all $\iota\in I$. If $C = \mathfrak{C}(s)\in \mathscr C^A$ is given by the equation \eqref{cs_eqn} then $\supp C = \sU$. We define $\phi\left( C'_x, C''_x\right) \bydef C_x$.

Suppose that $C_x \in\mathfrak{Grot}\left(A \right)= \mathscr X^{A}$ is represented by $C\in \mathscr C^A$ and $\sU = \supp C$. Similarly to the above part of this prove one can define pairs $\left(C'_\iota, C''_\iota  \right)  \bydef \varphi^{-1} \left(C|_{\sV_{\iota}} \right)$. Then similarly to the above construction one has a pair $C' \in \mathscr C^{A'}$ and $C'' \in \mathscr C^{A''}$ such that $\supp C' = \supp C'' = \sU$. A pair $\left(C'_x, C''_x\right)$ yields an element of $\mathscr X^{A'} \times_{\mathfrak X\left(\sX \right)}	 \mathscr X^{A''}$. We define
$$
\phi^{-1}\left(C_x \right) \bydef \left(C'_x, C''_x \right). 
$$
\end{proof}
