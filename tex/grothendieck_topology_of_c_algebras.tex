...
\end{definition}
\begin{empt}
Let  $\sX$ be a closed subset of  $\sY$ and a set  $\sY$ is locally connected.  Let $L$ be ring, $\mathscr {L}\bydef \mathfrak{Ass}\left(L_\sY \right)$ a sheaf associated with constant presheaf (cf. Definitions \ref{constant_presheaf_defn}, \ref{associated_sheaf_defn}). 
From the Proposition \ref{precosh_sh_prop}  it follows that the map
$$
\mathfrak L : \sU \mapsto H^c_0\left( \sU, \mathscr {L}\right) 
$$
is a cosheaf (cf. Definition \ref{top_cosh_defn}). Suppose that there is an open covering  $\mathscr U \bydef \left\{\sU_\a\right\}_{\a \in \A}$ of $\sX$. If  $\overline \sU_\a\subset \sX$ is a closure of $\sU_\a$ for any $\a\in \A$, then $\overline \sU_\a$ is a closed subset of $\sY$ and $^\neg  \sU_\a \bydef \sY \setminus \overline \sU_\a$ is an open subset of $\sY$. For any $\left(\a_0, ...\a_n\right)\in \A^n$ we denote by $\sU_{\a_0, ...\a_n}\bydef \sU_{   \a_0} \cap \dots \cap \sU_{   \a_n}$ and $^\neg \sU_{\a_0, ...\a_n}\bydef ~ ^\neg\sU_{   \a_0} \cup \dots \cup ^\neg \sU_{   \a_n}$.
 Let $^\neg \mathscr L = \mathfrak{Ass}\left(\neg  \mathfrak L\right)$ be the sheaf on $\mathfrak{Closed}\left( \sX\right)$ associated with with the negation presheaf $\neg  \mathfrak L$ (cf. Definition \ref{top_neg_defn}). 
 
 \begin{proof}
 Let 
 $$
 ^\neg \mathscr L\xrightarrow{\eps}\mathscr A^0  \xrightarrow{d}\mathscr A^{1}  \xrightarrow{d}\mathscr A^2\xrightarrow{d}\dots \xrightarrow{d}\mathscr A^n \to\dots
 $$
 be an injective resolution. Since $\check C^p\left( \mathscr U,\bullet\right)$ is an exact functor of presheaves, we have
 \bean
 ...
 \begin{tikzpicture}
 	...
 	{
 		& 0  & 0 & 0 & 0 &	\\ 
 		0 & \Ga\left( ^\neg \mathscr L\right) 		& \Ga\left(\mathscr A^0 \right) & \Ga\left(\mathscr A^1 \right)  &  \Ga\left(\mathscr A^2 \right)& \dots\\
 		0 & \check C^0\left(\mathscr U,  ^\neg \mathscr L \right) 		& \check C^0\left(\mathscr U, \mathscr A^0 \right) & \check C^0\left(\mathscr U, \mathscr A^1 \right)  &  \check C^0\left(\mathscr U, \mathscr A^2 \right)& \dots \\0 & \check C^1\left(\mathscr U, ^\neg \mathscr L \right) 		& \check C^1\left(\mathscr U, \mathscr A^0 \right) & \check C^1\left(\mathscr U, \mathscr A^1 \right)  &  \check C^1\left(\mathscr U, \mathscr A^2 \right)& \dots\\0 & \check C^2\left(\mathscr U,  ^\neg \mathscr L \right) 		& \check C^2\left(\mathscr U, \mathscr A^0 \right) & \check C^2\left(\mathscr U, \mathscr A^1 \right)  &  \check C^2\left(\mathscr U, \mathscr A^2 \right)& \dots\\& \vdots  & \vdots & \vdots & \vdots &
 		\\};
 	\path[-stealth]
...
\end{empt}
\paragraph{FINISH}
Let $\sX$ be a compact, locally connected, Hausdorff, orientable $n$-{dimensional homology manifold over} $L$ (cf. Definition \ref{top_ndiml_defn}). From \eqref{top_dim_eqn} it follows that $\dim_L \sX = n$. From the theorem \ref{top_c_ac_thm} it follows that any sheaf $\mathscr A$ of $L$-modules has a $c$-soft resolution (cf. Definition \ref{top_res_defn})
