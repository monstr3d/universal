...
\begin{empt}
Let $A,~ A', ~A''$ be $C^*$-algebras with inclusions $A \subset M\left( A'\right)$ and   $A \subset M\left( A''\right)$. If $\pi': A' \to B\left(\H'\right)$ and $\pi'': A'' \to B\left(\H''\right)$ are faithful representations then there is a the natural injective map 
...
\end{split}
\ee
where $_\sU\K\left( L^2\left(\mathbb{F}^n \right) \right)_\sU\subset \K\left( L^2\left(\mathbb{F}^n \right)\right) $ is a generated by $C_0\left( \sU\right) \K\left( L^2\left(\mathbb{F}^n \right)\right)  C_0\left( \sU\right)$ hereditary $C^*$-subalgebra.
...
\begin{empt}
	...
	Let us consider a set of generators and relations $\left(G_\sigma, \overline\phi^\sigma_\a\right)$ (cf. Appendix \ref{gen_rel_empt}) such that
	\begin{itemize}
