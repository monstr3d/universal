...
\begin{proof}
	...
	(a) Let $B, C\subset A$ be subalgebras. A covering of $B$ is a family $\left\{B_\iota\subset B\right\}_{\iota \in I}$ of subalgebras of $B$ such that $B$ is a generated by a union $\bigcup B_\iota$ hereditary $C^*$-subalgebra (cf. Definition \ref{hered_gen_defn}). If  $L\left( B\right)$, $L\left( C\right)$ and $L\left( B_\iota \right)$ are given by the equation \eqref{left_ideal_eqn} closed left ideals then $L\left(B\cap C \right) = L(B)\cap L(C)$, $~L\left(B_\iota \cap C \right)= L\left(B_\iota \right)\cap L\left( C \right)$ (cf. Lemma \ref{hered_ideal_lem}). Moreover $L\left(B\right)$ is the $C^*$-norm closure of an algebraic sum $\sum_\iota L\left( B_\iota\right)$. For any $\iota \in I$ one has $L\left(B_\iota\right)\cap L\left(C\right)\subset L\left(B\right)\cap L\left(C\right)$ it follows that $\sum_\iota \left( L\left(B_\iota\right)\cap L\left(C\right)\right) \subset L\left(B\right)\cap L\left(C\right)$, i.e. a generated by a union $\cup_\iota \left( B_\iota\cup C\right)$ hereditary subalgebra is a subalgebra of the intersection $B\cap C$. If $\eps > 0$ and $ b\in L\left(C\right)\cap L\left(B\right)$ then from the from the Theorem \ref{left_ideal_thm} it follows that there is a positive $u \in L\left(C\right)$ such that 
	\bean
	\left\|u \right\| \le 1,\\
...
we conclude that $b$ lies in the $C^*$-norm closure of $\sum_\iota \left( L\left(B_\iota\right)\cap L\left(C\right)\right)$. So $L\left(B\right)\cap L\left(C\right)$ equals to the $C^*$-norm closure of $\sum_\iota \left( L\left(B_\iota\right)\cap L\left(C\right)\right)$ and $B\cup C$ is a generated by $\cup_\iota \left(B_\iota\cap C \right)$ hereditary $C^*$-algebra. Thus 
	...
	\end{itemize}
	From $\sum_\iota L\left( B_\iota \right)\subset L\left(B \right)$ and $\sum_j L\left( C_{\iota_j} \right)\subset L\left(B_\iota \right)$ for any $\iota \in I$ it follows that $\sum_{\iota j} L\left( C_{\iota_j} \right)\subset L\left(B \right)$. However the ideal $\subset L\left(B \right)$ is $C^*$-norm closed so a $C^*$-norm closure of $\sum_{\iota j} L\left( C_{\iota_j} \right)$ is a subset of $ L\left(B \right)$. 
...
%	\bt\text{-}\lim_\la u_\la = 1_{ M\left( B\right) },\\
	...
	\end{split}
	\ee
	From the Lemma \ref{increasing_convergent_w_lem} it turns out that the net $\left\{\rho\left( u_\la\right)  \right\}$  is convergent  with respect to the strong topology of $B\left(\H \right)$ (cf. Definition \ref{strong_topology_defn}). If $p \bydef s$-$\lim\rho\left(  u_\la \right) \in B\left( \H\right)$ is a strong limit then $p$ lies in the strong closure of $\rho(A)$ (cf. Definition \ref{strong_topology_defn}). One can prove that $p$ is a projector.
...
If $A = CT\left(\sX, \dl \right)$ (cf. Notation \ref{ctr_not})  then we define a presheaf $\mathscr{P}'^\dl $ on $\sX$ (cf. Definition \ref{presheaf_defn}) such that for any pair  $\sU\subset\sV$ of open subsets of $\sX$ one has
\bean
...
there are natural equivalences of categories of presheaves and sheaves
	...
	\mathbf{PreSh}\left(\mathbf{T}_{CT\left(\sX, \dl \right)} \right) \cong 	\mathbf{PreSh}\left(\mathbf{T}_{\mathrm{Sp\acute{e}}\left(\mathscr P^{\dl} \right)'} \right),\\
		...
		\end{split}
	\eean
	where the notation of the Appendix \ref{appendix_etale_sec} is used. For any group $F$ and any $r > 0$ one has  natural homomorphisms
\bean
H^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^\delta  \right)'\right)\cong \check{H}^r\left(CT\left( \sX,\dl\right), F \right),\\
\check H^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^\delta  \right)'\right)\cong \check{H}^r\left(CT\left( \sX,\dl\right), F \right)
\eean
\end{rem}
...
\begin{theorem}\label{blowing_grothendiek_thm} 	Let $A$ be a continuous trace $C^*$-algebra (cf. Definition \ref{continuous_trace_c_alt_defn}), and let $F$ be an Abelian group. For all $r \ge 0$ there is a natural isomorphisms
	\bean
	\check{H}^r\left(CT\left( \sX,0\right), F \right)	\cong \check{H}^r\left(\sX, F  \right)	\oplus {X}^r
	\eean
	where  $X^r$ is an Abelian group and the Notation  \eqref{const_shef_coh_eqn} is  used.
\end{theorem}
\begin{proof}
	From the equation \eqref{cech_hom_eqn} it follows that for all $r> 0$ there are following homomorphisms:
	\bean
	\check{H}^r\left( j\right) 	: \check{H}^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^0  \right)', F \right)\to \check{H}^r\left(\sX, F \right),\\
	\check{H}^r\left(p^0\right) 	: \check{H}^r\left(\sX, F \right)\to \check{H}^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^0  \right)', F\right).
	\eean 
	...
	$$
	If $X^r \bydef \ker \check{H}^r\left( j\right)$ then from the above equations it follows that $	\check{H}^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^0  \right)'\right) 	\cong \check{H}^r\left(\sX, F  \right)	\oplus X^r$. Otherwise from the Remark \ref{equiv_rem} it follows that $$\check{H}^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^0  \right)'\right)\cong \check{H}^r\left(CT\left( \sX,0\right), F \right).$$
\end{proof}
...
\end{split}
\eean
One can prove that the map $\psi_\dl$ is continuous. From the equation  \eqref{cech_hom_eqn} and the Remark \ref{equiv_rem} it turns out that for all $r \ge 0$ and an Abelian group $F$ there is a natural homomorphism
\bean
...
\begin{definition}\label{pullback_defn}\cite{milne:lec}
	...
	\end{tikzpicture}
	\\
	A morphism $U \to U'$ making this diagram commute is also referred to as an $X$-morphism. The \textit{fibre product}  $U_1\times_X U_2$ of morphisms $\varphi_1: U_1\to X$,  $\varphi_2: U_2\to X$ in $\mathscr C$ is their product in the category $\mathscr C/X$. Thus, there is a commutative diagram 
	\newline
	\begin{tikzpicture}
		\matrix (m) [matrix of math nodes,row sep=3em,column sep=4em,minimum width=2em]
		{
...
\begin{definition}\label{etale_sheaf_defn}\cite{milne:lec}
	...
	$$
	f \mapsto \left\{f_{\iota }\right\}:\mathscr F \left(U\right) \to \prod_{\iota \in I} \mathscr F \left(U_\iota \right)
	$$
...
\ee
 The $r^{\text{th}}$ cohomology
group is given by 
\be\label{etale_coh_eqn}
H^r\left(X,\ \mathscr F \right)\bydef R^r\Ga\left(X, \mathscr F \right)= \ker~ \dl_{r+1} / \im~ \dl_r.
\ee
It is proven in \cite{johnstone:topos, milne:lec}  that these groups do not depend on the injective resolution.
...
%\end{remark}
\begin{remark}\label{dir_image_rem}
	Let $f : \sX \to \sY$ be a continuous map, and let $F$ be a set. If both $F_\sX$ and $F_\sY$ are constant presheaves  (cf. Definition \ref{constant_presheaf_defn}) on $\sX$ and $\sY$ then one has $\mathfrak{Ass}\left( F_\sX\right)  = f_*\mathfrak{Ass}\left( F_\sY\right)$ where 
	$\mathfrak{Ass}$  means the associated sheaf functor (cf. Definition \ref{site_sheaf_ass_defn}) and 	
	$f_*\mathfrak{Ass}\left( F_\sY\right) $ means the direct image (cf. Definition \ref{sheaf_inv_im_defn})
\end{remark}
...
\phi: \check{H}^\bullet\left(\sX, \mathscr P\otimes \mathscr P \right)\cong \check{H}^\bullet\left(\sX, \mathscr P \right).
	...
	\begin{lem}
		Let $\sX$ be a topological space. If $F$ is an Abelian group then any isomorphism $F \otimes_Z F \cong  F$ yields a product
		\bean
