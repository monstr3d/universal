...
A \xhookrightarrow{\widetilde \varphi} M\left(\widetilde A \right)
	\end{split}
	\ee
	be injective  hereditary full (cf. Definition \ref{hereditary_full_defn} homomorphisms of $\C$-algebras. If $\widetilde A \to B\left( \widetilde \H\right)$ ia a full faithful representation then left and right multipliers can be replaced with left and right multipliers respectively (cf. Proposition \ref{lrc_lem}). Suppose that following conditions hold.
...
\left(a', a'' \right)\mapsto \widetilde \phi \left(a' \right)  \widetilde \psi \left(a'' \right)
\end{split}
\ee
then from the bimodule condition \eqref{bimod_gen_eqn} it follows that
\bean
p\left(a'\varphi\left(a \right) , a'' \right)= p\left(a', \varphi\left(a \right) a'' \right)
\eean
so one has a map
...
\begin{split}
d: A'\otimes_A A'' \to B\left(\H'\ox_{A} \H'' \right)\\
a' \otimes a'' \mapsto \pi'\left(a'\right)\otimes \pi''\left(a''\right).
\end{split}
\ee
\end{empt}
\begin{lemma}
Let  $\widetilde A\subset B\left(\H'\ox_{A} \H'' \right)$ be the $C^*$-norm closure of $d\left(A'\otimes_A A'' \right)$. If $\widetilde A$ is a $C^*$-subalgebra then the natural the homomorphism $P: A' \otimes_A A'' \to \widetilde A$ of $A$-$A$ bimodules   is a {generalized Dixmier-Douady duality} (cf. Definition \ref{dd_dual_defn}).
\end{lemma}
\begin{proof}
...
\end{split}
\ee
so one has all homomorphisms described by the equations \eqref{dd_bimodule_eqn}. We leave to the reader the proof of that these homomorphisms satisfy to conditions \eqref{bimod_gen_eqn}-\eqref{bim_eqn}. From the equation \eqref{di_defn_eqn} it follows that $P$ is injective.  On the other hand from $P\left(  A'\ox_A A''\right)= d\left(A'\ox_A A'' \right)$ it follows that $P\left(  A'\ox_A A''\right)$ is dense in $\widetilde A$.
\end{proof}
