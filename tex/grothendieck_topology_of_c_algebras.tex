...
A \xhookrightarrow{\widetilde \varphi} M\left(\widetilde A \right)
	\end{split}
	\ee
	be injective  hereditary full (cf. Definition \ref{hereditary_full_defn} homomorphisms of $\C$-algebras. Suppose that following conditions hold.
		...
		\eea
		 If $\widetilde \pi:\widetilde A \to B\left( \widetilde \H\right)$ is a  faithful nondegenerate representation then left and right multipliers can be replaced with left and right multipliers respectively (cf. Proposition \ref{lrc_lem}), i.e. there are homomorphisms
		 \bean
		 ...
		 such that
		 \bean
\forall a' \in A'\quad \forall \widetilde a \in  \widetilde A \quad \widetilde \pi\left( \widetilde \phi\left(a' \right)\widetilde a \right)= \pi'\left( a' \right) \widetilde \pi \left( \widetilde a\right) .\\
 \forall a'' \in A''\quad \forall \widetilde a \in  \widetilde A \quad  \widetilde \pi\left( \widetilde a \widetilde \psi\left(a'' \right) \right)= \widetilde \pi \left( \widetilde a\right) \pi''\left( a'' \right) 	 
		 \eean
 If we define a pairing
\be\label{dd_pair_eqn}
\begin{split}
d: A' \times A''\to B\left( \widetilde \H\right) ,\\
...
\bean
d\left(a'\varphi\left(a \right) , a'' \right)= d\left(a', \varphi\left(a \right) a'' \right)
\eean
so one has a map
\bean
...
Consider the situation \ref{dd_dual_empt}. If 
	\bea\label{dd_in_eqn}
...
\eea
is a \textit{(generalized) Dixmier-Douady duality}.
\end{definition}
...
$\widetilde A\subset  B\left(\widetilde\H \right) $ be the $C^*$-norm closure of $P\left(A'\otimes_A A'' \right)\subset B\left(\H'\ox_{A} \H'' \right)\subset B\left(\widetilde\H \right)$. If $\widetilde A$ is a $C^*$-subalgebra of $B\left(\widetilde\H \right)$ and the inclusion $\widetilde A\subset B\left(\widetilde\H \right)$ is a faithful nondegenerate representation then the situation \ref{dd_hh_empt} yields a {Dixmier-Douady duality} $d: A'\otimes_A A''\to \widetilde A$  (cf. Definition \ref{dd_dual_defn}).
\end{lemma}
\begin{proof}
Using inclusions  $A \subset M\left( A'\right)$ and   $A \subset M\left( A''\right)$ one can find homomorphisms 
	\be\nonumber
\begin{split}
	A \xhookrightarrow{\phi} \mathbf{RC}\left(A' \right),\\
	A  \xhookrightarrow{\psi} \mathbf{LC}\left(A'' \right),\\
...
\end{split}
\ee
so one has all homomorphisms described by the equations \eqref{dd_bimodule_eqn}. We leave to the reader the proof of that these homomorphisms satisfy to conditions \eqref{bimod_gen_eqn}-\eqref{bim_eqn}.   On the other hand $P\left(  A'\ox_A A''\right)$ is dense in $\widetilde A$.
\end{proof}
...
\ee
If for any $x \in \sX$ the *-homomorphisms  $\pi_x^\dl: CT\left(\sX, \dl \right)\to \left( \H^\dl_x\right)$ and  $\pi_x^\rho: CT\left(\sX, \rho \right)\to \left( \H^\dl_x\right)$ are corresponding to $x$ irreducible representations and $\pi_a^\dl: CT\left(\sX, \dl \right)\to \left( \H^\dl_a\right)$ and $\pi_a^\rho: CT\left(\sX, \rho \right)\to \left( \H^\rho_a\right)$ are atomic representations (cf. Definition \ref{atomic_repr_defn}) then both $\H^\dl_a$ and $\H^\rho_a$ are completions of algebraic direct sums $\bigoplus_{x \in \sX}\H^\dl_x$ and $\bigoplus_{x \in \sX}\H^\dl_x$. The user can proof that the norm completion of $\H^\dl_a\ox_{C_0\left(\sX\right)}\H^\rho_a$ is a norm completion of an algebraic direct sum
	$$
\bigoplus_{x \in \sX}\H^\dl_x\ox_{\C}\H^\rho_x.	
	$$
	If $a^\dl \in CT\left(\sX, \dl \right)$ and $a^\rho \in CT\left(\sX, \rho \right)$  then $\pi^\dl_a\left( a^\dl\right) \otimes \pi^\rho_a\left( a^\rho\right)$ corresponds to a family
\be
	\left\{\pi^\dl_x\left( a^\dl\right) \otimes \pi^\rho_x\left( a^\rho\right)\right\}_{x \in \sX}.
\ee
From the proof of the Proposition \ref{ctr_d_prop} it follows that the family corresponds to an element of $\widetilde a \in CT\left(\sX, \dl + \rho\right)$. If $\widetilde \H_x$ is the completion of  $\H^\dl_x\ox_{\C}\H^\rho_x$ then there is the natural irreducible representation $\widetilde \pi_x: \in CT\left(\sX, \dl + \rho\right)\to B\left(\widetilde \H_x\right)$. 


Taking into account that for any $x \in \sX$ a natural representation $\widetilde A \to B\left(\ \right)$  
...
The situation \ref{dd_ctr_empt} there is a {generalized Dixmier-Douady duality} (cf. Definition \ref{dd_dual_defn}).
\end{lemma}
\begin{proof}
...
\eea
where $f, g \in \Ga_c\left(\G\left(M, \sF\right),\Om^{1/2}  \right)$ (cf. Appendix \ref{foli_alg_subsec}). Similarly to \eqref{groupoid_*_c_eqn} and \eqref{groupoid_inv_c_eqn} the equations  \eqref{foli_a_prod_eqn}, \eqref{foli_a_inv_eqn} yield a *-algebra, we denote it by  $\Ga_c\left(\G\left(M, \sF\right),\Om^{1/2} , \sigma \right)$. 
...
\ee
where $f_\L\in C_c^\infty\left(\L^2 \right) $,  $~g_\L\in C_c^\infty\left(\L^2 \right)$ and $\sigma_\L: \L^3 \to \T$ correspond to $f$, $g$ and $\sigma$. The multiplication \eqref{foli_l_m_eqn} corresponds to the action
\be\label{foli_act_egn}
\begin{split}
\bullet_{\sigma} :C_c^\infty\left(\L^2 \right)\times C_c^\infty\left(\L \right)\to C_c^\infty\left(\L \right),\\
\left(f_\L \bullet_{\sigma} u_\L \right) \left(x \right)\bydef \int
f_\L(x, y) \, u_\L(y) \, \sigma_\L\left(x, y, y \right)\d \mu^{y}_\L.
\end{split}
\ee
The space $C_c^\infty\left(\L \right)$ is dense in $L^2\left(\L \right)= C_c^\infty\left(\L, \mu_\L \right)$, so the action \eqref{foli_act_egn} can be uniquely extended up to
\be\label{foli_act_l2_egn}
\begin{split}
	\bullet_{\sigma} :C_c^\infty\left(\L^2 \right)\times L^2\left(\L \right)\to L^2\left(\L \right).
\end{split}
\ee
...
If $\pi_a: C^*_r\left(M,\sF, \sigma\right)\hookto B\left(\H_a \right) $ is the atomic representation (cf. Definition \ref{atomic_repr_defn}) then $C^*_r\left(M,\sF, \sigma\right)$ is the completion of $\Ga_c\left(\G\left(M, \sF\right),\Om^{1/2} , \sigma \right)$ with respect to the norm $a \mapsto \left\|\pi_a\left(a \right)  \right\|$.
...
$$
If  $a^\dl\in \Ga_c\left(\G\left(M, \sF\right),\Om^{1/2} , \dl \right)$, and $a^\rho\in \Ga_c\left(\G\left(M, \sF\right),\Om^{1/2} , \rho \right)$ then for any leaf $\L$ there are $a^\dl_\L \in C_c\left(\L\times \L \right)$ and  $a^\rho_\L \in C_c\left(\L\times \L \right)$ such that for any $u \in \Coo_c\left(\L\right)$ one has
\bean
