...
\begin{definition}\label{grothendieck_ca_t_defn}
	If $A$ is a $C^*$-algebra such that there is a topological space $\mathscr X^A$ such that both sites $\mathbf{T}_{A}$ arising from $A$ (cf. Definition \ref{grothendieck_ca_defn}) and $\mathbf{T}_{\mathscr X^A}$ arising from $\mathscr X^A$ (cf. Definition \ref{top_gro_definition}) are equivalent then we say that $\mathscr X^A$ is the \textit{Grothendieck} $A$-\textit{space}.
\end{definition}
\begin{remark}
...
\end{definition}
\begin{definition}
An inclusion $C_0\left( \sX\right) \hookto M\left(A \right)$ into multiplier $C^*$-algebra  with  a net $\left\{u_\la \right\}_{\la \in \La}\subset C_0\left(\sX \right)$ such that
$$
...
\begin{empt}
For any open subset $\sU$ denote by $_\sU A_\sU$ a generated by the set $C_0\left(\sU\right) AC_0\left(\sU\right)$ hereditary subalgebra of $A$. We define a presheaf of sets $\mathscr{P}^A $ on $\sX$ (cf. Definition \ref{presheaf_defn}) such that for any pair  $\sU\subset\sV$ of open subsets of $\sX$ one has
...
If $C^0\subset A$ is the  trivial hereditary $C^*$-subalgebra, then the set $\mathscr U'_{C^0}$ 
\bean
\mathscr X^A \bydef\quad  \text{the maximal open subset of }\quad \mathrm{Sp\acute{e}}\left(\mathscr P^A \right)\setminus \mathscr U'_{C^0}
\eean
and
...
$$
(cf. Exercise \ref{sheaf_etale_exer}). If $s\left(x \right)$ is known for every $x \in p'\circ \left(\mathscr X^A\right)$ such that $s\left(x \right)\in \mathscr X^A$ then  $s\left(x \right)= C^0_x$ for all $x \in p'\circ s\left( \mathscr U'_{C^0}\right)$. It follows that the restriction $s|_{p'\left( \mathscr X^A\cup\mathscr U'_{C^0}\right)  }$ known, taking into account that $ \mathscr X^A\cup\mathscr U'_{C^0}$ is dense in $\mathrm{Sp\acute{e}}\left(\mathscr P^A \right)$ one has the unique section $s : \sX \to \mathrm{Sp\acute{e}}\left(\mathscr P^A \right)$. Thus any commutative hereditary algebra can be uniquely defined by the open set $\mathscr U_C$.



If $B$ is a hereditary $C^*$-subalgebra of $B$ and
$$
...
\end{empt}
 ...
 If $A$ is a {hereditary commutative generated} $C^*$-algebra with  {Hausdorff blowing-up} $C_0\left( \sX\right) \hookto M\left(A \right)$ then $\mathscr X^A$ is the Grothendieck $A$-space. (cf. Definition \ref{grothendieck_ca_t_defn}).
\end{lemma}
\begin{proof}
...
between hereditary subalgebras of $A$ and open subsets of $\mathscr X^A$. So a category of open hereditary subalgebras of $B$ and their inclusions is equivalent to the category of open subsets of $A_{\mathscr X^A_B}$ and their inclusions.
 ...
 If  $\left\{\mathscr U_\iota\right\}_{\iota \in I}$ is a family of open sets of $ A_{\mathscr X^A_B}$ such that $\mathscr U = \cup \mathscr U_\iota$ then for all $\iota \in I$ one has  $A_{\mathscr U_\iota}\subset  A_{\mathscr U}$ It turns out that
\bean
...
If  $\left\{B_\iota \subset  A \right\}_{\iota \in I}$ is a family of hereditary $C^*$-subalgebras, and $B$ is a generated  by the union $\cup_{\iota \in I}B_\iota$ hereditary subalgebra then from the equation \ref{one_to_one_eqn} it follows that  $B_\iota=A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_{B_\iota}}$ and $B =A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_{B}}$. From the inclusion  $B_\iota \subset B$ it follows that $A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_{B_\iota}}\subset A_{\mathscr X^A_B}$. It turns out that $\mathscr X^A_{B_\iota}\subset \mathscr X^A_B$ and $\cup_{\iota \in I}\mathscr X^A_{B_\iota}\subset \mathscr X^A_B$. On the other hand from $\mathscr X^A_{B_\iota}\subset \cup_{\iota \in I}\mathscr X^A_{B_\iota}$ it follows that 
$$
...
\end{exercise}
\begin{definition}
	For any commutative hereditary  $C^*$-subalgebra $C \subset A$ an intersection of all subsets $\sU$ such that $C\subset ~_\sU A _\sU$ is said to be the \textit{support} of $C$. We write $\sU = \supp C$. A set of all supports is said to be a \textit{family of supports} $\mathscr K^A$. For any $k \in \mathscr K^A$ denote by
	\be
	\mathscr K^A_k = \left\{C \subset A| C \text{is commutative } \supp C = k \right\}
	\ee
\end{definition}
\begin{definition}
Let $A', A'', A$ be We say that $A$ is a \textit{local product} of $A'$ and $A''$ if 
\begin{enumerate}
	\item $\mathscr K^A=\mathscr K^{A'}= \mathscr K^{A'}$
	\item For all $x\in \sX$ there is an open ne $\sU$ such that for all $k \in \mathscr K^A$ such that $k \subset \sU$ there is a bijective map
	$$
\mathscr K^{A'}_k	\times \mathscr K^{A''}_k\cong \mathscr K^A_k.
	$$
\end{enumerate}
 ...
 \begin{theorem}
 	...
 	$$
  \mathscr X^A \cong  \mathscr X^{A'} \times_{ \mathscr X}	 \mathscr X^{A''}
 	$$
 \end{theorem}
...
Let  $\mathrm{Sp\acute{e}}\left(\mathscr S  \right)$ be the	\'espace etal\'e  (cf. Exercise \ref{sheaf_etale_exer}) of the sheaf $\mathscr S$. The empty set $\emptyset$ yields a global  section $s^\emptyset \in \mathscr S\left(\sX \right)$, this section gives an open subset $\mathrm{Sp\acute{e}}\left(\mathscr S  \right)_\emptyset \subset \mathrm{Sp\acute{e}}\left(\mathscr S  \right)$. Denote by $\mathscr X$ a maximal open subset of $\mathrm{Sp\acute{e}}\left(\mathscr S  \right)\setminus\mathrm{Sp\acute{e}}\left(\mathscr S  \right)_\emptyset$.
...
$$
\begin{exercise}
Prove that if $\mathscr X^A$ is given by the Lemma \ref{grot_com_lem}  Grothendieck $A$-space. (cf. Definition \ref{grothendieck_ca_t_defn}) then $P'_A$ maps $\mathscr X^A$ onto $\mathscr X$. Moreover the natural surjective map
\be
P_A: \mathscr X^A\to \mathscr X.
\ee
...
\paragraph*{}
In the situation of the Lemma \ref{grot_com_lem}
there are natural equivalences of categories of presheaves and sheaves
\bean
\begin{split}
	\mathbf{PreSh}\left(\mathbf{T}_{A} \right) \cong 	\mathbf{PreSh}\left(\mathbf{T}_{\mathscr X^A} \right),\\
	\mathbf{Sh}\left(\mathbf{T}_{A} \right) \cong 	\mathbf{Sh}\left(\mathbf{T}_{\mathscr X^A} \right),\\
	\mathbf{Ab}\left(\mathbf{T}_{A} \right) \cong 	\mathbf{Ab}\left(\mathbf{T}_{\mathscr X^A}\right)
\end{split}
\eean
...
\bea\label{hom_eqn}
H^r\left(\mathscr X^A,F\right)\cong \check{H}^r\left(A, F \right),\\
\label{chech_hom_eqn}
\check H^r\left(\mathscr X^A, F\right)\cong \check{H}^r\left(A, F \right)
\eea
