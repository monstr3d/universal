...
\end{definition}
\begin{empt}
Let $CT\left(\sX, \dl \right) \to B\left( \H\right)$ be the atomic representation  (cf. Definition \ref{atomic_repr_defn}). For any hereditary $C^*$-subalgebra denote by $P_B \subset B\left( \H\right)$ a set of projectors such that 
$$
p \in P_B \quad \Leftrightarrow \quad pB = \{0\}. 
$$
From the Lemma \ref{increasing_convergent_w_lem} it follows that there is the maximal element $p_B \in B$. Also denote by 
\be\label{ctr_sh_of_alg}
A\left( B\right)  \bydef p_B CT\left(\sX, \dl \right) p_B \subset B\left( \H\right). 
\ee
It is clear that $A\left(B \right)$ is a $C^*$-algebra and there is a surjective *-homomorphism $A \to A\left( B\right)$. Given $B', B'' \in  \mathbf{Hered}/CT\left(\sX, \dl \right)$ such that $B' \subset B''$ (i.e. there is $\mathbf{Hered}/CT\left(\sX, \dl \right)$-morphism $B'' \to B'$) there is   the natural surjective  *-homomorphism
\be\label{ctr_psh_of_alg}
A^{B'}_{B''}:A\left( B'\right) \to A\left( B''\right)
\ee 
It turns out that a map
\be\label{ctr_psh_defn}
B \mapsto A\left( B\right) 
\ee
is a $\mathbf{Hered}/CT\left(\sX, \dl \right)$-presheaf (cf. Definitions \ref{presheaf_defn} and \ref{hered_cat_defn}).
\end{empt}
...
\end{enumerate}
(i) Any $a \in A\left( B\right)$ is given by $p_B \widetilde a p_B$  where $\widetilde a \in CT\left(\sX, \dl \right)$. However if  $p_{B_\iota}\widetilde a p_{B_\iota}= 0$ for every $\iota = 1,..., n$ then $p_B \widetilde a p_B = 0$.\\
(ii)  Let  $P \subset  \left\{1, ..., n\right\}^2$ be such that for any $\left(\iota, j\right)\in P$ the generated by a union $B_\iota \cup B_j$ hereditary $C^*$-algebra $B_{\iota j}$ is not $\mathbf{Hered}/CT\left(\sX, \dl \right)$. It follows that $\im p_{B_\iota}\cap \im p_{B_j}\neq \{0\}$ for any $a \in A\left( B\right)$ then there is $\widetilde a \in CT\left(\sX, \dl \right)$ such that $a = p_B \widetilde a p_B$. If $a_\iota \bydef A^B_{B_\iota} \left(a \right)$ then $a_\iota =  p_{B_\iota} \widetilde a   p_{B_{\iota}}$. 
  One should prove that $A^{B_\iota}_{B_{\iota j}}\left( a_\iota \right) = A^{B_j}_{B_{\iota j}}\left( a_j \right)$. This fact follows from the following equation
\bean
A^{B_\iota}_{B_{\iota j}}\left( a_\iota \right)= A^{B_j}_{B_{\iota j}}\left( a_j \right)= p_{B_\iota j} \widetilde a   p_{B_{\iota j}}
\eean 
\end{proof}
Let $U \bydef U\left( M\left(CT\left(\sX, \dl \right)\right)\right) $ be a group of unitary elements of multiplier $C^*$-algebra.  If $B \subset CT\left(\sX, \dl \right)$ is a hereditary $C^*$-subalgebra  and  $L\left( B\right)$ is a given by the equation  \eqref{left_ideal_eqn} left ideal then From $M\left(CT\left(\sX, \dl \right) \right) L\left( B\right)= L\left( B\right)$ it follows that there is a natural surjective group homomorphism
$$
U \to U\left( CT\left(\sX, \dl \right)/ L\left( B\right) \right) 
$$
where  $U\left( CT\left(\sX, \dl \right)/ L\left( B\right) \right)$ is a subgroup of $\Aut_{C_0\left( \sX\right)} \left(  CT\left(\sX, \dl \right)/ L\left( B\right)\right)$. The following notation  
\be
U^A \left(B \right) \bydef U\left( CT\left(\sX, \dl \right)/ L\left( B\right) \right) 
\ee
will be used. 
...
$$
a \notin L\left(B \right) \quad  \widetilde u a \neq a.
$$
From $L\left( B\right)= \bigcap_{\iota =1}^n L\left( B_\iota\right)$ it follows that there is $\iota$ such that $a \notin L\left(B_\iota \right)$ so is $p^{B}_{B_\iota}$ is given by \eqref{ctr_ub_eqn} then $p^{B}_{B_\iota}\left( u\right) \neq 1_{U\left( CT\left(\sX, \dl \right)/ L\left( B\right) \right)}$.\\
  (ii) Let  $P \subset  \left\{1, ..., n\right\}^2$ be such that for any $\left(\iota, j\right)\in P$ the generated by a union $B_\iota \cup B_j$ hereditary $C^*$-algebra $B_{\iota j}$ is not $L \left( B_\iota\right)+ L \left( B_j\right)$. Equivalently $L\left( B_{\iota j}\right) = L \left( B_\iota\right)+ L \left( B_j\right)\neq CT\left(\sX, \dl \right)$ or $CT\left(\sX, \dl \right)/\left(L \left( B_\iota\right)+ L \left( B_j\right) \right)\neq 0$, so $U\left(B_{\iota j} \right)\neq \{1\}$ . If $u \in U\left( B\right)$ and $\widetilde u \in U$ is a representative  of $u$ then for any $\iota = 1, ...,n$ the element $u_\iota \bydef p^B_{B_\iota}\left( u\right) $ is represented by $\widetilde u$. One should prove that $p^{B_\iota}_{B_{\iota j}}\left( u_\iota \right) = p^{B_j}_{B_{\iota j}}\left( u_j \right)$. This fact follows from the following equation
 \bean
  p^B_{B_{\iota j}}=  p^{B_\iota }_{B_{\iota j}}\circ  p^B_{B_\iota}=  p^{B_j }_{B_{\iota j}}\circ  p^B_{B_j}.
 \eean 
\end{proof}
\begin{remark}
For any open subset $\sU \subset \sX$
\end{remark}
\begin{empt}
...
%\end{remark}
	...
	\begin{definition}\label{constant_presheaf_defn}\cite{johnstone:topos}
		...
		F_X \bydef \mathscr P.
		\ee
		A sheaf associated (cf. Definition \ref{associated_sheaf_defn}) with a constant sheaf will is named a \text{constant sheaf}. We use the same notation $F_X$ for a constant sheaf. The difference between the constant presheaf and the constant sheaf depends from the context.
	\end{definition}
...
\end{lemma}
	...
	When $\hat A$
	is a (locally compact) Hausdorff space $T$, we can localize at a point $t$ â that is,
	examine behavior in a neighborhood of $t$ â either by looking at the ideal $A_\sU$
