...
\paragraph{}
Let $A$ be a $C^*$-algebra with the Hausdorff spectrum $\sX$. Consider a pair $\left(\pi_x, e_x\right)$ where $\pi_x : A \to B\left(\H_x \right)$ is a corresponding to $x \in \sX$ irreducible representation and $e_x \in B\left(\H_x \right)$ is a rank-one projector, i. e. $e_x = \xi\left\rangle \right\langle\xi$. 
...
From 
	...
	$$
	i.e. $B_{\left(\pi_x, e_x\right)}$ satisfies to \eqref{pprime_eqn}. If $B\notin p^{-1}_{\left(\pi_x, e_x\right)}\left(0\right)$ then there is $b \in B$ such that $\pi_x\left(b\right)\H_x \not\perp e_x$. Moreover there is a rank-one projector $y\in \xi\left\rangle \right\langle\xi$
	
	
	 or  equivalently $\left(\pi_x\left(b \right) , e_x \right)_{\H_x} \neq 0$ where $\left(\pi_x\left(b \right) , e_x \right)_{\H_x}$ is a scalar product on $\H_x$.
	
	There is $y\in \pi_x\left(B_{\left(\pi_x, e_x\right)}\right)$ such that $\pi_x \left(b \right) + y = e_x$. So there is $b' \in B_{\left(\pi_x, e_x\right)}$ such that $\pi_x\left(b' \right) = y$ and $\pi_x\left(b' + b\right) = e_x$. If $a \in A$ is any element such that $\left(\pi_x\left(a \right) , e_x \right)_{\H_x} = k$ 
	
	
	then there if $a'\in B_{\left(\pi_x, e_x\right)}$
	
	
	
	Moreover $B_{\left[C, \I\right]}$ is a maximal among hereditary $C^*$-subalgebras $B$ such that
	$
	B \cap C \subset \I
	$.
