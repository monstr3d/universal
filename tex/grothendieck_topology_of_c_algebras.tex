...
\end{lemma}
\begin{proof}
If  $B \in \mathbf{Hered}/A$ and $\left\{B_\iota \to B\right\}_{\iota= 1,...,n}$ is a covering then from the Definition \ref{pullback_covering_ca_defn} it follows that $B = \bigcap_{\iota=1}^n B_\iota$. Element $u \in U\left( B \right)$ is trivial if for any $\iota= 1,...,n$ the image of $u$ in $ U\left( B_\iota \right)$ is trivial. If one!!!!
\end{proof}
