...
\paragraph{} Let $A \bydef CT\left(\sX, \dl \right)$ the stable
continuous-trace algebra  with DixmierÐDouady class $\delta \in \check{H}^3\left(\sX,\Z \right)$ (cf. Notation \ref{ctr_not}). Suppose that the space $\sX$ is compact. Let $U \bydef U\left( M\left(A\right)\right) $ be a group of unitary elements of multiplier $C^*$-algebra.  For any  hereditary $C^*$-subalgebra $B \subset A$ denote by  $L\left( B\right)$ is a given by the equation  \eqref{left_ideal_eqn} left ideal of $A$. Let 
\be\label{ctr_perp_eqn}
\begin{split}
L^\perp\left( B\right) \text{ is the }C^*-\text{norm completion of the union of left ideals } L \subset A \\\text{ such that }L \cap  L\left( B\right)= \{0\}
\end{split}
\ee
$L^\perp\left( B\right)$ is the $C^*$-norm completion of the union of left ideals $L \subset A$ such that $L \cap  L\left( B\right)= \{0\}$. For any hereditary $C^*$-subalgebra $B \subset A$ there is a subgroup
\bean
...
or $v  U_B v^{-1} =  U_B$, i.e. $U_B$ is a normal subgroup of $U$.
 If for any $B', B'' \in  \mathbf{Hered}/A$ such that $B' \subset B''$ (i.e. there is $\mathbf{Hered}/A$-morphism $B'' \to B'$) there are  natural inclusions $L\left({B'}\right)\subset L\left({B''}\right)$, $~L^\perp\left({B''}\right)\subset L^\perp\left({B'}\right)$ and $U_{B'}\subset U_{B''}$. So if $U\left( B\right)\bydef U / U_B$ then there  the natural surjective  homomorphism of groups
\be\label{ctr_ub_eqn}
U\left(B'\right) \xrightarrow{p^{B'}_{B''}} U\left(B'' \right). 
\ee
When  the map $B \mapsto  U\left( B \right)$ is a presheaf of groups on the category $\mathbf{Hered}/A$ (cf. Definitions \ref{hered_cat_defn} and  \ref{presheaf_defn}).
\begin{lemma}
The  map $B \mapsto  U\left( B \right)$ is a sheaf with respect to the Gelfand $A$-topology (cf. Definition \ref{gelfand_a_site_defn}).
\end{lemma}
\begin{proof}
...
\end{enumerate}
(i) If $u \in U\left( B\right)$ is not trivial and $\widetilde u \in U$ is a representative of $u$ such that
$$
