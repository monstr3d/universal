...
\begin{definition}\label{dd_dual_defn}
	We say that the situation \ref{dd_dual_empt} is a \textit{generalized Dixmier-Douady duality} if a following condition holds
	\bea\label{dd_in_eqn}
...
Let  $\widetilde \H$ be the norm completion of the pre-Hilbert space $\H' \otimes_A \to \H''$, and let
$\widetilde A\subset  B\left(\widetilde\H \right) $ be the $C^*$-norm closure of $P\left(A'\otimes_A A'' \right)\subset B\left(\H'\ox_{A} \H'' \right)\subset B\left(\widetilde\H \right)$. If $\widetilde A$ is a $C^*$-subalgebra of $B\left(\widetilde\H \right)$ and the inclusion $\widetilde A\subset B\left(\widetilde\H \right)$ is a faithful nondegenerate representation then the situation \ref{dd_hh_empt} yields a {generalized Dixmier-Douady duality} (cf. Definition \ref{dd_dual_defn}).
\end{lemma}
\begin{proof}
...
\end{split}
\ee
where $f, g \in \Ga_c\left(\G\left(M, \sF\right),\Om^{1/2}  \right)$ (cf. Appendix \ref{foli_alg_subsec}). Similarly to \eqref{groupoid_*_c_eqn} and \eqref{groupoid_inv_c_eqn} the equations  \eqref{foli_a_prod_eqn} yield a *-algebra, we denote it by  $\Ga_c\left(\G\left(M, \sF\right),\Om^{1/2} , \sigma \right)$. 
...
\be\nonumber
\begin{split}
	d: C^*_r\left(M,\sF, \dl\right)\otimes_{C_0\left( M\right) }C^*_r\left(M,\sF, \rho\right) \to B\left(\H^\dl_a\ox_{C_0\left( M\right)} \H^\rho_a \right),\\
...
$$
d_\L : L^2\left(\L\right) \times  L^2\left(\L\right)\to L^2\left(\L\right) \ox_{C_0\left( M\right)}  L^2\left(\L\right)\cong L^2\left(\L\right)
$$
...
\bean
a^\dl u\left( x\right)  = \int a^\dl \left(x, y\right) \dl\left(x, y \right)  u\left(x \right) ~dx,\\
a^\rho u\left( x\right)  = \int a^\rho \left(x, y\right) \rho\left(x, y \right)  u\left(x \right) ~dx
\eean
