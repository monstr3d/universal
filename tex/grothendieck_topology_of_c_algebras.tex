...
If $A = CT\left(\sX, \dl \right)$ (cf. Notation \ref{ctr_not})  then we define a presheaf $\mathscr{P}'^\dl $ on $\sX$ (cf. Definition \ref{presheaf_defn}) such that for a pair  $\sU\subset\sV$ of open subsets of $\sX$ one has
\bean
...
A $C^*$-algebra 	$A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}$ is generated by subalgebras of $B$ so one has an inclusion	$A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}\subset B$, i.e.    $A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}$ is a hereditary subalgebra of $B$.
	...
	\begin{itemize}
		\item if $\pi_x: B \to B\left(\H_x\right)$ is a representation which corresponds to $x\in \sX_B$ (cf. Remark \ref{ctr_spe_rem}) then
		$$
