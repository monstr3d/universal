...
Following conditions hold:
	\begin{enumerate}
		\item[(i)] If $\sX$ is a Hausdorff, locally compact space then there is a faithful functor  (cf. Definition \ref{funct_full_faithfull_defn})
		$$
		...
		\item[(ii)] For any $\mathscr F \in \mathbf{Ab}\left(\sX  \right)$ and $p \ge 0$ one has the natural isomorphism
		$$
	H^p\left(\sX, \mathscr F  \right) 	\cong H^p\left(\mathfrak{Locale} \left( C_0\left(  \sX\right)\right),  F\left(\mathscr F\right) \right).
		$$
	\end{enumerate}
\end{theorem}
\begin{proof}
(i) From the Lemma \ref{comm_alg_loc_lem} it follows that one should construct a faithful functor $F^{\mathrm{top}}_X : \mathbf{Ab}\left(\sX  \right) \hookto \mathbf{Ab}\left(\mathfrak{Adjoint} \left( \sX  \right)\right)$. For any  $\mathscr F \in \mathbf{Ab}\left(\sX  \right)$ and any $\sU \in  \mathfrak{Adjoint} \left( \sX  \right)$ we define 
$F_X \left( \mathscr F\right) \left(\sU \right)$ as group of continuous maps    $f:\sU\to \mathrm{Sp\acute{e}}\left(\mathscr F \right)$ to the	\'espace etal\'e such that $p\circ f = \Id_\sU$ (cf. Exercise \ref{sheaf_etale_exer} and the Remark \ref{sheaf_rem}). If $\sV \subset \sU$
then the required by the Definition \ref{presheaf_defn} map $F_X \left( \mathscr F\right) \left(\sU \right)\to F_X \left( \mathscr F\right) \left(\sV \right)$ is given by $f \mapsto f|_\sV$.
...
In other words, a cosheaf is a $\V^{\mathrm{op}}$-valued sheaf. 
\end{definition}
