...
0\to \mathscr A  \xrightarrow{\eps}\mathscr A^0  \xrightarrow{d}\mathscr A^{1}  \xrightarrow{d}\mathscr A^2\xrightarrow{d}\dots \xrightarrow{d}\mathscr A^n \to 0
 ...
 \ee
 The differential sheaf $\mathscr L^*$ is a $c$-soft {quasi}-
 $n$-{resolution} of $L$ (cf. Definition \ref{top_quasi_res}). From the Theorem \ref{top_from_sh_to_cosh_thm} one has a s a flabby quasi-$n$-coresolution of $L$. Also,
...
\
	...
	and let $M \to H^0\left( \mathscr L^*\right)$  be a homomorphism of presheaves (i.e., $M \to 
	H^0\left( \mathscr L^*\left( 
	\sU\right) \right)$  for all open $\sU$). Then $\mathscr L^*$, together with $\eps$, is called a \textit{quasi}-
	$n$-\textit{resolution} of $M$ provided that
	\begin{enumerate}
		\item[(a)]  $\mathscr L^*$ is bounded below,
		\item[	(b)] $\mathscr H^p\left( \mathscr L^*\right)= 0$  for all $0 \neq p \le  n$,
		\item[	(c)] $\eps$ induces an isomorphism $M \to \mathscr H^p\left( \mathscr L^*\right)$  of sheaves.
	\end{enumerate}
	
	Similarly, let $\mathfrak{L}_*$ be a differential cosheaf and $\eta: H_0\left( \mathfrak L_*\right)  \to L$ a homomorphism
	of precosheaves. Then $\mathfrak{L}_*$, together with $\eta$, is called a \textit{quasi}-$n$-\textit{coresolution}
	of $L$ provided that
	\begin{enumerate}
		...
		\item[	(c)] $\eta$ is a local isomorphism of precosheaves.
	\end{enumerate}
...
\end{remark}
\begin{empt}
Let  $\sX$ be a closed subset of  $\sY$ and a set  $\sY$ is locally connected.  Let $\mathscr {L}\bydef \mathfrak{Ass}\left(L_\sY \right)$ be a sheaf associated with constant presheaf (cf. Definitions \ref{constant_presheaf_defn}, \ref{associated_sheaf_defn}). From the Proposition \ref{precosh_sh_prop} it follows that the map
$$
...
points of A have disjoint open neighborhoods in X.
	...
	\begin{proposition}
		PAGE 282 
		1.6 Proposition. 
		If $\mathscr L$ is a $c$-soft sheaf then the precosheaf $\Ga_c\mathscr L$, where
		$$
