...
\paragraph{}
	...
	Let 4) be a family of supports on X and let L be a fixed
	ground ring with unit. We let dime,L X be the least integer n (or oo) such
	that H (X; 4) = 0 for all sheaves 4 of L-modules and all k > n.
\end{definition}
\begin{remark}  PAGE 69
	The following result shows that softness is a "local" property. Recall
	that 
	\be\label{top_ep_eqn}
...
\end{remark}
\begin{theorem}
	16.4. Definition. PAGE 111 Let 4) be a family of supports on X and let L be a fixed
	ground ring with unit. We let dime,L X be the least integer n (or oo) such
	that H (X; 4) = 0 for all sheaves 4 of L-modules and all k > n.
...
\begin{definition}\label{top_loc_para_defn}
	PAGE 112 We say that a space X is locally paracompact if it is Hausdorff and each
...
\begin{proposition}\label{top_loc_para_prop}
	PAGE 112 16.5. Proposition. If 4i and T are paracompactifying and E(4) C E(LY),
	then any 41-soft sheaf is 4i-soft and dim4,,L X < dim,y,L X.
\end{proposition}
\begin{definition}\label{top_diml_defn}
	PAGE 112.
	X is locally paracompact then we define (using \ref{top_loc_para_prop})
	$\dim_L\sX$ = dime,L X, where 4i is paracompactifying and E(4)) = X.
\end{definition}
\begin{proposition}
	PAGE 285
...
\begin{enumerate}
	\item[(i)] (i) The natural map $\lim_K  H_*\left(K , M\right)\to H_*\left(\sX , M\right)$ is an isomorphism.
	KE-D
	(ii) The canonical map H!(X;,Itf) -* is an isomorphism
	for any replete resolution T' of L.
\end{enumerate}

If t = c, then these statements hold for arbitrary sheaves x.11\end{theorem}
\begin{definition}\label{top_ndiml_defn}\cite{bredon:sheaf}
	PAGE 329 9.1. Definition. 
	
	A locally compact space X is called an $n$-\textit{dimensional
		weak homology manifold over} $L$ (abbreviated as $n-whm_L$) if $\dim L X < \infty$
	and $\mathscr H_p (X; L) = 0$ for $p\neq n$ and is torsion-free if $p=n$.  $\mathscr H_p (X; L) = 0$) is
	called the \textit{orientation sheaf} of X and will be denoted by $\mathscr O = \mathscr O_\sX$. An
	n-whmL X is said to be an $n$-\textit{dimensional homology manifold over} $L$
	(abbreviated as n-hmL) if 6 is locally constant18 with stalks isomorphic to
	L. It is said to be\textit{orientable} if 0 is constant.
\end{definition}
...
\begin{theorem}
	...
	Theorem. Let X be a connected n-cmL, let F (; X be a proper
	closed subset, let U C X be any nonempty open subset, and let .Al be an
	elementary sheaf on X. Then:
	\begin{enumerate}
		\item (a) %If o1 has stalks L, then Hn(F;.Af) = 0 and Hn_1(F;.dl) as torsionfree;
		\item[	(b)] $H^n_c\left(\mathscr O \right)$  is the free $L$ -module on the components of $\sU$;
		\item[	(c)] H, (F; .AO = 0;
		(d) jX u : Hrn(U;.A1) Hn(X; A) is surjective;
		(e) jX u : Hrn (U; 0) - H2 (X; B) is an isomorphism if U is connected;
		§16. Generalized manifolds 381
...
\begin{empt}\label{top_taut_empt}\cite{spanier:at}
	...
	\begin{empt}
		PAGE 292,\
		Consider the canonical injective resolution ,f (X; L) of L (see 11-3) and
		note that J'(X;L)IU = ,4'(U;L) for U C X open. For a sheaf 4 on X,
		we define
...
\end{split}		
		...
		and
		Hp (X; d) = Hp(C?(X;,d)).
		'Caution: Hp(X;4) corresponds to homology based on infinite, locally
		finite chains, while HH(X;4) is analogous to classical homology.] Recall
		that by our notational conventions,
	\end{empt}
