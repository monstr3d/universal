...
A hereditary $C^*$-subalgebra $B \subset A$ is said to be \textit{coconnected} if there are no hereditary $C^*$-subalgebras $B'\subset $ and $B''\subset$ such that  $B = B' \cap B''$ and $A$ is generated by $B'\cup B''$ hereditary $C^*$-subalgebra of $A$.
\end{definition}
...
\begin{lemma}\label{const_sheaf_morphism_lem}
	...
	F_{\mathfrak{Site}\left(\widetilde  A\right) }= \mathfrak{Topos}(\varphi)^* F_{\mathfrak{Site}\left(  A\right) }
	\ee
