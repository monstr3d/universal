...
\ee
 ...
 \eea
	...
	\varphi_{\a'\a''} : \K\left( L^2\left( \mathcal N_{\a', \a''}\right)\right) \otimes C_0\left(\sZ_{a'\a''} \right)\hookto \K\left( L^2\left( \mathcal N_{\a'}\right)\right) \otimes C_0\left(\sZ_{\a'} \right),\\
		\varphi_{\a''\a'} : \K\left( L^2\left( \mathcal N_{\a', \a''}\right)\right) \otimes C_0\left(\sZ_{a'\a''} \right)\hookto \K\left( L^2\left( \mathcal N_{\a''}\right)\right) \otimes C_0\left(\sZ_{\a''} \right)
	\eean
	which are induced by an inclusion \eqref{foli_hm_inc_eqm}.
...
\forall a \in \K\left( L^2\left( \mathcal N_{\a', \a''}\right)\right) \otimes C_0\left(\sZ_{a'\a''} \right)\quad		\phi^\sigma_{\a',\a''}\left(a \right) = \phi^\sigma_{\a'',\a'}\left(\overline\Phi^\sigma_{\a'\a''}\left(a \right)  \right).
		$$
		where  $\phi^\sigma_{\a',\a''}\left(a \right)\in \K\left(  L^2\left( \mathcal N_{\a'}\right)\right)  \otimes C_0\left(\sZ_{\a'\a''}\right)$ and $\phi^\sigma_{\a'',\a'}\left(\overline\Phi^\sigma_{\a'\a''}\left(a \right)  \right)\in \K\left(  L^2\left( \mathcal N_{\a''}\right)\right)  \otimes C_0\left(\sZ_{\a''}\right)$
	\end{itemize}
...
From the Proposition \ref{cohom_groupoid_prop} and the Exercise \ref{ca_exer} it follows that $C^*\left(G_\sigma, R_\sigma \right)$ up to $*$-isomorphism depends on a class $[\sigma]$ of the cycle $\sigma$ in $H^2\left( \G\left(M,\sF, \sigma\right) \right)$. 
\end{remark}


If $\pi: C^*_r\left(M,\sF, \sigma\right)\to B\left(\H_\pi \right)$ is an irreducible representation then from the Theorem \ref{foli_irred_hol_thm} it follows that $\sH_\pi \cong L^2\left( L\right)$ where $L\subset M$ is leaf  a having no holonomy. From the inclusion $L\subset M$ one has an action $\Coo_c\left( M\right) \times  L^2\left( L\right)\to L^2\left(L\right)$, so there is a representation $\rho_M^L: \Coo_c\left( M\right) \to B\left(L^2\left( L\right) \right)$. For any $a \in \Ga^\infty_c\left(\G, \Om^{1/2}\right)$, $~\xi\in L^2\left( L\right)$ and $f \in \Coo_c\left( M\right)$ one has
\be\label{irre_eqn}
\begin{split}
