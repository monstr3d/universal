...
\bea
\check{H}^2\left(\T \right)\left( \Dl\left(\left[\a_{\iota j}\right] \right)\right) = 0,\\
...
where $\check{H}^2\left(\T \right)$ and $\check{H}^3\left(\Z \right)$ are given by the equations \eqref{ctr_h2_eqn} and \eqref{ctr_h3_eqn} respectively.
 \end{lem}
\begin{proof}
The transition functions $\a_{\iota j} \in A\left(F_{\iota j} \right)$ can be regarded as transition functions  $\a^A_{\iota j} \in A^A\left(	CT\left(\sX, \dl \right)\left(F_{\iota j}  \right) \right)$  where hereditary subalgebras 	$CT\left(\sX, \dl \right)\left(F_{\iota j}  \right)$  are given by the equation \eqref{ctr_cosupp_eqn}. However all $\a^A_{\iota j}$ are identical isomorphisms. It follows that $\left[\a^A_{\iota j}\right] \in D\left( C^0\left( \mathscr U, A^A\right)\right) $ where $D$ is given by \eqref{nonab_perm_eqn}. !!! It turns out that $\left[\a^A_{\iota j}\right]$ is trivial in $\check H^1\left( \mathscr U, A^A\right)$. But $\check{H}^2\left(\T \right)\left( \Dl\left(\left[\a_{\iota j}\right] \right)\right)$ is the image of $\left[\a^A_{\iota j}\right]$ so it is also trivial, it follows that $\check{H}^2\left(\T \right)\left( \Dl\left(\left[\a_{\iota j}\right] \right)\right)= 0$. Since $\check{H}^3\left(\Z \right)\left(\dl \right)$ is an image of $\check{H}^2\left(\T \right)\left( \Dl\left(\left[\a_{\iota j}\right] \right)\right)$  one has $\check{H}^3\left(\Z \right)\left(\dl \right)= 0$.
\end{proof}
\paragraph{!!! ESSENTIAL}
