...
\end{lemma}
\begin{proof}
Consider frames $\mathfrak{Frame}\left(  A\right)$ and   $\mathfrak{Frame}\left( \widetilde A\right)$ given by the Definition \ref{grothendieck_ca_defn}. One can define a morphism of frames
...
\end{lemma}
\begin{proof}
	Let us define a frame homomorphism 
...
\eean
\end{example}
\subsection{Pullbacks of locales}

\paragraph{}Let $X', X'', X$ be locales with surjections (cf. Definition \ref{loc_emp_sup_defn}) $f': X'\to X$ and $f'': X''\to X$. If $U' \in X'$ then $U'$ can be regarded as locale and there is an embedding $f'_{U'}: U' \subset X'$ (cf. ref{$U' \in X'$}). So there is a map $f_{U'}\bydef f' \circ f'_{U'}:U' \to X$. From the Theorem \ref{locale_decomp_thm} it follows that there is a nucleus $j_{U'}$ on $X$ for which $f_{U'}$
factors through the embedding $i_{U'} : X_{j_{U'}} \hookto X$ via a surjection $p_{U'} \to X_{j_{U'}}$, i.e. $f_{j_{U'}} = i_{{U'}} \circ p_{U'}$. Moreover from the Theorem \ref{locale_decomp_uni_thm} if follows that 
\be\label{inc_loc_eqn}
\forall V' \in X' \quad V' \le U' \quad f_{V'} = i_{{V'}} \circ p_{U'}
\ee
Similarly for any $U'' \in X''$ one can define a nucleus $j_{U''}$ on $X$
\begin{definition}\label{compatible_defn}
A pair $\left(U', U''\right) \in X'\times X''$ is said to be \textit{compatible} if $j_{U'}= j_{U''}$
\end{definition}
\begin{lemma}
A family $X'\times_X X''$ of compatible pairs has the natural structure of locale.
\end{lemma}
\begin{proof}
Let $\left\{\left(U'_\iota, U''_\iota \right)\right\}_{\iota \in I}\subset X'\times_X X'$ and let $U' \bydef \bigvee U'_\iota$, $~ U'' \bydef \bigvee U''_\iota$. Then from the equation \ref{inc_loc_eqn} for all $\iota \in I$ there is a natural morphism $f''_\iota: U''_\iota \to X_{j_{U'}}$. One can define a morphism $f''_{U''}:U'' \to X_{j_{U'}}$ by the following way. If $V \in  X_{j_{U'}}$ then for any $\iota \in I$ one has $V''_\iota  \bydef f''^{-1}_{U''_\iota}\left( V\right) \in U''_\iota$. Otherwise $V''_\iota  \in U''$ because $U''_\iota  \le U''$. We define $f''^{-1}_{V}\bydef \bigvee  f''^{-1}_{U''_\iota}\left( V\right)$. From this fact it follows that there is an embedding $X_{j_{U''}} \hookto X_{j_{U'}}$.  Similarly  one has an embedding $X_{j_{U'}} \hookto X_{j_{U''}}$, so we obtain $X_{j_{U'}} = X_{j_{U''}}$ and 
$$
\left( \bigvee U'_\iota, \bigvee U''_\iota\right) \in X'\times_X X'.
$$
We define
$$
 \bigvee \left(  U'_\iota,  U''_\iota\right) \bydef \left( \bigvee U'_\iota, \bigvee U''_\iota\right).
$$
If  $\left(U'_1, U''_1 \right), ~ \left(U'_2, U''_2 \right)\in   X'\times_X X''$ then $\left(U'_1\bigvee U'_2, U''_1 \bigvee U''_2 \right)\in  X'\times_X X''$ and
$$
j_{U'_1\bigvee U'_2}= j_{U''_1\bigvee U''_2} = j.
$$
From \eqref{wedge_eqn} it follows that 
$$
j\left(U'_1\wedge U'_2 \right) = j U'_1 \wedge j U'_2=j U''_1 \wedge j U''_2 = j\left(U''_1\wedge U''_2 \right).
$$
So one has $\left( U'_1\wedge U'_2 , U''_1\wedge U''_2 \right) \in X'\times_X X''$.
we define
$$
\left(U'_1, U''_1 \right)\wedge \left(U'_2, U''_2\right)\bydef \left( U'_1\wedge U'_2 , U''_1\wedge U''_2 \right).
$$
We leave to the reader a proof of the condition \ref{frame_eqn}.
\end{proof}
\begin{definition}\label{locale_pullback_defn}
One has a following  pullback diagram
		\newline
\begin{tikzcd}
	X'\times_X X'' \arrow[d] \arrow[r]
	& X' \arrow[d]\\
	X''\arrow[r]	& X
\end{tikzcd}
\\
In this case we say that $	X'\times_X X''$ is a \textit{pullback of locales}.
\end{definition}
...
There is an isomorphism between the   {Gelfand}  $A$-{locale} $\mathfrak{Locale}\left(A \right)$ (cf. Definition \ref{grothendieck_ca_defn}) and the locale $\mathfrak{L}_\H$ of subspaces of $\H$ giben by $B\H$ where $B$ is a hereditary of $A$.
The locale $\mathfrak{L}_\H$ is a measure locale (cf. \cite{pavlov_duality}). If a point $x \in \sX$ is a $\mu$-\textit{atom} (i.e. $\mu\left(\left\{x\right\} \right)> 0$) then there is a nontrivial subspace $\H_x \subset \H$. Let us define 
$$
...
\begin{enumerate}
	\item[(i)] there is a natural bijection $\pt\left( \K\right) \cong P\H$, where $\pt\left( \K\right)$ is a set of points of Gelfand $\K$-locale $\mathfrak{Locale}(\K)$, and $P\H$ is a projective $\H$, 
	\item[(ii)] The locale $\mathfrak{Locale}(\K)$ is spatial (cf. Definition \ref{enough_points_defn})
\end{enumerate}
\end{lemma}
\begin{proof}
(i) Let $p^{-1}:	\mathfrak{Frame}\left(\K \right)\to \left\{0, 1\right\}$
 ...
 $$ 
 (cf. \eqref{p_p_eqn}) then $P$ is a hereditary subalgebra of $\K$. From the equation \eqref{pprime_eqn} it follows that $P \subsetneqq \K$ and from the Lemma \ref{compact_hered_lem} it follows that $\pi\left(P \right)\H \subsetneqq \H$. If there are $\H', \H'' \subset \H$ such that 
 \be\label{comp_sp_eqn}
 \begin{split}
 	\H' \neq \pi\left(P \right)\H,\\
 	\H'' \neq \pi\left(P \right)\H,\\
 	\H' \cap \H'' = \pi\left(P \right)\H
 \end{split}
 		\ee
 		then one has
 		\be\label{comp_op_eqn}
 		\begin{split}
 			\K\left(\H' \right) \neq P,\\
 			\K\left(\H'' \right) \neq P,\\
 			\K\left(\H' \right) \cap \K\left(\H'' \right)= P
 		\end{split}
 		\ee 
 		The equations \eqref{comp_op_eqn} contradict with  \eqref{prime_eqn} ones, so the conditions \eqref{comp_op_eqn} are impossible. Since  \eqref{comp_op_eqn} are consequences of \ref{comp_sp_eqn}, the system of equations are  \ref{comp_sp_eqn} are also impossible. If there is a space $\K'\subset \H$ such that $\pi\left(P \right)\H \subsetneqq K' \subsetneqq \H$ then there is $K''\subsetneqq \H$ such that
 		\bean
 		K' \neq \pi\left(P \right)\H,\\
 	K'' \neq \pi\left(P \right)\H,\\
 			K' \cap K'' = \pi\left(P \right)\H
 		\eean
 		but it is impossible. So $\pi\left(P \right)\H$ is a codimension one subspace of $\H$. There is the unique one-dimensional subspace $L \subset \H$ such that $L\perp \H$. This space corresponds to a point of $\mathfrak{pt} \in P\H$. Henceforth we write $\mathfrak{pt} \perp \pi\left(P \right)\H$. 
 	If $B \subset \K$ is a hereditary subalgebra then from the construction  it follows that if a $p_x$ is a corresponding to $x$ point then
 	$$
p_{\mathfrak{pt}}^{-1}\left(0 \right)  = \left\{ \K\left(\H' \right) | \H' \subset \H,\quad \H' \perp \mathfrak{pt} \right\}.	
 	$$	
 Taking into account  the equation \ref{point_set_eqn} one has
 	\be\label{comp_open_set_eqn}
 	\mathrm{pt}(B) = \left\{p_{\mathfrak{pt}}| \mathfrak{pt}\in P\H, \quad \pi(B)\H \not\perp x\right\}
 	\ee
 (ii) If $B', B'' \subset \K$ are two hereditary subalgebras such that $B'\neq B''$ then from the Lemma \ref{compact_hered_lem}  it follows that there is subsets $\H', \H''\subset \H$ such that $B' = \K\left(\H' \right)$ and  $B' = \K\left(\H' \right)$. One can suppose that $\H'' \subsetneqq \H'$, so there is $\xi \in \H'$ such that $\xi \perp \H''$. If $x \in P\H$ corresponds to a line $\C\xi$ then
$p_x\left( B'\right) = 1$ and $p_x\left( B''\right) = 0$. Using the equation one concludes that $\mathfrak{Locale}(\K)$ is spatial.
\end{proof}
\begin{remark}
...
\begin{empt}
	...
	\eean	
	of locales. 
Consider a pullback of locales (cf. Definition \ref{locale_pullback_defn})
	\newline
	\begin{tikzcd}
...
\eean
where $\smile$ is a cup product (cf. the equation \eqref{cup_prod_eqn} and the Remark \ref{ring_homo_rem} !!! \ref{topos_hom_chech_hom_r_eqn}  \ref{topos_hom_hom_eqn}  !!!). From \eqref{cup_eq_eqn} it follows that the above pairing yields a following group homomorphism
\be\label{dd_coh_eqn}
...
\end{proposition}
\begin{lemma}\label{loc_adj_lem}
%Lemma 1. 
A morphism $\Phi: A \to B$ of frames considered as a map
of posets has a right adjoint $\Psi : B \to A$
such that
\be\label{loc_adj_eqn}
\Psi\left( U\right)  \bydef \bigvee \left\{V \in B | \Phi\left( V\right) \le U \right\}.
\ee
\end{lemma}
Recall that for a map $f: Y\to X$ of locales, the corresponding frame
map $f^{-1}: \mathcal{O}(Y)\to \mathcal{O}(X)$, considered as a map of posets, has a right
adjoint $f_*: \mathcal{O}(Y)\to \mathcal{O}(X)$ (Lemma \ref{loc_adj_lem}). The unit and counit of this
adjunction between posets state that $U \le f_*f^{-1} (U)$ for any $U \in \mathcal{O}(X)$,
and that  $f^{-1}f_* (V) < V$ for any $V \in \mathcal{O}(Y)$. Moreover, the triangular
identities for the adjunction reduce to the equalities
\be\label{redu_eqn}
 f_*f^{-1}f_*=f^{-1}, \quad f^{-1}f_*f^{-1} = f_*, 
\ee
...
\end{definition}
\begin{lemma}\cite{topos:intro}
%Lemma 2. 
Let $f : X \to Y$ be a map of locales. The following three
conditions are equivalent:
\begin{enumerate}
	\item [(a)] $f$ is a surjection of locales (i.e., 1-1 is one-to-one);
	\item 	[(b)] $f_*f^{-1} = \Id: \mathcal{O}(X)\cong \mathcal{O}(X)$,
	\item 	[(c)] $f_*: \mathcal{O}(Y)\to \mathcal{O}(X)$ is a surjection of posets.
	
\end{enumerate}

Also, the following are equivalent:
\begin{enumerate}
	\item [(a')] $f$ is an embedding of locales (i.e., 1-1 is one-to-one);
	\item 	[(b')] $f^{-1}f_* = \Id: \mathcal{O}(Y)\cong \mathcal{O}(Y)$,
	\item 	[(c')] $f_*: \mathcal{O}(Y)\to \mathcal{O}(X)$ is  is injective (one-to-one).
	\end{enumerate}
\end{lemma}
Consider an embedding $f: X \to Y$ of locales. Thus, by (c') above,
$f_*: \mathcal{O}(Y)\to \mathcal{O}(X)$ is injective, and again by \eqref{redu_eqn} its image consists of
precisely those elements $U\in \mathcal{O}(X)$ which are fixed under the operator
$f_*f^{-1}: \mathcal{O}(X)\to \mathcal{O}(X)$. This operator $j \bydef f_*f^{-1}$ is (the underlying
functor of) the monad of the adjunction $f^{-1} \vdash f_*$, while the unit and
multiplication of this monad are maps which can be expressed by the
following inequalities, for each $U$ in the poset $\mathcal{O}(X)$:
\bea\label{lju_eqn}
U\le jU,\\
\label{llju_eqn}
jj U \le jU.
\eea
Notice that by the functoriality of $j = f_*f^{-1}$, \eqref{lju_eqn} implies that $j U\le j^2 U$ so that \eqref{llju_eqn} is in fact an identity 
\be\label{lllju_eqn}
j^2 U = j U
\ee

Notice also that, since $f_*$ and $f^{-1}$ both preserve finite meets ($f_*$ as
a right adjoint and $f^{-1}$ as a frame morphism), we have for any $U, U' \in 
 \mathcal{O}(X)$
 \be\label{wedge_eqn}
 j\left( U\wedge U'\right) = jU \wedge j U'
 \ee
An operator $j:  \mathcal{O}(X)\to  \mathcal{O}(X)$ satisfying the identities (2), (3'), (4) is called a \textit{nucleus} on the locale $X$. This nucleus determines the domain locale $Y$. Indeed, it is  stated  if $f: Y \to X$ is an embedding of locales, then $\mathcal{O}(Y)$ is isomorphic to the set of fixed points $\left\{U \in \mathcal{O}(X)| jU = U\right\}$ of the nucleus $j f_*f^{-1}$. The converse of this observation also holds:
\begin{proposition}
Let $j: \mathcal{O}(X)\to \mathcal{O}(X)$ be a nucleus on a locale $X$.
Then the poset of $j$-fixed points $\left\{U \in \mathcal{O}(X)| jU = U\right\}$ is a frame,
and $j$ defines a surjective frame-morphism from $\mathcal{O}(X)$ into this frame of
fixed points.
\end{proposition}
The locale corresponding to this frame of fixed points  $\left\{U \in \mathcal{O}(X)| jU = U\right\}$ is usually denoted by $X_j$-so the corresponding frame is
\be
\mathcal{O}(X_j) = \left\{U \in \mathcal{O}(X)| jU = U\right\}
\ee
The proposition then states that the nucleus $j$ on $X$ determines an
embedding of locales
\be
i: X_j \hookto X
\ee
given by the frame morphism $i^{-1}:\mathcal{O}(X) \to \mathcal{O}(X_j) $ defined by $i^{-1}U =jU$
[Its right adjoint $i_*:  \mathcal{O}(X_j) \to  \mathcal{O}(X)$ is simply the inclusion: $i^*U = U$ for all $U \in \mathcal{O}(X_j)$.] Locales of the form $X_j$ where $j$ is a nucleus on $X$
are called the \textit{sublocales} of $X$.
\begin{theorem}\label{locale_decomp_thm}
Let $f: Y \to X$
 be a map of locales. Then there exists a nucleus $j$ on $X$ for which $f$
factors through the embedding $i : X_j \hookto X$ via a surjection $p: Y \to X_j$, i.e. $f = i \circ p$.
\end{theorem}
\begin{empt}
	Fragment !!!
The given map $f: X \to Y$ of locales yields
adjoint functors $\mathcal{O}(X) \substack{ \xrightarrow{f^{-1}}\\\xleftarrow[f_*]{}}\mathcal{O}(Y)$. Define $j$ as the composite
$j : f_*f^{-1}:\mathcal{O}(Y) \to \mathcal{O}(X)$; then $j$ is a nucleus.
\end{empt}
\begin{theorem}\label{locale_decomp_uni_thm}
%Theorem 6 (Factorization Theorem, uniqueness). 
Let $f : Y \to X$ be a map of locales, while $Y \xrightarrow{p} A \xrightarrow{u} X$ and $Y \xrightarrow{q}B \xrightarrow{v} X$
are two factorizations of $f$. If $v$ is an embedding and $p$ is a surjection,
then there exists a unique map $g: A \to B$ of locales such that $gp = q$
and $vg = u$. %as in the commutative diagram
%Y q ) B
%pL/~//~ Iv
%A -"",u.,---+l X.
Moreover, if $u$ is also an embedding and $q$ is also a surjection, then $g$ is
an isomorphism.
\end{theorem}
\begin{empt}\label{open_sublocale_empt}
For a given locale $X$ and an element $U\in \mathcal{O}(X)$, the set (the downward
closure) $\downarrow U = \left\{W \in \mathcal{O}(X)| W \le U\right\}$. 
is clearly a frame, and there is
an evident surjective frame morphism
\be\label{emb_eqn}
U \wedge \cdot : \mathcal{O}(X) \to \downarrow U\quad W \mapsto W \wedge U
\ee
Let us also write $U$ for the locale given by this frame $\downarrow U$; i.e., the locale
$U$ defined by the frame $ \mathcal{O}(U) = \downarrow U$. Then \eqref{emb_eqn} describes an embedding
\be
f : U \hookto X, \quad f^{-1}\left(W \right) = U\wedge W \quad W \in  \mathcal{O}(X)
\ee
By the definition of implication as an adjoint, the right adjoint $f_*$ of $f^{-1}$ 
is the map $V \mapsto U \Rightarrow V$, for $V \le U$. It follows that $U$ is isomorphic to
the sublocale $X_j$ , for the nucleus $j = f_*f^{-1}$; i.e., $jW = (U \Rightarrow (U\wedge W)) =
(U \Rightarrow W)$ for each $W \in  \mathcal{O}(X)$
Sublocales of this form are called \textit{open sublocales} of $X$. In other words,
any element $U \in  \mathcal{O}(X)$ defines a unique open sublocale of $X$, usually
called $U \hookto X$.
\end{empt}
%\begin{definition}\label{open_locale_mor_defn}\cite{topos:intro}
%Definition 1. 
%A map $f : X \to Y$ of locales is \textit{open} if and only if the corresponding map $f^{-1}: \mathcal O\left( Y\right) \to  \mathcal O\left( X\right)$ of posets has a left adjoint $f_!$ which satisfies the Frobenius identity
...
Prove that a given by \eqref{cech_hom_eqn} map $\check{H}^\bullet\left(f\right):\check{H}^\bullet\left(\sY, F \right)\to \check{H}^\bullet\left(\sX, F \right)$ is a homomorphism of rings.
	...
	\begin{remark}\label{ring_homo_rem} !!! \ref{topos_hom_chech_hom_r_eqn}  \ref{topos_hom_hom_eqn}  !!!
