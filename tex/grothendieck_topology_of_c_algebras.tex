...
\begin{definition}\label{grothendieck_ca_t_defn}
	If $A$ is a $C^*$-algebra such that there is a topological space $\mathfrak{Grot}\left(A \right) $ such that a site $\mathbf{T}_{A}$ 	
...
\section{$C^*$-algebras generated by their commutative subalgebras}
\subsection{Grothendieck spaces}
\paragraph{}
Here we consider a class of $C^*$-algebras which possesses Grothendieck spaces.
\begin{definition}\label{hered_com_defn}
A $C^*$-algebra $A$ is said to be \textit{commutative generated} if a $A$ is generated by all its commutative subalgebras,
\end{definition}
\begin{exercise}
Prove that any commutative generated $C^*$-algebra is generated by its  commutative hereditary $C^*$-subalgebras,
\end{exercise}
Denote by
	\be\label{com_cov_eqn}
\mathscr C^A \bydef \left\{C\text{ is a  commutative hereditary } C^*\text{-subalgebra of } A \right\}
\ee	
...
\begin{empt}
For any open subset $\sU$ denote by $_\sU A_\sU$ a generated by the set $C_0\left(\sU\right) AC_0\left(\sU\right)$ hereditary subalgebra of $A$ (cf. Definition \ref{hered_generated_defn}). We define a presheaf of sets $\mathscr{P}^A $ on $\sX$ (cf. Definition \ref{presheaf_defn}) such that for any pair  $\sV\subset\sU$ of open subsets of $\sX$ one has
...
For any commutative $C^*$-subalgebra $C \subset A$ denote by a union of open sets (cf. n$^{\text{o}}$ 1 of the Exercise \ref{sheaf_etale_open_exer})
\bean
...
\ee
is an open subset of $\mathscr X^A$. Any hereditary commutative hereditary subalgebra $C$ corresponds to a continuous  section
$$
...
$$ 
then from the Lemma \ref{grothendieck_ca_lem} it follows that $B$ is generated by the union
$
...
\end{empt}
 ...
 If $A$ is a {hereditary commutative generated} $C^*$-algebra with  {Hausdorff blowing-up} $C_0\left( \sX\right) \hookto M\left(A \right)$ then $\mathscr X^A$ is the Grothendieck $A$-space $\mathfrak{Grot}\left(A \right)$ (cf. Definition \ref{grothendieck_ca_t_defn}).
\end{lemma}
\begin{proof}
From the equation  \eqref{one_to_one_eqn} it follows that there is a natural one to one correspondence 
$$
B \leftrightarrow A_{\mathscr X^A_B}
$$
between hereditary subalgebras of $A$ and open subsets of $\mathscr X^A$. So a category of open hereditary subalgebras of $B$ and their inclusions is equivalent to the category of open subsets of $A_{\mathscr X^A_B}$ and their inclusions.
...
If  $\left\{B_\iota \subset  A \right\}_{\iota \in I}$ is a family of hereditary $C^*$-subalgebras, and $B$ is a generated  by the union $\cup_{\iota \in I}B_\iota$ hereditary subalgebra then from the equation \ref{one_to_one_eqn} it follows that  $B_\iota=A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_{B_\iota}}$ and $B =A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_{B}}$. From the inclusion  $B_\iota \subset B$ it follows that $A_{\mathscr X^A_{B_\iota}}\subset A_{\mathscr X^A_B}$. It turns out that $\mathscr X^A_{B_\iota}\subset \mathscr X^A_B$ and $\cup_{\iota \in I}\mathscr X^A_{B_\iota}\subset \mathscr X^A_B$. On the other hand from $\mathscr X^A_{B_\iota}\subset \cup_{\iota \in I}\mathscr X^A_{B_\iota}$ it follows that 
$$
...
\end{definition}
Note that if $ \mathcal W= \supp C$ then $ \mathcal W\cap \sV =\supp C\cap ~_\sV A _\sV$. 
Let $\Om_\sX$ be {sheaf of stalks of open sets} (cf. Appendix \ref{open_sheaf_empt}). Let  $\mathrm{Sp\acute{e}}\left(\Om_\sX \right)$ be the	\'espace etal\'e  (cf. Exercise \ref{sheaf_etale_exer}) of the sheaf $\Om_\sX$. The empty set $\emptyset$ yields a global  section $s^\emptyset \in \Om_\sX\left(\sX \right)$, this section gives an open subset $\mathrm{Sp\acute{e}}\left(\Om_\sX  \right)_\emptyset \subset \mathrm{Sp\acute{e}}\left(\mathscr S  \right)$. Denote by 
\be
\mathfrak X\left(\sX \right)  \bydef  \mathrm{Sp\acute{e}}\left(\Om_\sX  \right)\setminus\mathrm{Sp\acute{e}}\left(\Om_\sX \right)_\emptyset.
\ee
If  $\mathscr{P}^{ A}$ is a   given by \eqref{comm_alg_sheaf}  presheaf then there  is a surjective morphism of sheaves (cf. Definition \ref{sheaf_defn})
\bean
\psi : \mathfrak{Ass}\left( \mathscr{P}^A\right)  \to \mathscr S;\\
\psi_\sU \left( C\right) \bydef \supp C
\eean
where $\mathfrak{Ass}$ means the associated sheaf functor (cf. Definition \ref{associated_sheaf_defn}). This morphism yields a surjective local homeomorphism 
$$
P'_A:\mathrm{Sp\acute{e}}\left(\mathscr P^A  \right) \to \mathrm{Sp\acute{e}}\left(\Om_\sX  \right).
$$
\begin{exercise}
Prove that if $\mathscr X^A$ is given by the Lemma \ref{grot_com_lem}  Grothendieck $A$-space. (cf. Definition \ref{grothendieck_ca_t_defn}) then $P'_A$ maps $\mathscr X^A$ onto $\mathfrak X\left(\sX \right)$. Moreover the natural surjective maps
\bea
P_A: \mathscr X^A\to \mathfrak X\left(\sX \right),\\
P_\sX :  \mathfrak X\left(\sX \right)\to \sX
\eea
are a local homeomorphisms.

\end{exercise}
\begin{definition}\label{local_prod_defn}
Let $A, A', A''$ be  commutative generated $C^*$-algebras (cf. Definition \ref{hered_com_defn}). If $C_0\left( \sX\right) \hookto M\left(A \right)$, $~C_0\left( \sX\right) \hookto M\left(A \right)$, $~C_0\left( \sX\right) \hookto M\left(A \right)$ be Hausdorff blowing-ups (cf. Definition \ref{blowing_up_defn}) then
 we say that $A$ is a \textit{local product} of $A'$ and $A''$ if for any $x \in \sX$ there is an open neighborhood $\sU$ such that for every open subset $\sV \subset \sU$ there is a bijective map
	\be\label{local_prod_eqn}
\varphi_{\sV} :		\mathfrak{Supp}\left( A'\right)_{\sV}	\times \mathfrak{Supp}\left( A''\right)_{\sV}\cong \mathfrak{Supp}\left( A\right)_{\sV}.
	\ee
\end{definition}

\begin{theorem}\label{local_prod_thm}
	If  $A$ is a {local product} of $A'$ and $A''$ then there is a homeomorphism
$$
\phi: \mathfrak{Grot}\left(A' \right)\times_{\mathfrak X\left(\sX \right)}\mathfrak{Grot}\left(A'' \right)\cong \mathfrak{Grot}\left(A \right).
$$
\end{theorem}
\begin{proof}
	Any element of 
$\mathfrak{Grot}\left(A' \right)\times_{\mathfrak X\left(\sX \right)}\mathfrak{Grot}\left(A'' \right)=\mathscr X^{A'} \times_{\mathfrak X\left(\sX \right)}	 \mathscr X^{A''}$ corresponds to a pair $C'_x \in  \mathfrak{Grot}\left(A' \right)$, $~C''_x \in  \mathfrak{Grot}\left(A'' \right)$ such that $P_{A'}\left( C'_x\right)= P_{A''}\left( C''_x\right)$. Suppose that both $C'_x$ and $C''_x$ are represented by  hereditary commutative $C^*$-algebras $C' \in \mathscr C^{A'}$ and $C'' \in \mathscr C^{A''}$ such that $\supp C' = \supp C''$. If $\sU$ is an open neighborhood of $x$ which satisfies to the Definition \ref{local_prod_defn} then  both $C'_x$ and $C''_x$ can be represented by $C'\cap ~_\sU A _\sU$ and $C''\cap ~_\sU A _\sU$ such that $\supp C''\cap ~_\sU A _\sU= \supp C''\cap ~_\sU A _\sU = \sV \bydef \supp C' \cap \sU =\supp C'' \cap \sU$. If  $C = \varphi_\sV \left( C'\cap ~_\sU A' _\sU, C''\cap ~_\sU A'' _\sU\right)$  then the map $\phi$ is given by $\left( C'_x, C''_x\right) \mapsto C_x$.
	
Any  element of $\mathfrak{Grot}\left(A \right)= \mathscr X^A$ is  a stalk $C_x$ of a hereditary commutative $C^*$-algebra $C\in \mathscr C^A$. If  $\sU$ is the given by the  the Definition \ref{local_prod_defn} open neighborhood of $x$ then $C_x$ can be represented by $C|_\sU = C\cap ~_\sU A _\sU$. If $\sV= \supp C|_\sU=\supp C \cap \sU$ then from the the equation \eqref{local_prod_eqn} there are unique  $C'\in 	\mathscr C^{A'}$ and $C''\in 	\mathscr C^{A''}$ such that $\supp C' = \supp C'' = \sV$ and $C|_\sU = \varphi_{\sV} \left(C', C'' \right)$. However a pair $\left(C', C''\right)$ yields an element of $\mathscr X^{A'} \times_{\mathfrak X\left(\sX \right)}	 \mathscr X^{A''} =  \mathfrak{Grot}\left(A' \right)\times_{\mathfrak X\left(\sX \right)}\mathfrak{Grot}\left(A'' \right)$. Thus one has an inverse to $\phi$ continuous map.
...
If $F$ is an Abelian group such that $F\ox_\Z F \cong F$ then from the Theorem \ref{local_prod_thm}, the Lemma \ref{gr_ring_lem} and the Exercise \ref{ring_homo_exer} one can obtain following homomorphisms
	...
	\check{H}^\bullet\left( A', F\right) \ox_{\check{H}^\bullet\left( \sX,F\right) } 	\check{H}^\bullet\left( A'', F\right)\to 	\check{H}^\bullet\left( A, F\right),\\
		\check{H}^\bullet\left( A', F\right) \ox_{\check{H}^\bullet\left( \mathfrak X\left(\sX \right),F\right) } 	\check{H}^\bullet\left( A''\right)\to 	\check{H}^\bullet\left( A, F\right)
		\eean 
		of $\check{H}^\bullet\left( \sX, F \right)$ and $\check{H}^\bullet\left( \mathfrak X\left(\sX \right), F\right)$ bimodules respectively. 
\end{remark}
...
\begin{lemma}\label{blowing_grothendieck_sh_lem} 	
	If $F$ is an Abelian group then thee homomorphisms of groups
		\bean\label{blowing_grothendieck_sh_eqn}
...
\eean
 the notation  \eqref{etale_hom_a_eqn} is  used. If there is an isomorphism $F\otimes_\Z F \cong F$ then \eqref{blowing_grothendieck_shc_eqn} is a homomorphism of rings.
\end{lemma}
\begin{proof}
If $p_A:\mathscr X^A\to\sX$ is a natural local homeomorphism, and both $F_\sX$ and $F_{\mathscr X^A}$ are constant presheaves arising from $F$ then from the Proposition  \ref{spectral_sequence_prop} if follows that for all $r\ge 0$ there are homomorphisms
	\bean\label{x_seqc_eqn}
	\begin{split}
		{H}^r\left(\sX, \mathfrak{Ass}\left( F_\sX\right)  \right)\xrightarrow{H^r\left(p \right) } {H}^r\left(\mathscr X^A, p^*\mathfrak{Ass}\left( F_\sX\right) \right) 
			\end{split}
	\eean
		where $\mathfrak{Ass}$ means the associated sheaf functor, and $p^*$ is an inverse image (cf. Definitions \ref{geom_mor_defn} and \ref{sheaf_inv_im_defn}).
	 Otherwise the map $p$ is a local homeomorphism (cf. Exercise \ref{sheaf_etale_open_exer}), it follows that 
\bean\label{x_seqcu_eqn}
	p^*\mathfrak{Ass}\left( F_\sX\right)\cong 	\mathfrak{Ass}\left( F_{\mathscr X^A}\right)
\eean
...
$$
		{H}^r\left(\sX, \mathfrak{Ass}\left( F_\sX\right)  \right)\xrightarrow{H^r\left(p \right) } {H}^r\left(\mathscr X^A, \mathfrak{Ass}\left(F_{\mathscr X^A}\right) \right). 
$$
...
From the Theorem \ref{dauns_hofmann_thm} it follows that there is  Hausdorff blowing-up. From the Lemma \ref{grot_com_lem} it follows that there is the Grothendieck $CT\left(\sX, \dl \right)$-space $\mathfrak{Grot}\left(CT\left(\sX, \dl \right)\right) =\mathscr X^{CT\left(\sX, \dl \right)}$.
\end{proof}
\begin{lemma}
...
\end{lemma}
\begin{proof}
If $x \in \sX$ is any point than there is a neighborhood of $\sU$ of $x$ such that 
$$
_\sU CT\left(\sX, \dl \right) _\sU\cong ~_\sU CT\left(\sX, \rho \right) _\sU\cong ~_\sU CT\left(\sX, \dl + \rho \right) _\sU\cong C_0\left( \sU\right) \otimes \K
$$
corresponding to $x$ irreducible representations.
For any $x \in \sX$ denote by 
\bean
\pi^{ \dl }_x:  CT\left(\sX, \dl  \right)\to B\left( \H^{\dl}_x\right),\\  
\pi^{  \rho}_x:  CT\left(\sX, \dl + \rho \right)\to B\left( \H^{ \rho}_x\right),\\
 \pi^{ \dl + \rho}_x:  CT\left(\sX, \dl + \rho \right)\to B\left( \H^{\dl + \rho}_x\right)
\eean
If $\sV \subset \sU$ and $$C^{\delta } \in \mathfrak{Supp}\left( CT\left(\sX, \dl \right)\right)_{\sV}, \quad C^{ \rho} \in \mathfrak{Supp}\left( CT\left(\sX, \rho \right)\right)_{\sV}, \quad  C^{\delta + \rho} \in \mathfrak{Supp}\left( CT\left(\sX, \dl + \rho \right)\right)_{\sV}$$
then there are following correspondences:
\begin{itemize}
	\item $C^{\delta}$ corresponds to a family of rank-one projectors $\left\{p_x^{\dl}\in B\left(\H^{\dl}_x\right)\right\}_{x \in\sV}$ onto spaces $\pi^{\dl}\left( C^{\dl}\right)\H^{\dl}$,
	\item $C^{\rho }$ corresponds to a family  of rank-one projectors $\left\{p_x^{\rho}\in B\left(\H^{\rho}_x\right)\right\}_{x \in\sV}$ onto spaces $\pi^{\rho}\left( C^{\dl}\right)\H^{\dl}$,
	\item $C^{\delta + \rho }$ corresponds to a family  of rank-one projectors $\left\{p_x^{\dl+\rho}\in B\left(\H^{\dl + \rho}_x\right)\right\}_{x \in\sV}$ onto spaces $\pi^{\dl + \rho}\left( C^{\dl + \rho}\right)\H^{\dl + \rho}$.
\end{itemize}
Using  a bijective map $\varphi_{\dl \rho}	:CT\left(\sX, \dl \right)\times_\sX CT\left(\sX, \rho \right)\cong CT\left(\sX, \dl +\rho\right)$ (cf. equitation \eqref{bundle_prod_iso})  we can find a one to one correspondence between pairs $\left(\left\{p_x^{\dl}\right\}_{x \in\sV}, \left\{p_x^{\rho}\right\}_{x \in\sV} \right)$ and families $\left\{p_x^{\dl+\rho}\right\}_{x \in\sV}$ this correspondence yields required by the Definition \ref{local_prod_defn} bijective map
$$
	\mathfrak{Supp}\left(CT\left(\sX, \dl \right)\right)_{\sV}	\times \mathfrak{Supp}\left(CT\left(\sX, \rho \right)\right)_{\sV}\cong \mathfrak{Supp}\left(CT\left(\sX, \dl + \rho \right)\right)_{\sV}.
$$
\end{proof}
From the Theorem \ref{local_prod_thm}  one can obtain a homeomorphism
\bea\label{space_homeo_eqn}
\mathfrak{Grot}\left(CT\left(\sX, \dl \right)\right)\times_{\mathfrak X\left(\sX \right)}\mathfrak{Grot}\left(CT\left(\sX, \rho \right)\right) \cong \mathfrak{Grot}\left(CT\left(\sX, \dl + \rho\right)\right).
\eea
...
\bea\label{hom1_homeo_eqn}
	...
	\check{H}^\bullet\left( CT\left(\sX, \dl \right), F\right) \ox_{\check{H}^\bullet\left(\mathfrak X\left(\sX \right),F\right) } 	\check{H}^\bullet\left( CT\left(\sX, \rho \right), F\right)\to 	\check{H}^\bullet\left( CT\left(\sX, \dl + \rho \right), F\right)
\eea
...
for all $x \in \sX$. A generated by $a$ hereditary commutative  subalgebra $C^a$ (cf. Definition \ref{hered_generated_defn}) is commutative, so it defines a global section $s^a\in \mathscr{P}^{CT\left(\sX, 0 \right)} \left(\sX\right)$ (cf. equation \eqref{comm_alg_sheaf}), such that $s^a_x \in \mathscr X^{CT\left(\sX, 0 \right)}$ (cf. equation \eqref{xa_eqn}) for all $x \in \sX$.
...
p_0 \circ j = \Id_{\sX}.
\ee
where $p_0: \mathscr X^{CT\left(\sX, 0 \right)}\to \sX$  is the natural surjective local homeomorphism.
...
\begin{proof}
	...
	\be\label{x_seq_eqn}
	\begin{split}
		{H}^r\left(\sX, \mathfrak{Ass}\left( F_\sX\right)  \right)\xrightarrow{H^r\left(p_0 \right) } {H}^r\left(\mathscr X^{CT\left(\sX, 0 \right)}, p_0^*\mathfrak{Ass}\left( F_\sX\right) \right) \xrightarrow{H^r\left(j \right) }\\\xrightarrow{H^r\left(j \right) } {H}^r\left(\sX, j^*p_0^*\mathfrak{Ass}\left( F_\sX\right) \right)
	\end{split}
	\ee
	...
	$$
	p_0^*\mathfrak{Ass}\left( F_\sX\right)\cong 	\mathfrak{Ass}\left( F_{\mathscr X^{CT\left(\sX, 0 \right)}}\right)
	$$
	(cf. the Exercise \ref{dir_image_exer}). In result the equation \eqref{x_seq_eqn} is equivalent to the following one
	\bean
	\begin{split}
		{H}^r\left(\sX, \mathfrak{Ass}\left( F_\sX\right)  \right)\xrightarrow{H^r\left(p_0 \right) } {H}^r\left(\mathscr X^{CT\left(\sX, 0 \right)}, \mathfrak{Ass}\left( F_{\mathscr X^{CT\left(\sX, 0 \right)}}\right) \right) \xrightarrow{H^r\left(j \right) }\\\xrightarrow{H^r\left(j \right) } {H}^r\left(\sX, \mathfrak{Ass}\left( F_\sX\right) \right).
	\end{split}
	\eean
	...
	\check{H}^r\left(p_0\right) 	:\check{H}^r\left(\sX, F \right)\to \check{H}^r\left(\mathscr X^{CT\left(\sX, 0 \right)}, F\right).
	\eean 
