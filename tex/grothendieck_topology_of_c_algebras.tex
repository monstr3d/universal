...
\begin{empt}\label{top_paracompact_empt}
	Henceforth we suppose that $\sX$ is paracompact, so since $\sX$ from the Theorems \ref{gelfand-naimark_thm} and \ref{top_paracompact_thm} it follows that $\sX$ is metrizable. Any subspace of $\sU \subset \sX$ is metrizable  so $\sU$ is paracompact. Let $\mathscr{ A}\in \mathbf{Ab}\left(\sX\right)$ be a sheaf of Abelian groups. From the Remark \ref{sheaf_rem} it follows that there is a presheaf $^\neg \mathscr{ A}\in \mathbf{Ab}\left(\mathfrak{Closed}\left(\sX \right) \right)$ such that
	\be\label{top_paracompact_eqn}
	\begin{split}
	\forall \sU \in \mathfrak{Closed}\left(\sX \right)\quad ^\neg \mathscr{ A}\left(\sU \right) \bydef \text{ a group of continuous sections }  \iota :\sU\to \mathrm{Sp\acute{e}}\left(\mathscr A \right),\\
	\forall ~\sV , \sU 	\in \mathfrak{Closed}\left(\sX \right) , \quad \sV \subset \sU \quad ^\neg\mathscr{ A}\left(\sU \right)\to ^\neg\mathscr{ A}\left(\sV \right); \quad \iota \mapsto \iota|_\sV.
	\end{split}
	\ee
\end{empt}
...
$$
Let  $\left\lbrace s_\iota \in ^\neg \mathscr{ A}\left(\sU_\iota  \right)\right\rbrace$ be a family with
$$
\forall \iota, j \in I \quad s_\iota|_j = s_j|_\iota
$$
Since $\sX$ is metrizable, for any $\sU_\iota$ there is  fundamental system of
paracompact neighborhoods $\left\{\sV_{\iota, \a}\right\}$ of $\sU_\iota$  such that $\mathscr A\left( \sU_\iota\right) = \varinjlim_{\sV_{\iota, \a}} \mathscr A\left(
\sV_{\iota, \a} \right)$ (cf. Theorem \ref{paracompact_n_thm}). For any $\iota$ there is an open set $\sV_\iota \in  \left\{\sV_{\iota, \a}\right\}$ and $t_\iota \in A\left(
\sV_{\iota}\right)$ such that   $s_\iota$ is the "restriction" of $t_\iota$. A space $\bigcup \sU_\iota$ is paracompact (cf. \ref{top_paracompact_empt}, so there is a locally finite family   $\left\{\mathcal W_{\bt, \iota}\right\}_{(\bt, \iota) \in \B\times I}$ of open sets such that
\begin{itemize}
	\item $\mathcal W_{\bt, \iota} \subset \sV_{\iota}$,
	\item $\bigcup \mathcal W_{\bt, \iota} = \bigcup \sU_\iota$,
	\item the family $\left\{\mathcal W_{\bt, \iota}\right\}$ is locally finite.
\end{itemize}
From the Lemma \ref{top_paracompact_lem} it follows  that for any $\left( \bt, \iota\right) \in \B\times I$ there is 
\end{proof}

\paragraph{!!! DELETE !!! FINISH}
...
\end{definition}
\begin{empt}
Consider the canonical injective resolution $\mathcal J^*\left(\sX, L \right)$  of $L$ and
note that $\mathcal J^*\left(\sX, L \right)|\sU  = \mathcal J^*\left(\sU, L \right)$ for $\sU\subset \sX$ open. For a sheaf $\mathscr A$ on $\sX$,
we define
\bean
