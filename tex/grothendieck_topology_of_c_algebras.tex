...
\bean
\bigvee_\iota B_\iota \bydef \text{ a generated by a union } \bigcup_\iota B_\iota \text{ hereditary } C^*\text{-algebra.}
\eean
\end{remark}
...
The explained in  \ref{grothendieck_ca_empt} Grothendieck topology $J_A$  is said to be an  $A$-\textit{topology}. A corresponding  site (cf. Definition \ref{grotendieck_topolgy_defn}) $\mathbf{T}_A\bydef \left(\mathbf{Hered}/A, J_A\right)$ is said to be \textit{arising} from $A$ (cf. Definition \ref{top_gro_definition}). A Grothendieck topos $\mathscr{E}_A\bydef \mathbf{Sh}\left(\mathbf{T}_A \right)$ is said to be $A$-\textit{topos}.
\end{definition}
\begin{definition}\label{a_locale_defn}
	From the Theorem \ref{locale_thm} it follows that the $A$-{topos} $\mathscr{E}_A$ is localic. It corresponds to a locale $\mathfrak{Locale}\left(A \right)$, we say that  $\mathfrak{Locale}\left(A \right)$ is the \textit{Grothendieck}  $A$-\textit{locale}, so one 
has
\be\label{a_locale_eqn}
\mathscr{E}_A = \mathbf{Sh}\left(\mathfrak{Locale}\left(A \right) \right). 
\ee	
...
\begin{definition}\label{grothendieck_ca_t_defn}
	Let $A$ be a $C^*$-algebra. !!!! SUFFICIENT !!! If  there is a topological space $\mathfrak{Space}\left(A \right) $ such that a site $\mathbf{T}_{A}\bydef \left(\mathbf{Hered}/A, J_A\right)$ 	
	arising from $A$ (cf. Definition \ref{grothendieck_ca_defn}) is equivalent to a site $\mathbf{T}_{\mathfrak{Space}\left(A \right)}$ arising from $\mathfrak{Space}\left(A \right)$ (cf. Definition \ref{top_gro_definition}) then we say that $ \mathfrak{Space}\left(A \right)$ is the \textit{Grothendieck} $A$-\textit{space}.  In this case the  $A$-{topos} $\mathscr{E}_A\bydef \mathbf{Sh}\left(\mathbf{T}_A \right)\cong \mathbf{Sh}\left( \mathfrak{Space}\left(A \right)\right)$ is spatial (cf. Appendix \ref{spatial_toposes_sec}).
\end{definition}
\begin{remark}
If  the {Grothendieck} $A$-{space} exists it should not be unique, different topological spaces can correspond to a single Grothendieck locale,
\end{remark}
\begin{remark}
If $\mathfrak{Space}\left(A \right)$  is the {Grothendieck} $A$-\textit{space} and $\mathscr{E}_A\bydef \mathbf{Sh}\left(\mathbf{T}_A \right)\cong\mathbf{Sh}\left(\mathfrak{Space}\left(A \right) \right) $ is the $A$-{topos}, then from the Example \ref{top_points_exm} it follows that any $x\in \mathfrak{Space}\left(A \right)$ yields a point of  $\mathscr{E}_A$ in the sense of the Definition \ref{topos_point_defn}. Moreover $\mathfrak{Space}\left(A \right)$ is a sufficient class of points (cf. Definition \ref{topos_point_defn}).
\end{remark}
...
\bea\label{hom_eqn}
H^r\left(\mathfrak{Space}\left(A \right),F\right)\cong \check{H}^r\left(A, F \right),\\
\label{chech_hom_eqn}
...
\subsection{Morphism of locales}
...
\end{lemma}
\begin{proof}
Consider frames $\mathbf{ Frm}_A$ and   $\mathbf{ Frm}_{\widetilde A}$ given by the Remark \ref{locale_rem}. One can define a morphism of frames
\bean
\mathbf{ Frm}_{ A}\to \mathbf{ Frm}_{\widetilde A},\\
...
$$
\mathbf{Locale}_{\widetilde A} \to \mathbf{ Locale}_{ A},
$$
The Theorem \ref{locale_thm} yields a morphism of toposes $\mathscr E_{\widetilde A}\to \mathscr E_{A}$.
\end{proof}
\section{Topoi of commutative von Neumann algebras}
\paragraph*{}
Let $\sX$ be a topological space, end let $\mu$ be a nondegenerate positive Radon measure on $\sX$ such that $\mu\left( \sX\right)= 1$.
Let $\H \bydef L^2\left(\sX, \mu \right)$ be a Hilbert space of square $\mu$-integrable functions, an let   $C_0\left(\sX\right)\hookto B\left( \H\right)$ be a natural representation. If $A \bydef C_0\left(\sX\right)''$ is the bicommutant in $B\left( \H\right)$ then $A$ is a commutative von Neumann $C^*$ algebra (cf. Theorem \ref{von_Neumann_thm}). There is an isomorphism between a locale of  hereditary $C^*$-algebras of $B \subset A$ and a locale $L_\H$ of some  subspaces of $\H$ such that
$$
B \mapsto  B \H \subset \H.
$$
On the other hand the locale $L_\H$ is a measure locale. A point $x \in \sX$ is a $\mu$-\textit{atom}, i.e. $\mu\left(\left\{x\right\} \right)> 0$ then there is a nontrivial subspace $\H_x \subset \H$ and a point (cf. Definition \ref{locale_point_defn}) $L_\H\mapsto f:\left\{\mathbf{true}, \mathbf{false}\right\}$ such that $\H_x\mapsto \mathbf{true}$ and 
$$
\forall \H' \in L_\H \quad \H'\perp \H_x \quad \Rightarrow \quad \H'\mapsto \mathbf{false}.
$$ 
If $L_\H$ has no atoms then for any $\H' \in \H$ there are nontrivial $\H'_1, \H'_2 \in L_\H$ such that $\H'_1 \perp \H'_2 \Leftrightarrow \H'_1 \wedge \H'_2 = \mathbf{false}$ and $ \H'_1 \oplus \H'_2= \H \Leftrightarrow \H'_1 \vee \H'_2 = \H'$. From the Lemma \ref{topos_point_lem} it follows that the locale of hereditary subalgebras of $A$ has no points.

\begin{remark}
From this section it follows that $A$-locale $\mathfrak{Locale}\left(A \right)$ is a measure locale (cf. \cite{pavlov_duality}). Moreover Grothendieck topos yields the described in \cite{pavlov_duality} Gelfand-type duality  between commutative von Neumann algebra and measure locales. On the other hand from the Remark \ref{c_0_x_rem} it follows that this theory yields a Gelfand duality between locally compact Hausdorff spaces and commutative $C^*$-algebras.
\end{remark}
...
\end{itemize}
\begin{remark}
If $C_0\left( \sX\right) \hookto M\left(A \right)$ is  \textit{Hausdorff blowing-up of } $A$ then form the Lemma \ref{lolale_lem} it follows that there is a geometric morphism $\mathscr E_A \xrightarrow{f}\mathscr E_{C_0\left( \sX\right) }$.  If $F$ is an Abelian group then from the  Proposition \ref{spectral_sequence_prop} for any $r\ge 0$ there is a homomorphism
\bean
H^r\left(\mathscr E_{C_0\left( \sX\right) }, F \right) \xrightarrow{} H^r \left(\mathscr E_{A }, f^*\mathfrak{Ass} \left( F_A\right)\right).
\eean
...
\begin{exercise}
	...
	\begin{enumerate}
		...
		\item  for any $B \in \mathscr C^A$ set $\left\{B|_{\sU_\iota}\right\}_{\iota \in I}$ is a covering of $B|_\sU$ (cf. Equation \eqref{c_res_eqn}).
	\end{enumerate}
\end{exercise}
...
\section{Topoi of $C^*$-algebras with Hausdorff spectrum}
\subsection{Grothendieck spaces}
\paragraph{}
Here we prove that any  $C^*$-algebra with Hausdorff spectrum possesses a Grothendieck space.
If $A$ is a $C^*$ with Hausdorff spectrum then from the Theorem \ref{dauns_hofmann_thm} it follows that there is natural Hausdorff blowing-up $C_0\left(\sX \right) \hookto A$. Denote by
	\be\label{ca_f_eqn}
	\mathscr H^A \bydef \left\{B \subset A | B \text{ is a hereditary  } C^*\text{-subalgebra of }  A\right\}.
	\ee
...
\be\label{h_alg_sheaf}
\begin{split}
	\mathscr{P}^A_{\mathrm{full}}\left( \sU\right) \bydef \left\{B\cap~ _\sU A_\sU\left| B \in \mathscr H^A \right. \right\}= \left\{\left.B\in \mathscr H^A \right| B \subset~ _\sU A _\sU \right\};\\
	\rho_{\sU \sV}: \mathscr{P}^A \left(\sU\right)\to  \mathscr{P}^A \left(\sV\right), \quad B \mapsto B \cap~_\sV A_{\sV}.
\end{split}
\ee
\begin{remark}\label{flabby_f_rem}
	We denote by
	\be\label{c_res_f_eqn}
	B|_\sU \bydef B \cap~ _\sU A_\sU.
	\ee $\mathscr{P}^A$
	Since $B|_\sU$ is a hereditary $C^*$-subalgebra of $A$ the presheaf  $\mathscr{P}^A$  is flabby (cf. Definition \ref{sheaf_flabby_defn}),  so $\mathscr{P}^A$ is a sheaf (cf. Definition \ref{sheaf_defn}).
\end{remark}

\section{Topoi of $C^*$-algebras of type $I_0$}
\subsection{Grothendieck spaces}
\paragraph{}
...
\begin{lemma}
There is the {Grothendieck} $CT\left(\sX, \dl \right)$-{space} $\mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)$ (cf. Definition \ref{grothendieck_ca_t_defn}) with the surjective local homeomorphism $\mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)\to \mathrm{Sp\acute{e}}\left(\Om_\sX \right)$. 
\end{lemma}
\begin{proof}
From the Remark \ref{ctr_gen_rem} it follows that $CT\left(\sX, \dl \right)$ is of type $I_0$ (cf. Definition \ref{type_i0_defn}).
From the Theorem \ref{dauns_hofmann_thm} it follows that there is  Hausdorff blowing-up $C_0\left( \sX\right)\hookto CT\left(\sX, \dl \right)$. From the Theorem \ref{grot_type_I_thm} it follows that there is the Grothendieck $CT\left(\sX, \dl \right)$-space\\ $\mathfrak{Space}\left(CT\left(\sX, \dl \right)\right) =\mathscr X^{CT\left(\sX, \dl \right)}$.
\end{proof}
...
\end{proof}
\begin{remark}
From the  Theorem \ref{grot_type_I_thm} and Lemmas \ref{foli_t1_lem}, \ref{blowing_foli_lem} it follows that there is the Grothendieck $C^*_r\left(M,\sF, \sigma\right)$-space $\mathfrak{Space}\left(C^*_r\left(M,\sF, \sigma\right) \right)$ (cf. Definition \ref{grothendieck_ca_t_defn}) and natural surjective local homeomorphism
\bean
...
\section{Comparison with the Bohr topos}
...
\end{definition}
	\begin{theorem}\label{locale_thm}
...
\begin{proposition}\label{locale_funct_prop}
%Proposition 2. 
...
%\end{definition}	
\subsection{Alexandrov topology}\label{alex_top}\cite{johnstone:stone_spaces}.
...
topology, since it is closed under arbitrary unions and intersections. % And we define the \textit{upper interval topology} $\Phi(X, \le)$ (sometimes called the weak topology) to be the smallest topology for which all sets of the form $\downarrow\left(  x\right) $ are closed, i.e. the topology based by sets of the form 
%$$
%X\setminus  \left( \downarrow\left(  x_1\right)\cup ... \cup \downarrow\left(  x_n\right)\right) 
%$$
\subsection{Spatial topoi}\label{spatial_toposes_sec}
\paragraph*{}	
...
\begin{definition}\label{topos_point_defn}
Let $\mathscr S$ be a category of sets and functions, and let $\mathscr E \xrightarrow{\ga} \mathscr S$ be an $\mathcal S$-topos (cf. Definition \ref{relative_topos_defn}). By a \text{point} of $\mathcal E$ we mean a Geometric morphism $\mathcal S \xrightarrow{p} \mathcal E$ over $\mathscr S$. If $K$ is a class of points, we say that $K$ is \textit{sufficient} if the family of functors $\left\{\left.p^*\right| p\in K\right\}$ is conservative, i.e. if any $X \xrightarrow{f} Y$ in $\mathcal E$ such that $p^*\left(f \right)$ is iso for all $K$ is necessary iso. We say that $\mathcal E$ \text{has enough points} if the class of all points of $\mathcal E$ is sufficient.
\end{definition}
...
\end{remark}
\begin{definition}\label{locale_point_defn}
	A \textit{point} of a locale $A$ is a frame homomorphism $A \to \left\{\mathbf{true}, \mathbf{false}\right\}$.
\end{definition}
\begin{lemma}\label{topos_point_lem}
Let $p: A \to \left\{\mathbf{true}, \mathbf{false}\right\}$ be a point and $X = p^{-1}\left(\left\lbrace \mathbf{true}\right\rbrace \right)$.  there is no nontrivial decomposition $X = X'\vee X''$ such that $X'\wedge X'' =  \mathbf{false}$.
\end{lemma}
\begin{proof}
	 If $p\left(X' \right) =\mathbf{false}$ or $p\left(X'' \right) =\mathbf{false}$ then $p\left(X \right)=  p\left(X' \right) \vee p\left(X'' \right)=\left\{\mathbf{true}, \mathbf{false}\right\}$. It follows that  $p\left(X' \right)=  p\left(X'' \right) =\mathbf{true}$ and $p\left(X' \right)\wedge  p\left(X'' \right)= \mathbf{true}$. On the $p\left(X' \right)\wedge  p\left(X'' \right) = p\left(X'\wedge  X'' \right)= p\left( \mathbf{false}\right) = \mathbf{false}$.
\end{proof}
\begin{empt}\label{locale_to_space_empt}
	Let $X$ be a locale, and let $\mathbf{Sh}\left(X \right)$ be a localic topos. 
	 It is proven in \cite{topos:intro} that If $\mathrm{pt}\left( X\right) $ is a set of points then there is the natural topology on $\mathrm{pt}\left( X\right)$. Moreover there is a functor
	 \be\label{locale_to_space_eqn}
	 X \mapsto \mathrm{pt}\left( X\right)
	 \ee
	 from the category of locales to the category of spaces. If a set of points is sufficient then there is an isomorphism of toposes
	 \be\label{locale_to_space_iso_eqn}
	 \mathbf{Sh}\left( X\right) \cong  \mathbf{Sh}\left( \mathrm{pt}\left( X\right)\right).
	 \ee
\end{empt}
...
intrinsically carries the intuitionistic logical structure of a Heyting algebra (cf. Remark \ref{spatial_toposes_rem}).
Here I follow to \cite{joost:borification}. For any $C^*$-algebra 
...
\\
\\
A partially ordered set yields a locale $L(A)$.
\begin{definition}
	The \textit{Bohr topos} of $C^*$-algebra $A$ is a localic topos $\mathbf{Sh}\left( L(A)\right)$. This topos is spatial.
\end{definition}
