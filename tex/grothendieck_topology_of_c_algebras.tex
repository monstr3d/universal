...
From the Remark \ref{pretopology_to_topology_rem} it follows that the given by the Lemma Grothendieck pretopology can be replaced by the natural Grothendieck topology $J_A$(cf Definition \ref{grotendieck_topolgy_defn})
\end{empt}

\begin{definition}\label{grothendieck_ca_defn}
The explained in  \ref{grothendieck_ca_empt} Grothendieck topology $J_A$  is said to be an  $A$-\textit{topology}. A corresponding  site (cf. Definition \ref{grotendieck_topolgy_defn}  ) $\mathbf{T}_A\bydef \left(\mathbf{Hered}/A, J_A\right)$ is said to be \textit{arising} from $A$ (cf. Definition \ref{top_gro_definition}).
\end{definition}
\begin{definition}\label{grothendieck_ca_t_defn}
	Let $A$ be a $C^*$-algebra. If  there is a topological space $\mathfrak{Grot}\left(A \right) $ such that a site $\mathbf{T}_{A}\bydef \left(\mathbf{Hered}/A, J_A\right)$ 	
...
\begin{enumerate}
	...
	\item[(b)]  $\mathcal E$ is \textit{cartesian closed}, i.e. for each object $X$ we have an \textit{exponential functor} $\left(\cdot\right)^X$ which is right adjoint to the functor $\left(\cdot\right)\times X$.
...
\end{definition}
	...
	\begin{example}\label{top_gro_exm}
		...
		$$
		If $\sU \subset \sX$ is an open subset then we assume that a family  $\left\{\sU_\iota \subset \sU\right\}_{\iota\in I}$, is a covering of $\sU$ (cf. Definition \ref{pretopology_defn}   !!! NO SITE !!!) if and only if  $\sU = \cup_{\iota\in I}\sU_\iota$. This system of coverings is a specialization of Grothendieck topology. 
	\end{example}
