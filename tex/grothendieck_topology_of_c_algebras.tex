...
\ee
will be used. 
Given $B', B'' \in  \mathbf{Hered}/A$ such that $B' \subset B''$ (i.e. there is $\mathbf{Hered}/A$-morphism $B'' \to B'$) there is the   natural inclusion  the natural surjective  homomorphism of groups
\be\label{ctr_ub_eqn}
...
\end{lemma}
\begin{proof}
If $B$ is a   $\mathbf{Hered}/CT\left(\sX, \dl \right)$-object then a covering of $B$ is a finite family $\left\{B_1, ..., B_n\right\}$ of  $\mathbf{Hered}/CT\left(\sX, \dl \right)$-objects such that  $B = \bigcap_{\iota=1}^n B_\iota$. If $L\left( B\right)$ and $L\left( B_\iota \right)$ are given by the equation \eqref{left_ideal_eqn} then $ L\left( B\right)= \bigcap_{\iota =1}^n L\left( B_\iota\right)$.  Further proof contains two parts:
\begin{enumerate}
...
\begin{lemma}
PAGE 141. Lemma 5.47. Suppose A is a continuous-trace C?-algebra with spectrum T, ? ? AutC0(T) A, and t ? T. Then there are a compact neighbourhood F of t and a
unitary multiplier u ? UM(AF ) such that ?F = Ad u.
\end{lemma}
\paragraph{!!!DOWN}
