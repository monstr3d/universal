...
(a) Let $B, C\subset A$ be subalgebras. A covering of $B$ is a family $\left\{B_\iota\subset B\right\}_{\iota \in I}$ of subalgebras of $B$ such that $B$ is a generated by a union $\bigcup B_\iota$ hereditary $C^*$-subalgebra (cf. Definition \ref{hered_gen_defn}). If  $L\left( B\right)$, $L\left( C\right)$ and $L\left( B_\iota \right)$ are given by the equation \eqref{left_ideal_eqn} left ideals then $L\left(B\cap C \right) = L(B)\cap L(C)$, $~L\left(B_\iota \cap C \right)= L\left(B_\iota \right)\cap L\left( C \right)$. Moreover $L\left(B\right)$ is the minimal left ideal which contains a union $\cup_\iota L\left( B_\iota\right)$. From this fact it follows that $L(B)\cap L(C)$ is the minimal left ideal which contains a union $\cup_\iota L\left( B_\iota\cap C\right)$. Now using the Lemma \ref{hered_ideal_lem} one can deduce that $B\cap C$ is a generated by a union $\cup_\iota\left(  B_\iota \cap C\right) $ hereditary subalgebra (cf. Definition \ref{hered_gen_defn}), i.e. 
...
\begin{itemize}
	\item $L\left( B_\iota \right)$ is a minimal left ideal which contains a union $\cup_\iota L\left( B_\iota \right)$,
	\item for all $\iota$ the set $L\left( B_\iota \right)$ is a minimal left ideal which contains a union $\cup_j L\left( C_{\iota j} \right)$.
\end{itemize}
From these facts it follows that $L\left( B_\iota \right)$ is a minimal left ideal which contains a union $\cup_{\iota j} L\left( C_{\iota j} \right)$. Taking into account  the Lemma \ref{hered_ideal_lem} one can deduce that $B$ is a generated by a union $\cup_{\iota j}  C_{\iota j}$ hereditary subalgebra (cf. Definition \ref{hered_gen_defn}), i.e. 
