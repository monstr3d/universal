...
\begin{definition}\label{grothendieck_ca_defn}
	The given by the Exercise \ref{grothendieck_ca_exer} topology  is said to be a \textit{Grothendieck} $A$-\textit{topology}.
\end{definition}
...
$$
where $\rho_x: A \to B\left(\H_x\right)$ is an irreducible representation which corresponds to $x$. We define a presheaf $\mathscr{P}^{'\text{comm}}$ on $\sX$ such that if $\sU$ and $\sV$ are open subsets of $\sX$ then
\bean
...
Let $\mathscr{P}^{\text{comm}}\bydef \mathfrak{Ass} \left(\mathscr{P}^{'\text{comm}} \right)  $ be an associated sheaf (cf. Definition \ref{site_sheaf_ass_defn}).
Let $\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)$ be 	\'espace etal\'e  (cf. Exercise \ref{sheaf_etale_exer}). of the presheaf $\mathscr{P}^{\text{comm}}\left(\sU\right)$. 
The space  $\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)$ is a union of all stalks of the presheaf $\mathscr{P}^{\text{comm}}$. There is a surjective continuous map $p_\sX: \mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)\to \sX$ (cf. Exercise \ref{sheaf_etale_exer}). If $A_0\bydef \{0\}\subset A$ is a trivial commutative subalgebra then it defines a section $s^0  \in \mathscr{P}^{\text{comm}}\left(\sX\right)$. Let
\bean
\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)_0 \bydef \bigcup_{x \in \sX} s^0_x,\\
\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)' \bydef \mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)\setminus \mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)_0
\eean  
where $s^0_x$ is a stalk of $s^0$ at $x$ (cf. Definition \ref{stalk_defn}). For any commutative $C^*$-subalgebra $C \subset A$ denote by
\be\label{ctr_top_eqn}
\mathscr U_C \bydef \left\{ \left. y \in \mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}}\right)' \right| C \quad \text{ is representative of}\quad  y\right\}	
\ee
\begin{exercise}\label{ctr_open_exer}
	Prove that a family of the given by \eqref{ctr_top_eqn} sets is a basis of the topology of $\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}}\right)'$.
\end{exercise}
...
$$
\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}}\right)'_B\bigcup_{C \in  \text{Com}\left(B\right)} \mathscr U_C 
$$
is an open subset of $\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}}\right)'$. Conversely if $\mathscr U\subset \mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}}\right)'$ is an open subset then for all $y \in \mathscr U$ there is a  commutative subalgebra $C \subset A$ such that if a section $s^C \in \mathscr P^{\text{comm}}\left(\sX \right)$ corresponds to $C$ then one has
\bean
y = s^C_{p_\sX\left(y \right) },\\
\forall x \in \sX \quad s^C_x \in \mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}}\right)' \quad \Rightarrow \quad  s^C_x\in \mathscr U.
\eean  
...
\end{proof} 
\begin{corollary}
		If $A$ is a continuous trace $C^*$-algebra (cf. Definition \ref{continuous_trace_c_alt_defn}) then the {Grothendieck} $A$-{topology} (cf. Definition \ref{grothendieck_ca_defn}) corresponds   the	space $\mathrm{Sp\acute{e}}\left(\mathscr P^{\mathrm{comm}} \right)'$.
\end{corollary}
\begin{proof}
...
q_\sX\bydef p_\sX|_{\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)'}: \mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)' \to \sX.
\ee
(cf. Exercise \ref{sheaf_etale_exer}). Suppose that there is a commutative subalgebra $C \subset A$ such that $\rho\left(C \right) \neq \{0\}$ for any irreducible representation $\rho : A \to B\left(\H_\rho\right)$.
...
\eean
Let $F$ be an Abelian group, and let $\sF'$ be a corresponding to $F$ constant presheaf on $\sX$ (cf. Definition \ref{constant_presheaf_defn}). Let $\sF$ be a sheaf associated with $\F'$ (cf. Definitions \ref{site_sheaf_ass_defn} and \ref{sheaf_prdf}). If $q^{-1} \sF$ is an inverse image (cf. Definition \ref{sheaf_inv_im_defn}) then $q^{-1} \sF\left( \sV \right)\cong F$ for any connected open set $\sV \subset (\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)'$. It follows that $q^{-1} \sF$ is an associated to a constant sheaf on $\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)'$ which corresponds to $F$. From \eqref{blowing_p_j_eqn} it follows that there is an isomorphism $j^{-1}q^{-1} \sF\cong \F$. From (i) of the Proposition \ref{spectral_sequence_prop} for any $r\ge 0$ one has homomorphisms.
\bean
H^r\left( \sX, \sF\right) \xrightarrow{H^r\left( q^{-1}\right) } H^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)', q^{-1}\F  \right);\\
H^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)', q^{-1}\F  \right) \xrightarrow{H^r\left( j^{-1}\right) } H^r\left( \sX, \sF\right).
\eean
From \eqref{blowing_p_j_eqn} it follows that
\be\label{blowing__p_j_c_eqn}
H^r\left( j^{-1}\right)\circ H^r\left( q^{-1}\right)= \Id_{H^r\left( \sX, \sF\right)}.
\ee


\begin{theorem}\label{blowing_grothendiek_thm} 	Let $A$ be a continuous trace $C^*$-algebra (cf. Definition \ref{continuous_trace_c_alt_defn}) and let $F$ be an Abelian group. If there is a commutative subalgebra $C \subset A$ such that $\rho\left(C \right) \neq \{0\}$ for any irreducible representation $\rho : A \to B\left(\H_\rho\right)$ then for all $r \ge 0$ there is an isomorphism
		$$
		H^r\left(A, F \right)	\cong H^r\left(\sX, F  \right)	\oplus X^r
		$$
		where  $X^p$ is an Abelian group and the notations \eqref{a_const_shef_coh_eqn}, \eqref{const_shef_coh_eqn} are  used.
\end{theorem} 
\begin{proof}
 From the above construction it turns out that  that $p^{-1} \sF$ is an associated to a constant sheaf on $\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)'$ which corresponds to $F$. If $$X^r \bydef \ker \left(   H^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)', p^{-1}\F  \right) \xrightarrow{H^r\left( j^{-1}\right) } H^r\left( \sX, \sF\right)\right) $$ then from the equation \eqref{blowing__p_j_c_eqn} it turns out that
	$$
	H^r\left(\mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)', F \right)	\cong H^r\left(\sX, F  \right)	\oplus X^r.
	$$
...
If  $A$ is a  stable separable  continuous trace $C^*$-algebra with spectrum $\sX$ then from the Proposition \ref{ctr_bundle_prop}
it follows that  there is a locally
trivial bundle $\pi: \F\to \sX$ with fibre $\K(\H)$ and structure group $\Aut \left( \K(\H)\right)$ such that $A$ is $C_0\left( \sX\right)$ -isomorphic to the space of sections $\Ga_0\left(\sX, \F \right)$. The space $\F$ as a set equals to a union 
$$
...
$$
\E  \bydef \bigcup_{x\in\sX} \left\{e \in \K\left(\H_x \right)| ~ e\quad\text{is a rank-one projector}\right\}
$$
then one has a locally trivial subbundle $\pi|_\E \E\to  \sX$, 
and a fiber of the bundle equals to $\C P^\infty$. If $s_x \in \mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)'_x$ is represented by a commutative $C^*$-subalgebra $C\subset A$ then one has a one-dimensional 
subspace $\rho_x\left(C \right) \H_x \subset \H_x$. If we denote by $p_x\in \K\left(\H_x \right)$ a rank one projector onto $\rho_c\left(C \right) \H_x$ then one has a surjective map
\bean
\begin{split}
\psi : \mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)'\to \E,\\
s_x \mapsto p_x.
\end{split}
\eean
One can prove that the map $\psi$ is continuous.
\begin{theorem}	If  $A$ is a  stable separable  continuous trace $C^*$-algebra with spectrum $\sX$  then for all $r \ge 0$ there is a natural homomorphism
	$$
	H^r\left(\E, F \right)	\to H^r\left(A, F  \right)
	$$
	where  the notations \eqref{a_const_shef_coh_eqn} and \eqref{const_shef_coh_eqn} are  used.
\end{theorem} 
\begin{proof}
Follows from the existence of the continuous surjective  map $\psi : \mathrm{Sp\acute{e}}\left(\mathscr P^{\text{comm}} \right)'\to \E$ Remark \ref{geometric_morphism_rem} and (i) of the Proposition \ref{spectral_sequence_prop}.
\end{proof}
There is a sheaf $\mathscr F$ of local sections of a the bundle $\F$. Let $\mathscr G'$ be a presheaf given by
$$
\mathscr G' \left(\sU \right) \bydef \left\{s \in  \mathscr F\left( \sU\right) | \exists a \in A \quad \forall x \in \sU \quad \rep_x\left( s\right) \quad\text{is a rank-one projector or zero}  \right\}
$$
and let Let $\mathscr{G}\bydef \mathfrak{Ass} \left(\mathscr{G'} \right)  $ be an associated sheaf (cf. Definition \ref{site_sheaf_ass_defn})be an associated sheaf.
\begin{lem}
There is a natural isomorphism of $\mathscr{G} \cong \mathscr  P^{\text{comm}}$ of sheaves
\end{lem}
\begin{proof}
	!!!
\end{proof}
...
\begin{example}\label{top_gro_exm}
	If $\sX$ is a topological space then one has a category of open subsets and inclusions. If $\sU \subset \sX$ is an open subset then we assume that a family  $\left\{\sU_\iota \to \sU\right\}_{\iota\in I}$, is a covering of $\sU$ (cf. Definition \ref{site_defn}) if  $\sU = \cup_{\iota\in I}\sU_\iota$. This system of coverings is Grothendieck topology.
\end{example}
...
\end{enumerate}
\begin{empt}
	For any Abelian group $A$ one can define a constant presheaf $\mathscr P$ of Abelian groups (cf. Definition \ref{constant_presheaf_defn}). 
...
\begin{exercise}
\end{exercise}\label{sheaf_etale_open_exer}
...
\begin{empt}
	!!! MAP !!!
Let us suppress the coefficient Abelian group $A$. For each topological space $\sX$ we obtain the Cech system . %This is an inverse system in the homotopy category dexed by the set of open covers U? of X? which is partially ordered by U≤V?≤? if V? refines U?. The term XU?? is the geometric realization of the nerve N(U)?(?) of U?, and the bonding map pVU:XV?XU???:????? is the (uniquely determined) homotopy class of any "refinement map" rVU:N(V)?N(U)???:?(?)??(?) which chooses for V?V??? a member rVU(V)?U???(?)?? such that V?rVU(V)?????(?). Usually one restricts to normal covers (these have a subordinated partition of unity), but this only a technical point.Application of Hn?? produces a direct system Hn(C(X))??(?(?)) of abelian groups whose direct limit is defined to be the n?-th Cech cohomology group of X?.Each map f:X?Y?:??? induces a map of index sets f#:U(Y)?U(X)?#:?(?)??(?) and a map of inverse systems f––:C(X)?C(Y)?_:?(?)??(?). The latter induces Hn(f––):Hn(C(Y))?Hn(C(X))??(?_):??(?(?))???(?(?)). The direct limit functor gives you an induced map on the Cech cohomology groups.

\end{empt}

\begin{empt}\cite{bryl:loop}
...
\begin{lemma}\cite{hartshorne:ag}
	!!!!
	Let $\sX$ be a topological space, and U an open covering. Then 
	for each $r \ge 0$ there is a natural map, functorial in $\mathscr F$.
	$$
	\check{H}^r\left( \right) \to H^r\left( \right) 
	$$
\end{lemma}
\begin{corollary}
	CHECH
...
\begin{proposition}\label{ctr_bundle_prop}\cite{rae:ctr_morita}
	...
	trivial bundle $\left( \mathscr F, \pi,\sX\right) $ with fibre $\K(\H)$ and structure group $\Aut \left( \K(\H)\right)$ such that $A$ is $C_0\left( \sX\right)$ -isomorphic to the space of sections $\Ga_0\left(\sX, \mathscr F \right)$. 
