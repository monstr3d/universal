...
A $C^*$-algebra 	$A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}$ is generated by subalgebras of $B$ so one has an inclusion	$A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}\subset B$.
	From the Proposition \ref{ctr_hered_prop} it follows that both $C^*$-algebras $B$ and $A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}$ have continuous trace. 
	If $B \neq A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)'_B}$ then  $ A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}\subsetneqq B$, and taking into account the Remark \ref{ctr_gen_rem} there is a positive element $a \in B_+\setminus A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)'_B}$ such that that $\rank \pi\left(a \right) \le 1$ for any irreducible representation $\pi: B \to B\left(\H \right)$. Since Pedersen's ideal $K(B)$  of $B$ is dense in $A$ one can suppose that $a \in K\left( B\right)$. It turns out that there is a compact subset of $\sX$ such that $\pi_x\left( a\right)=0$ for all $x \in \sX \setminus \sU$ where $\pi_x : B \to B\left(\H_x\right)$ is a corresponding to $x$ irreducible representation. There is $\eps > 0$ such that 
\be\label{gee_eqn}
	\left\| a - a'\right\| \ge \eps
	\ee 
	for all $a' \in A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)'_B}$. Let up prove that there is $x_0\in \sX$ such that $\left\| \pi_{x_0}\left( a-a'\right)\right\| \ge \eps$. If not then for any $x \in \sX$ there is $a_x \in A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)'_B}$ such that $\left\| \pi_{x}\left( a-a'\right)\right\| <\eps$. From the 
Lemma \ref{ctr_rep_eq_lem} it turns out that there is a neighborhood $\sU_x$ of $x$ such that $\left\| \pi_{x'}\left( a-a'\right)\right\| <\eps$ for all $x' \in \sU_x$. From $\sU \subset \bigcup_{x\in \sU}\sU_x$ and taking into account that the set $\sU$ is compact there is a finite set $\left\{x_1, ..., x_n\right\}\in \sU$ such that $\sU \subset \bigcup_{j = 1}^n\sU_{x_j}$. There is a set $f_1, ..., f_n \in C_c\left( \sX\right)_+$ of positive functions such that for all $j \in {1,...,n}$ support of $f_j$ is a subset of $\sU_{x_j}$ and
$$
\forall x \in \sU \quad \sum_{j = 1}^nf_j\left(x  \right) = 1.
$$
If $a' \bydef \sum_{j = 1}^nf_j a_{x_j} $ then from our construction it follows that $a' \in A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)'_B}$ and $\left\| a - a'\right\| \ge \eps$, this fact contradicts with the equation \eqref{gee_eqn}, so there is $x_0\in \sX$ such that $\left\| \pi_{x_0}\left( a-a'\right)\right\| \ge \eps$ for all $a' \in A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)'_B}$. If $B^a$ is generated by $a$ hereditary commutative subalgebra of $B$ then 
from $\pi_{x_0}\left( B^a\right)\not\subset  \pi_{x_0}\left(A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)'_B}\right)$  it follows that
	$$
	B^a \cap A|_{\sV}\not\subset A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}
	$$
	where $\sV$ is an open neighborhood of $x$.
