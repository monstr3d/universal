...
If $\sX$ is a compact metrizable space then one has:
\begin{enumerate}
	\item [(i)] for any finite covering $\left\{\sU_\iota \right\}_{\iota \in I}$ of $\sX$ by open subsets there is a finite  covering $\left\{\sV_\iota \right\}_{\iota \in I}$ of $\sX$ by closed  subsets such that 
...
\end{split}
\ee
\item[(ii)]  for any finite covering $\left\{\sV_\iota \right\}_{\iota \in I}$ of $\sX$ by closed subsets there is a finite  covering $\left\{\sU_\iota \right\}_{\iota \in I}$ of $\sX$ by open  subsets which satisfies to the condition \eqref{top_open_closed_cech_eqn}.
\end{enumerate}
\end{lemma}
\begin{proof}
(i)  Let $\mathfrak{dist}\left(\cdot, \cdot \right): \sX^2 \to \R$ be the distance on $\sX$.  If $r$ be a positive real number then the set
\bean
\sU^r_\iota \bydef \left\{x \in \sU_\iota | \forall y \in \sX\setminus   \sU_\iota \quad \mathfrak{dist}\left(x, y \right)> r\right\}\subset \sX
\eean
is open. Moreover 
$$
\sU_\iota = \bigcap_{r > 0} \sU^r_\iota
$$
It follows that 
$$
\sX = \bigcup_{\iota \in I} \bigcup_{r > 0} \sU^r_\iota
$$
Since $\sX$ is compact there is $r_0 > 0$ such that
$$
\forall r \in \R \quad  0 < r < r_0 \quad \Rightarrow \quad  \sX = \bigcup_{\iota \in I}  \sU^r_\iota.
$$
If  $\left(\iota_0, ..., \iota_{n-1}\right)\in I^n$ is such that   $\sU_{\iota_0,..., \iota_{n-1}}\bydef\sU_{\iota_0} \cap ... \cap \sU_{n-1}\neq \emptyset$ then for any point $x \in \sU_{\iota_0} \cap ... \cap \sU_{n-1}$ one has
$$
\inf_{y \in \sX \setminus \sU_{\iota_0,..., \iota_{n-1}} } \mathfrak{dist}\left(x, y \right) > 0.
$$
For all $\sU_{\iota_0,..., \iota_{n-1}}$ we select an arbitrary point $x \in \sU_{\iota_0,..., \iota_{n-1}}$ and denote by
$$
r_{\iota_0,..., \iota_{n-1}}\bydef \frac{\inf_{y \in \sX \setminus \sU_{\iota_0,..., \iota_{n-1}} } \mathfrak{dist}\left(x, y \right) }{2}.
$$
If $r_f < r_0$ and $r < r_{\iota_0,..., \iota_{n-1}}$ for all $\sU_{\iota_0,..., \iota_{n-1}}\neq \emptyset$ then one has
\bean
 \sX = \bigcup_{\iota \in I}  \sU^{r_f}_\iota,
 \forall \iota \\
 \forall \left(\iota_0, ..., \iota_{n-1}\right)\in I^n \quad \sU_{\iota_0} \cap ... \cap \sU_{n-1}\neq \emptyset\quad \Leftrightarrow \quad \sU^{r_f}_{\iota_0} \cap ... \cap \sU^{r_f}_{n-1}\neq \emptyset,
\eean 
If
\be\label{top_open_closed_c_cech_eqn}
 \sV^{r_f}_\iota\bydef \text{ the closure of } \sU^{r_f}_{\iota}
 \ee 
  then the family $\left\{\sV^{r_f}_\iota \right\}_{\iota \in I}$ satisfies to the condition \ref{top_open_closed_cech_eqn}.\\
(ii) Let $\left\{\sV_\iota \right\}_{\iota \in I}$ be a finite closed covering of $\sX$.  If  $\sV_{\iota_0} \cap ... \cap \sV_{n-1}=\emptyset$ then
$$ 
\inf_{\substack{x \in \sX \setminus \sV_{\iota_0}\\ y \in \sV_{\iota_1,..., \iota_{n-1}} }} \mathfrak{dist}\left(x, y \right) > 0
$$
If 
 \bean
r_{\iota_0,..., \iota_{n-1}}\bydef \frac{1}{2}{\inf_{\substack{x \in \sX \setminus \sV_{\iota_0}\\ y \in \sV_{\iota_1,..., \iota_{n-1}} }} \mathfrak{dist}\left(x, y \right)},\\ \\
r_f \in \R \quad \sV_{\iota_0} \cap ... \cap \sV_{n-1}=\emptyset \Rightarrow r_f < r_{\iota_0,..., \iota_{n-1}}
\eean
and
$$
\sU^{r_f}_\iota \bydef \left\{x \in \sX | \exists y \in \sV_\iota\quad  \mathfrak{dist}\left(x, y \right)< r_f\right\}
$$
then a family $\left\{\sU^{r_f}_\iota \right\}_{\iota \in I}$ satisfies to the conditions \eqref{top_open_closed_cech_eqn}.
\end{proof}
...
\end{lemma}
\begin{proof}
Let $\left\{\sV^{r_{c}}_\la\right\}_{\la \in \La}$  be a closed covering of $\sX$ given by (i)  of the Lemma \ref{top_open_closed_cech_lem} i.e
\bean
\begin{split}
	\forall \la \quad \sV_\la \subset \sU_\la,\\
	\forall \left(\la_0, ..., \la_{n-1}\right)\in I^n \quad \sU_{\la_0} \cap ... \cap \sU_{\la_{n-1}}\neq \emptyset\quad \Leftrightarrow \quad \sV^{r_{c}}_{\la_0} \cap ... \cap \sV^{r_{c}}_{\la_{n-1}}\neq \emptyset.
\end{split}\eean 
If $\left\{\sV_\iota \right\}_{\iota \in I}$ is a refinement of both $\left\{\sV^{r_{c}}_\la\right\}_{\la \in \La}$ and $\left\{\sV_\th\right\}_{\th \in \Th}$ then there is a map $\phi : I \to \La$ such that $\sV_\iota \subset \sV_{\phi\left(\iota \right) }$. Otherwise from (ii) of the Lemma   \ref{top_open_closed_cech_lem} is $r_o$ such that for any $r \le r_o$ one has
\bean
\begin{split}
	\forall \iota \quad \sV_\iota \subset \sU^{r_o}_\iota,\\
	\forall \left(\iota_0, ..., \iota_{n-1}\right)\in I^n \quad \sU^{r_o}_{\iota_0} \cap ... \cap \sU^{r_0}_{\iota_{n-1}}\neq \emptyset\quad \Leftrightarrow \quad \sV_{\iota_o} \cap ... \cap \sV_{\iota_{n-1}}\neq \emptyset,
\end{split}
\eean
If $r < r_0$ and $r < r_c$ and $\sU_\iota \bydef \sV^r_\iota$ then for all $\iota \in I$ one has $\sU_\iota \subset \sU_{\phi\left(\iota \right) }$ i.e. $\left\{\sU_\iota\right\}_{\iota \in I}$ is a refinement of $\left\{\sU_\la\right\}_{\la \in \La}$. Moreover one has
\
...
If $\sX$ is a compact metrizable space then for $\mathscr A\in \mathbf{Ab}\left( \sX\right)$ and  any $p$ there is a natural isomorphism
\be\label{cech_n_eqn}
...
Suppose that $\mathscr F$ is a flasque sheaf on $\sX$.
	...
	(I, §1) for irreducible topological spaces. 
%	(b) If 0 —> .?' -> & -> .?" -> 0 is an exact sequence of sheaves, and if .?' is 
%	flasque, then for any open set l\ the sequence 0 -> W\\j) -> #(U) -> 
	3?"(lr) -> 0 of abelian groups is also exact. * 
%	(c) If 0 -> J^' -> J? -> .?" -> 0 is an exact sequence of sheaves, and if &' and 3F 
	are flasque, then 5F" is flasque. 
