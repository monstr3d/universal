...
\begin{definition}\label{conn_defn!} !!! DELETE !!!
	We say that a $C^*$-algebra $A$ is \textit{connected} if it is not a direct sum $A' \oplus A''$ of nontrivial $C^*$-algebras. We say that a $C^*$-algebra $A$ is \textit{finitely connected} if it is a finite direct sum of connected $C^*$-algebras, i.e $A \cong A_1 \oplus ... \oplus A_n$. If $A = A' \oplus A''$ and $A'$ is a connected $C^*$-algebra then we say that $A'$ is a \textit{connected component} of $A$.
\end{definition}
...
\end{exercise}
\begin{lemma}\label{const_sheaf_morphism_lem}
...
\begin{proof}
Firstly we construct a functor 
\bean
A_{\overline\varphi} \bydef \mathbf{Hered}/  A \xrightarrow{ \overline\varphi} \mathbf{Hered}/ \widetilde A \xrightarrow{\mathbf{y}} \mathbf{Sets}^{\mathbf{Hered}/ \widetilde A^{\mathrm{op}}} \xrightarrow{\mathfrak{Ass}} \mathbf{Sh}\left(\mathbf{Hered}/ \widetilde A, J_{\widetilde A} \right)
\eean 
(cf. \eqref{geom_ep_im_eqn}) where $\overline \varphi$ is given by \eqref{geom_functor_eqn}. iF $B$ is a 
\end{proof}
