...
\end{lemma}
\begin{proof}
If $A$ be a $C^*$-algebra, then one needs check that hereditary $C^*$-subalgebras of $A$ satisfy to conditions (a)-(c) of the Definition \ref{site_defn}.\\
(a) Let $B, C\subset A$ be subalgebras. A covering of $B$ is a family $\left\{B_\iota\subset B\right\}_{\iota \in I}$ of $C^*$-subalgebras  such that $B$ is a generated by a union $\bigcup B_\iota$ hereditary $C^*$-subalgebra. Form the Lemma \ref{hered_lem} it follows that $B$ is a $C^*$-norm completion of *-subalgebra $B'$ such that any $b \in B'$ is given by
$$
b= \sum_{k = 1}^n b'_k a_k b''_k \quad b'_k, b''_k \in \bigcup B_\iota \quad a_k \in A.
$$
Similarly a generated by  a union $\cup \left( B_\iota\cap C\right)$ hereditary $C^*$-subalgebra $B_\cap$ is a $C^*$-norm completion of *-subalgebra $B'_\cap$ such that any $b \in B'_\cap$ is given by
$$
b= \sum_{k = 1}^n b'_k a_k b''_k \quad b'_k, b''_k \in \bigcup B_\iota\cap C \quad a_k \in A.
$$
 From 	$\cup \left( B_\iota\cap C\right)\subset B\cap C$ it follows that $B_\cap \subset B\cap C$. If $\left\{u_\a\right\}_{\a \in \A}$ is an approximate identity of $C$ and $b'_\cap \in  B'\cap C$  then one has:
 \begin{itemize}
 	\item $\lim_\a u_\a b'_\cap u_\a = b'_\cap$,
 	\item  $ u_\a b'_\cap u_\a\in B_\cap$ for any $\a \in \A$.
 \end{itemize}
 It follows that $B'\cap C \subset B_\cap$. If $b \in B\cap C$ and  $\eps > 0$ then there is $b' \in B'$ such that $\left\| b' - b\right\| < \eps/2$.  Moreover there is $\a_0 \in \A$ such that $\left\| u_{\a_0} b u_{\a_0} - b\right\| < \eps/2$. From $\left\| u_{\a_0} \right\|\le 1$ it follows that
 $$
 \left\|b- u_{\a_0} b' u_{\a_0}\right\|\le \left\| u_{\a_0}\left(b-  b'\right)  u_{\a_0}\right\|+ \left\| b - u_{\a_0}b  u_{\a_0}\right\|< \left\| u_{\a_0}\right\|^2 \left\| b' - b\right\|+ \eps/2< \eps.
 $$
However $u_{\a_0} b' u_{\a_0}\in B'\cap C\subset B_\cap$, and  since $\eps$ is arbitrary small and $B_\cap$ is $C^*$-norm closed one has $b \in B_\cap$. It follows that $B_\cap= B\cap C$.
So the intersection $B\cap C$ is a generated by a union $\cup_\iota\left(  B_\iota \cap C\right) $ hereditary subalgebra (cf. Definition \ref{hered_gen_defn}), i.e. 
a family $\left\{B_\iota\cap C \subset B\cap C\right\}_{\iota\in I}$ is a {covering} of $B\cap C$ (cf. Definition \ref{hered_cov_defn}).
\\
(b) Let a family $\left\{B_\iota\subset B\right\}$ be a covering of $B$, and let for all $\iota$ a family $\left\{C_{\iota j}\subset B_\iota \right\}$ be a covering of $B_\iota$. From $\cup_{\iota j}C_{\iota j}\subset B$ it follows that if $B_\cup$ is generated by a union $\cup_{\iota j}C_{\iota j}$ hereditary algebra then $B_\cup \subset B$. If $b \in B$ then for any $\eps > 0$ there is a sum
$$
b'= \sum_{k = 1}^n b'_k a_k b''_k \quad b'_k, b''_k \in \bigcup B_\iota \quad a_k \in A
$$
such that $\left\|b- b'\right\|< \eps/2$. For any $k =1...,n$ there is a sum
$$
c_k= \sum_{l = 1}^{m_k} c'_{lk} a_{lk} c''_{lk} \quad c'_{lk}, c''_{lk} \in \bigcup C_{\iota j} \quad a_{lk} \in A
$$
such that $\left\|c_k- b'_k a_k b''_k\right\|< \frac{\eps}{2n}$. It follows that
$$
\left\|b - \sum_{k = 1}^n\sum_{l = 1}^{m_k} c'_{lk} a_{lk} c''_{lk} \right\|< \eps,
$$
Since $\sum_{k = 1}^n\sum_{l = 1}^{m_k} c'_{lk} a_{lk} c''_{lk}\in B_\cup$, number $\eps$ is arbitrary small and $B_\cup$ is $C^*$-norm closed we conclude that $b \in B_\cup$, i.e. $B = B_\cup$.
\\
(c) Evident.
\end{proof}
...
%	\bt\text{-}\lim_\la u_\la = 1_{ M\left( B\right) },\\
	...
	\end{split}
	\ee
	From the Lemma \ref{increasing_convergent_w_lem} the net $\left\{\rho\left( u_\la\right)  \right\}$  is convergent  with respect to the strong topology of $B\left(\H \right)$ (cf. Definition \ref{strong_topology_defn}). If $p \bydef s$-$\lim\rho\left(  u_\la \right) \in B\left( \H\right)$ is a strong limit then $p$ lies in the strong closure of $\rho(A)$ (cf. Definition \ref{strong_topology_defn}). One can prove that $p$ is a projector.
...
\end{lemma}
\begin{proof}
	For any $\xi \in p\H\setminus\{0\}$ there is $a \in A$ such that $a \xi \neq 0$. From the equation \eqref{four_decompositon_eqn} we can suppose that $a$ is positive. If  $\left\{u_\la \right\}_{\la \in \La} \subset  B_+$ is an  approximate unit of $B$ (cf. Definition \ref{approximate_unit_defn}) then one   has a norm limit $\lim_{\la \in \La}u_\la \xi = \xi$, it follows that  $\lim_{\la \in \La}a u_\la \xi = a\xi$. There is $\la_0\in \La$ such that $a u_{\la_0} \xi \neq 0$, so one has
	\bean
...
\end{proof}
A combination of the above results yields the following.
...
If $A$  is a $C^*$-algebra with Hausdorff locally compact spectrum  $\sX$ then any ideal of $A$ corresponds to an open set $\sU\subset\sX$ we denote this ideal by 
...
If $A = CT\left(\sX, \dl \right)$ (cf. Notation \ref{ctr_not})  then we define a presheaf $\mathscr{P}'^\dl $ on $\sX$ such that for pair  $\sU\subset\sV\subset\sX$ of open subsets one has
\bean
...
As a set the space  $\mathrm{Sp\acute{e}}\left(\mathscr P^\dl  \right)$ is a union of all stalks of the presheaf $\mathscr{P}'^\dl $. There is a surjective continuous map $\mathrm{Sp\acute{e}}\left(\mathscr P^\dl  \right)\to \sX$ (cf. Exercise \ref{sheaf_etale_exer}).
The set
\bean
...
!!! CONTINUE !!!
\begin{proof}
A $C^*$-algebra 	$A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}$ is generated by subalgebras of $B$ so one has an inclusion	$A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}\subset B$, i.e.    $A_{\mathrm{Sp\acute{e}}\left(\mathscr P^\dl \right)_B}$ is a hereditary subalgebra of $B$.
...
trivial bundle $\pi_\dl: \F^\dl\to \sX$ with fibre $\K(\H)$ and structure group $\Aut \left( \K(\H)\right)$ such that $A$ is $C_0\left( \sX\right)$ -isomorphic to the space of sections $\Ga_0\left(\sX, \F \right)$. The space $\F^\dl$ as a set equals to a union 
$$
...
and with fibre $\C P^\infty$ and structure group $\Aut \left( \K(\H)\right)$. If $s_x \in \mathrm{Sp\acute{e}}\left(\mathscr P^\dl  \right)'_x$ is represented by a commutative $C^*$-subalgebra $C\subset A$ then one has a one-dimensional 
...
\end{split}
\eean
One can prove that the map $\psi_\dl$ is continuous. From the equations \eqref{hom_f_eqn}, \eqref{cech_hom_eqn} and the Remark the Remark \ref{equiv_rem} it turns out that for all $r \ge 0$ and an Abelian group $F$ there are homomorphisms
\bean
...
\eean 
are homomorphisms of rings (cf. Exercise \ref{ring_homo_exer}), so both $\check{H}^\bullet\left(\mathrm{Sp\acute{e}}\left(\mathscr G^{\delta } \right) , F\right)$ and $  \check{H}^\bullet\left(\mathrm{Sp\acute{e}}\left(\mathscr G^{ \rho} \right) , F \right)$ are $\check{H}^\bullet\left(\sX, F \right)$- bimodules. From $\mathrm{pr}_1^\sX \circ \mathrm{pr}_1=\mathrm{pr}_2^\sX \circ \mathrm{pr}_2$ and the Proposition \ref{cup_ass_prop} it follows that
...
a described in  \cite{milne:lec}  \'etale cohomology of algebraic varieties. Here we have obtained an integer cohomology of $C^*$-algebras.
...
\begin{definition}\label{geom_mor_defn}\cite{johnstone:topos}
	If both $\mathscr E$, $\mathscr F$ are toposes then \textit{geometric morphism} $\mathscr F\xrightarrow{f}\mathscr E$ consists of a pair of functors $\mathscr E\xrightarrow{f_*}\mathscr F$  and $\mathscr E\xrightarrow{f^*}\mathscr F$ such that $f^*$ is left adjoint to $f_*$ and $f^*$ is left exact. 
\end{definition}
\begin{definition}\label{constant_presheaf_defn}\cite{johnstone:topos}
	For any set $F$ there is a \textit{constant presheaf} $\mathscr P$ such that $\mathscr P\left( U\right) = F$ and $\rho^\sU_\sV = \Id_F$ for all $V\subset U$. If $X$ is the terminal object of $\mathrm{Cat}\left( \mathbf T\right)$ (cf. Definition \ref{site_defn})
...
\paragraph{}
Here I follow to \cite{milne:lec}. Let $\mathbf T$ is a site and $X$ is an object of $\mathrm{Cat}\left( \mathbf T\right)$. The functor
\bean
...
\paragraph{}
The explained below theories are specializations of the described  in the Sections  \ref{etale_presheaf_defn} and  \ref{etale_sheaf_defn} ones.
...
$$
\mathscr P_\sX\left(\sU_{\iota_0,.... \iota_r} \right)\cong \mathscr P_\sY\left(f^{-1}\left( \sU_{\iota_0,.... \iota_r} \right)\right) \cong F.
$$
...
\phi: \check{H}^\bullet\left(\sX, \mathscr P\otimes \mathscr P \right)\cong \check{H}^\bullet\left(\sX, \mathscr P \right).
	...
	\begin{lem}
		Let $\sX$ be a topological space. If $F$ is an Abelian group then any isomorphism $F \otimes_Z F \to  F$ yields a product
		\bean
	...
	Let  $F$ be an Abelian group with a homomorphism $F \otimes_Z F \cong F$. Prove that a given by \eqref{cech_hom_eqn} map $\check{H}^\bullet\left(f\right):\check{H}^\bullet\left(\sY, F \right)\to \check{H}^\bullet\left(\sX, F \right)$ is a homomorphism of rings.
	\end{exercise}
...
\end{defn}
	...
	\begin{itemize}
		...
		\item $M$ is strongly closed  (cf. Definition \ref{strong_topology_defn}).
	\end{itemize}
\end{thm}
