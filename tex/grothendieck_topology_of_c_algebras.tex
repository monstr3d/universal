...
From the Definition \ref{local_prod_defn} and the Theorem \ref{grot_type_I_thm} it follows that one should find a homeomorphism $\phi: \mathscr X^{CT\left(\sX, \dl \right)}\times_{\mathrm{Sp\acute{e}}\left(\Om_\sX \right)} \mathscr X^{CT\left(\sX, \dl \right)}\to \mathscr X^{CT\left(\sX, \dl+\rho \right)}$.
	...
	by $s^\dl \in \mathcal X^{CT\left(\sX, \dl \right)}$ and $s^\rho \in \mathcal X^{CT\left(\sX, \rho \right)}$ such that $P_A\left( s^\dl\right) = P_A\left( s^\dl\right)$. If $x \bydef p_A\left(s^\dl \right)= p_A\left(s^\rho \right)$  (cf. equation \eqref{loical_hom_eqn}) then there a commutative hereditary subalgebras $C^\dl \subset {CT\left(\sX, \dl \right)}$ and $C^\rho \subset {CT\left(\sX, \rho \right)}$
	such that
	\begin{itemize}
		\item $s^\dl = C^\dl_x$ and $s^\rho = C^\rho_x$,
		\item both $\supp C^\dl$ and $\supp C^\rho$ are representatives of $P_A\left( s^\dl\right) = P_A\left( s^\rho\right)$.
	\end{itemize}
	If $\sU \bydef  \supp C^\dl \cap \supp C^\rho$ then both $s^\dl$ and $s^\rho$ can be represented by $\left.C^\dl\right|_\sU$ and $\left.C^\rho\right|_\sU$  respectively (cf. the equation \eqref{c_res_eqn}. For the Lemma \ref{gen_lem} it follows that there are Abelian elements $a^\dl \in  {CT\left(\sX, \dl \right)}$ and $a^\rho \in {CT\left(\sX, \rho \right)}$ such that both $\left.C^\dl\right|_\sU$ and $\left.C^\rho\right|_\sU$ (cf. a generated by $\left\{a^\dl\right\}$ and $\left\{a^\rho\right\}$ hereditary $C^*$-algebras (cf. Definition \ref{hered_generated_defn}). If $\varphi_{\dl\rho}$ is given by the equation \eqref{bundle_prod_iso}	then an element
	$$
