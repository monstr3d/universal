...
\end{definition}
\begin{remark}\label{localic_topos_rem}
	From the Theorem \ref{locale_thm} it follows that the $A$-{topos} is localic.
\end{remark}
\begin{definition}\label{grothendieck_ca_t_defn}
	...
	arising from $A$ (cf. Definition \ref{grothendieck_ca_defn}) is equivalent to a site $\mathbf{T}_{\mathfrak{Grot}\left(A \right)}$ arising from $\mathfrak{Grot}\left(A \right)$ (cf. Definition \ref{top_gro_definition}) then we say that $ \mathfrak{Grot}\left(A \right)$ is the \textit{Grothendieck} $A$-\textit{space}.  In this case the  $A$-{topos} $\mathscr{E}_A\bydef \mathbf{Sh}\left(\mathbf{T}_A \right)\cong \mathbf{Sh}\left( \mathfrak{Grot}\left(A \right)\right)$ is spatial (cf. Appendix \ref{spatial_toposes_sec}).
\end{definition}
\begin{remark}
The   is the {Grothendieck} $A$-{space} should not be unique, different topological spaces can correspond to a single Grothendieck topology,
\end{remark}
\begin{remark}
If $\mathfrak{Grot}\left(A \right)$  is the {Grothendieck} $A$-\textit{space} and $\mathscr{E}_A\bydef \mathbf{Sh}\left(\mathbf{T}_A \right)\cong\mathbf{Sh}\left(\mathfrak{Grot}\left(A \right) \right) $ is the $A$-{topos}, then from the Example \ref{top_points_exm} it follows that any $x\in \mathfrak{Grot}\left(A \right)$ yields a point of  $\mathscr{E}_A$ in the sense of the Definition \ref{topos_point_defn}. Moreover $\mathfrak{Grot}\left(A \right)$ is a sufficient class of points (cf. Definition \ref{topos_point_defn}).
\end{remark}
...
Let $\mathcal S$ be a category of sets and functions, and let $\mathcal E \xrightarrow{\ga} \mathcal S$ be an $\mathcal S$-topos (cf. Definition \ref{relative_topos_defn}). By a \text{point} of $\mathcal E$ we mean a Geometric morphism $\mathcal S \xrightarrow{p} \mathcal E$ over $\mathcal S$. If $K$ is a class of points, we say that $K$ is \textit{sufficient} if the family of functors $\left\{\left.p^*\right| p\in K\right\}$ is conservative, i.e. if any $X \xrightarrow{f} Y$ in $\mathcal E$ such that $p^*\left(f \right)$ is iso for all $K$ is necessary iso. We say that $\mathcal E$ \text{has enough points} if the class of all points of $\mathcal E$ is sufficient.
\end{definition}
...
\begin{proposition}
	...
	There is a functor $\mathcal C : \mathbf{Cstar} \to \mathbf{Poset}$, from the category of $C^*$-algebras and partially ordered sets defined
	on objects as
	\[
	\mathcal C\left(A \right) = \{C \subseteq A | C \in \mathbf{cCstar} \},
	\]
	ordered by inclusion.
	On a morphism $f:A \to B$ of $\mathbf{Cstar}$,	it acts as $\mathcal{C}(f) : \mathcal C\left( A\right)  \to	\mathcal{C}(B)$ by the direct image $C \mapsto f(C)$.
\end{proposition}
The collection of
functors  $\mathcal C(A) \to \mathscr S$ forms a topos
$\mathcal T({A})=\mathscr S^{\mathcal C(A)}$. This is the \emph{topos associated
	to $A$}. 
entities internal to $\mathcal T({A})$. The subobject classifier ${\Omega}$ in
$\mathcal T({A})$ has already been given in \ref{cosieves}.
...
morphisms  by $\mathbf{Topos}$.	
\begin{proposition}
	\label{prop:asstopos}
	There is a functor $\mathcal T : \mathbf{Cstar} \to \mathbf{Topos}$, defined
	on objects by $\mathcal T({A})=\mathscr S^{\mathcal C(A)}$, the
	category of functors from $\mathcal C(A)$ to the ambient topos.
\end{proposition}
\begin{remark}
	content...
\end{remark}	
