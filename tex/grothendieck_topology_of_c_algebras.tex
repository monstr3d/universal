...
\end{tikzcd}
	...
	In the described above situation one has a Grothendieck topology.
	\end{theorem}
	\begin{proof}
	One needs check conditions (a) - (c) of the Definition \ref{pretopology_defn},\\
	(a) If $B$ and $C$ are $\mathbf{Hered}/A$-objects such that there is a morphism $C \to B$ then $B \subset C$. Moreover $B\times_A C= C$. If $\left\{B_\iota \to B\right\}_{\iota\in I}$ is a covering of $B$  then $B = \bigcap_{\iota \in I}B_\iota$.hen $C \subset B$. Moreover $B\times_A C= C$ 
	From 	
	$$
	C = 	B \cup C = \left( \bigcap_{\iota \in I}B_\iota \right) \cup C = \bigcap_{\iota \in I}\left( B_\iota\cup C\right) 
	$$
	it follows that the family $\left\{ B_\iota\cap C\right\}_{\iota \in I}$ is a covering of $C$.\\
...
\section{Grothendieck topology of commutative $C^*$-algebras}
...
\forall \sU \in \mathfrak{Closed}\left( \sX\right)\quad \mathscr F^c\left( \sU\right) \bydef \text{a space of continuous sections of } \mathscr F \text{ over }\sU
\ee
...
\end{lemma}
\begin{proof}
Follows from the Theorem \ref{closed_cov_thm}.
\end{proof}
...
Any $\mathfrak{Closed}\left( \sX\right)$ flasque sheaf $\mathcal A \in \mathbf{Ab}\left(\mathfrak{Closed}\left( \sX\right), J_\sX\right)$ is acyclic (cf. Definition \ref{acyc_defn})
\end{proposition}
\begin{proof}
The proof of this proposition is an analog of the \ref{sheaf_flasque_ext_prop} one.
\end{proof}
\begin{lemma}
Let $\mathscr F, \mathscr G\in \mathbf{Sh}\left( \sX\right)$  are sheaves, and let $\mathscr F\xrightarrow{f} \mathscr G$ is a $\mathbf{Sh}\left( \sX\right)$ 
...
\paragraph{}
For topological space $\sX$ and any sheaf  $\mathscr A\in \mathbf{Ab}\left( \sX\right)$ there is the  given by  \eqref{canonical_resolution_eqn} canonical resolution 	
...
then for all $p$ the  sheaf $\mathscr{C}^{0}\left(\sX, \mathscr A \right)$ flasque. From 
From the Proposition \ref{sheaf_flasque_prop} it follows that 
$H^p\left(\sX, 	\mathscr{C}^{p}\left(\sX, \mathscr A \right)  \right)  = 0$ for all $q$ and  $p > 0$.
If we apply the functor $\Ga\left(\sX, \cdot \right)$ then we obtain a complex
\bean
\Ga\left(X,\ 	\mathscr{C}^{0}\left(\sX, \mathscr A \right)  \right) \xrightarrow{\Ga\left( d_0 \right)} \Ga\left(X,	\mathscr{C}^{1}\left(\sX, \mathscr A \right) \right) \xrightarrow{\Ga\left( d_1 \right)} \Ga\left(X, 	\mathscr{C}^{2}\left(\sX, \mathscr A \right)  \right) \to ... 
\eean
For any $p$ a sheaf $\mathscr{C}^{0}\left(\sX, \mathscr A \right)$ is flasque, so it is acyclic 
from the Theorem \ref{acyc_prop} one has  the $p^{\text{th}}$ cohomology
group 
...
\end{split}
\eean
On the other hand from Lemma \ref{top_comm_flasque_lem} it follows that 
!!!! PROOF THAT RESOLUTION.
\end{proof}
...
\begin{theorem}\label{top_alg_loc_thm} Following conditions hold:
	\begin{enumerate}
		...
		\item[(iii)] The double negation $\mathfrak{Closed}\left( \sX\right)_{\neg\neg}$ (cf. Definition \ref{double_neg_loc_defn}) of the locale $\mathfrak{Closed}\left( \sX\right)$ contains all regular closed subspaces of $\sX$ (cf. Definition \ref{top_reg_defn}).
	\end{enumerate}
\end{theorem}
...
\begin{proof}
	...
	\ee	
	is a nonempty open set. If $\sX$ has no isolated points then since $\sX$ is Hausdorff (cf. Theorem \ref{gelfand-naimark_thm}) there are two open subsets $^\neg P', ^\neg P'' \subset ^\neg P$ such that
	\bean
	^\neg P' \subsetneqq ^\neg P,\\
	^\neg P'' \subsetneqq ^\neg P,\\
	^\neg P  = ^\neg P' \cup ^\neg P''.
...
\end{proof}
\begin{empt} Let $\sX$ be a compact, locally connected, Hausdorff, orientable $n$-{dimensional homology manifold over} $L$ (cf. Definition \ref{top_ndiml_defn}). From \eqref{top_dim_eqn} it follows that $\dim_L \sX = n$. From the theorem \ref{top_c_ac_thm} it follows that any sheaf $\mathscr A$ of $L$-modules has a $c$-soft resolution (cf. Definition \ref{top_res_defn})
	\be\label{top_res_eqn}
	0\to \mathscr A  \xrightarrow{\eps}\mathscr A^0  \xrightarrow{d}\mathscr A^{1}  \xrightarrow{d}\mathscr A^2\xrightarrow{d}\dots \xrightarrow{d}\mathscr A^n \to 0
	\ee
	If $\mathscr L\bydef \mathfrak{Ass}\left(L_\sX \right) $ sheaf associated with a constant presheaf 
	\be\label{top_resh_eqn}
	0\to \mathscr L   \xrightarrow{\eps}\mathscr L^0  \xrightarrow{d}\mathscr L^{1}  \xrightarrow{d}\mathscr L^2\xrightarrow{d}\dots \xrightarrow{d}\mathscr L^n \to 0
	\ee
	or equivalently
	\be\label{top_reshh_eqn}
	0\to \mathscr H^0\left( \mathscr L^0\right)   \xrightarrow{\eps}\mathscr L^0  \xrightarrow{d}\mathscr L^{1}  \xrightarrow{d}\mathscr L^2\xrightarrow{d}\dots \xrightarrow{d}\mathscr L^n \to 0
	\ee
	The differential sheaf $\mathscr L^*$ is a $c$-soft {quasi}-$n$-{resolution} of $L$ (cf. Definition \ref{top_quasi_res}). From the Theorem \ref{top_from_sh_to_cosh_thm} one has a s a flabby quasi-$n$-coresolution of $L$. Also,
	...
	for $p \le n$.
	From the Theorem \ref{top_soft_flabby_thm} it follows that the sheaf  $\mathscr L^p$ is  is flabby for all $p > 1$.
	
	For any $\sF \in \mathfrak{Closed}\left( \sX\right)_{\neg\neg}$ a covering of $\sF$ is a family $\left\{\sU_\a\right\}$ of open subsets of $\sY$ such that $ \sX \setminus \sU \subset \sU_\a$ and $\sU$ is a closure of  $\cap \sU_\a$.  Let $L$ be an unital ring, and let $\mathscr L \bydef \mathfrak{Ass}\left(L_\sY \right)$ be a sheaf associated with constant presheaf.
	
	
	and let $\mathfrak A \bydef H^c_0\left(\bullet, L \right) $ be a cosheaf given by the Proposition \ref{precosh_sh_prop} 
\end{empt}
\begin{definition}
...
\begin{definition}
	A covering of a topological space $\sX$ by regular closed sets is said to be \textit{good} 
\end{definition}
\begin{empt}
	Let $\sX$ be a compact Hausdorff space, and let $\mathscr U = \left\{\sU_\a\right\}_{\a \in I}$ a covering by regular closed spaces  of $\sX$. Let $N\left(\mathscr U \right)$ be the nerve of $\mathscr U$ (cf. Definition \ref{})
	
	
	having the property that $H^p\left( \sU_{\sigma^n};\mathscr A \right)= 0$  for $p > 0$ and
	all $\sigma^n \in N\left(\mathscr U \right)$ , all $n$.
\end{empt}
\begin{theorem}
	PAGE 193 
...
\begin{remark}
	The above construction does not depend on the inclusion $\sX \to \sY$. The inclusion will be used later.
\end{remark}

\begin{definition}\label{top_neg_defn}
	Let $\mathfrak{A}$ be a precosheaf on $\sY$ (cf. Definition \ref{top_cosh_defn}). There is a presheaf $^\neg \mathfrak{A}$ on the locale $\mathfrak{Closed}\left( \sX\right)$ such that
	\be\label{top_neg_eqn}
	\forall \sU \in \mathfrak{Closed}\left( \sX\right) \quad ^\neg \mathfrak{A}\left( \sU\right) \bydef \mathfrak{A}\left( ^\neg \sU\right).
	\ee
	We say that $^\neg \mathfrak{A}$ is the \textit{negation presheaf of} $\mathfrak{A}$.
\end{definition}
...
\begin{remark}
	Both the Lemma \ref{comm_alg_loc_thm}  !!! THEOREM !!! and the Theorem \ref{comm_alg_loc_thm} a motivations of the term "Gelfand locale" (cf. Theorem \ref{gelfand-naimark_thm}).
\end{remark}
\begin{lemma}
...
The Gelfand $\K$-locale (cf. Definition \ref{gelfand_a_site_defn} !!! SITE !!!) is not spatial (cf. Definition \ref{spatial_locale_defn}).
\end{lemma}
\begin{proof}
...
\be\label{psersh_im_eqn}
	U \mapsto f \mathscr F\left(\sU \right). 
\ee
	\end{remark}
...
\begin{proof}	
  ...
  If $\mathscr F \xrightarrow{\varphi} \mathscr G$ is a $\mathbf{Sh}\left(\mathbf T \right)$-morphism then the given by \eqref{psersh_im_eqn} presheaf is not necessary a sheaf. So the image (cf. Definition \ref{image_defn}) of $\varphi$ is the sheaf associated with the presheaf given by \eqref{psersh_im_eqn}.
  \end{remark}
...
functor $f^*: \mathbf{Sets}^{\mathbf{C}^{\mathrm{op}}}\to \E$ preserves colimits, and every presheaf $R$ on $\mathbb{C}$
...
More explicitly, each presheaf $R \in  \mathbf{Sets}^{\mathbf{C}^{\mathrm{op}}}$ is canonically a colimit
$\varinjlim H$ of representables. Indeed, $R$ determines the category $\int R$ of its
elements, with objects the pairs $(C, r)$, where $r \in R(C)$ and $C \in \mathbf{C}$,
while $H: \int  R \to \mathbf{Sets}^{\mathbf{C}^{\mathrm{op}}}$ sends this object $(C, r)$ to $\Hom( -, C) \bydef  \mathbf{y}C$.
This colimit $\varinjlim H$ can be presented as a coequalizer,
with coproducts over the arrows $u: (C', r') -t (C, r)$ and then over the
objects of $\int R$. This gives the following coequalizer in $\mathbf{Sets}^{\mathbf{C}^{\mathrm{op}}}$:
\be\label{coprod_topos_eqn}
\coprod_{\substack{u: C'\to C\\r \in R(C)}}\mathbf{y}C' \substack{ \xrightarrow{\th}\\\xrightarrow[\tau]{}} \coprod_{\substack{C \in \mathbf{C}\\r \in R(C)}}\mathbf{y}C\xrightarrow{\eps} R;
\ee
specifically, if we write elements of $\coprod_{u,r} \mathbf{y}C'$ as triples $(r, u, v)$ with
$r \in  R(C)$, $~u: C' \to C$ and $v \in  \mathbf{y}(C')(B)'= \mathbb{C}\left( B, C'\right)$  for some object
...
\coprod_{\substack{u: C'\to C\\r \in R(C)}}f^*\mathbf{y}C' \substack{ \xrightarrow{\th}\\\xrightarrow[\tau]{}} \coprod_{\substack{C \in \mathbf{C}\\r \in R(C)}}f^*\mathbf{y}C\xrightarrow{\eps} f^*R;
\ee
...
$-\otimes_{\mathbf{C}}A:\mathbf{Sets}^{\mathbf{C}^{\mathrm{op}}}\to \E$. Explicitly, for any $R \in \mathbf{Sets}^{\mathbf{C}^{\mathrm{op}}}$ the object $R\otimes_{\mathbf{C}}A$ of $\E$ is defined by the following coequalizer:
\be\label{coprod_topos_a_eqn}
\coprod_{\substack{u: C'\to C\\r \in R(C)}}A\left(C' \right)  \substack{ \xrightarrow{\th}\\\xrightarrow[\tau]{}} \coprod_{\substack{C \in \mathbf{C}\\r \in R(C)}}A\left(C \right)\xrightarrow{\eps} R\otimes_{\mathbf{C}}A;
\ee
...
\end{tikzcd}
\\
commutes. Indeed, this is just the canonical isomorphism
...
\ee
This adjunction  assumes that $\E$ is cocomplete. The
co completeness hypothesis "$\E$ has all small colimits" may be replaced by
"$\E$ is a topos over sets"%, provided the functor $A: \mathbf{C}\to \E$ is also replacedby a left ll(C) object (an "internal diagram") on the category objectI (C)in E. See V. 7 (5). Here ll: Sets -+ E is the canonical functor,§1(8).
\subsection{Geometric morphisms into Grothendieck topoi}
\begin{definition}\cite{topos:intro}
Suppose $\left(\mathbf{C}, J)\right)$ and $\left(\mathbf{D}, K\right)$  are two sites, and suppose $\mathbf{C}$ and $\mathbf{C}$ are
...
such morphism of sites $\phi: \mathbf{C}\to \mathbf{D}$ induces a geometric morphism $\mathbf{Sh}\left(\mathbf{D}, K\right)\to \mathbf{Sh}\left(\mathbf{C}, J\right)$.
%f: Sh(D, K) -+ Sh(C, J); the direct image functor f*: Sh(D, K) -+Sh(C, J) sends a sheaf F on D to the composition f*(F) = F 0 ¢, and
the inverse image 1*: Sh(C, J) -+ Sh(D, K) sends a sheaf G on C to
the tensor product G Q9c A.p, where
!!!
%A.p = a 0 y 0 ¢: C ~ D Y ) SetsDOP _--!a~-+) Sh(D, K).
\end{theorem}
...
\begin{definition}\label{nerve_defn}\cite{gelfand_manin}
...
\begin{empt}\label{top_taut_empt}\cite{spanier:at}
	...
	\end{definition}
	\begin{theorem}\label{top_soft_flabby_thm}
...
\end{proof}
\begin{empt}
	Let $\sY$ be a compact  $n$-\textit{dimensional homology manifold over} principal ideal domain $L$ (cf. Definition \ref{top_ndiml_defn}). One has $\dim_L \sY = n$ (cf. the equation \eqref{top_dim_eqn}, and the Definition \ref{top_diml_defn}). If $L_\sY\in \mathbf{PreShAb}\left(\sY \right) $ is a constant presheaf and $\mathscr{L} \bydef \mathfrak{Ass}\left( L_\sY\right)\in \mathbf{ShAb}\left(\sY \right) $ is the associated sheaf  then there is the canonical injective resolution (cf. \ref{canonical_resolution_empt}). From the Theorem \ref{top_diml_thm} the resolution looks like
	\bean
