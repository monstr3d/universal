...
\eea 
Let $\Om_\sX$ be {sheaf of stalks of open sets} (cf. Appendix \ref{open_sheaf_empt}). Let  $\mathrm{Sp\acute{e}}\left(\Om_\sX \right)$ be the	\'espace etal\'e  (cf. Exercise \ref{sheaf_etale_exer})
...
$$
\begin{exercise}
Prove that if $\mathscr X^A$ is given by the Lemma \ref{grot_com_lem}  Grothendieck $A$-space. (cf. Definition \ref{grothendieck_ca_t_defn}) then $P'_A$ maps $\mathscr X^A$ onto $\mathrm{Sp\acute{e}}\left(\Om_\sX \right)$. Moreover the natural surjective maps
\bea\label{pa_eqn}
