...
\paragraph{}
Here we prove that for   $C^*$-algebra of type $I_0$ (cf. Definition \ref{type_i0_defn}) the  Grothendieck $A$-locale is spatial. If $C_0\left(\sX \right)$ is a commutative $C^*$-algebra, $C \subset C_0\left(\sX \right)$ is a subalgebra and $\sU$ is an open subset then we denote by $C|_\sU \bydef C \cap C_0\left( \sU\right)$. 

\begin{definition}

Let $A$ be a $C^*$-algebra of type $I_0$. An $A$-\textit{representative}  is a triple $\left( j, x, C\right)$ where 
\begin{enumerate}
	\item[(a)]  $j: C_0\left(\sX \right) \hookto A$ is an inclusion.
	\item[(b)] $x \in \sX$ is a point.
	\item[(c)] $C \subset C_0\left(\sX \right)$ is  $C^*$-subalgebra such that $C|_\sU \neq \{0\}$ for any open neighborhood of $x$.
\end{enumerate}
Two $A$-{representatives} $\left( j', x', C'\right)$ and  $\left( j'', x'', C''\right)$ are \textit{equivalent} if there are open neighborhoods $\sU'$ and $\sU''$ of $x'$ and $x''$ respectively such that
$$
j'\left(C'|_{\sU'} \right) = j''\left(C''|_{\sU''} \right).
$$
Equivalence classes of representatives a said to be \textit{Grothendieck} $A$- \textit{points}. A represented by  $\left( j, x, C\right)$ Grothendieck $A$ point will be denoted by $C_x$.  We denote by $\mathfrak{Points}\left(A \right)$ a set of Grothendieck $A$ points.  If $B\subset A$ is a hereditary subalgebra then we say than $C_x$ is in 
$B$ if there is a representative $\left( j, x, C\right)$ of $C_x$ such that $j(C)\subset B$. We write $C_x \in B$, We denote by
$$
\mathfrak{Points}\left(B \right)\bydef \left\{C_x \in \mathfrak{Points}\left(A \right)| C_x \in B  \right\}
$$
\end{definition}
\begin{lemma}
The sets $\mathfrak{Points}\left(B \right)$ yield a topology on $\mathfrak{Points}\left(A \right)$
\end{lemma}
\begin{proof}
Clearly $\mathfrak{Points}\left(B' \right)\cap \mathfrak{Points}\left(B'' \right)= \mathfrak{Points}\left(B'\wedge B'' \right)$ (cf. \eqref{locale_eqn}) and $\bigcup_{\iota\in I} \mathfrak{Points}\left(B_\iota\right) \subset \mathfrak{Points}\left( \bigvee_{\iota\in I} B_\iota\right)$. Let $C_x \in \mathfrak{Points}\left( \bigvee_{\iota\in I} B_\iota\right)$ be represented by $\left( j, x, C\right)$, such that $j\left(C \right)  \subset  \bigvee_{\iota\in I} B_\iota$. From the Lemma \ref{grothendieck_ca_lem} it follows that if $C_\iota = j\left(C \right) \cap B_\iota$ then $j\left(C \right)=  \bigvee_{\iota\in I} C_\iota$,
