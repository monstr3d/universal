...
\begin{lemma}\label{comm_alg_loc_lem}. Following conditions hold:
	\begin{enumerate}
		\item [(i)] The locale $\mathfrak{Closed}\left( \sX\right)$ is naturally isomorphic to the Gelfand $C_0\left(  \sX\right)$-locale $\mathfrak{Locale}\left(C_0\left(  \sX\right) \right)\cong \mathfrak{Closed}\left( \sX\right)$,
		\item[(ii)] If $\sX$ has no isolated points then the  locale $\mathfrak{Locale}\left(C_0\left(  \sX\right) \right)$.
	\end{enumerate}
\end{lemma}
...
%	Lemma 6.1.2. The categories Ind(C) and Pro(C) are U-categories.
	...
	\begin{lemma}\cite{munkres:topology}
		%	'Lemma 41.6.
...
\end{empt}
\begin{theorem}\label{jj}\cite{bredon:sheaf}
%9.5. Theorem. 
