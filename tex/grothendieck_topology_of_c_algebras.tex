...
subspace $\rho_c\left(C \right) \H_x \subset \H_x$. If we denote by $p_x\in \K\left(\H_x \right)$ a rank one projector onto $\rho_c\left(C \right) \H_x$ then one has a surjective map
\bean
\begin{split}
...
group  $$
H^r\left(X,\ \mathscr F \right)\bydef R^r\Ga\left(X, \F \right)= \ker~ d_r / \im~ d_{r-1} 
$$
...
Let $s \in \F\left(\sU\right)$ be a section. Prove following statements.
\begin{enumerate}
	\item The set 
	\be\label{sheaf_u_s_eqn}
\mathscr U_s\bydef \left\{s_x\right\}_{x \in \sU}
	\ee
is open subset  of the \'espace etal\'e  of $\mathrm{Sp\acute{e}}\left(\mathscr F \right)$ of the presheaf $\F$.
...
\begin{proposition}\label{ctr_bundle_prop}\cite{rae:ctr_morita}
	...
	trivial bundle $\left( \F, \pi,\sX\right) $ with fibre $\K(\H)$ and structure group $\Aut \left( \K(\H)\right)$ such that $A$ is $C_0\left( \sX\right)$ -isomorphic to the space of sections $\Ga_0\left(\sX, \F \right)$. 
