...
\ee
Since $C|_\sU$ is a hereditary $C^*$-subalgebra of $A$ the presheaf is flabby (cf. Definition \ref{sheaf_flabby_defn}),  so $\mathscr{P}^A$ is a sheaf.
\end{remark}
...
\end{lemma}
\begin{proof}
From the equation \eqref{comm_alg_sheaf} it turns out that $C$ corresponds to a global section $s' : \sX \to \mathrm{Sp\acute{e}}\left(\mathscr P^A  \right)$. If $s^0 : \sX \to \mathrm{Sp\acute{e}}\left(\mathscr P^A  \right)$ is a section of the trivial subalgebra $\{0\}\subset A$ then denote by 
$$
...
\begin{remark}
	...
	\eean
	of the Dixmier-Douady theory (cf. equation \eqref{bundle_prod_iso}).
...
From the Definition \ref{local_prod_defn} and the Theorem \ref{grot_type_I_thm} it follows that one should find a homeomorphism $\phi: \mathscr X^{CT\left(\sX, \dl \right)}\times_{\mathrm{Sp\acute{e}}\left(\Om_\sX \right)} \mathscr X^{CT\left(\sX, \dl \right)}\to \mathscr X^{CT\left(\sX, \dl+\rho \right)}$.
	...
	by $s^\dl \in \mathscr X^{CT\left(\sX, \dl \right)}$ and $s^\rho \in \mathscr X^{CT\left(\sX, \rho \right)}$ such that $P_{CT\left(\sX, \dl \right)}\left( s^\dl\right) = P_{CT\left(\sX, \dl \right)}\left( s^\dl\right)$ where the notation \eqref{pa_eqn} is used. If $x \bydef p_{CT\left(\sX, \dl \right)}\left(s^\dl \right)= p_{CT\left(\sX, \rho \right)}\left(s^\rho \right)\in \sX$  (cf. equation \eqref{loical_hom_eqn}) then there are commutative hereditary subalgebras $C^\dl \subset {CT\left(\sX, \dl \right)}$ and $C^\rho \subset {CT\left(\sX, \rho \right)}$
	such that
	\begin{itemize}
		\item $s^\dl = C^\dl_x$ and $s^\rho = C^\rho_x$,
		\item both $\supp C^\dl$ and $\supp C^\rho$ are representatives of $P_{CT\left(\sX, \dl \right)}\left( s^\dl\right) = P_{CT\left(\sX, \rho \right)}\left( s^\rho\right)$.
	\end{itemize}
	...
	$$
	is Abelian. If $C$ is generated by $\left\{a\right\}$  hereditary $C^*$-algebra then we define
	$$
...
\end{proof}
From the Lemma \ref{ctr_local_prod_thm}   one can obtain a homeomorphism
