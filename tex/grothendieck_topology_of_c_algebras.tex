...
\end{proof}
\begin{empt}
Let $\sX$ be an orientable $n$-\textit{dimensional homology manifold over} $L$. From \eqref{top_dim_eqn} it follows that $\dim_L \sX= n$. If $\mathscr J^* \left(\sX, L \right)$ the canonical injective resolution of $L$ (cf. \ref{canonical_resolution_empt}) then from the equation \eqref{top_dim_eqn}  $\mathscr J^p \left(\sX, L \right)= 0$ for any $p > n$ (cf. the equation \eqref{top_dim_eqn})
\end{empt}
...
\begin{empt}
	...
	\be
	\mathfrak A \leftarrow \mathfrak A_1 \leftarrow \dots \leftarrow\mathfrak A_n 
	\ee
...
\end{empt}
	...
	such that the sequence $\mathscr A  \xrightarrow{\eps}\mathscr L^0  \xrightarrow{d}\mathscr L^{1}  \xrightarrow{d}\mathscr L^2\to \dots $ is exact.
	\end{definition}
...
\begin{enumerate}
	...
	Moreover, if 4> is paracompactifying, then "4>-acyclic" in (c) and (d) can
	
\end{enumerate}	

	be replaced by "4>-soft."
\end{theorem}
...
\ee		
	...
	\begin{definition}
		...
		\textit{resolution} of $M$ provided that
		\begin{enumerate}
	...
	\begin{theorem}\label{top_from_sh_to_cosh_thm}
		PAGE 352. 12.9. Theorem. Suppose that X is clcL+1 Then $\mathfrak D \left(\mathscr L^*\right)$  is a flabby quasi n-
		coresolution of L for any c-soft quasi-(n + 1)-resolution !' of L. Also,
		X is hlc' and there as a natural isomorphism Hp(D(Y*)(U)) HH(U;L)
		for p < n.
