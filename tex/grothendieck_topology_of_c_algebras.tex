...
Let  $\widetilde A\subset B\left(\H'\ox_{A} \H'' \right)$ be the $C^*$-norm closure of $P\left(A'\otimes_A A'' \right)$. If $\widetilde A$ is a $C^*$-subalgebra and the inclusion $\widetilde A\subset B\left(\H'\ox_{A} \H'' \right)$ is a faithful nondegenerate representation then the situation \ref{dd_hh_empt} yields a {generalized Dixmier-Douady duality} (cf. Definition \ref{dd_dual_defn}).
\end{lemma}
\begin{proof}
...
\ee
For any $x \in \sX$ denote by $\pi^\dl: CT\left(\sX, \dl \right)\to \left( \H^\dl_x\right)$ and  $\pi^\rho: CT\left(\sX, \rho \right)\to \left( \H^\dl_x\right)$ 
