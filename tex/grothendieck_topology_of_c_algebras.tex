...
\subsection{$C^*$-algebras generated by their commutative subalgebras}
...
\eean
Let $\mathrm{Sp\acute{e}}\left(\mathscr P^A  \right)$ be an	\'espace etal\'e  (cf. Exercise \ref{sheaf_etale_exer}) of the sheaf $\mathscr{P}^A$. 
...
\ee
is an open subset of $\mathscr X_A$. Any hereditary commutative hereditary subalgebra $C$ corresponds to a continuous  section
$$
...
\end{empt}
 ...
 Let $A$ be a {hereditary commutative generated} $C^*$-algebra with  {Hausdorff blowing-up} $C_0\left( \sX\right) \hookto M\left(A \right)$.
If  $\mathscr X^A$ is an arising from $A$ site (cf. Definition \ref{grothendieck_ca_defn}) then there is a natural equivalence
$$
\mathbf{T}_{A}\cong \mathbf{T}_{\mathscr X^A}
$$
where $\mathbf{T}_{\mathscr X^A}$ is a site arising from $\mathscr X^A$ (cf. Definition \ref{top_gro_definition}).
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
If $p:\mathscr X^A\to\sX$ is a natural local homeomorphism, and both $F_\sX$ and $F_{\mathscr X^A}$ are constant presheaves arising from $F$ then from the Proposition  \ref{spectral_sequence_prop} if follows that for all $r\ge 0$ there are homomorphisms
...
$$
The Exercise   \ref{ring_homo_exer} yields a ring homomorphism $\check{H}^\bullet \left(\sX, F  \right)\to	\check{H}^\bullet\left(\mathscr X^A, F \right)$. Now this lemma follows from the Remark \ref{equiv_rem}.
\end{proof}


\subsection{Pairings of cohomology}
\begin{lemma}
	If $F$ is an Abelian  group with $F\ox_\Z F \cong F$ and both $p':\sX' \to \sX$ and $p'':\sX'' \to \sX$ are surjective  continuous maps then there is a natural pairing
	$$
	\check{H}^\bullet\left(\sX', F \right) \ox_{\check{H}^\bullet\left(\sX, F \right)} \check{H}^\bullet\left(\sX'', F \right)\to \check{H}^\bullet\left(\sX'\times_\sX \sX'', F \right)
	$$
	where $\sX'\times_\sX \sX''$ is a pullback (cf. Definition \ref{pullback_defn}).
\end{lemma}
\begin{proof}
	...
	\be\label{cross_eqn}
	\begin{split}
		\check{H}^\bullet\left(\sX'  , F\right) \times  \check{H}^\bullet\left(\sX'' , F \right) \to \check{H}^\bullet\left(\sX'\times_\sX \sX'' , F \right)\\
		(\a, \bt) \mapsto \check{H}^\bullet\left(q'\right) \left( \a\right) \smile \check{H}^\bullet\left(q''\right) \left( \bt\right)
	\end{split}
	\ee
	where $\smile$ is a cup product (cf. \ref{tens_prop_c_empt}).  Both maps \bean
	\check{H}^\bullet\left( p'\right): \check{H}^\bullet\left(\sX, F \right) \to \check{H}^\bullet\left(\sX' , F\right),\\
	\check{H}^\bullet\left( p''\right): \check{H}^\bullet\left(\sX, F \right) \to \check{H}^\bullet\left((\sX'' , F\right)
	\eean 
	are homomorphisms of rings (cf. Exercise \ref{ring_homo_exer}), so both $\check{H}^\bullet\left(\sX' , F\right)$ and $  \check{H}^\bullet\left(\sX'' , F \right)$ are $\check{H}^\bullet\left(\sX, F \right)$- bimodules. From $p'\circ q' = p''\circ q''$,  the equation \eqref{cech_hom_comp_eqn}  and the Proposition \ref{cup_ass_prop} it follows that
	\be\label{cross_r_eqn}
	\begin{split}
		\check{H}^\bullet\left( q'\right) \left( \a\right)\smile \check{H}^\bullet\left( q''\right)\left(  \check{H}^\bullet\left(p''\right) \left( \ga\right)\smile   \bt\right)=\\=
		\check{H}^\bullet\left(q'\right) \left( \a\smile \check{H}^\bullet\left(p'\right) \left( \ga\right)\right)\smile \check{H}^\bullet\left( q''\right)\left(     \bt\right)
	\end{split}
	\ee
