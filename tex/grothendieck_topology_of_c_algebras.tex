...
continuous-trace algebra  with DixmierÐDouady class $\delta \in \check{H}^3\left(\sX,\Z \right)$ (cf. Notation \ref{ctr_not}). Suppose that the space $\sX$ is compact. Let $U \subset M\left(A \right)$ be a group of unitary elements.  For any hereditary $C^*$ subalgebra $B \subset A$ there is a normal subgroup
\be\label{ctr_ub_eqn}
U_B \bydef \left\{u \in U | \forall b \in B \quad u b = b\right\}\subset U.
\ee
For any  $\mathbf{Hered}/A$-object denote by
\be
\forall B \in \mathbf{Hered}/A \quad U\left( B \right) \bydef U / U_B.
\ee
When one  for any $B', B'' \in  \mathbf{Hered}/A$ such that $B' \subset B''$ (i.e. there is $\mathbf{Hered}/A$-morphism $B'' \to B'$)  then there is a natural injective  homomorphism of groups:
\bean
 U\left( B'' \right)\to U\left( B' \right)
\eean 
So the map $B \mapsto  U\left( B \right)$ is a presheaf with respect to the Gelfand $A$-topology (cf. Definition \ref{gelfand_a_site_defn})
\begin{lemma}
The  map $B \mapsto  U\left( B \right)$ is a sheaf. 
\end{lemma}
\begin{proof}
