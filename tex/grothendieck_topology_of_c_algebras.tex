...
%	\bt\text{-}\lim_\la u_\la = 1_{ M\left( B\right) },\\
	...
	\end{split}
	\ee
	From the Lemma \ref{increasing_convergent_w_lem} it turns out that the net $\left\{\rho\left( u_\la\right)  \right\}$  is convergent  with respect to the strong topology of $B\left(\H \right)$ (cf. Definition \ref{strong_topology_defn}). If $p \bydef s$-$\lim\rho\left(  u_\la \right) \in B\left( \H\right)$ is a strong limit then one can prove that $p$ is a projector.
...
\begin{lemma}\label{blowing_grothendiek_lem} 	If $F$ is an Abelian group then for all $r \ge 0$ there is an isomorphisms
	\bean
...
\end{itemize}
So there is a homeomorphism $\mathrm{Sp\acute{e}}\left(\mathscr P^\delta  \right)'\cong \mathrm{Sp\acute{e}}\left(\mathscr G^\dl \right)$. This theorem follows from the Remark \ref{equiv_rem} and the equation \eqref{hh_p_eqn}.
