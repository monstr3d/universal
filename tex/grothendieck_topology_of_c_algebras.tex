...
contains more the two points, then there are two closed sets $\sU, \sV \in \mathfrak{Adjont}\left( \sX\right)$  such that
\bean
\sU \not\subset  \sX_P,\\
\sV \not\subset \sX_P,\\
\sU \wedge \sV = \sU \cap \sV= \sX_P.
\eean 
...
Any topological $\sX$ space yields a spatial locale (cf. Definition \ref{spatial_locale_defn})
\be
\begin{split}
\mathfrak{Standard}(\sX) \bydef \text{ the set of open subsets of } \sX,\\
\sU \wedge_s \sV \bydef \sU \cap \sV,\\
\bigvee_s \sU_\iota \bydef \bigcup \sU_\iota.
\end{split}
\ee
However there is an unversed  locale
\be
\begin{split}
	1_s = \sX,\\
	0_s = \emptyset\\
	\mathfrak{Inversed}(\sX) \bydef \text{ the set of open subsets of } \sX,\\
	\sU \wedge_i \sV \bydef \sU \cup \sV,\\
	\bigvee_i \sU_\iota \bydef  \text{the maximal open subset of } \bigcap \sU_\iota.
\end{split}
\ee

\subsection{Gelfand topoi}
\paragraph*{}
...
There is an isomorphism between the   {Gelfand}  $A$-{locale} $\mathfrak{Locale}\left(A \right)$ (cf. Definition \ref{gelfand_a_locale_defn}) and the locale $\mathfrak{L}_\H$ of subspaces of $\H$ giben by $B\H$ where $B$ is a hereditary of $A$.
The locale $\mathfrak{L}_\H$ is a measure locale (cf. \cite{pavlov_duality}). If a point $x \in \sX$ is a $\mu$-\textit{atom} (i.e. $\mu\left(\left\{x\right\} \right)> 0$) then there is a nontrivial one-dimensional  subspace $\H_x \subset \H$. It follows that there are no subspaces $\H_1, \H_2 \subset \H$ such that
\bean
\H_1 \neq \H_x,\\
\H_2 \neq \H_x,\\
\H_x = \H_1 \wedge \H_2 = \H_1 + \H_2,
\eean
i.e, $\H_x$ is properly prime (cf. equations \eqref{prime_eqn} and \eqref{pprime_eqn}). 
If $\mathfrak{L}_\H$ has no atoms then for any nontrivial  $\H' \in \mathfrak{L}_\H$ then there are $\H_1, \H_2 \subset \H$ such that
\bean
\H_1 \neq \H',\\
\H_2 \neq \H',\\
\H' = \H_1 \wedge \H_2 = \H_1 + \H_2,
\eean
i.e. $\H'$ does not satisfies to conditions  \eqref{prime_eqn} and \eqref{pprime_eqn}. So if $\mu$ has no atoms then it has no points. In result one has.
\begin{itemize}
