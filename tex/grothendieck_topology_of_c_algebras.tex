...
\ee
	where the right part means the product of operators. If $\sigma \in H^2\left(\G\left(M, \sF\right), \T \right)$ then for any $\a\in \mathscr A$ there is an inclusion
	$\phi_\a: \K\left( L^2\left( \mathcal N_{\a}\right)\right) \otimes C_0\left(\sZ_\a \right) \hookto C^*_r\left(M, \F, \sigma \right)$ which does not unique. If $\a', \a'' \in \mathscr A$ are such that $\sU_{\a'}\cap \sU_{a''} \neq \emptyset$ then there are inclusions 
	\bean
	...
	$$
	\phi_{a'}\circ \varphi_{\a'\a''} = 	\phi_{a''}\circ \varphi_{\a''\a'} \circ \overline\Phi^\sigma_{\a'\a''}
	$$
	If we consider a given by \eqref{foli_a_prod_eqn} twisted product $*_\sigma$ then similarly to the equation \eqref{star_eqn} one has
	$$
	a_n *_\sigma  ... *_\sigma a_1 = b_n \cdot \overline\Phi^\sigma_{\a_n,\a_{n-1}}\cdot b_{n-1} \cdot  ...\cdot b_2\cdot  \overline\Phi^\sigma_{\a_2,\a_1}\cdot b_1
	$$
	There is an automorphism $\Phi^\sigma_{\a'\a''}$ of the $a$
!!!	
...
\end{lemma}
\begin{proof}
	From the inclusion $\Coo_c\left( M\right)\hookto M\left( C^*_r\left(M,\sF, \sigma\right)\right)$ and taking into account that $\Coo_c\left( M\right)$ is dense in $C_0\left( M\right)$ we have an inclusion $C_0\left( M\right)\hookto M\left( C^*_r\left(M,\sF, \sigma\right)\right)$. If $\left\{u_\la\right\}\subset C_0\left( M\right)$ is an approximate unit (cf. Definition \ref{approximate_unit_defn}) for  $C_0\left( M\right)$ then
$$
\forall a \in \Ga^\infty_c\left(\G, \Om^{1/2}\right) \quad \lim_{\la\in \La}\left\| a - u_\la a u_\la\right\|= 0.
$$
Taking into account that $\Ga^\infty_c\left(\G, \Om^{1/2}\right)$ dense in $C^*_r\left(M,\sF, \sigma\right)$ one has
$$
...
\subsection{Dixmier-Douady theory}
\paragraph{}
Let both $\delta, \rho \in  H^2\left(\G\left(M, \sF\right), \T \right)$  be cocycles. Consider a set  the set of generators and relations $\left(G, R\right)$ (cf. Appendix \ref{gen_rel_empt}) such that
\begin{itemize}
...
\begin{proof}
	...
	by $s^\dl \in \mathscr X^{C^*_r\left(M,\sF, \delta\right)}$ and $s^\rho \in \mathscr X^{C^*_r\left(M,\sF,  \rho\right)}$ such that $P_{C^*_r\left(M,\sF, \delta\right)}\left( s^\dl\right) = P_{C^*_r\left(M,\sF, \rho\right)}\left( s^\rho\right)$ where the notation \eqref{pa_eqn} is used. If $x \bydef p_{C^*_r\left(M,\sF, \dl\right)}\left(s^\dl \right)= p_{C^*_r\left(M,\sF, \rho\right)}\left(s^\rho \right)$  (cf. equation \eqref{loical_hom_eqn}) then there a commutative hereditary subalgebras $C^\dl \subset C^*_r\left(M,\sF, \dl\right)$ and $C^\rho \subset C^*_r\left(M,\sF, \rho\right)$
	such that
	\begin{itemize}
		\item $s^\dl = C^\dl_x$ and $s^\rho = C^\rho_x$,
	...
	\bean
	C^*_r\left(\sU, \F|_\sU, \dl\right) \cong \K\left(  L^2\left( \mathcal N_\a\right)\right)   \otimes C_0\left( \sZ_\a\right),\\
		C^*_r\left(\sU, \F|_\sU, \rho \right) \cong \K\left(  L^2\left( \mathcal N_\a\right)\right)   \otimes C_0\left( \sZ_\a\right).
	\eean
...
$$
\phi\left( s^\dl, s^\rho\right) \bydef C_x.
$$
	We leave to the reader a proof of following statements.
\begin{itemize}
	\item $\phi \left(s^\dl, s^\rho \right)$ is uniquely depends on both $s^\dl$ and $s^\rho$, i.e. it  does not depend on a choice of $C^\dl$ and $C^\rho$ etc.
