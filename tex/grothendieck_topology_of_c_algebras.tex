...
\end{theorem}
	...
	%9.5. Theorem. 
	!!! USE THIS THEOREM IN PROOF  FLABBY !!!
...
\begin{proof}
	!!!! 2.4 HARTSHORNE !!!
If we consider  the  given by  \eqref{canonical_resolution_eqn} canonical resolution, i.e.
	\be\label{canonical_resolution_c_eqn}
0 \to \mathscr F \xrightarrow{\eps} 	\mathscr{C}^{0}\left(\sX, \mathscr F \right) \xrightarrow{d_0 } \mathscr{C}^{1}\left(\sX, \mathscr F \right)\xrightarrow{d_1 } \mathscr{C}^{2}\left(\sX, \mathscr F \right)\xrightarrow{d_2}\dots
\ee	
then for all $p$ the  sheaf $\mathscr{C}^{0}\left(\sX, \mathscr F \right)$ flabby.
From the Proposition \ref{sheaf_flasque_prop} it follows that 
$H^p\left(\sX, \mathscr F \right)  = 0$ for all $q$ and  $p > 0$.

and apply the functor $\Ga\left(X, \cdot \right)$ to obtain a complex
\bean
\Ga\left(X,\ 	\mathscr{C}^{0}\left(\sX, \mathscr F \right)  \right) \xrightarrow{\Ga\left( d_0 \right)} \Ga\left(X,	\mathscr{C}^{1}\left(\sX, \mathscr F \right) \right) \xrightarrow{\Ga\left( d_1 \right)} \Ga\left(X, 	\mathscr{C}^{2}\left(\sX, \mathscr F \right)  \right) \to ... 
\eean
For any $p$ a sheaf $\mathscr{C}^{0}\left(\sX, \mathscr F \right)$ is flabby, so it is acyclic 
The complex \eqref{complex_eqn}  and the Theorem \ref{derived_finctor_thm} define the $p^{\text{th}}$ cohomology
group 

\bean
\begin{split}
	H^p\left(X, \mathscr F \right)=\begin{cases}
...
\begin{exercise}\cite{hartshorne:ag}\label{sheaf_flasque_exer}
	...
	\begin{itemize}
		...
		\item[(b)] If  $\mathscr F' \to \mathscr F \to \mathscr F''$ is an exact sequence of sheaves, and if $\mathscr F'$ is 
		flasque, and if $\mathscr F'$ and $\mathscr F$
		are flasque, then $F''$ is flasque. 
