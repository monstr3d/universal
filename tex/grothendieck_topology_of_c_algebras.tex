...
There is an alternative description of $\mathfrak{Site}\left(\K \right)$. Let $d \in \N$ or $d = \infty$. If $d = \infty$ we suppose that $\C^d \bydef \ell^2\left(\N \right)$. There is a complex projective plane $\C P^d$. Now
\begin{enumerate}
	\item[(a)] $\mathbf{Hered}/\K$-objects are subsets of $\C P^d$ given by
	\be\label{compact_zar_open_eqn}
	\sU = \C P^d \setminus \bigcup_{j = 1}^n L_j
\ee
...
\end{empt}
\begin{empt}
Suppose that $d$ is finite, and consider $\C P^d$ as an algebraic manifold with Zariski topology $J_{\mathrm{Zar}}$. On the other there is a category  $\mathbf{Cat}\left( \C P^d\right) $  of open with respect $J_{\mathrm{Zar}}$  subsets of $\C P^d$ and their inclusions. Since any given by \eqref{compact_zar_open_eqn} set is open one has a functor
\be\label{compac_to_cp_eqn}
\mathbf{Hered}/\K \xrightarrow\phi \mathbf{Cat}\left( \C P^d\right).
\ee
There is a site $\left(\C P^d, J_{\mathrm{Zar}}\right)$ and  one can prove that the functor \ref{compac_to_cp_eqn} is  a morphism of sites (cf. Definition \ref{morphism_of_sites_defn}). From the Theorem \ref{site_mor_thm} it follows that there is a geometric morphism
\be\label{compac_geom_eqn}
 \mathbf{Sh}\left(\C P^d, J_{\mathrm{Zar}} \right)\xrightarrow{f} \mathbf{Sh}\left(\mathbf{Hered}/\K, J_\K \right)
\ee
Let $F$ be a  Abelian group, and let both $F_{\mathbf{Hered}/\K} \in \mathbf{Ab}^{\left(\mathbf{Hered}/\K \right)^{\text{op} }}$ and $F_{\mathbf{Cat}\left( \C P^d\right)} \in \mathbf{Ab}^{\left(\C P^d \right)^{\text{op} }}$ be constant presheaves (cf. Definition \ref{constant_presheaf_defn}) of Abelian groups. It is known that if $\sU$ is an open with respect to Zariski topology subset of connected complex manifold then $\sU$ is connected. From this fact it follows that both  $F_{\mathbf{Hered}/\K}$ and $F_{\mathbf{Cat}\left( \C P^d\right)}$ are sheaves, i.e. $F_{\mathbf{Hered}/\K}\in \mathbf{Ab}\left(\mathbf{Hered}/\K, J_\K \right)$ and $F_{\mathbf{Cat}\left( \C P^d\right)}\in \mathbf{Ab}\left(\C P^d, J_{\mathrm{Zar}}\right) $ and
...
\ee
specifically, if we write elements of $\coprod_{u,r} \mathbf{y}C'$ as triples $(r, u, v)$ with
...
graded ring with identity. It is called the \text{Chow ring} of $\sX$. Roughly speaking  $\sY.\sZ$ correspond   to the  intersection $\sY\cap\sZ$ of subvarieties.
\end{remark}
