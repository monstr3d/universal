...
\begin{definition}\label{a_locale_defn}
	From the Theorem \ref{locale_thm} it follows that the $A$-{topos} $\mathscr{E}_A$ is localic. It corresponds to a locale $\mathfrak{Grot}\left(A \right)$, we say that  $\mathfrak{Grot}\left(A \right)$ is the \textit{Grothendieck} $A$-\textit{locale}, so one 
has
\be\label{a_locale_eqn}
\mathscr{E}_A = \mathbf{Sh}\left(\mathfrak{Grot}\left(A \right) \right). 
\ee	
...
\end{proof}
\section{Grothendieck topology of $C^*$-algebras with Hausdorff spectrum}
