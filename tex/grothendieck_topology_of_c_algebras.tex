...
\end{lemma}
\begin{proof}
	If for any $x \in \sX$ the *-homomorphisms  $\pi_x^\dl: CT\left(\sX, \dl \right)\to B\left( \H^\dl_x\right)$ and  $\pi_x^\rho: CT\left(\sX, \rho \right)\to B\left( \H^\dl_x\right)$ are corresponding to $x$ irreducible representations and $\pi_a^\dl: CT\left(\sX, \dl \right)\to B\left( \H^\dl_a\right)$ and $\pi_a^\rho: CT\left(\sX, \rho \right)\to B\left( \H^\rho_a\right)$ are atomic representations (cf. Definition \ref{atomic_repr_defn}) then both $\H^\dl_a$ and $\H^\rho_a$ are completions of algebraic direct sums $\bigoplus_{x \in \sX}\H^\dl_x$ and $\bigoplus_{x \in \sX}\H^\dl_x$. The reader  can easily proof that the norm completion of $\H^\dl_a\ox_{C_0\left(\sX\right)}\H^\rho_a$ is a norm completion of an algebraic direct sum
	$$
	...
	\ee
	From the proof of the Proposition \ref{ctr_d_prop} it follows that the family corresponds to an element of $\widetilde a \in CT\left(\sX, \dl + \rho\right)$. If $\widetilde \H_x= \H^{\dl + \rho}_z$ is the completion of  $\H^\dl_x\ox_{\C}\H^\rho_x$ then there is the natural irreducible representation $\widetilde \pi_x: \in CT\left(\sX, \dl + \rho\right)\to B\left(\widetilde \H_x\right)$. If $\widetilde \H_a = \H^{\dl + \rho}_a$ is a norm completion of 
	$$	\bigoplus_{x \in \sX}\H^\dl_x\ox_{\C}\H^\rho_x\quad\text{and/or}\quad
	\bigoplus_{x \in \sX}\H^{\dl +\rho}_x
	$$
	then the natural 
\end{proof}
