...
\forall \left(\iota_0, ..., \iota_{n-1}\right)\in I^n \quad \sU_{\iota_0} \cap ... \cap \sU_{n-1}\neq \emptyset\quad \Leftrightarrow \quad \sV_{\iota_0} \cap ... \cap \sV_{n-1}\neq \emptyset,
\end{split}
\ee
\item[(ii)]  for any finite covering $\left\{\sV_\iota \right\}_{\iota \in I}$ of $\sX$ by closed subsets there is a finite  covering $\left\{\sV_\iota \right\}_{\iota \in I}$ of $\sX$ by open  subsets which satisfies to the condition \eqref{top_open_closed_cech_eqn}.
\end{enumerate}
\end{lemma}
\begin{proof}
...
\eean 
If $\sV_\iota$ is the closure of $\sU^{r_f}_{\iota}$ then the family $\left\{\sV_\iota \right\}_{\iota \in I}$ satisfies to the condition \ref{top_open_closed_cech_eqn}.\\
(ii) Let $\left\{\sV_\iota \right\}_{\iota \in I}$ be a finite closed covering of $\sX$. Denote by $\A$ the set of finite subsets of $I$ such that
!!!
\end{proof}
...
\end{theorem}
\begin{proof}
According to the Remark \ref{top_chech_cof_rem} one can use only finite open coverings of $\sX$ for calculation of $\check H^p\left(\sX,  \mathscr A\right)$, Let us define a homomorphism $\check H^p\left(\sX,  \mathscr A\right) \to \check H^p\left(\mathfrak{Closed}\left( \sX\right),  \mathscr A^c\right)$. If 
\end{proof}
