...
If $A$ is a continuous trace $C^*$-algebras then the spectrum $\sX$ of is a Hausdorff space. Any ideal of $A$ corresponds to an open set $\sU$ we denote this ideal by $A|_{\sU}$. We define a presheaf $\mathscr{P}^{\text{comm}}$ on $\sX$ such that if $\sU$ and $\sV$ are open subsets of $\sX$ then
\bean
...
\begin{proof}
	...
	Let $\eps> 0$ be given. From (b) of the Lemma \ref{ctr_rep_eq_lem} it turns out that for all $\la\in \La$ the map
	$$
	x\mapsto \left\|  \rep_x\left(b_\la\right)\right\|
	$$
	is continuous, so the set 
	$$
	\sX_\la \bydef \left\{x \in \sX \left|\left\|  \rep_x\left(b_\la\right)\right\|\le\eps \right. \right\}
	$$
	...
	we see that $x\notin \sX_\la$ if $\la$ is sufficiently large. Thus $x \notin \bigcap \sX_\la$. In other words, 
	$\bigcap   \sX_\la$ is empty. Hence $\sX_{\la_0}$ is empty for some $\la_0\in\La$ %(Theorem 2.36). 
	It follows that  $0\le \left\|  \rep_x\left(b_\la\right)\right\| < \eps$ for all $x \in \sX$  and for all $\la\ge \la_0$. This proves the theorem.
\end{proof}
...
\subsection{Cohomology. Definition and basic properties}\label{grothendieck_cohomology_section}
\paragraph{}
