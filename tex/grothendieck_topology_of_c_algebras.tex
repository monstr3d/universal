...
\begin{enumerate}
\item If $\left\{\sU\right\}_{\iota \in I}$ is a family of open subsets of $\sX$ then a set   $\left\{_{\sU_\iota}A_{\sU_\iota}\right\}_{\iota \in I}$ is a covering of $_{\cup_{\iota \in I} \sU_\iota}A_{\cup_{\iota \in I}\sU_\iota}$ (cf. Definition \ref{hered_cov_defn})
\item  !!!
\end{enumerate}
\end{exercise}
...
\end{empt}
 ...
 If $A$ is a {commutative generated} $C^*$-algebra with  {Hausdorff blowing-up} $C_0\left( \sX\right) \hookto M\left(A \right)$ then $\mathscr X^A$ is the Grothendieck $A$-space $\mathfrak{Grot}\left(A \right)$ (cf. Definition \ref{grothendieck_ca_t_defn}).
\end{lemma}
\begin{proof}
...
Let $\Om_\sX$ be {sheaf of stalks of open sets} (cf. Appendix \ref{open_sheaf_empt}). Let  $\mathrm{Sp\acute{e}}\left(\Om_\sX \right)$ be the	\'espace etal\'e  (cf. Exercise \ref{sheaf_etale_exer}) of the sheaf $\Om_\sX$. The empty set $\emptyset$ yields a global  section $s^\emptyset \in \Om_\sX\left(\sX \right)$, this section gives an open subset $\mathrm{Sp\acute{e}}\left(\Om_\sX  \right)_\emptyset \subset \mathrm{Sp\acute{e}}\left(\Om_\sX \right)$. Denote by 
\be
...
\ee
If  $\mathscr{P}^{ A}$ is a   given by \eqref{comm_alg_sheaf}  presheaf then there  is a surjective morphism of sheaves (cf. Definition \ref{sheaf_morphism_defn})
\bean
...
\begin{definition}\label{local_prod_defn}
	...
	!!! THERE IS A SYSTEM OF OPEN SETS !!!
Let $A, A', A''$ be  commutative generated $C^*$-algebras (cf. Definition \ref{hered_com_defn}). If $C_0\left( \sX\right) \hookto M\left(A \right)$, $~C_0\left( \sX\right) \hookto M\left(A' \right)$, $~C_0\left( \sX\right) \hookto M\left(A'' \right)$ are Hausdorff blowing-ups (cf. Definition \ref{blowing_up_defn}) then
...
\eean
corresponding to $x$ irreducible representations. If $\sV \subset \sU$ and $$C^{\delta } \in \mathfrak{Supp}\left( CT\left(\sX, \dl \right)\right)_{\sV}, \quad C^{ \rho} \in \mathfrak{Supp}\left( CT\left(\sX, \rho \right)\right)_{\sV}, \quad  C^{\delta + \rho} \in \mathfrak{Supp}\left( CT\left(\sX, \dl + \rho \right)\right)_{\sV}$$
(cf. equation \eqref{supp_set_eqn}) then there are following correspondences:
\begin{itemize}
	\item $C^{\delta}$ corresponds to a family of rank-one projectors $\left\{p_x^{\dl}\in B\left(\H^{\dl}_x\right)\right\}_{x \in\sV}$ onto subspaces $\pi^{\dl}_x\left( C^{\dl}\right)\H^{\dl}_x\subset \H^{\dl}_x$,
	\item $C^{\rho }$ corresponds to a family  of rank-one projectors $\left\{p_x^{\rho}\in B\left(\H^{\rho}_x\right)\right\}_{x \in\sV}$ onto spaces $\pi^{\rho}_x\left( C^{\dl}\right)\H^{\dl}_x\subset \H^{\rho}_x$,
	\item $C^{\delta + \rho }$ corresponds to a family  of rank-one projectors $\left\{p_x^{\dl+\rho}\in B\left(\H^{\dl + \rho}_x\right)\right\}_{x \in\sV}$ onto spaces $\pi^{\dl + \rho}_x\left( C^{\dl + \rho}\right)\H^{\dl + \rho}_x\subset \H^{\dl+\rho}_x$.
\end{itemize}
Using  a bijective map $\varphi_{\dl \rho}	:CT\left(\sX, \dl \right)\times_\sX CT\left(\sX, \rho \right)\cong CT\left(\sX, \dl +\rho\right)$ (cf. equitation \eqref{bundle_prod_iso})  we can find a one to one correspondence between pairs $\left(\left\{p_x^{\dl}\right\}_{x \in\sV}, \left\{p_x^{\rho}\right\}_{x \in\sV} \right)$ of families and families $\left\{p_x^{\dl+\rho}\right\}_{x \in\sV}$. This correspondence yields required by the Definition \ref{local_prod_defn} bijective map
$$
