...
\end{remark}
\begin{empt}
Let  $\sX$ be a closed subset of  $\sY$ and a set  $\sY$ is locally connected.  Let $\mathscr {L}\bydef \mathfrak{Ass}\left(L_\sY \right)$ be a sheaf associated with constant presheaf (cf. Definitions \ref{constant_presheaf_defn}, \ref{associated_sheaf_defn}). From the Proposition \ref{comm_alg_loc_thm}  !!! THEOREM !!!\ref{sh_sh_prop} it follows that the map
$$
\mathfrak A : \sU \mapsto H^c_0\left( \sU, \mathscr {L}\right) 
$$
is a cosheaf (cf. Definition \ref{top_cosh_defn})
\end{empt}
 \begin{lemma}
 The given by
\bean
...
\bean
H^q\left(\check C^p\left( \mathscr U; \mathscr A^*\right)\right) = \check C^p\left( \mathscr U;\check H^q\left( \Ga \mathscr A^*|_\bullet\right)\right)  = C^p\left( \mathscr U;\check H^q\left( \bullet, \mathscr  A^*\right)\right) = \\
\begin{cases}
