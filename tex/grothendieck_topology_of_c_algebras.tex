...
\begin{lemma}\label{sh_co_lem}
	...
	\varphi_\sU:	\mathscr L\left(\sU \right) \cong \mathfrak D \left( \mathscr L \right) \left(\sU \right)  
		...
		\item[(ii)] A map
		$$
		\sU \in \mathfrak{Closed}\left( \sX\right)\quad\sU \mapsto \mathfrak A\left(^\neg \sU \right) 
		$$
		is a sheaf on the locale $\mathfrak{Closed}\left( \sX\right)$.
	\end{enumerate}
\end{lemma}
\begin{proof}(i)
...
(ii) If $\sV \subset \sU$ then $^\neg \sU\subset ^\neg \sV$, and there is the natural map
$$
i_{\sV\sU}:\mathfrak A\left(^\neg \sV\ \right) \to \mathfrak A\left(^\neg \sU\ \right). 
$$
If $\left\{\sU_{   \a}\right\}_{\a \in \A}\subset \mathfrak{Closed}\left( \sX\right)$ then $\sU \bydef \bigvee \sU_\a$ is a closure of  $\bigcup \sU_\a$. So $^\neg \sU$ is a maximal open subset of $\bigcap ^\neg \sU_\a$.  Consider a family $\left\{s_\a \in \mathfrak A\left(^\neg \sU_\a \right) \right\}$ such that for all $\a', \a''\in \A$ one has
\bean
\rho_{\sU_{   \a''},\sX}\left( s_{\a'}\right) = \rho_{\sU_{   \a''},\sX}\left( s_{\a''}\right)
\eean
From the above equation and (i) it follows that 
$$
\varphi^{-1}_{\sX}\left( \rho_{\sU_{   \a''},\sX}\left( s_{\a'}\right)\right)  = \varphi^{-1}_{\sX}\left( \rho_{\sU_{   \a''},\sX}\left( s_{\a''}\right)\right)
$$
where $\varphi_{\sX}$ is given by \eqref{sh_co_eqn}, so one has
$$
\left|  \varphi^{-1}_{\sX}\left( \rho_{\sU_{   \a''},\sX}\left( s_{\a'}\right)\right)\right|= \left|  \varphi^{-1}_{\sX}\left( \rho_{\sU_{   \a''},\sX}\left( s_{\a''}\right)\right)\right|
$$
