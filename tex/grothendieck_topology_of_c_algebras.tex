...
are the identity transformations $1_F$ and $1_G$ on $F$ and $G$ respectively. $F$ is said to be \textit{left adjoint} $G$.
	...
	\end{definition}
	\begin{definition}\label{site_defn}\cite{johnstone:topos,milne:lec}
	...
	Two different Grothendieck pretopologies can give the same sheaves. To remove this we consider those families which are "saturated" in the sense that $\left(V \xrightarrow{\a}U\right) \in R$ implies $\left(W\xrightarrow{\a\bt}U\right)\in \R$ for any $W \xrightarrow{\bt} V$. Such a family is called a \textit{sieve}.
	\end{empt}
...
\end{defn}
\begin{rem}\cite{johnstone:topos}
