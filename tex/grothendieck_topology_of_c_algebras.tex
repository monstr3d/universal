...
\begin{definition}\label{a_locale_defn}
	From the Theorem \ref{locale_thm} it follows that the {hereditary} $A$-{topos} $\mathfrak{Topos}\left( A\right)$ is localic (cf. Definition \ref{localic_topos_defn}. It corresponds to a locale $\mathfrak{Locale}\left(A \right)$, we say that  $\mathfrak{Locale}\left(A \right)$ is the \textit{hereditary}  $A$-\textit{locale}, so one 
	has
	\be\label{a_locale_eqn}
	\mathfrak{Topos}\left( A\right)  = \mathbf{Sh}\left(\mathfrak{Locale}\left(A \right) \right). 
	\ee	
	The corresponding frame $\mathfrak{Frame}\left(A \right)$ is said to be the \textit{hereditary}  $A$-\textit{frame}.
\end{definition}
...
\begin{definition}\label{grothendieck_ca_t_defn}
	Let $A$ be a $C^*$-algebra. If the {Hereditary}  $A$-{locale} $\mathfrak{Locale}\left(A \right)$ is spatial (cf. Definition \ref{enoght_points_defn}) then we say that the $\mathfrak{Space}\left(A \right)\bydef \mathrm{pt}\left( \mathfrak{Locale}\left(A \right)\right) $ is the \textit{Hereditary} $A$-\textit{space}.  
\end{definition}
\begin{exercise}
	Prove that for any commutative $C^*$-algebra the {Hereditary}  $A$-{locale} $\mathfrak{Locale}\left(A \right)$ is naturally isomorphic to the Bohr $A$-locale $\mathfrak{BohrLocale}\left(A \right)$ of the Bohr topos (cf. Definition \ref{bohr_locale_defn}.
\end{exercise}
\begin{remark}\label{c_0_x_rem}
	For any locally compact Hausdorff space $\sX$ there is a one to one correspondence between open subsets of $\sX$ and hereditary subalgebras of $C_0\left(\sX\right)$, So an arising from $\sX$ site $\mathbf{T}_\sX$  is naturally equivalent to a site arising from $C_0\left( \sX\right)$ (cf. Definition \ref{grothendieck_ca_defn}), i.e. $\sX$ is the {Hereditary} $C_0\left(\sX\right)$-{space}.
\end{remark}
...
\bea\label{hom_eqn}
H^r\left(\mathfrak{Space}\left(A \right),F\right)\cong {H}^r\left(A, F \right),\\
\label{chech_hom_eqn}
\check H^r\left(\mathfrak{Space}\left(A \right), F\right)\cong \check{H}^r\left(A, F \right)
\eea
...
$$
\check H^\bullet\left(\mathfrak{Space}\left(A \right), F\right)\cong \check{H}^\bullet\left(A, F \right)
$$
...
Any hereditary full *-homomorphism yields a morphism of locales, so it defines a localic (geometric) morphism of toposes.
\end{lemma}
\begin{proof}
Consider frames $\mathfrak{Locale}\left(  A\right)$ and   $\mathfrak{Locale}\left( \widetilde A\right)$ given by the Remark \ref{locale_rem}. One can define a morphism of frames
\be\label{frame_moprhism_eqn}
\begin{split}
f^{-1}:\mathfrak{Frame}\left(  A\right)\to \mathfrak{Frame}\left( \widetilde A\right)\\
B \mapsto \quad \text{a generated by } \varphi\left(B \right)\widetilde{A} \varphi\left(B \right) \text{ hereditary subalgebra of } \widetilde A
\end{split}
\ee
...
\end{proof}
The Theorem \ref{locale_thm} yields a geometric  morphism of toposes $\mathfrak{Topos}\left( \widetilde A\right)\to\mathfrak{Topos}\left( A\right)$.

\begin{lemma}\label{locale_open_lem}
	Any hereditary full injective *-homomorphism $f: A \hookto \widetilde A$ yields an open map of locales  (cf. Definition \ref{open_locale_mor_defn}).
\end{lemma}
\begin{proof}
		Let us define 
...
\begin{empt}\label{dd_dual_empt}
	...
	\bean
	M\left(A \right) \xhookrightarrow{\phi} M\left(A' \right),\\
	M\left(A \right) \xhookrightarrow{\psi} M\left(A'' \right),\\
	M\left(A' \right) \xhookrightarrow{\widetilde \phi} M\left(\widetilde A \right),\\
	M\left(A'' \right) \xhookrightarrow{\widetilde \psi} M\left(\widetilde A \right),\\
	\eean 
	be injective *-homomorphisms. Suppose following conditions hold:
	\begin{enumerate}
		\item [(a)] The restrictions $\phi|_A$, $~h_A$, $~\widetilde \phi|_{A'}$, $~\widetilde h|_{A''}$ hereditary full (cf. Definition \ref{hereditary_full_defn}
		\item[(b)] 
		\bea\label{dd_dual_empt_1_eqn}
		\widetilde \phi\circ \phi = \widetilde \psi\circ \psi,\\
		\label{dd_dual_empt_2_eqn}
		\forall	a ' \in A' \quad \forall	a '' \in A'' \quad \widetilde \phi\left(a'\right)\widetilde \psi\left(a''\right)\in \widetilde A
		\eea	
		From the above situation one has an $A$-$A$ bimodule homomorphism
		\be\label{dd_dual_empt_eqn}
		\begin{split}
		\varphi: A'\ox_A A'' \to \widetilde A,\\
			a' \otimes a'' \mapsto \widetilde \phi\left(a'\right)\widetilde \psi\left(a''\right).
		\end{split}
		\ee
		where $ A'\ox_A A''$ is an algebraic tensor product.
	\end{enumerate}
\end{empt}

\begin{definition}\label{dd_dual_defn}
	We say that the situation \ref{dd_dual_empt} is a \textit{generalized Dixmier-Douady duality} the homomorphism \ref{dd_dual_empt_eqn} is injective and $\varphi\left(  A'\ox_A A''\right)$ is dense in $\widetilde{A}$.
\end{definition}
...
\eean	
of locales. For any $\widetilde B \in \mathfrak{Locale}\left( \widetilde A\right) $ one has $f_! \circ \widetilde f_! \left(\widetilde B \right)= g_! \circ \widetilde g_! \left(\widetilde B \right)$, i.e. $\widetilde B$ yields an element of $\left(\widetilde f_! \left(\widetilde B \right), \widetilde g_! \left(\widetilde B \right) \right) 
...
\begin{remark}\label{local_prod_rem}
	If $R$ is a ring then  the equation \eqref{local_prodg_eqn} the  Lemma \ref{gr_ring_lem} and the Exercise \ref{ring_homo_exer} one can obtain following homomorphism
	\bean
\check{H}^\bullet\left(\mathfrak{Locale}\left( 'A\right), R\right) \ox_{\check{H}^\bullet\left(  \mathfrak{Locale}\left( A\right),R\right) } 	\check{H}^\bullet\left( \mathfrak{Locale}\left( A''\right), R\right)\to \check{H}^\bullet\left(  \mathfrak{Locale}\left(\widetilde A\right), R\right)
\eean 
The above equation is isomorphic to the following one
	
	\be\label{dix_do_co_eqn}
		\check{H}^\bullet\left( A', R\right) \ox_{\check{H}^\bullet\left( A,R\right) } 	\check{H}^\bullet\left( A', R\right)\to \check{H}^\bullet\left( \widetilde A, R\right)
	\ee 
 These homomorphisms can be regarded as analogs of the following formula
	\bean
...
There is an isomorphism between the   {Hereditary}  $A$-{locale} $\mathfrak{Locale}\left(A \right)$ (cf. Definition \ref{a_locale_defn}) and the locale $\mathfrak{L}_\H$ of subspaces of $\H$ giben by $B\H$ where $B$ is a hereditary of $A$.
...
From the Lemma \ref{lolale_lem} it follows that there is a morphism 
\be\label{p_loc_eqn}
p_{loc}:\mathfrak{Locale}\left(A \right) \to \sX.
\ee
of locales.
...
\section{Locales for   $C^*$-algebras with Hausdorff spectrum}

\paragraph{}
Let $A$ be a $C^*$-algebra with the Hausdorff spectrum $\sX$. Consider a pair $\left(\pi_x, e_x\right)$ where $\pi_x : A \to B\left(\H_x \right)$ is a corresponding to $x \in \sX$ irreducible representation and $e_x \in B\left(\H_x \right)$ is a rank-one projector. 
\begin{lemma}\label{haus_point_lem} For any pair $\left(\pi_x, e_x\right)$ given by
	\be\label{haus_point_map_eqn}
	\begin{split}
		p_{\left(\pi_x, e_x\right)}:	\mathfrak{Frame}\left(A \right)\to \left\{0, 1\right\},\\
		B \mapsto \begin{cases}
			\pi_x\left(B\right)  \perp e_x & 0\\
			e_x \in 	\pi_x\left(B\right) & 1
		\end{cases}.\end{split}
	\ee
...
\begin{proof}
	...
	From the Lemma \ref{point_lem} it follows that one needs proof that the given by \eqref{p_p_eqn} hereditary subalgebra $B_{\left[C, \I\right]}$ satisfies to the conditions  \eqref{prime_eqn} and \eqref{pprime_eqn}. 
	If $\left\{B_\iota \right\}_{\iota \in I}\bydef p^{-1}\left(0 \right)$ then $B_{\left[C, \I\right]} = \bigvee_{\iota \in I} B_\iota$. If left ideals $L\left( B_{\left[C, \I\right]}\right)$ and  $L\left( B_\iota\right)$ are given by \eqref{left_ideal_eqn} then 
	$L\left( B_{\left[C, \I\right]}\right)$ 
	is the the $C^*$-norm closure of an algebraic sum $\sum_{\iota\in I}L\left(B_\iota \right)$. If $f: C \to A/\I$ is a natural surjective map and  $c \in C$ is such that $\left\|f(c)\right\|> 0$ and 
	$$
	\inf_{b \in \sum_{\iota\in I}L\left(B_\iota \right)} \left\|b - c\right\| \ge \left\|f(c)\right\| \quad \Rightarrow \quad \inf_{b \in B_{\left[C, \I\right]}} \left\|b - c\right\| \ge \left\|f(c)\right\| \quad \Rightarrow B_{\left[C, \I\right]} \neq A
	$$
	i.e. $B_{\left[C, \I\right]}$ satisfies to \eqref{pprime_eqn}. Moreover $B_{\left[C, \I\right]}$ is a maximal among hereditary $C^*$-subalgebras $B$ such that
	$
	B \cap C \subset \I
	$.
	If $B_{\left[C, \I\right]}\subsetneqq B'$ then there is $c \in C\setminus\I$  such that $c \in B'$. The ideal is $\I$ maximal it follows that $C \subset B'$ and $B'$ contains all  commutative $C^*$-subalgebras.  Since $A$ is generated by all its commutative $C^*$-algebra one has $B' = A$,
	If $B_{\left[C, \I\right]}$ does not satisfy to condition \eqref{prime_eqn} then there are hereditary subalgebras $B', B'' \subset A$ such that 
	\bean
	B'\cap B'' = B_{\left[C, \I\right]},\\
	B', B''\neq  B_{\left[C, \I\right]}.
	\eean
	However $B', B'' = A$ so $B'\cap B'' = A$.
\end{proof}
\  


Here we consider stable separable continuous trace  $C^*$-algebras $CT\left(\sX, \dl \right)$ (cf. Notation \ref{ctr_not}).
\subsection{Hereditary spaces}

\begin{lemma}
	There is the {Hereditary} $CT\left(\sX, \dl \right)$-{space} $\mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)$ (cf. Definition \ref{grothendieck_ca_t_defn}) with the surjective local homeomorphism $\mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)\to \mathrm{Sp\acute{e}}\left(\Om_\sX \right)$
\end{lemma}
\begin{proof}
	From the Remark \ref{ctr_gen_rem} it follows that $CT\left(\sX, \dl \right)$ is of type $I_0$ (cf. Definition \ref{type_i0_defn}).
	From the Theorem \ref{dauns_hofmann_thm} it follows that there is  Hausdorff blowing-up $C_0\left( \sX\right)\hookto CT\left(\sX, \dl \right)$. From the Theorem \ref{type_0_thm} and the  equation \eqref{blowing_grothendieck_sh_eqn} it follows that there is the Hereditary $CT\left(\sX, \dl \right)$-space $\mathfrak{Space}\left(CT\left(\sX, \dl \right)\right) =\mathfrak{Points}\left(CT\left(\sX, \dl \right)\right)$ with the given by \eqref{P_A_eqn} map $P_{CT\left(\sX, \dl \right)}:\mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)\to \mathrm{Sp\acute{e}}\left(\Om_\sX \right)$. We leave to the reader the proof of that $P_{CT\left(\sX, \dl \right)}$ is a local homeomorphism.
\end{proof}
...
\end{proof}
\subsection{Dixmier-Douady product}
\begin{lemma}\label{ctr_local_prod_lem}
	If $\dl, \rho \in \check{H}^3\left(\sX, \Z \right)$  then $CT\left(\sX, \dl + \rho \right)$ is the Dixmier-Douady product of $CT\left(\sX, \dl \right)$ and $CT\left(\sX, \rho \right)$ (cf. Definition \ref{local_prod_defn}).
\end{lemma}
\begin{proof}
	One should find a homeomorphism $\phi: \mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)\times_{\mathrm{Sp\acute{e}}\left(\Om_\sX \right)} \mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)\to \mathfrak{Space}\left(CT\left(\sX, \dl +\rho\right)\right)$.
	Any element of $\mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)\times_{\mathrm{Sp\acute{e}}\left(\Om_\sX \right)} \mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)$ can be represented 
	by $s^\dl \in \mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)$ and $s^\rho \in \mathfrak{Space}\left(CT\left(\sX, \rho \right)\right)$ such that $P_{CT\left(\sX, \dl \right)}\left( s^\dl\right) = P_{CT\left(\sX, \rho \right)}\left( s^\rho\right)$ where the notation \eqref{P_A_eqn} is used. If $x \bydef p_{CT\left(\sX, \dl \right)}\left(s^\dl \right)= p_{CT\left(\sX, \rho \right)}\left(s^\rho \right)\in \sX$  (cf. equation \eqref{loical_hom_eqn}) then there are commutative hereditary subalgebras $C^\dl \subset {CT\left(\sX, \dl \right)}$ and $C^\rho \subset {CT\left(\sX, \rho \right)}$
	such that
	\begin{itemize}
	...
	$$
	\phi \left(s^\dl, s^\rho \right) 	\bydef C_x\in \mathfrak{Space}\left(CT\left(\sX, \dl + \rho \right)\right).
	$$
	...
	\begin{itemize}
		...
		\item The map  $\phi: \mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)\times_{\mathrm{Sp\acute{e}}\left(\Om_\sX \right)} \mathfrak{Space}\left(CT\left(\sX, \dl \right)\right)\cong \mathscr X^{CT\left(\sX, \dl +\rho\right)}$ is a homeomorphism.
	\end{itemize}
...
$CT\left(\sX, 0 \right) \cong C_0\left(\sX, \K \right)$. So there is a positive $a \in  C_0\left(\sX, \K \right)_+$ such that $\rank \pi_x \left(a\right)= 1$ The generated by $\{a\}$  \ref{hered_generated_defn} commutative  hereditary subalgebra $C^a$ is isomorphic to $C_0\left(\sX \right)$, there is the natural *-isomorphism $C_0\left(\sX \right) \to C^a$. If $p_{CT\left(\sX, 0 \right) }: \mathfrak{Space}\left( CT\left(\sX, 0 \right) \right) \to \sX$ is given by  \eqref{p_a_eqn} map then there is a continuous map
\be\label{j_eqn}
\begin{split}
	\sX \xrightarrow{s } \mathfrak{Space}\left( CT\left(\sX, 0 \right)\right) = \mathfrak{Points}\left( CT\left(\sX, 0 \right)\right) ,\\
	x \mapsto \left[j, x, C_0\left( \sX\right) \right]
\end{split}
\ee 
\begin{exercise}
	Prove that $p_{CT\left(\sX, 0 \right) }: \mathfrak{Space}\left( CT\left(\sX, 0 \right) \right) \to \sX$ is a local homeomorphism.
\end{exercise}
...
\section{Locales of $C^*$-algebras of type $I_0$}
\subsection{Hereditary spaces}
\paragraph{}
Here we prove that for   $C^*$-algebra of type $I_0$ (cf. Definition \ref{type_i0_defn}) the  Hereditary $A$-locale is spatial. 
...
Let $A$ be a $C^*$-algebra of type $I_0$. An $A$-\textit{representative}  is a pair
$\left(C, \I \right)$ where $C\subset A$ is a commutative $C^*$-subalgebra and $\I\subset C$ is maximal ideal. Two $A$-{representatives} $\left(C', I'\right) $ and  $\left(C'', \I''\right) $ are \textit{equivalent} if and only if
\bean
C' \cap \I'' = C'' \I' \neq \{0\}
\eean
Equivalence classes of representatives a said to be   $A$-\textit{points}. An $A$ point represented by  $\left( C, \I\right)$   will be denoted by $\left[C, I\right]$.  We denote by $\mathfrak{Points}\left(A \right)$ a set of  $A$-points.  
\end{definition}


\begin{lemma}\label{i0_point_lem} If $A$ is $C^*$-algebras of type $I_0$ then
for any $A$-point $\left[C, \I\right]$ given by
\be\label{i0_point_map_eqn}
\begin{split}
p_{\left[C, \I\right]}:	\mathfrak{Frame}\left(A \right)\to \left\{0, 1\right\},\\
	B \mapsto \begin{cases}
		B \cap C \subset \I & 0\\
		B \cap C \not\subset \I & 1
	\end{cases}.\end{split}
...
\end{proof}
\begin{corollary}
	If $A$ is $C^*$-algebras of type $I_0$ then  hereditary $A$-locale $\mathfrak{Locale}(A)$ is spatial (cf. Definition \ref{enoght_points_defn}).
\end{corollary}
\begin{proof}
	If $B', B'' \subset A$ be hereditary $C^*$-subalgebras such that $B'\neq B''$ then one can suppose that  $B'\not\subset  B''$. There is an Abelian element $c \in B'\setminus B''$. If $C$ is the generated by $c$ hereditary commutative $C^*$-subalgebra then $C \cap B'' \neq C$. There is a maximal ideal $\I\subset C$ such that $B'' \cap C \subset \I$.
if $p_{\left[C, \I\right]}  : \mathfrak{Frame}(A)\to \left\{0, 1\right\} $ is given by the equation \eqref{i0_point_map_eqn} then $p_{\left[C, \I\right]}\left( B'\right) = 1$ and $p_{\left[C, \I\right]}\left( B''\right) = 0$. From the Definition \ref{enoght_points_defn} it follows that the locale $\mathfrak{Locale}(A)$ is spatial.
\end{proof}
\begin{lemma}\label{type_I_lem}
If $A$ is a $C^*$-algebra of type $I_0$ then one has:
\begin{enumerate}
	\item [(i)] the family of sets $\mathfrak{Points}\left(B \right)$ yield a topology on $\mathfrak{Points}\left(A \right)$,
	\item[(ii)] the space $\mathfrak{Points}\left(A \right)$ is sober.
\end{enumerate}
\end{lemma}
\begin{proof}
(i) Clearly $\mathfrak{Points}\left(B' \right)\cap \mathfrak{Points}\left(B'' \right)= \mathfrak{Points}\left(B'\wedge B'' \right)$ (cf. \eqref{locale_eqn}) and $\bigcup_{\iota\in I} \mathfrak{Points}\left(B_\iota\right) \subset \mathfrak{Points}\left( \bigvee_{\iota\in I} B_\iota\right)$. If $\left[\left( j, y,  C_0\left( \sY\right)\right)\right] \in \mathfrak{Points}\left( \bigvee_{\iota\in I} B_\iota\right)$ If $B = \bigvee_{\iota\in I} B_\iota$ then from the Lemma \ref{grothendieck_ca_lem} it follows that $j\left( C_0\left( \sY\right)\right)\cap B$ is a hereditary subalgebra of $j\left( C_0\left( \sY\right)\right)$. There is an open subset $\sU \subset \sY$ such that $j\left( C_0\left( \sY\right)\right)\cap B= j\left( C_0\left( \sU\right)\right)$. From $\left[\left( j, y,  C_0\left( \sY\right)\right)\right] \in B$ it follows that $x \in \sU$. Similarly if $\sU_\iota \subset \sU$ is such that $j\left(C_0\left(\sU_\iota \right)  \right) = j\left(C_0\left(\sU \right)  \right)\cap B_\iota$ then from the Lemma \ref{grothendieck_ca_lem} it follows that
$$
C_0\left( \sU\right) = \bigvee_{\iota\in I} C_0\left( \sU_\iota\right).
$$
From the above equation it follows that $\sU = \bigcup_{\iota\in I}\sU_\iota$ and there is $\iota_0 \in I$ such that $x \in \sU_{\iota_0}$. So $\left[\left( j, y,  C_0\left( \sY\right)\right)\right]$ is represented by $\left(j|_{C_0\left( \sU_{\iota_0}\right)}, C_0\left( \sU_{\iota_0}\right), x \right)$. From $j\left(C_0\left( \sU_{\iota_0}\right) \right)\subset B_{\iota_0}$ it follows that
$$
\left[\left( j, y,  C_0\left( \sY\right)\right)\right] \in \mathfrak{Points}\left(B_{\iota_0} \right) \subset \bigcup_{\iota\in I} \mathfrak{Points}\left(B_\iota\right).
$$
(ii) Let $\left[\left( j, y,  C_0\left( \sY\right)\right)\right]\in \mathfrak{Points}\left(A \right)$, and let $B \subset A$ hereditary $C^*$-subalgebra generated by commutative subalgebras $C\subset A$ such that there is no representative $\left( j', y',  C_0\left( \sY'\right)\right)$ such that
\bean
\left[\left( j, y,  C_0\left( \sY\right)\right)\right]= \left[\left( j', x',  C_0\left( \sY'\right)\right)\right],\\
j'\left( C_0\left( \sY'\right)\right) \subset C.
\eean 
From our construction it follows that $\mathfrak{Points}\left(B \right)= \mathfrak{Points}\left(A \right)\setminus \left\{\left[\left( j, y,  C_0\left( \sY\right)\right)\right]\right\}$. It means that any set $\left\{\left[\left( j, y,  C_0\left( \sY\right)\right)\right]\right\}$ is closed, so from the Remark \ref{sober_rem} it turns out that $\mathfrak{Points}\left(A \right)$ is sober.
\end{proof}

\begin{definition}
Two $A$-{representatives} $\left( j', y',  C_0\left( \sY'\right)\right)$ and  $\left( j'', y'',  C_0\left( \sY''\right)\right)$ are \textit{Bohr equivalent} if there are open neighborhoods $\sU'$ and $\sU''$ of $x'$ and $x''$ respectively such that
$$
j'\left(C_0\left( 
...
If $A$ is a $C^*$-algebra of type $I_0$ then the Hereditary $A$-locale $\mathfrak{Locale}(A)$ is spatial, and the Hereditary $A$-space  $\mathfrak{Space}(A)$ is homeomorphic to $\mathfrak{Points}(A)$.
\end{theorem}
\begin{proof}
...
\end{exercise}
\begin{remark}
From the inclusion $\mathfrak{BohrLocale}(A)\subsetneqq\mathfrak{Locale}(A)$ it follows that the Hereditary $A$-locale can be more informative than the Bohr one.
\end{remark}
\begin{remark}
	If $C_0\left( \sX\right) \to M\left(A\right)$ is Hausdorff blowing-up then from \eqref{p_loc_eqn} it follows that there is a surjective continuous map
		\be\label{p_a_eqn}
...
\ee
For any open set $\sU \subset \sX$ denote by
\be\label{stalk_eqn}
\left[\sU, x\right] \bydef \text{ a represented by }  \sU \text{ stalk  of }\Om_\sX \text{ at } x \in \sX.
\ee
...
\end{definition}
Let as define a continuous map
\be\label{P_A_eqn}
\begin{split}
P_A : \mathfrak{Points}\left(A \right) \to  \mathrm{Sp\acute{e}}\left(\Om_\sX \right),\\
\left[\left( j, y,  C_0\left( \sY\right)\right)\right]\mapsto \left[ \supp j\left(  C_0\left(\sY \right)\right) , p_A\left(\left[\left( j, y,  C_0\left( \sY\right)\right)\right]\right)  \right].
\end{split}
\ee
where the notation \eqref{stalk_eqn} is used.
...
\begin{definition}\label{local_prod_defn}
 ...
 we say that $A$ is a \textit{Dixmier-Douady product} of $A'$ and $A''$ if   there is a homeomorphism
...
or equivalently
$$
 \mathfrak{Points}\left(A' \right)\times_{\mathrm{Sp\acute{e}}\left(\Om_\sX \right)}\mathfrak{Points}\left(A'' \right)\cong \mathfrak{Points}\left(A \right).
$$
\end{definition}
...
\begin{definition}\label{etale_haus_defn}
	If $A$ is a $C^*$-algebra of type $I_0$ then Hausdorff blowing-up $C_0\left(\sX \right) \to M\left(A \right)$ is said to be \textit{etal\'e} if the give by  \eqref{p_a_eqn} map
	\bean
	p_A: \mathfrak{Space}(A)\to \sX
	\eean
...
\begin{lemma}\label{blowing_grothendieck_sh_lem} 	
	If $F$ is an Abelian group and $C_0\left(\sX \right) \to M\left(A \right)$ is a  Hausdorff blowing-up then there is a homomorphism 
	\be\label{blowing_grothendieck_shc_eqn}
	\check{H}^\bullet \left(\sX, F  \right)\to	\check{H}^\bullet\left(A, F \right),
	\ee
	Moreover if  there is an isomorphism $F\otimes_\Z F \cong F$ then \eqref{blowing_grothendieck_shc_eqn} is a homomorphism of rings. If Hausdorff blowing-up is {etal\'e} then there is a homomorphism
\begin{eqnarray}\label{blowing_grothendieck_sh_eqn}
{H}^\bullet\left(\sX, F  \right)\to	{H}^\bullet\left(A, F \right),
\end{eqnarray}
where the notations \eqref{const_shef_coh_eqn} and \eqref{etale_hom_a_eqn} are  used. 
\end{lemma}
\begin{proof}
	The  given by the equation  \eqref{p_a_eqn} continuous map 
	$p_A: \mathfrak{Space}(A)\to \sX$ and the equation \eqref{cech_hom_eqn}  one has a homomorphism $\check{H}^\bullet \left(\sX, F  \right)\to	\check{H}^\bullet\left(\mathrm{Space}(A), F \right)$. The homomorphism \eqref{blowing_grothendieck_shc_eqn} follows from \eqref{chech_hom_eqn}. If $F\otimes_\Z F \cong F$ then from the 	Exercise   \ref{ring_homo_exer} we obtain a ring homomorphism. 
If $p_A:\mathscr \mathrm{Space}(A)\to\sX$ is local homeomorphism, and both $F_\sX$ and $F_{\mathscr X^A}$ are constant presheaves arising from $F$ then from the Proposition  \ref{spectral_sequence_prop} and/or the equation \eqref{hom_to_x_eqn} if follows that for all $r\ge 0$ there is a homomorphism
	\bean\label{x_seqc_eqn}
	\begin{split}
		{H}^r\left(\sX, \mathfrak{Ass}\left( F_\sX\right)  \right)\xrightarrow{H^r\left(p_A \right) } {H}^r\left(\mathrm{Space}(A), p^*_A\mathfrak{Ass}\left( F_\sX\right) \right) 
			\end{split}
	\eean
...
\bean\label{x_seqcu_eqn}
	p^*_A\mathfrak{Ass}\left( F_\sX\right)\cong 	\mathfrak{Ass}\left( F_{\mathrm{Space}(A)}\right)
\eean
...
$$
		{H}^r\left(\sX, \mathfrak{Ass}\left( F_\sX\right)  \right)\xrightarrow{H^r\left(p_A \right) } {H}^r\left(\mathrm{Space}(A), \mathfrak{Ass}\left(F_{\mathscr X^A}\right) \right). 
$$

\end{proof}


\section{Locales of  $C^*$-algebras of foliations}
\subsection{Preliminaries. Hereditary spaces}
\paragraph*{}
...
\end{proof}
\begin{remark}
From the  Theorem \ref{grot_type_I_thm} and Lemmas \ref{foli_t1_lem}, \ref{blowing_foli_lem} it follows that there is the Hereditary $C^*_r\left(M,\sF, \sigma\right)$-space $\mathfrak{Space}\left(C^*_r\left(M,\sF, \sigma\right) \right)$ (cf. Definition \ref{grothendieck_ca_t_defn}) and natural surjective local homeomorphism
\bean
\mathfrak{Space}\left(C^*_r\left(M,\sF, \sigma\right) \right)\to \mathrm{Sp\acute{e}}\left(\Om_M \right).
\eean
...
\end{remark}
\section{Hereditary locales versus Bohr locales}
\paragraph*{}
Hereditary locales are equivalent  Bohr ones in case of $C^*$-algebras of type $I_0$. However these locales are very different in general. If $\th \in \R\setminus\Q$ than one has a $C^*$- algebra $C\left(\T_\th\right)$ of noncommutative torus.  !!!!
\begin{appendices}
...
\begin{definition}\label{open_locale_mor_defn}
%Definition 1. 
...
\end{remark}
\begin{definition}\label{enoght_points_defn}\cite{topos:intro}
...
$p : 1 \to X$ such that $p^{-1}\left( U\right)\neq p^{-1}\left(V \right)$. Equivalently, $X$ has enough points if and only if, for any  $U, V \in  \mathcal O(X)$.
$$
\mathrm{pt}(U) = \mathrm{pt}(V) \quad \Rightarrow \quad U = V.
$$
\end{definition}

\begin{proposition}\cite{topos:intro}
For any locale X, the following are equivalent:
\begin{enumerate}
	\item[(i)] $X$ has enough points;
\item[(ii)] the counit $\mathrm{Loc}\mathrm{pt} X \to X$ is an isomorphism of locales;
...
\paragraph{!!!}
	...
	\begin{corollary}\label{cup_products_cor}
%Corollary 3.7.
 Cup products can be defined for the category of sheaves of modules on
any site $\left( \mathscr C, J\right)$. They are natural maps Hp(U; A)
Hq(U; B)!Hp+q(U; A
B) satisfying
...
\end{corollary}
	...
	\begin{remark}
	In \cite{milne:lec} and \cite{swan:cup} CUP PRODUCT.
	\end{remark}
