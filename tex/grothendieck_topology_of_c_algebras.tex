...
\paragraph{INSERT}
PAGE 95.
We now want to relate bundles over $\Z$ to sheaf cohomology. We fix a bundle
$p : \sX \to \sZ$ with fibre $F$ and structure group $G$, and choose an open cover $\left\{\sU_\iota\right\}$ of
$\sZ$ for which there are local trivializations $h_\iota: p^{-1}\left(\sU_\iota  \right)\to \sU_\iota F$ . For each pair $\iota, j$
