...
If the space $\sX$  is compact then for any sheaf $\mathscr{ A}$  of Abelian groups on $\sX$ one has:
\begin{enumerate}
...
\end{theorem}
\begin{proof}
	(i)	If $\left\{\sU_\iota\right\}_{\iota \in I}\subset \mathfrak{Closed}\left(\sX \right)$ then from the equation \eqref{top_adj_eqn} it follows that 
	$$
	 = \text{ a closure of } \bigcup \sU_\iota.
	$$
	...
	\forall x' \in 	\sU_x\cap \left|s_\iota \right|\quad s_\iota\left(x' \right) = a^x\left( x'\right) 
		$$ 
	\end{itemize}
		The set $\bigcup_{x \in \bigcup \sU_\iota} \sU_x$ is open and $\bigcup \sU_\iota\subset \bigcup_{x \in \bigcup \sU_\iota} \sU_x$. It follows that the closure $\overline{\sU}$ of  $\bigcup \sU_\iota$ is a subset of $\bigcup_{x \in \bigcup \sU_\iota} \sU_x$. On the other hand $\overline{\sU}$  is a closed subset of the compact set $\sX$, i.e.  $\overline{\sU}$ is compact.  There is a finite set $\left\{x_1, ..., x_n\right\}$ such that $\overline{\sU} \subset \bigcup_{j=1}^n \sU_{x_j}$. From the \ref{top_paracompact_lem} it follows that there is a family $\left\{\sV_1, ..., \sV_n\right\}$ of closed  subsets of $\sX$ such that $\sV_j \subset \sU_{x_j}$ and $\overline{\sU} \subset \bigcup_{j=1}^n \sV_{j}$. For any $j = 1,..., n$ there is $\iota \in I$ such that $\sV_j\cap \bigcup \sU_\iota \subset \left|s_\iota \right|$. We denote by $s_j$ a the restriction of $s_\iota$. From the Theorem \ref{closed_cov_thm} it follows that there is $t \in \mathscr{ A}\left(\bigcup \sU_\iota \right)$. Such that  $t|_{\sV_j}= s_j$. From the our construction it turns out that $t|_{\sU_\iota}= s_\iota$. From the Theorem \ref{sheaf_neigh_en_thm} it follows that there is an open neighborhood $\sV$ of $\bigcup \sU_\iota$ and a section $q \in \mathscr A\left(  \sV\right) $ such that $t = q|_{\bigcup \sU_\iota}$. Otherwise $\overline \sU \subset \sV$ because  $\bigcup \sU_\iota$ is dense in $\overline \sU$. We define the required section $s \in \mathscr A\left(  \overline \sU\right)$ as $s \bydef q|_{\overline \sU}$.
		\\
...
\end{theorem}
\begin{proof} (i)
	For any presheaf $\mathscr F \in \mathbf{PreSh}\left( \mathfrak{Closed}\left( \sX\right)\right)$ we define a precosheaf \\$^\neg\mathscr F \in \mathbf{PreCSh}\left( \mathfrak{Locale}\left( \sX\right)\right)$ such that
	$$
	\forall \sU \in \mathfrak{Closd}\left( \sX\right)\quad \neg\mathscr F\left(\mathfrak{Inv}\left(\sU \right)\right)   \bydef \mathscr F\left( \sU\right) 
	$$
...
%	Lemma 6.1.2. The categories Ind(C) and Pro(C) are U-categories.
	...
	\section{Topological sheaves and sheaves}
...
\begin{equation}\label{topcoeq cosh}
	\bigoplus_{\bt, \ga} \mathfrak{A}(\sU_\bt \cap \sU_\ga) \xrightarrow{g} \bigoplus_{i \in I}  \mathfrak{A}(\sU_i) \xrightarrow{f}  \mathfrak{A}(\sU)
\end{equation}
