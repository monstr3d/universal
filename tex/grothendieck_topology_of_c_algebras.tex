...
\begin{definition}\label{locale_point_defn}
	A \textit{point} of a locale $A$ is a frame homomorphism $A \to \left\{0, 1\right\}$.
\end{definition}
\begin{lemma}\label{topos_point_lem}
Let $p: A \to \left\{0,1\right\}$ be a point and $X = p^{-1}\left(\left\lbrace 1\right\rbrace \right)$.  there is no nontrivial decomposition $X = X'\vee X''$ such that $X'\wedge X'' =  0$.
\end{lemma}
\begin{proof}
	 If $p\left(X' \right) =0$ or $p\left(X'' \right) =0$ then $p\left(X \right)=  p\left(X' \right) \vee p\left(X'' \right)=\left\{0, 1\right\}$, so $X = X'\vee X'' \neq  p^{-1}\left(1 \right)$. It follows that  $p\left(X' \right)=  p\left(X'' \right) =1$ and $p\left(X' \right)\wedge  p\left(X'' \right)= 1$. On the other hand $p\left(X' \right)\wedge  p\left(X'' \right) = p\left(X'\wedge  X'' \right)= p\left(0\right) = 0$.
\end{proof}
