...
\paragraph{}
\begin{definition}
A $C^*$-algebra $A$ is said to be \textit{hereditary commutative} if following conditions hold.
\begin{enumerate}
	\item [(a)] A family
...
\end{definition}
If  $A$ is  a $C^*$-algebra generated by its hereditary commutative $C^*$-subalgebras then .
...
\mathscr C^A_B \bydef \left\{C \cap B| C \in \mathscr C^A\right\}
\ee	
is a covering of $B$. Suppose that there is an inclusion $C_0\left( \sX\right) \hookto M\left(A \right)$ into multiplier $C^*$-algebra  with  a net $\left\{u_\la \right\}_{\la \in \La}\subset C_0\left(\sX \right)$ such that
$$
\forall a \in A \quad \lim_{\la\in \La}\left\| a - u_\la a u_\la\right\|= 0 
$$
For any open $\sU \subset \sX$ denote by $A_\sU$
