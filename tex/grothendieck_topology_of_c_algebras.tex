...
\paragraph{}
Let $\sX$ be a locally compact, Hausdorff topological space, and let $A \bydef C_0\left( \sX\right)$ be a corresponding commutative $C^*$-algebra. Any hereditary $C^*$-subalgebra $B \subset A$ is an closed ideal which uniquely depends  on a closed subset
\be\label{top_xb_eqn}
\sX_B \bydef\left\{x \in \sX\quad |\quad \forall a \in B \quad a\left(x \right) = 0\right\}.
\ee
...
\emptyset \bydef 0,\\
\sX \bydef 1,\\
\bigvee \sU_\iota \bydef \quad \text{ a closure of } \quad \bigcup\sU_\iota,\\ 
\sV \wedge \sU\bydef \sV \cap \sU.
\end{split}
\ee
We leave to the reader proof of that the locale $\mathfrak{Adjont}\left( \sX\right)$ is naturally isomorphic to the Gelfand $C_0\left(  \sX\right)$-locale $\mathfrak{Locale}\left(C_0\left(  \sX\right) \right)$ (cf. Definition \ref{gelfand_a_locale_defn}).
Let $p^{-1}:	\mathfrak{Frame}\left(C_0\left(  \sX\right) \right)\to \left\{0, 1\right\}$
be a point of  the Gelfand $A$-frame	$\mathfrak{Frame}\left(A \right)$  (cf. Definition \ref{locale_point_defn}). If $K \bydef
...
$$ 
(cf. \eqref{p_p_eqn}) then $P$ is a hereditary subalgebra of $C_0\left(  \sX\right)$. From the equation \eqref{pprime_eqn} it follows that $P \neq A$. If $\sX_P$ is given by \eqref{top_xb_eqn} then from $P \neq A$ it follows that $\sX_P\neq \sX$. If  $x', x'' \in\sX\setminus  \sX_P$ and $\sU = \sX_P\cup \{x'\}$, $~\sV = \sX_P\cup \{x''\}$ then
\bean
\sX_p = \sU \cap \sV,\\
\sU \not\subset \sX_p,\quad 
\sV \not\subset \sX_p,
\eean
i.e, there is a contradiction with \eqref{prime_eqn}. It turns out that there is $x \in  \in\sX$ such that $\sX_P= \sX\setminus \{x\}$. If $x$ has no one-point neighborhoods then the set $\sX_P= \sX\setminus \{x\}$ is not closed. So if $\sX$ has no points with  one-point neighborhoods then the locale $\mathfrak{Adjont}\left( \sX\right)\cong \mathfrak{Locale}\left( C_0\left(  \sX\right)\right)$ has no points.
\begin{theorem}\label{comm_sh_thm}
If $\sX$ is a Hausdorff, locally compact space then there is a faithful functor  (cf. Definition \ref{funct_full_faithfull_defn})
$$
F_X : \mathbf{Ab}\left(\sX  \right) \hookto \mathbf{Ab}\left(\mathfrak{Locale} \left( C_0\left(  \sX\right)\right)  \right).
$$
between categories of sheaves of Abelian groups (cf. Remark \ref{ab_uss_rem}).		
\end{theorem}
...
B' \wedge B'' = P \quad \Leftrightarrow \quad  \pi\left(P \right) \H =  \pi\left(B' \right) \H+ \pi\left(B'' \right) \H,
 ...
 \end{cases}
 \ee 
  From the equation i\eqref{k_eno_eqn} it follows that 
 $$
 \forall B \in \mathfrak{Frame}\left( \K\right) \quad \forall p \in  \mathrm{pt}(\K) \quad \dim \pi\left(B \right)\H > 1 \quad  p\left(B \right) = 1
 $$
 (cf. equation \eqref{point_set_eqn}). If  $\dim \H> 2$ then there is an infinite set of different $B \in \mathfrak{Locale}\left( \K\right)$ such that $\dim \pi\left(B \right)\H > 1\quad \Rightarrow\quad \mathrm{pt}(B)=1$. From this fact it follows that $\mathfrak{Locale}\left( \K\right)$ is not spatial (cf. Definition \ref{spatial_locale_defn}).
 \end{proof}
\begin{remark}
...
\section{Adjoint functors}\label{adjoint_functor_sec}
\paragraph*{}
...
\begin{empt}\cite{bredon:sheaf}					
 For any sheaf $\sF$ of Abelian groups on $\sX$ and open set $\sU\subset \sX$  we let $C^0(\left(\sU, \sF \right)$ be the
	...
	\be
	C^0(\left(\sU, \sF \right)\bydef \prod_{x \in \sX} \sF_x
	\ee
	!!! WHERE $F_x$ !!!
	Under point-wise operations, this is an Abelian group, and the functor $\sU \mapsto 	C^0(\left(\sU, \sF \right)$, $\sX$. Indeed it is a sheaf (cf. \cite{bredon:sheaf}) which will be denoted by $\mathscr C^0(\left(\sU, \sF \right)$. Note that if $\sX_d$ denotes the point set of $\sX$ 
	with the discrete topology and if $\phi : \sX_d \to \sX$ is the canonical map, then $\mathscr C^0(\left(\sU, \sF \right)\cong f_*f^* \sF$.
	Inclusion of the collection of sections of $\sF$ in the collection of all serrations
	gives an inclusion $\sF\left(\sU \right)\mapsto   C^0(\left(\sU, \sF \right)= \mathscr C^0(\left(\sU, \sF \right)\left(\sU \right)$  and hence
	provides a natural monomorphism
	\be\label{sheaf_can_res}
	\eps : \sF \hookto \mathscr C^0\left(\sX, \sF \right)
	\ee
	(For $\phi : \sX_d \to \sX$ as above, this inclusion coincides with the monomorphism
	$\bt :\sF \hookto f_*f^*\sF$). 
	Let $\mathscr{Z}^{1}\left(\sX, \sF \right) \bydef \coker\left\{\sF \hookto \mathscr C^0\left(\sX, \sF \right)\right\}$. so that the sequence
	$$
	\{0\}\to \sF \to \mathscr{C}^{0}\left(\sX, \sF \right) \to \mathscr{Z}^{1}\left(\sX, \sF \right)\to {0}
	$$
	is exact. We also define, inductively,
\bean
	\mathscr{C}^{n}\left(\sX, \sF \right) \bydef \mathscr{C}^{0}\left( \mathscr{Z}^{n}\left(\sX, \sF \right)\right),\\
	\mathscr{Z}^{n+1}\left(\sX, \sF \right) \bydef \mathscr{Z}^{1}\left( \mathscr{Z}^{n}\left(\sX, \sF \right)\right)  
\eean
	so that
\bean
		\mathscr{Z}^{n}\left(\sX, \sF \right) \xrightarrow{\eps} \mathscr{C}^{n}\left(\sX, \sF \right)\xrightarrow{\partial}
	\mathscr{Z}^{n+1}\left(\sX, \sF \right)
\eean

	is exact. Let $d \bydef \eps \circ \partial$ be the composition
	$$
	\mathscr{C}^{n}\left(\sX, \sF \right) \xrightarrow{\partial }  \mathscr{Z}^{n + 1}\left(\sX, \sF \right)\xrightarrow{\eps}\mathscr{C}^{n+1}\left(\sX, \sF \right) 
$$

	Then the sequence 
	\be
0 \to \sF \xrightarrow{\eps} 	\mathscr{C}^{0}\left(\sX, \sF \right) \xrightarrow{d } \mathscr{C}^{1}\left(\sX, \sF \right)\xrightarrow{d } \mathscr{C}^{2}\left(\sX, \sF \right)\xrightarrow{d}\dots
	\ee
	is exact. That is, $\mathscr{C}^{\bullet}\left(\sX, \sF \right)$ is a resolution of $\sF$. It is called the \textit{canonical
	resolution} of $\sF$.
\end{empt}
...
Prove that a given by \eqref{cech_hom_eqn} map $\check{H}^\bullet\left(f\right):\check{H}^\bullet\left(\sY, F \right)\to \check{H}^\bullet\left(\sX, F \right)$ is a homomorphism of rings.
	...
	\begin{corollary}
	%Corollary. 
	For $\Phi$ paracompactifying and sheaves $\sF$ on $\sX$ there as
	a natural isomorphism
	$$
\check{H}_\Phi^\bullet\left(\sX, \sF \right) \cong {H}_\Phi^\bullet\left(\sX, \sF \right) 
	$$
	\end{corollary}
