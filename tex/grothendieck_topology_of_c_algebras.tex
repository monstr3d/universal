...
\begin{proof}
	...
	From $p_0\circ j = \Id_\sX$ if follows that $j^*p_0^*\mathfrak{Ass}\left( F_\sX\right)\cong \mathfrak{Ass}\left( F_\sX\right)$ and the composition $H^r\left(j \right)\circ H^r\left(p_0 \right)$ can be regarded as 
...
\begin{exercise}\label{dir_image_exer}
	...
	\mathfrak{Ass}\left( F_\sY\right)  = f_*\mathfrak{Ass}\left( F_\sX\right).
	\eean
	 where 
	$\mathfrak{Ass}$  means the associated sheaf functor (cf. Definition \ref{site_sheaf_ass_defn}) and 	
	$f_*$ means the direct image (cf. Definition \ref{sheaf_inv_im_defn}). Moreover if $f$ ia a local homeomorphism then there is a natural isomorphism
	\be\label{inv_image_eqn}
\mathfrak{Ass}\left( F_\sY\right)  = f^*\mathfrak{Ass}\left( F_\sX\right).
\ee
