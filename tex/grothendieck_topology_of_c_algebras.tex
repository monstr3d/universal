...
A $C^*$-algebra $A$ is said to be \textit{hereditary commutative generated} if a family
	\be\label{com_cov_eqn}
	\mathscr C^A \bydef \left\{C\text{ is a  commutative hereditary } C^*\text{-subalgebra of } A \right\}
	\ee	
	is a covering of $A$ (cf. Definition \ref{hered_cov_defn}).
...
\be\label{xa_eqn}
\mathscr X^A \quad \bydef\quad  \text{the maximal open subset of }\quad \mathrm{Sp\acute{e}}\left(\mathscr P^A \right)\setminus \mathscr U'_{C^0}
\ee
and
...
$$
(cf. Exercise \ref{sheaf_etale_exer}). If $s\left(x \right)$ is known for every $x \in p'\circ \left(\mathscr X^A\right)$ such that  then  $s\left(x \right)= C^0_x$ for all $x \in p'\circ s\left( \mathscr U'_{C^0}\right)$. It follows that the restriction $s|_{p'\left( \mathscr X^A\cup\mathscr U'_{C^0}\right)  }$ known, taking into account that $ \mathscr X^A\cup\mathscr U'_{C^0}$ is dense in $\mathrm{Sp\acute{e}}\left(\mathscr P^A \right)$ one has the unique section $s : \sX \to \mathrm{Sp\acute{e}}\left(\mathscr P^A \right)$. Thus any commutative hereditary algebra can be uniquely defined by the open set $\mathscr U_C$. If $B$ is a hereditary $C^*$-subalgebra of $B$ and
$$
...
\end{exercise}
\begin{definition}
	For any commutative hereditary  $C^*$-subalgebra $C \subset A$ an intersection of all subsets $\sU$ such that $C\subset ~_\sU A _\sU$ is said to be the \textit{support} of $C$. We write $\sU = \supp C$. A set of all supports is said to be a \textit{family of supports} $\mathfrak{Supp}\left( A\right) $. For any $\mathscr K \in\mathfrak{Supp}\left( A\right) $ denote by
	\be
	\mathfrak{Supp}\left( A\right)_{\mathscr K} = \left\{C \in \mathscr C^A|  \supp C = \mathscr K \right\}
	\ee
\end{definition}

Let us define a sheaf on $\mathscr S$ on $\sX$ 
\bean
\mathscr S\left(\sU \right)  \bydef  \text{the family of all supports of commutative $C^*$-algebras of } _\sU A _\sU;\\
...
\be
\mathfrak X\left(A \right)  \bydef ~ \text{the  maximal open subset of }~ \mathrm{Sp\acute{e}}\left(\mathscr S  \right)\setminus\mathrm{Sp\acute{e}}\left(\mathscr S  \right)_\emptyset.
\ee

If  $\mathscr{P}^{ A}$ be a presheaf given by \eqref{comm_alg_sheaf} then there  is a surjective morphism of sheaves (cf. Definition \ref{sheaf_defn})
\bean
...
$$
\begin{exercise}
Prove that if $\mathscr X^A$ is given by the Lemma \ref{grot_com_lem}  Grothendieck $A$-space. (cf. Definition \ref{grothendieck_ca_t_defn}) then $P'_A$ maps $\mathscr X^A$ onto $\mathfrak X\left(A \right)$. Moreover the natural surjective map
\be
P_A: \mathscr X^A\to \mathfrak X\left(A \right).
\ee
is a local homeomorphism.
\end{exercise}
\begin{definition}\label{local_prod_defn}
!!!	Let $A, A', A''$ be We say that $A$ is a \textit{local product} of $A'$ and $A''$ if 
	\begin{enumerate}
		\item[(a)] $\mathfrak{Supp}\left( A\right)=\mathfrak{Supp}\left( A'\right)=\mathfrak{Supp}\left( A''\right)$
		\item[(b)] For all $x\in \sX$ there is an open neighborhood $\sU$ such that for all $\mathscr K\in \mathfrak{Supp}\left( A\right)$ such that $\mathscr K \subset \sU$ there is a bijective map
	\be\label{local_prod_eqn}
\varphi_{\mathscr K} :		\mathfrak{Supp}\left( A'\right)_{\mathscr K}	\times \mathfrak{Supp}\left( A''\right)_{\mathscr K}\cong \mathfrak{Supp}\left( A\right)_{\mathscr K}.
	\ee
	\end{enumerate}
\end{definition}
...
$$
\mathfrak{Grot}\left(A' \right)\times_{\mathfrak X\left(A \right)}\mathfrak{Grot}\left(A'' \right)\cong \mathfrak{Grot}\left(A \right) \cong 
$$
\end{theorem}
\begin{proof}
	Any element of 
$\mathfrak{Grot}\left(A' \right)\times_{\mathfrak X\left(A \right)}\mathfrak{Grot}\left(A'' \right)$ !!!!	
	
Any  element of $\mathfrak{Grot}\left(A \right)= \mathscr X^A$ is  a stalk $C_x$ of a hereditary commutative $C^*$-algebra $C$. If  $\sU$ is the given by the condition (b) of the Definition \ref{local_prod_defn} then $C_x$ can be represented by $C|_\sU = C\cap ~_\sU A _\sU$. If $\mathscr K= \supp C|_\sU$ then there are unique  $C'\in 	\mathscr C^{A'}$ and $C''\in 	\mathscr C^{A''}$ such that $\supp C' = \supp C'' = \mathscr K$ and $C|_\sU = \varphi_{\mathscr K} \left(C', C'' \right)$. However a pair $\left(C', C''\right)$ yields an element of $\mathscr X^{A'} \times_{\mathfrak X\left(A \right)}	 \mathscr X^{A''} =  \mathfrak{Supp}\left( A'\right)_{\mathscr K}	\times \mathfrak{Supp}\left( A''\right)_{\mathscr K}$
$$
\mathscr X^A \cong  \mathscr X^{A'} \times_{\mathfrak X\left(A \right)}	 \mathscr X^{A''}
$$
