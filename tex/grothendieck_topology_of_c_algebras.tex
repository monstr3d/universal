...
\end{tikzcd}
	...
	\begin{itemize}
		...
		\item[(b)] for any nontrivial hereditary $C^*$-subalgebra $D\subset A$ there is a nontrivial hereditary $C^*$-subalgebra $D'\subset D$ such that a set
		$
		\left\{\iota \in I| D' \subsetneqq B_\iota \right\}$ is finite.
	\end{itemize}
 ...
 Recall that morphism any $B'\to B''$ is an inclusion $B'' \subset B'$. 
	...
	$$
	it follows that the family of of generated by  $B_\iota\cup C$ hereditary subalgebras satisfies to the condition (a) of the Definition \ref{pullback_covering_ca_defn}. The condition (b) follows from the equation
	$$
D' \subset B_\iota \quad \Rightarrow \quad 	D' \subset B_\iota\cup C.
	$$
	(b) If $B = \bigcap_{\iota\in I} B_\iota$ and $B_\iota =  \bigcap_{j\in J_\iota}B_{\iota,j}$ for all $\iota \in I$ then $B = \bigcap_{\iota, j} B_{\iota,j}$, so the condition (a) of the Definition \ref{pullback_covering_ca_defn} holds. For any hereditary $C^*$-subalgebra $D$ there is a nontrivial hereditary $C^*$-subalgebra $D'\subset D$ such that a set
	$$
		\left\{\iota \in I| D' \subsetneqq B_\iota \right\} = \left\{\iota_1, ...,\iota_n\right\}
	$$
	is finite. There is a nontrivial  $C^*$-subalgebra $D_1\subset D'$ such that the set
	$
\left\{j \in J_{\iota_1} | D_1 \subsetneqq C_{1 j}\right\}	
	$ is finite. Similarly there is   a nontrivial  $C^*$-subalgebra $D_2\subset D_1$  such that the set $\left\{j \in J_{\iota_2} | D_2 \subsetneqq C_{2 j}\right\}$ is finite. Using the induction we have a nontrivial $C^*$- subalgebra $D_n$ which satisfies to the condition (b) of the Definition \ref{pullback_covering_ca_defn}.\\
	(c) Evident.
	\end{proof}
...
\paragraph{}
Let $\sX$ be a locally compact, Hausdorff topological space, and let $ C_0\left( \sX\right)$ be a corresponding commutative $C^*$-algebra (cf. Theorem \ref{gelfand-naimark_thm}). Any hereditary $C^*$-subalgebra $B \subset A$ is an closed ideal which uniquely depends  an closed subset
\be\label{top_xb_eqn}
