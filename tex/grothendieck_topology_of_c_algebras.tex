...
continuous-trace algebra  with DixmierÐDouady class $\delta \in \check{H}^3\left(\sX,\Z \right)$ (cf. Notation \ref{ctr_not}). Suppose that the space $\sX$ is compact. Let $U \bydef \Aut_{C_0\left(\sX \right) }\left(A\right)$ be a group of automorphisms.  For any  hereditary $C^*$-subalgebra $B \subset A$ denote by  $L\left( B\right)$ is a given by the equation  \eqref{left_ideal_eqn} left ideal of $A$. Let 
