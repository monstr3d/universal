...
continuous-trace algebra  with DixmierÐDouady class $\delta \in \check{H}^3\left(\sX,\Z \right)$ (cf. Notation \ref{ctr_not}). Suppose that the space $\sX$ is compact. Let $U \subset M\left(A \right)$ be a group of unitary elements.  For any  hereditary $C^*$-subalgebra $B \subset A$ denote by  $L\left( B\right)$ is a given by the equation  \eqref{left_ideal_eqn} left ideal of $A$. Let $L^\perp\left( B\right)$ is the $C^*$-norm completion of the union of left ideals $L \subset A$ such that $L \cap  L\left( B\right)= \{0\}$. For any hereditary $C^*$-subalgebra $B \subset A$ there is a subgroup
\bean
...
\be\label{ctr_ub_eqn}
U\left(B'\right) \to U\left(B'' \right). 
\ee
When  the map $B \mapsto  U\left( B \right)$ is a presheaf of groups on a category $\mathbf{Hered}/A$ (cf. Definitions \ref{hered_cat_defn} and  \ref{presheaf_defn}).
\begin{lemma}
The  map $B \mapsto  U\left( B \right)$ is a sheaf with respect to the Gelfand $A$-topology (cf. Definition \ref{gelfand_a_site_defn}).
\end{lemma}
\begin{proof}
	Firstly we prove that the presheaf is separated (cf. Definition \ref{sheaf_defn}).
If  $B \in \mathbf{Hered}/A$ and $\left\{B_\iota \to B\right\}_{\iota= 1,...,n}$ is a covering then from the Definition \ref{pullback_covering_ca_defn} it follows that $B = \bigcap_{\iota=1}^n B_\iota$. Element $u \in U\left( B \right)$  is trivial (i.e. equals to unity) if and only if for any $\iota= 1,...,n$ the image of $u$ in $ U\left( B_\iota \right)$ is trivial. 


Suppose that  for any pair $\left(\iota, j\right) \in \left\{1, ..., n\right\}^2$ a hereditary $C^*$-subalgebra $B_{\iota j}\subset A$ is generated by  a union $B_\iota \cup B_j$. For each pair there are natural homomorphisms $\phi_{\iota j}: U\left( B_\iota\right)  \to U\left(  B_{\iota j}\right) $ and $\phi_{j \iota }: U\left(  B_j \right) \to U\left( B_{\iota j}\right) $.  If a family  $\left\{u_\iota \in U\left( B_\iota \right)\right\}_{\iota = 1,...,n}$ is such that 
$$
B_{\iota j} \neq A \quad \Rightarrow \quad \phi_{\iota j} \left(u_\iota\right)= \phi_{j\iota } \left(u_j\right).
$$

and both $\widetilde u_\iota, \widetilde u_j \in U$ are representatives of $u_\iota$ and $u_j$ respectively then one has
$$
\widetilde u_\iota = \widetilde u_j \text{ modulo } U_{B_{\iota j}}
$$
(cf. equation \eqref{ctr_ub_eqn})
i.e. both $\widetilde u_\iota$ and $\widetilde u_j$ represent the same element of $U\left(B_{\iota j} \right)$. 
%and  for any pair $\left(\iota, j\right) \in \left\{1, ..., n\right\}^2$ a hereditary $C*$-subalgebra $B_{\iota j}\subset A$ is generated by  a union $B_\iota \cup B_j$.

such that a union $B_\iota \cup B_j$ does not generate $A$ 
