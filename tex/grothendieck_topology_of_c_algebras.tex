...
If $A$ is a connected  $C^*$-algebra then there is a category $\mathbf{Hered}/A$ such that:
\begin{itemize}
	\item $\mathbf{Hered}/A$-objects are hereditary $C^*$-subalgebras (cf. Definition \ref{hered_defn}) of $A$,
...
\begin{proof}
	...
	If we consider a finite diagram (cf. Section \ref{limit_sec}) with $\mathbf{Hered}/A$-objects $\left\{B_1, ..., B_n\right\}$ then the limit (cf Definition \ref{lim_defn}) of it is a generated by a union $B_1 \cup ... \cup B_n$ hereditary $C^*$-subalgebra of $A$.
\end{proof}
\begin{defn}\label{ultra_filter_ca_defn}
Let $A$ be a $C^*$-algebra. An indexed by a directed set $\La$ family $\left\{D_\la\right\}_{\la \in \La}$ of hereditary $C^*$-subalgebras of $A$ is said to be an $A$-\textit{filter} if one has:
\begin{enumerate}
	\item [(a)] $D_\la$ is not trivial,
	\item[(b)] any $\mathbf{Hered}/A$-object $D$ following condition holds
	$$
	\exists D_\la \in \left\{D_\la\right\} \quad D_\a \subset D \quad \Rightarrow \quad D \in \left\{D_\la\right\},
	$$ 
	\item[(c)]
	$
\forall \la', \la'' \in \La \quad D_{\la'}\cap D_{\la''}\in \left\{D_\la\right\}.
	$
	A maximal $A$-filter is an $A$-\textit{ultrafilter}, i.e. if  ultrafilter is a subset of another filter then both filters coincide.
\end{enumerate}
\end{defn}
...
\end{tikzcd}
	...
	For any $\mathbf{Hered}/A$-object $B$ a  finite family of morphisms $\left\{B_\iota \right\}_{\iota\in I}~$, called a \textit{covering} of $B$ if one has
	\begin{itemize}
		\item[(a)] $\bigcap_{\iota\in I} B_\iota = B$,
		\item[(b)] For any ultrafilter  $\left\{D_\la\right\}_{\la \in \La}$ (cf. Definition \ref{ultra_filter_ca_defn}) there is $\la_0 \in \La$ such that a set
		$\left\{\iota \in I| D_{\la_0} \subsetneqq B_\iota \right\}$ is finite.
	\end{itemize}
	
	
 Recall that morphism any $B'\to B''$ is an inclusion $B'' \subset B'$. 
	\end{definition} 
	...
	(a) If $B$ and $C$ are $\mathbf{Hered}/A$-objects such that there is a morphism $C \to B$ then $B \subset C$. Moreover $B\times_{\{0\}} C= C$. If $\left\{B_\iota \to B\right\}_{\iota\in I}$ is a covering of $B$  then $B = \bigcap_{\iota\in I} B_\iota$. Moreover $B\times_{\{0\}} C= C$. 
	From 	
	$$
	C = 	B \cup C = \left( \bigcap_{\iota\in I} B_\iota \right) \cup C = \bigcap_{\iota\in I}\left( B_\iota\cup C\right) 
	$$
	it follows that the family of of generated by  $B_\iota\cup C$ hereditary subalgebras satisfies to the condition (a) of the Definition \ref{pullback_covering_ca_defn}. 
	
	
	The condition (b) follows from the equation
	$$
D_{\la_0} \not\subset B_\iota \cup C\quad \Rightarrow \quad 	D_{\la_0} \not\subset B_\iota.
	$$
	(b) If $B = \bigcap_{\iota\in I} B_\iota$ and $B_\iota =  \bigcap_{j\in J_\iota}B_{\iota,j}$ for all $\iota \in I$ then $B = \bigcap_{\iota, j} B_{\iota,j}$, so the condition (a) of the Definition \ref{pullback_covering_ca_defn} holds. 
	If  $\left\{D_\la\right\}_{\la \in \La}$ is an ultrafilter then there is $\la_0 \in \La$ such that the set 
	$$
\left\{\iota \in I| D_{\la_0} \subsetneqq B_\iota \right\}	= \left\{\iota_1, ..., \iota_n\right\}
	$$
	If finite. Otherwise for any $j \in \{1,...,n\}$ there is $\la_j \in \La$ such that a set
	$$
\left\{\iota \in I| D_{\la_j} \subsetneqq B_{\iota, j }\right\}	
	$$
	is finite. If $\mu \ge \la_{j}$ for any $j=1,..., n$ then a set
$$
\left\{B_{\iota, j}| D_{\mu} \subsetneqq B_{\iota, j }\right\}
$$
	is finite.\\
	(c) Evident.
	\end{proof}
...
\end{notation}
\begin{definition}\label{conn_comp_defn}  !!! MAY BE DELETE !!!
	If $\mathscr U\bydef \left\{B_1,..., B_n\right\}$ is a covering of $B$, then there is a minimal equivalence relation $\sim$ on $\mathscr U$ such that
	$$
	B_{\iota'}\cap B_{\iota''}\neq \{0\}\quad \Rightarrow 	B_{\iota'}\sim B_{\iota''}
	$$
The equivalence classes are said to be \textit{connected components} of $\mathscr U$.
\end{definition}
HERE \ref{constant_sheaf_lem} !!!
...
\paragraph{}
If both be $A$ and $\widetilde{A}$ be $C^*$-algebras then  an injective  ring homomorphism of $\C$-algebras
\bean
\varphi_R: A \hookto \mathbf{RC}\left(\widetilde A\right)
\eean
(where $\mathbf{RC}\left(\widetilde A\right)$ is a space of right centralizers cf. Definition \ref{lrc_defn}) then there is a functor
\be\label{geom_functor_eqn}
\begin{split}
	 \varphi : \mathbf{Hered}/A \to \mathbf{Hered}/\widetilde A ,\\
...
\begin{theorem}\label{geometric_morphism_thm}
	If for any nonzero $\widetilde a \in \widetilde{A}$ one has 
	\be\label{non_deg_eqn}
\widetilde a	\varphi_R \left( A\right) \neq \{0\}
	\ee
then the functor \eqref{geom_functor_eqn} yields a  morphism of sites 
...
\begin{enumerate}
	\item[(i)] $\varphi$ preserves finite limits,
	\item[(ii)] $\varphi$ preserves coverings.
\end{enumerate}
(i) From the proof of the Lemma \ref{site_lim_lem} it follows that if we consider a finite diagram (cf. Section \ref{limit_sec}) with $\mathbf{Hered}/A$-objects $\left\{B_1, ..., B_n\right\}$ then the limit (cf Definition \ref{lim_defn}) of it is a generated by a union $B_1 \cup ... \cup B_n$ hereditary $C^*$-subalgebra $B \subset A$. On the other hand $\varphi\left( B\right)$ is a generated by a union $\varphi\left( B_1\right)  \cup ... \cup \varphi\left( B_n\right)$ hereditary $C^*$-subalgebra of $\widetilde A$.\\
(ii) From
$
\bigcap \varphi\left(B_\iota  \right) = \varphi\left( \bigcap B_\iota\right). 
$  it follows that the family $\left\{\varpi \left( B_\iota\right) \right\}$ satisfies to the condition (a) of the Definition \ref{pullback_covering_ca_defn}.

If $\left\{\widetilde A_\la\right\}_{\la \in \La}$ is an ultrafilter (cf. Definition \ref{ultra_filter_ca_defn}) and if for any $\la \in \La$ one has
$$
A_\la \bydef \bigcap \left\{A \in \mathbf{Hered}/A | \widetilde A_\la\subset \varphi\left(A \right) \right\}.
$$
then a family $\left\{ A_\la\right\}_{\la \in \La}$ is an ultrafilter. There is $\la_0\in \La$ such that a set
$
\left\{\iota \in I |  A_{\la_0} \not\subset B_\iota \right\}
$ is finite. From $\widetilde A_\la \subset \varphi \left(A_\a \right)$ it follows that 
$$
\left\{\iota \in I | \widetilde A_{\la_0} \not\subset \varphi\left(  B_\iota\right)  \right\}\subset \left\{\iota \in I |  A_{\la_0} \not\subset B_\iota \right\}
$$ 
so the set $\left\{\iota \in I | \widetilde A_{\la_0} \not\subset \varphi\left(  B_\iota\right)  \right\}$ is finite, i.e. the family $\left\{\varphi\left(B_\iota  \right)\right\}$ satisfies to the condition (b) of the Definition \ref{pullback_covering_ca_defn}.

\end{proof}
\begin{empt}
		Let both $\left(\mathbf{C}, J\right)$ and $\left(\mathbf{D}, K\right)$  are two sites, and suppose that $\phi:\mathbf{C}\to \mathbf{D}$ satisfies to the Definition \ref{morphism_of_sites_defn}. From the Theorem \ref{morphism_of_sites_thm} it follows that there is a geometric morphism  $\mathbf{Sh}\left(\mathbf{D}, K\right)\xrightarrow{f} \mathbf{Sh}\left(\mathbf{C}, J\right)$. A following lemma is an analog of (i) of the Proposition \ref{spectral_sequence_prop}
		\end{empt}
...
\begin{definition}\label{connected_grot_defn}
	A hereditary $C^*$-subalgebra $B \subset A$ is said to be \textit{coconnected} if there are no hereditary $C^*$-subalgebras $B'\subsetneqq A $ and $B''\subset$ such that  $B = B' \cap B''$ and $A$ is generated by $B'\cup B''$ hereditary $C^*$-subalgebra of $A$. If $B, B_1, B_2 \subset A$ are hereditary $C^*$-subalgebras such that $B'$ is coconnected, $B = B' \cap B''$ and $A$  is generated by $B'\cup B''$ hereditary $C^*$-subalgebra of $A$ then we say that $B'$ is a \textit{coconnected component} of $B$.
\end{definition}
\begin{exercise}\label{connected_grot_exer}
	Let $B \subset A$ be a hereditary $C^*$-subalgebra, and let $\mathscr U_B = \left\{B_\a\right\}$. Let $F$ be a set, and let $\mathscr F \bydef F_{\mathfrak{Site}\left( A\right)}$ be a constant sheaf (cf. Definition \ref{constant_presheaf_defn}). Prove that $\mathscr F\left(B \right)$ is a set of maps $\phi: \mathscr U \to  F$ such that an image $\im ~\phi = \phi\left(\mathscr U \right)$ is finite. 
\end{exercise}
\begin{empt}
Let $F$ be a set let both $F_{\mathfrak{Site}\left( A \right)}\in \mathbf{Sh}\left(\mathfrak{Site}\left( A \right) \right) $ and $F_{\mathfrak{Site}\left(\widetilde A \right)}\in \mathbf{Sh}\left( \mathfrak{Site}\left(\widetilde A \right)\right) $ are constant sheaves (cf. Definition \ref{constant_presheaf_eqn}. If $\mathbf{Sh}\left(\mathfrak{Site}\left(\widetilde A \right) \right)\xrightarrow{f} \mathbf{Sh}\left(\mathfrak{Site}\left( A \right) \right)$ is a given by \eqref{geometric_morphism_eqn} geometric morphism (cf. Definition \ref{geometric_morphism_defn}) such that both $f^*$ and $f_*$ are its direct and inverse image then $f^*$ is left adjoint to $f_*$, i.e. for any $\mathscr  F\in \mathbf{Sh}\left(\mathfrak{Site}\left( A \right) \right)$ and $\widetilde {\mathscr  G} \in \mathbf{Sh}\left(\mathfrak{Site}\left(\widetilde A \right) \right)$ one has
\be\label{geom_adj_eqn}
\Hom_{\mathfrak{Site}\left(\widetilde A \right)}\left( f^*\mathscr  F, \widetilde {\mathscr  G}\right) \cong \Hom_{\mathfrak{Site}\left(A \right)}\left( \mathscr  F, f_* \widetilde {\mathscr  G}\right).
\ee
In particular 
$$
	\Hom_{\mathfrak{Site}\left(\widetilde A \right)}\left( f^* F_{\mathfrak{Site}\left( A \right)}, \widetilde {\mathscr  G}\right) \cong \Hom_{\mathfrak{Site}\left(A \right)}\left( F_{\mathfrak{Site}\left( A \right)}, f_*\widetilde {\mathscr  G}\right) 
$$
If $B \in \mathbf{Hered}/A$ be coconnected then $F_{\mathfrak{Site}\left( A \right)}\cong F$. If $\widetilde B \bydef \varphi\left(  B\right) $ (cf. \eqref{geom_functor_eqn}) then 
 $f_*\widetilde {\mathscr  G}\left(B \right) =  \widetilde {\mathscr  G}\left(\widetilde B\right)$ (cf. \eqref{geom_dir_im_eqn}).  $\left\{ \widetilde B_\a\right\}$ is a family of coconnected components of $\widetilde B$ then $\widetilde {\mathscr  G}\left(\widetilde B\right)= \prod_\a \widetilde {\mathscr  G}\left( \widetilde B_\a\right)$. So 
 $
 \Hom_{\mathfrak{Site}\left(A \right)}\left( F_{\mathfrak{Site}\left( A \right)}, f_*\widetilde {\mathscr  G}\right)\left(B \right) 
 $ if a set of maps $F \to \prod_\a \widetilde {\mathscr  G}\left( \widetilde B_\a\right)$. Any map $F \to \prod_\a \widetilde {\mathscr  G}\left( \widetilde B_\a\right)$ yields a family of maps $\left\{F \to \widetilde {\mathscr  G}\left( \widetilde B_\a\right)\right\}$ and vice versa. On the other hand for any coconnected $\widetilde B' \in \mathbf{Hered}/\widetilde{A}$  the set $\Hom_{\mathfrak{Site}\left(\widetilde A \right)}\left( F_{\mathfrak{Site}\left( \widetilde A \right)},\widetilde {\mathscr  G}\right)\left( \widetilde{B}'\right)$ is a family of maps $F \to \widetilde {\mathscr  G}\left( \widetilde{B}'\right)$. It follows that any element of $\Hom_{\mathfrak{Site}\left(\widetilde A \right)}\left( F_{\mathfrak{Site}\left( \widetilde A \right)},\widetilde {\mathscr  G}\right)\left( \widetilde{B}'\right)$ uniquely depends on a family of maps $\left\{F \to \widetilde {\mathscr  G}\left( \widetilde B_\a\right)\right\}$. One has a following bijective map
 $$
  	\Hom_{\mathfrak{Site}\left(\widetilde A \right)}\left( F_{\mathfrak{Site}\left(\widetilde A \right)}, \widetilde {\mathscr  G}\right) \cong \Hom_{\mathfrak{Site}\left(A \right)}\left( F_{\mathfrak{Site}\left( A \right)}, f_*\widetilde {\mathscr  G}\right) 
 $$
 Taking into account \eqref{geom_adj_eqn} one has
\be\label{constant_sheaf_morphism_eqn}
 f^* F_{\mathfrak{Site}\left( A \right)} =  F_{\mathfrak{Site}\left( \widetilde A \right)}.
\ee
...
\paragraph{}
Let $\sX$ be a locally compact, Hausdorff topological space, and let $ C_0\left( \sX\right)$ be a corresponding commutative $C^*$-algebra (cf. Theorem \ref{gelfand-naimark_thm}). Any hereditary $C^*$-subalgebra $B \subset C_0\left( \sX\right)$ is an closed ideal which uniquely depends  an closed subset
\be\label{top_xb_eqn}
\sX_B \bydef\left\{x \in \sX\quad |\quad \exists a \in B \quad a\left(x \right)= 0\right\}.
\ee
It follows that the given by the Definition \ref{hered_cat_defn} category $\mathbf{Hered}/C_0\left(\sX \right)$ is equivalent to the category $\mathfrak{Closed}\left( \sX\right)$ such that:
\begin{enumerate}
...
\end{tikzpicture}
\end{enumerate}
\begin{lemma}\label{top_cov_lem}
If  $B \subset C_0\left( \sX\right)$ is a  hereditary $C^*$-subalgebra any covering of $B$ (cf. Definition \ref{pullback_covering_ca_defn}) corresponds 	to a locally finite  covering of $\sX_B$ (cf. \eqref{top_xb_eqn}) by closed subsets of $\sX$.
\end{lemma}
\begin{proof}
If $\left\{B_\iota\right\}_{\iota \in I}$ is a covering of $B$ then $B = \bigcap B_\iota$. It follows that $\sX_B = \bigcap_{\iota \in I} \sX_{B_\iota}$. Now one should prove that for any $\sX$ there is neighborhood $\sU_x$ such that the set
\be\label{top_fin_eqn}
\left\{\iota \in I| \sX_{B_\iota}\cap \sU_x\neq \emptyset\right\}
\ee
is finite. For any $x\in \sX$ there is an ultrafilter $\left\{A \right\}_{\la \in \La_x}\subset \mathbf{Hered}/C_0\left(\sX \right)$ such that for all $\la \in \La_x$ there is $a_\la \in A_\la$ such that $a_\la\left(x\right)\neq 0$. From the condition (b) of the Definition \ref{pullback_covering_ca_defn} it follows that there is $\la_0 \in \La$ such that a set
\bean
\left\{\iota \in I | A_{\la_0}\not\subset B_\iota\right\}
\eean
is finite. If 
$$
\sU_x \bydef \left\{y \in \sX | \exists a_{\la_0}\in A_{\la_0}\quad a_{\la_0}\left(y \right)\neq 0 \right\}
$$
then $\sU_x$ is an open neighborhood of $x$ and 
$$
\left\{\iota \in I | A_{\la_0}\not\subset B_\iota\right\}= \left\{\iota \in I |  \sX_{B_\iota}\cap  \sU_x   \neq \emptyset\right\},
$$
i. e. $\sX$ satisfies to the condition \ref{top_fin_eqn}.
\end{proof}
We denote by $J_\sX$ a Grothendieck topology which corresponds to a family of  locally finite coverings by closed sets.
...
\ee
where the Grothendieck topology $J_\sX$ is corresponds to the described above coverings and $J_{C_0\left(\sX \right) }$ is a Gelfand $C_0\left(\sX\right)$-topology (cf. Definition \ref{gelfand_a_site_defn}).
\begin{remark}\label{top_geometric_morphism_rem}
	If $\sX \to \sY$ is a continuous map then it is clear that there is a morphism of sites $\mathfrak{Closed}\left( \sY\right)\to \mathfrak{Closed}\left( \sX\right)$ (cf. Definition \ref{morphism_of_sites_defn}). 
	From the Theorem \ref{morphism_of_sites_thm} it follows that that there 
there is a geometric morphism
\be\label{top_geom_eqn}
\mathbf{Sh}\left( \mathfrak{Closed}\left( \sX\right)\right)\xrightarrow{f}\mathbf{Sh}\left( \mathfrak{Closed}\left( \sY\right)\right). 
\ee
If $F$ is a set then from the Lemma \ref{}	
\end{remark}
...
If the space $\sX$ is compact then given by \eqref{top_closed_sheaf_eqn} presheaf $\mathscr F^c$ is a sheaf on the site $\mathfrak{Closed}\left( \sX\right)$, i.e. $\mathscr F^c\in \mathbf{Sh}\left(\mathfrak{Closed}\left( \sX\right) \right)= \mathbf{Sh}\left(\mathfrak{Site}\left( C\left(\sX \right) \right) \right)$.

\end{lemma}
\begin{proof}
	Let $\sU \in \mathfrak{Closed}\left( \sX\right)$ and $\left\{\sU_1, ..., \sU_n\right\}$ is a covering of $\sU$ (cf. Section \ref{top_closed_sec}). If sections $s_\iota \in \mathscr{F}^c\left(\sU_\iota \right)$ are such that $s_\iota|_{\sU_\iota\cap \sU_j}=s_j|_{\sU_\iota\cap \sU_j}$ then from  the Theorem  \ref{loc_fin_french_thm} (cf. English translation  \ref{loc_fin_eng_thm}) it follows that there is $s \in \mathscr{F}^c\left(\sU_\iota \right)$ such that $s|_{\sU_\iota}= s_\iota$. It turns out that $\mathscr F^c\in \mathbf{Sh}\left(\mathfrak{Closed}\left( \sX\right) \right)$.
\end{proof}
...
\begin{lemma}
If $\sX$ is a compact space and $F$ is an Abelian group then for all $p > 0$ and any topological space $\sY$ there is an injective homomorphism
\be\label{top_prod_mor_eqn}
H^p\left(\mathfrak{Closed}\left( \sX\right), F_{\mathfrak{Closed}\left( \sX\right)}\right)  \hookto H^p\left(\mathfrak{Closed}\left( \sX\times \sY\right), F_{\mathfrak{Closed}\left( \sX\times \sY\right)}\right) .
\ee
\end{lemma}
\begin{proof}
If $\mathbf{Sh}\left( \mathfrak{Closed}\left( \sX\times\sY\right)\right)\xrightarrow{f}\mathbf{Sh}\left( \mathfrak{Closed}\left( \sX\right)\right)$ is a specialization of the given by \eqref{top_geom_eqn} geometric morphism then from the Proposition  \ref{spectral_sequence_prop} it follows that for all $p\ge 0$ there is the natural homomorphism 

\bean
H^p\left(\mathfrak{Closed}\left( \sX\right), F_{\mathfrak{Closed}\left( \sX\right)}\right)  \to H^p\left(\mathfrak{Closed}\left( \sX\times \sY\right), f^*F_{\mathfrak{Closed}\left( \sX\right)}\right).
\eean
where $f^*$ is an inverse image of $f$. One the other hand from  \eqref{constant_sheaf_morphism_eqn} it follows that $f^*F_{\mathfrak{Closed}\left( \sX\right)}\cong F_{\mathfrak{Closed}\left( \sX\otimes \sY\right)}$ so there is a homomorphism 
\bean
H^p\left(\mathfrak{Closed}\left( \sX\right), F_{\mathfrak{Closed}\left( \sX\right)}\right)  \to H^p\left(\mathfrak{Closed}\left( \sX\times \sY\right), F_{\mathfrak{Closed}\left( \sX\times \sY\right)}\right).
\eean
!!! INJECTIVE PROOF !!!
...
$$
Let $A^A \in \mathbf{Sh}\left(\mathfrak{Site}\left( CT\left(\sX, \dl \right)\right)  \right)$ be the given by \eqref{ctr_a_eqn} sheaf of groups, and let $\a \in \Aut_{C\left(\sX\right)}\left(CT\left(\sX, \dl \right) \right)$. For any $\sU_\iota \in \mathscr U$ element the restriction $\a|_{\sU_\iota}$ is represented by a continuous map $\a_\iota : \sU_\iota \to \Aut\left(\K \right)$. From \eqref{ctr_trans_eqn} it follows that 
$$
\forall x \in \sU_\iota \cap \sU_j \quad \a_\iota \left(x \right) \a_j^{-1} \left(x \right)= \a_\iota 1_{\Aut\left(\K \right)}\a_j^{-1} = \a_{\iota, j}\left( x\right) 
$$
From the above equation it follows that $ \a_{\iota, j}\in \left( D\left(\left\{\a^A_\iota \right\}\right).\left\{1_{\sU_{\iota, j}}\right\}\right)_{\iota, j}$ (cf. \eqref{nonab_perm_eqn}) so $\a_{\iota, j}$ represents a trivial class of $\check H^1\left(\mathfrak{Site} \left(CT\left(\sX, \dl \right) \right) A^A\right)$. It turns out that $\Dl \left(\a^A_{\iota j} \right) = 0 \in H^2\left(\mathfrak{Site} \left(CT\left(\sX, \dl \right) \right), S^A\right)$.



$\left\{\a^A_\iota \right\}\in C^0\left( \mathscr U^A, A^A\right)$ be a 0-chain (cf. the Definition \ref{nonab_coh_defn} and the Remark \ref{nonab_coh_rem}). If $\left\{1_{\sU_{\iota, j}}\right\}\in Z^1\left( \mathscr U^A, A^A\right)$ is a trivial cocycle then 
$$
...
\begin{lemma} !!! MAY BE DELETE !!!
		...
		$$
\cosupp B_{\iota_1} \cup ... \cup \cosupp B_{\iota_m}= \sX.		
		$$
...
%\end{remark}
	...
	F_{\mathbf T} \bydef \mathfrak{Ass}\left(F_{\mathrm{Cat}\left( \mathbf T\right)} \right)\in \mathbf{Sh}\left(\mathbf T \right).
		\ee
		  associated (cf. Definition \ref{associated_sheaf_defn}) with a constant sheaf  $F_{\mathrm{Cat}\left( \mathbf T\right)}$ will be named a \textit{constant sheaf}.
	\end{definition}

\begin{definition}\label{yoneda_defn}\cite{topos:intro}
...
\begin{lemma}\label{represented_presheaf_lem}\cite{johnstone:topos} % 0.12
...
preserves finite limits as well as coverings. %Here ¢ preserves covers meansthat if S E J(C) is a covering sieve of C in C, then the sieve (¢(S))generated by the set {¢( u) I u: C' -+ C is in S} is a covering sieve of¢(C) in D.
\end{definition}

\begin{theorem}\label{morphism_of_sites_thm}\cite{topos:intro}
%Theorem 2. 
...
Here we explicitly describe the given by the Theorem \ref{morphism_of_sites_thm} geometric morphism $\mathbf{Sh}\left(\mathbf{D}, K\right)\xrightarrow{f} \mathbf{Sh}\left(\mathbf{C}, J\right)$. The direct image is given by
...
\end{empt}
\begin{empt}
There is a category of sites and morphisms of sites.  This category has a final object $\mathbf{T}_{\mathrm{fin}}\bydef \left( \text{pt}, J_{\text{pt}}\right)$ where $\text{pt}$ is a category with a single object and single identical morphism $\text{pt} \xrightarrow{\Id_{\text{pt}}}\text{pt}$ which is a unique covering of  $\text{pt}$. For any site  $\left(\mathbf{C}, J\right)$ there is the unique functor $\mathbf{C}\xrightarrow{\phi_\mathbf{C}} \text{pt}$. It is clear that $\phi_\mathbf{C}$ is a morphism of sites (cf. Definition \ref{morphism_of_sites_defn}), so from the  Theorem \ref{morphism_of_sites_thm} it follows that there is a geometric morphism $\mathbf{Sh}\left(\mathbf{C}, J\right)\xrightarrow{f_{\left(\mathbf{C}, J\right)}} \mathbf{Sh}\left( \text{pt}, J_{\text{pt}}\right)$. If $F$ is a and $F_{\mathbf{Sh}\left( \text{pt}, J_{\text{pt}}\right)}$ is a constant sheaf (cf.  \eqref{constant_sheaf_eqn}) then one has
$$
F_{\left(\mathbf{C}, J\right)} = f^*_{\left(\mathbf{C}, J\right)} F_{\mathbf{Sh}\left( \text{pt}, J_{\text{pt}}\right)}
$$
where  $f^*_{\left(\mathbf{C}, J\right)}$ is an inverse image of $f_{\left(\mathbf{C}, J\right)}$. From this fact one can it follow that for any given by the Theorem \ref{morphism_of_sites_thm} geometric morphism $\mathbf{Sh}\left(\mathbf{D}, K\right)\xrightarrow{f} \mathbf{Sh}\left(\mathbf{C}, J\right)$  and any set $F$ one has
\be\label{constant_sheaf_geom_morphism_eqn}
F_{\mathbf{Sh}\left(\mathbf{D}, K\right)} = f^* F_{\mathbf{Sh}\left(\mathbf{C}, J\right)}.
\ee
\end{empt}
\begin{exercise}
	!!! MAY BE DELETE !!!
Proof that if $F$ is a set then in the situation of the Theorem \ref{morphism_of_sites_thm} one has
\be\label{const_sheaf_morphism_m_eqn}
...
\end{exercise}
\begin{lemma}\label{const_sheaf_morphism_lem} !!! DELETE !!!
	Consider a situation of the Theorem \ref{geometric_morphism_thm}. If $F$ is a set,  both $F_{\mathfrak{Site}\left( A\right) }\in \mathbf{Sh}\left(\mathfrak{Site}\left( A\right) \right)$ and  $F_{\mathfrak{Site}\left(\widetilde  A\right) }\in \mathbf{Sh}\left(\mathfrak{Site}\left(\widetilde A\right) \right) $ are constant sheaves (cf. \eqref{constant_sheaf_eqn}) then 
	\be\label{const_sheaf_morphism_eqn}
	F_{\mathfrak{Site}\left(\widetilde  A\right) }= \mathfrak{Topos}(\varphi)^*\left(  F_{\mathfrak{Site}\left(  A\right) }\right) 
	\ee
	where  $\mathfrak{Topos}(\varphi)^*$ is the inverse image (cf. Definition \ref{geometric_morphism_defn}) of the given by \eqref{geometric_morphism_eqn}.
\end{lemma}
!! UP !!!

\begin{proof}
Firstly we construct a given by \eqref{geom_ep_im_eqn} covariant functor 
\bean
A_{\overline\varphi} \bydef \mathbf{Hered}/  A \xrightarrow{ \overline\varphi} \mathbf{Hered}/ \widetilde A \xrightarrow{\mathbf{y}} \mathbf{Sets}^{\left( \mathbf{Hered}/ \widetilde A\right) ^{\mathrm{op}}} \xrightarrow{\mathfrak{Ass}} \mathbf{Sh}\left(\mathbf{Hered}/ \widetilde A, J_{\widetilde A} \right)
\eean 
where $\overline \varphi$ is given by \eqref{geom_functor_eqn}. iF $B$ is a hereditary $C^*$-subalgebra of $A$ then
\bean
{\mathbf{y}}\circ \overline \varphi \left(B \right) = \widetilde B \mapsto \Hom_{\mathbf{Hered}/ \widetilde A }\left( \widetilde B, \overline \varphi B\right). 
\eean
If $ \overline \varphi B\subset  \widetilde B$ then $\Hom_{\mathbf{Hered}/ \widetilde A} \left( \widetilde B, \overline \varphi B\right)$ contains a single $\mathbf{Hered}/ \widetilde A$-morphism (cf. Definition \ref{hered_cat_defn}) and if $ \overline \varphi B\subsetneqq  \widetilde B$ then $\Hom_{\mathbf{Hered}/ \widetilde A} \left( \widetilde B, \overline \varphi B\right)= \emptyset$.  So for any $\widetilde B= \overline\varphi B$ a set $\Hom_{\mathbf{Hered}/ \widetilde A} \left( \widetilde B, \overline \varphi B\right)$ contains a single $\mathbf{Hered}/ \widetilde A$-morphism 

If $\widetilde B$ is  any hereditary $C^*$-subalgebra of $\widetilde A$ and
\bean
B_{\widetilde{B}}\bydef \bigcap \left\{B \in \mathbf{Hered}/ \widetilde A | \widetilde B \subset \varphi B \right\},\\
\widetilde{B}_{\widetilde{B}} \bydef \overline \varphi \left( B_{\widetilde{B}}\right) 
\eean
then a pair $\left\{\widetilde{B}_{\widetilde{B}}, \widetilde{B} \right\}$ is a covering of $\widetilde{B}$ (cf. Definition \ref{hered_cat_defn}). such that $\Hom_{\mathbf{Hered}/ \widetilde A} \left(\widetilde{B}_{\widetilde{B}}, \overline \varphi B\right)= \Hom_{\mathbf{Hered}/ \widetilde A} \left(\widetilde{B}_{\widetilde{B}}, \widetilde{B}_{\widetilde{B}}\right)$ contains a single $\mathbf{Hered}/ \widetilde A$-morphism $\Id_{\widetilde{B}_{\widetilde{B}}}$ (cf. Definition \ref{hered_cat_defn}) and if $ \overline \varphi B\subsetneqq  \widetilde B$ then $\Hom_{\mathbf{Hered}/ \widetilde A} \left( \widetilde B, \overline \varphi B\right)= \emptyset$. It follows that 
$\mathfrak{Ass}\circ {\mathbf{y}}\circ \overline \varphi \left(B \right)$ contains $\Id_{\widetilde{B}_{\widetilde{B}}}$
