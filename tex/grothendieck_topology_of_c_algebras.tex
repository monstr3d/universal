...
\eean
 $B$ is generated by the family $\left\{B_\iota\subset B\right\}$ if and only if a Hilbert subspace $p_B \H$ is a norm closure of a sum $\sum_\iota p_\iota\H$. If $\H^\cap \bydef p_B \H \cap p_C \H$, $\H^\cap_\iota =  p_B \H \cap p_\iota \H$  and $p^\cap$, $p^\cap_\iota$ are a projectors  onto $\H^\cap$ and $\H^\cap_\iota$ then one has
\bean
...
\eean
$B\cap C$ is a generated by the family $\left\{B_\iota\subset B\right\}$ hereditary algebra if $\H^\cap = p_B \H \cap p_C \H$ is closure of $\sum \H^\cap_\iota = \sum p_B \H \cap p_\iota \H$. But this fact follows from that the sum $\sum_\iota p_\iota\H$ is dense in $p_B\H$.\\
...
\begin{definition}\label{grothendieck_ca_defn}
	The given by the Lemma \ref{grothendieck_ca_lem} topology  is said to be a \textit{Grothendieck} $A$-\textit{topology}.
\end{definition}
\begin{exercise}
...
If $F$ is an Abelian group with a linear map $F\ox_\Z F \to F$ then ane can define pa pairing
\be\label{cross_eqn}
\begin{split}
