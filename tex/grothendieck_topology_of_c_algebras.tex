...
\end{proof}
\begin{defn}\label{ca_ultra_filter_defn}
Let $A$ be a $C^*$-algebra. An indexed by a directed set $\La$ family $\left\{D_\la\right\}_{\la \in \La}$ of hereditary $C^*$-subalgebras of $A$ is said to be a \textit{filter} if one has:
\begin{enumerate}
...
\forall \la', \la'' \in \La \quad D_{\la'}\cap D_{\la''}\in \left\{D_\la\right\}.
	$
	A maximal filter is an \text{ultrafilter}, i.e. if  ultrafilter is a subset of another filter then both filters coincide.
\end{enumerate}
\end{defn}
...
\end{tikzcd}
	...
	\begin{itemize}
		...
		\item[(b)] For any ultrafilter  $\left\{D_\la\right\}_{\la \in \La}$ (cf. Definition \ref{ca_ultra_filter_defn}) there is $\la_0 \in \La$ such that a set
