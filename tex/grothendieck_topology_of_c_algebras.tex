...
\bean
\varphi: A \hookto \mathbf{RC}\left(\widetilde A\right)
\eean
...
\begin{lemma}\label{locale_open_lem}
	Any hereditary full injective homomorphism $f: A \hookto \mathbf{RC}\left(\widetilde A\right)$ yields an open map of locales  (cf. Definition \ref{open_locale_mor_defn}). Moreover the map is a surjection (cf. Definition \ref{loc_emp_sup_defn})
\end{lemma}
\begin{proof}
		Let us define 
...
A \xhookrightarrow{\widetilde \varphi} M\left(\widetilde A \right)
	...
	\begin{enumerate}
		...
		$\left\{\widetilde \pi \left(a' \right)\widetilde u_\la \widetilde \psi \left(a'' \right)  \right\}$ is $C^*$-norm convergent.
	\end{enumerate}
If define a pairing
\bean
...
\eean
then from the bimodule conditions it follows that
$$
\left(a'\varphi\left(a \right) , a'' \right)= \left(a', \varphi\left(a \right) a'' \right)
$$
so one has a map
...
Prove that a given by \eqref{cech_hom_eqn} map $\check{H}^\bullet\left(f\right):\check{H}^\bullet\left(\sY, F \right)\to \check{H}^\bullet\left(\sX, F \right)$ is a homomorphism of rings.
	...
	\end{lemma}
	\begin{definition}\label{dc_defn}\cite{matro:hcm}
