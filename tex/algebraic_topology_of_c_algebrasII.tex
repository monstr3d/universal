...
\begin{definition}\label{gelfand_space_defn}
	...
	Aa \in \xi.
	$$
	The \textit{Gelfand space} $\mathfrak{Gelfand}\left(A \right)$ of $C^*$-algebra $A$ is a space  of finite points with the final topology (cf. Definition \ref{top_final_defn}) given by the natural inclusion $\mathfrak{Gelfand}\left(A \right) \hookto \mathfrak{Ultrafilters}\left(A \right)$. If for any closed left ideal $L \subset A$ one has
	\bean
\mathfrak{Gelfand}\left(A \right)_L		\bydef \left\{\left. \xi \in \mathfrak{Gelfand}\left(A \right) \right| \exists L' \in \xi\quad L'\cap L = \{0\} \right\}
\eean
and  $\left\{L\right\}$ is the  set of all left closed ideals of $A$
then  subbasis $\mathscr{S}_A$ of the topology (cf. Definition \ref{top_subbasis_defn}) is given by
\be \label{gelfand_space_sub_eqn}
\mathscr{S}_A \bydef \left\{\left.\mathfrak{Gelfand}\left(A \right)_L	\subset \mathfrak{Gelfand}\left(A \right)\right| L \in \left\{L\right\}	\right\}.
\ee	
\end{definition}
...
\begin{definition}\label{covering_defn}
	...
	\eean 
The $*$-homomorphism $\lift$ is a {\it noncommutative covering} if for any $x \in \mathfrak{Gelfand}\left(A \right)$ there is a  neighborhood $\sU$ such that
\begin{enumerate}
	\item[(a)] $\sU\in \mathscr{S}_A$ where $\mathscr{S}_A$ is the given by \eqref{gelfand_space_sub_eqn} subbasis of the topology of $\mathfrak{Gelfand}\left(A \right)$.
	\item[(b)] $\mathfrak{Gelfand}\left(\lift\right)^{-1}\left( \sU\right)$  is the disjoint union of closed  subsets of $\mathfrak{Gelfand}\left(\widetilde A \right)$ each of which is mapped homeomorphically onto $\mathcal{U}$.
\end{enumerate}
\end{definition}
