...
\subsection{Hausdorff blowing-up}
	...
	\begin{remark}\label{blowing_rem}
		$	\mathfrak {Blowing}_{\sX-A }\left( 	C_c\left( \sX\right)\right)\cdot A$ is dense in $A$ if and only if $A\cdot 	\mathfrak {Blowing}_{\sX-A }\left( 	C_c\left( \sX\right)\right)$ is dense in $A$ (cf. equations \eqref{blowing_eqn}), i.e. both equations inclusions  \eqref{blowing_eqn} are equivalent.
	\end{remark}
\begin{remark}
	The inclusion $
