...
\begin{definition}\label{covering_defn}
	Let  both $A$ and  $\widetilde{A}$ be  connected $C^*$-algebras.  Let  $\lift: A  \hookto M\left( \widetilde{A}\right)$ be a good  $*$-homomorphism (cf. Definition 
	\ref{good_defn})  with 
	the given by \eqref{gelfand_map_eqn} surjective  continuous map	\bean
	\mathfrak{Gelfand}\left(\widetilde A \right)\xrightarrow{\mathfrak{Gelfand}\left(\lift \right)}\mathfrak{Gelfand}\left(A \right).
	\eean 
The $*$-homomorphism $\lift$ is a {\it noncommutative covering} if for any $x \in \mathfrak{Gelfand}\left(A \right)$ there is a closed neighborhood $\sU$ such that $\mathfrak{Gelfand}\left(\lift\right)^{-1}\left( \sU\right)$  is the disjoint union of closed  subsets of $\mathfrak{Gelfand}\left(\widetilde A \right)$ each of which is mapped homeomorphically onto $\mathcal{U}$
\end{definition}
...
\end{remark}
	...
	\begin{lemma}
		Let  $ C_0\left( \sX\right)\subset  M\left( A\right) $ be  Hausdorff blowing-up of $A$ (cf. Definition \ref{blowing_defn}). If the space $\sX$ is locally connected  then  for any open $\sU\subset \sX$ subset following conditions are equivalent:
		\begin{enumerate}
			\item[(a)] the space $\sU$ is connected,
			\item[(b)] the the $ \sU$-{subalgebra} 	$~_\sU A\cap  A_\sU$ (cf. Definition \ref{blowing_ideals_au_ua_defn}) is connected (cf. Definition \ref{connected_c_a_defn}),
		\end{enumerate}
	\end{lemma}
...
!!!
\subsubsection{Weak fundamental groups}
\paragraph{}
