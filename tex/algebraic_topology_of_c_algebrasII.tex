...
\begin{definition}\label{top_subbasis_defn}\cite{munkres:topology}
	Let $\sX$ be a topological space.
A {\it subbasis } $\mathscr S$ for a topology on $\sX$ is a collection of open subsets of $\sX$ such that
\begin{itemize}
...
\begin{empt}\label{principal_div_empt}
	...
	\textit{rationally equivalent} if they differ by a sum 
...
\paragraph{}Here I follow to \cite{berger:geometryI,projective_geometry}.
...
\begin{definition}\label{collineatioin_defn}\cite{projective_geometry}
	A {\it collineation} of $P$ is a bijective map from the point set and line set
	of $P$ onto themselves that preserves incidence.
	\end{definition}
\begin{definition}\cite{projective_geometry}
%	Definition.
 A collineation of a projective space $P(\sV)$ is called projective if it is
	induced by a linear map of $\sV$.
\end{definition}
\begin{definition}\cite{projective_geometry}
	%Definition. 
	...
	\end{itemize}
\end{definition}
\begin{theorem}\cite{projective_geometry}
%	2.2.1 Theorem. 
Let $\sV$ be a vector space of dimension $d + 1$ over a division ring	$F$. Then the theorem of Desargues holds in $P(\sV)$.
\end{theorem}
\begin{definition}\cite{projective_geometry}
...
Let V be a vector space over the division ring F. Then the theorem
	of Pappus holds in $P(\sV)$ if and only if F is commutative (in other words, if
	F is afield).
\end{theorem}
\begin{theorem}\cite{projective_geometry}
%	2.2.2 Theorem. 
Let $\sV$ be a vector space over the division ring $F$. Then the theorem
	of Pappus holds in $P(\sV)$ if and only if F is commutative (in other words, if
	F is afield).
\end{theorem}
\begin{theorem}\cite{projective_geometry}
...
Let $P$ be an arbitrary projective space. If the theorem
	of Pappus holds in $P$ then the theorem of Desargues is also true in $P$.
	\end{theorem}
...
\bibitem{harboush_inf} William J. Haboush {\it Infinite dimensional algebraic geometry; algebraic structures on $p$-adic groups and their homogeneous spaces}. 	Tohoku Mathematical Journal
Volume	57, 2005.!!!
