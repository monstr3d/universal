...
\begin{split}
	...
	L \mapsto \text{ the $C^*$-norm closure of } \widetilde A \varphi_M\left(L \right).
\end{split}
\ee
...
\begin{defn}\label{good_defn}
	Let both $A$ and $\widetilde A$ be $C^*$-algebras, and let $\varphi_M : A \hookto M\left( \widetilde A\right)$  be an injective 	$*$-homomorphism satisfying to the condition \ref{closed_p_eqn}. We say that both $\varphi_M$ is {\it good} if the given by \eqref{ultra_mor_eqn} map $\mathfrak{Ultrafilters}\left(\varphi_M \right)$ yields the natural
	map
...
There is the {\it Gelfand category} $\mathfrak{Gelfand}$ such that
\begin{enumerate}
	\item [(a)] $\mathfrak{Gelfand}$-objects are connected $C^*$-algebras,
	\item [(b)] $\mathfrak{Gelfand}$-morphisms are good $*$-homomorphisms.
\end{enumerate}
\end{definition}
