...
\begin{empt}\label{topology_empt}
	...
	\mathfrak{Ultrafilters}\left(A \right)_L	\bydef \left\{\left. \xi \in \mathfrak{Ultrafilters}\left(A \right) \right| \exists L' \in \xi\quad L'\subset L \right\}
	\ee
	set is open.
\end{empt}
\begin{remark}
...
\end{lemma}
\begin{proof}
	If $L \subset A$ is a closed left ideal and $\widetilde L$ is the $C^*$-norm closure of $\varphi_M\left( L\right) \widetilde A$ then
	\bean
	\mathfrak{Gelfand}\left(\varphi_M \right)^{-1}\left( \mathfrak{Gelfand}\left(A \right)_L\right) = \mathfrak{Gelfand}\left(\widetilde A \right)_{\widetilde L}
	\eean 
	So any  set of basis (cf. Definition \ref{top_base_defn}) of the topology of  $\mathfrak{Gelfand}\left(A\right)$  corresponds to a set of basis of the topology of $\mathfrak{Gelfand}\left(\widetilde A\right)$. From (c) of the Theorem \ref{top_continuous_thm} it turns out turns out that the map \ref{gelfand_map_eqn} is continuous. Any element of basis of the topology of $\mathfrak{Gelfand}\left(\widetilde A \right)$ is given by $\mathfrak{Gelfand}\left(\widetilde A \right)_{\widetilde L}$. The intersection
	\bean
	L = \bigcap \left\{L' \subset A \left| {\widetilde L} \subset \varphi_M\left( L\right) \widetilde A\right.\right\}\subset A
	\eean
	...
	Any $\xi \in \mathfrak{Gelfand}\left(A \right)_L$ is given by the ultrafilter $\left\{L_\la\subset L\right\}$. If any $\widetilde L_\la$ is the $C^*$-norm closure of $\varphi_M\left( L_\la\right) \widetilde A$ then  there is filter $\left\{\widetilde L_\la\right\}$. If there is $\widetilde L_{\la_0}\in \left\{\widetilde L_\la\right\}$ such that $\widetilde L_{\la_0}\cap \widetilde L=\{0\}$ then there is a nontrivial intersection 
	\bean
	L'' \bydef \bigcap \left\{L' \subset L\left| \widetilde L_{\la_0}\subset \varphi_M\left( L'\right) \widetilde A \right.\right\}
	\eean 
	and the $C^*$-norm closure $L'''$ of the union of left ideals 
	\bean
	\bigcap \left\{L' \subset L | L' \cap L'' = \{0\}\right\}
	\eean 
	is not trivial. From our construction it follows that $L'''\subsetneqq L$ and the closure of $\widetilde L\subset \varphi_M\left( L'\right) \widetilde A$ equals to $\widetilde L$. On the other hand $L$ is the minimal among the closed ideals $L''''$ such that $\widetilde L\subset \varphi_M\left( L'''''\right) \widetilde A$. There is a contradiction. From the contradiction it follows that 
	\be 
	\forall \la \quad \widetilde L_\la\cap \widetilde L \neq \{0\}.
	\ee
	There is a filter $\left\{\widetilde L_\la\cap \widetilde L\right\}$ and there exists an ultrafilter $\left\{\widetilde L_\a\right\}$ such that  
	\bean \left\{\widetilde L_\la\cap \widetilde L\right\}\subset \left\{\widetilde L_\a\right\},\\
	\forall \a \quad L_\a \subset L.
	 \eean If $\widetilde \xi\in \mathfrak{Gelfand}\left(\widetilde A \right)$ is represented by $\left\{\widetilde L_\a\right\}$ then from our construction it follows that
	\bean
	\widetilde \xi \in \mathfrak{Gelfand}\left(\widetilde A \right)_{\widetilde L},\\
	\varphi_M\left(\widetilde \xi \right) = \xi,
	\eean
	i.e.
	\bean
	...
	\eean 
The above equation means that $\mathfrak{Gelfand}\left(A \right)_L \subset \mathfrak{Gelfand}\left(\varphi_M \right)\left( \mathfrak{Gelfand}\left(\widetilde A \right)_{\widetilde L}\right) $.	Taking into account $\mathfrak{Gelfand}\left(\varphi_M \right)\left( \mathfrak{Gelfand}\left(\widetilde A \right)_{\widetilde L}\right)\subset \mathfrak{Gelfand}\left(A \right)_L$ one concludes that
	$$
	...
	$$
	i.e. the image of an element   $\mathfrak{Gelfand}\left(\widetilde A \right)_{\widetilde L}$  of base of topology $\mathfrak{Gelfand}\left(\widetilde A \right)$ is an element  $\mathfrak{Gelfand}\left( A \right)_{ L}$ of base of topology $\mathfrak{Gelfand}\left( A \right)$. it means the the map $\varphi_M$ is open.
\end{proof}
\begin{lemma}
...
\subsection{Definition of coverings}
...
%A {\it subbasis } $\mathscr S$ for a topology on $\sX$ is a collection of subsets of $\sX$ whose union equals $\sX$. The {\it topology generated by the subbasis} $\mathscr S$ is defined to be the  collection $\mathscr T$ of all unions of finite intersections of elements of $\mathscr S$.
%\end{definition}		
