...
(*) $\sX$ is a Noetherian integral separated scheme which is regular in 
codimension one. 
\begin{definition}
Definition. Let $\sX$ satisfy (*). A {prime divisor} on I is a closed integral subscheme $\sY$ of codimension one. A {Weil divisor} is an element of the free Abelian group $\mathrm{Div}\sX$ generated by the prime divisors. We write a divisor as $D =\sum_{j=1}^n $  where tne Yt are prime divisors, the n( are integers, and 
only finitely many /?• are different from zero. If all the /?,- ^ 0, we say 
that D is effective. 
\end{definition}
