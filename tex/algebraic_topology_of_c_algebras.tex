...
\bean
\mathrm{cl}\left( A\right) \smile \mathrm{cl}\left( B\right)\in  H^{4n - 2a - 2b}\left(\sX, \sX \aetminus \left( A \cap B\right)  \right) %\cong H_{4n - 2a - 2b}\left(\ A \cap B\right)
\eean
(?4?-2?-26(*, *- (? ? 
and thus A - ? lives in H2a+2b_2n(A ? 5). Note that if the intersection is 
proper, this last group is free on the classes of irreducible components of ? ? ?, 
so this determines A ¥ ? as a cycle; in other words, this gives a topological 
construction of intersection multiplicities. 
Complex vector bundles Ein X have Chern classes c,(?) in ?2'*, satisfying: 
(i)co(?)= l;c,(?) = 0if/> rank(?). 
(ii) If/: Y -+X,c,{f*E) = /%(?). 
	(iii) If 0 -* E' -+ E -* E" -È 0 is an exact sequence, then 
	...
	For a line bundle L e ?^?',©^), c,(L) e H2(X,Z) may be constructed as 
