...
\begin{empt}
Let $A$ be a $C^*$ -algebra and let $\sY$ be the spectrum of $A$. An element $\xi \in \mathfrak{Gelfand}\left(A \right)$  \textit{belongs to } $y \in \sY$ if one has
\bean
\forall L \in \xi \quad \rho_y\left(L \right) \neq \rho_y\left(A \right) 
\eean
where $\rho_y : A \to B\left(\H_y \right)$ is an irreducible representation (cf. Definition  \ref{irred_defn}). If $y', y'' \in \sY$ with $y' \neq y''$ and $\xi$ belongs to both $y'$, and $y''$ then the generated by the set $\left\{ L \cap \ker_{y'}\right\}$ filter exceeds $\xi$ it is impossible since $\xi$ is an ultrafilter. Similarly bicommutant $\xi''$ \text{belongs} if $\rho_y\left(\xi'' \right) \neq \rho_y\left(A \right)$.
There are natural maps
\bea\label{to_spec_eqn}
\mathfrak{Gelfand}\left(A \right) \to \sY,\\
\label{to_spec''_eqn}\mathfrak{Gelfand}\left(A \right)'' \to \sY,
\eea
\end{empt}
