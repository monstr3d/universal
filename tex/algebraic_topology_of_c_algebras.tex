...
\begin{split}
\mathfrak{LeftMax}\left(A \right)\onto \hat A,\\
\mathfrak{LeftMax}\left(A \right)\onto \check A
\end{split}
\ee
from the space of maximal left ideals onto the prime spectrum.
\end{lemma}
...
\begin{proof}
If $t \in \mathfrak{LeftMax}\left(A \right)$ then $t\cap t^*$ is the maximal hereditary $C^*$-algebra (cf. \ref{hered_ideal_lem}), From \ref{hered_spectrum_prop}
\end{proof}
...
\end{lemma}
\begin{proof}
According to the Definition \ref{jackobseon_defn}!!!
\end{proof}
\begin{proof}
