...
\begin{definition}\label{good_ultrafilter_defn}
	An ultrafilter of open subsets sets  on locally compact Hausdorff  space  is {\it good} if contains an open set with compact closure.
\end{definition}
...
\end{remark}
	...
	$C^*$-algebra. Then for any $n \in \N$ there is an $*$-isomorphisms $A\otimes \K \cong \mathbb{M}_n\left( A\otimes \K\right)$ which yields a homeomorphism $\mathfrak{Gelfand}_r\left(\mathbb{M}_n\left(A \right)\right)\cong \mathfrak{Gelfand}_r\left(A \right)$. So any projector in $\mathbb{M}_n\left(A\right)$  yield a projector in $C_0\left(\mathfrak{Gelfand}_r\left(A \right) \right)$ and vice versa.
	According to the  Shilov Idempotent Theorem any projector in $C_0\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right) \right)$ corresponds to an element of the group of cohomology $H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)$ with compact support (cf. \ref{commutative_k_empt}).  There are natural actions 
	\bea\label{g_act_eqn}
...
\begin{proof}
The group $H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)$ is a free Abelian group generated by quasicomponents of $\mathfrak{Gelfand}_r\left(A\otimes \K \right)$. Moreover if both $\sU', \sU''\subset \mathfrak{Gelfand}_r\left(A\otimes \K \right)$ are quasicomponents then
\be\label{cup_u_eqn}
\left[\sU'\right]\smile \left[\sU''\right]= \begin{cases}
\left[\sU'\right]& \sU'= \sU''\\
0 & \sU'\neq \sU''
\end{cases}
\ee
Element $g$ yields a homeomorphism of $\mathfrak{Gelfand}_r\left(A\otimes \K \right)$, so it corresponds to a transposition of quasicomponents. This lemma follows from \eqref{cup_u_eqn}
\end{proof}
