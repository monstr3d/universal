...
\begin{tikzcd}
~
	& A  \arrow[d, "\varphi^2"] & & V^{\mathrm{nat}}\left(A \right) \arrow[r, "\subset"] \arrow[d, "\varphi_*"]& V\left( A\right)   \arrow[d, "\varphi^2_*"]\\
	C\left( \sX^+\right) \arrow[r, "\varphi^1"]	& M\left( A\right)& & 	V\left( C\left( \sX^+\right)\right)  \arrow[r, "\varphi^1_*"]	& V\left( M\left( A\right)\right) 
\end{tikzcd}
\eean
...
\begin{definition}
	Under the above hypothesis for any $\sV\in V^{\mathrm{nat}}\left(A \right)$ 
	 we define  {\it Chern classes} $c_j\left(\sV \right)\in H^{2j}\left(\sX^+, \Z \right)$ of $\sV$ as {Chern classes} of $\varphi_*\left( \sV\right) \in V\left( C\left( \sX^+\right)\right)$ (cf. Theorem \ref{k:char_thm})
 \end{definition}
...
\begin{rem}
	...
	$K$-groups.
\end{rem}
\begin{empt}\label{seimfunct_empt}\cite{blackadar:ko}
%5.2.1. Functoriality. 
If $\varphi : A \to B$ is a homomorphism (not necessarily
continuous!) of local Banach algebras, then $\phi$ induces a map $\phi_*: \mathrm{Proj}\left(A \right) \to \mathrm{Proj}\left(B \right)$. $\phi$ extends to a homomorphism from $\mathbb{M}\left(A \right)_\infty$ to $\mathbb{M}\left(A \right)_\infty$, which induces
a semigroup homomorphism, also denoted $\phi_*$, from $V (A)$ to $V (B)$. So $V$ is a
covariant functor from the category of local Banach algebras to the category of
Abelian semigroups.
\end{empt}
