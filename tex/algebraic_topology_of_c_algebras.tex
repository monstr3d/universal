...
\begin{lemma}\label{ctr_g_lem}
	If  $A$ is a {continuous-trace} $C^*$-{algebra} then for any $\xi \in \mathfrak{Gelfand}\left(A \right)$ (cf. Definition \ref{gelfand_space_defn}) there is the natural  pair $\left(x_\xi, y_\xi \right)$ such that $x_\xi \in \sX \bydef  \hat A$ and $y_\xi \in \mathbb{P}\left(\H_x \right)_{\mathfrak{Gelfand}}$ (cf. the equation \eqref{gefand_projective_eqn}) where $\rep_x: A \to B\left(\sH_x \right)$ is the corresponding to $x$ irreducible representation  (cf. Definitions \ref{irred_defn} and \ref{spectrum_prime_primtive_defn}).
\end{lemma}
\begin{proof}
From the Theorem \ref{dauns_hofmann_thm} and the Lemma \ref{hausdorff_spectrum_lem} it follows that there is Hausdorff blowing-up (cf. Definition \ref{blowing_defn})
\be 
\mathfrak {Blowing}_{\sX-A }:C_0\left( \sX\right) \hookto M\left(A \right)
\ee
where $\sX\bydef \hat A$ is the spectrum of $A$ (cf. Definition \ref{spectrum_prime_primtive_defn}). So from the equation \eqref{phi_a_eqn} and the Lemma  \ref{lem_g_lem} it follows that there is the natural surjective continuous  map

\be\label{ctr_g_eqn}
\begin{split}
\phi_{A-\sX}:	\mathfrak{Gelfand}\left(A \right) \onto \sX.
\end{split}
\ee
We define $x 
\bydef \phi_{A-\sX}\left(\xi \right)$ then for any $L \in \xi $ one has $\rep_x\left( L\right) \neq \{0\}$  If
$
\bigcap_{L \in \xi} \rep_x\left(L \right) = \{0\}
$ then there are $L', L'' \in \xi$ with $\rep_x\left(L' \right)\cap \rep_x\left(L'' \right)= \{0\}$ and the generated by the set
\be\label{int_d_eqn}
\left\{L \in \xi\quad  \left| \quad \rep_x\left(L \right) \cap \rep_x\left(L' \right)\neq  \{0\}\quad  \rep_x\left(L \right) \cap \rep_x\left(L'' \right)= \{0\}\right. \right\}.
\ee
The generated by \eqref{int_d_eqn} filter exceeds $\xi$. It is impossible since $\xi$ is an ultrafilter. From the equation \eqref{gefand_projective_eqn} it follows that  $\bigcap_{L \in \xi} \rep_x\left(L \right)$ corresponds to the subset of $\mathbb{P}\left(\H_x \right)_{\mathfrak{Gelfand}}$ . If $y', y'' \in \bigcap_{L \in \xi} \rep_x\left(L \right)$ are such that $y' \neq y''$ then $\bigcap_{L \in \xi} \rep_x\left(L \right)\setminus \{y''\}$ is an open subset of $\bigcap_{L \in \xi} \rep_x\left(L \right)$ and a generated by the set
\bean
\left\{L \in \xi\quad  \left| \quad \bigcap_{L \in \xi} \rep_x\left(L \right)\setminus \{y''\}\right. \right\}.
\eean
exceeds $\xi$. It is impossible since $\xi$ is an ultrafilter. So there is $y_\xi \in \mathbb{P}\left(\H_x \right)_{\mathfrak{Gelfand}}$ with
\be\label{int_yd_eqn}
\bigcap_{L \in \xi} \rep_x\left(L \right)= \{y_\xi\} \subset \mathbb{P}\left(\H_x \right)_{\mathfrak{Gelfand}}
\ee
A given by
 $$
 \eta \bydef \left\{ a \in A \left|\rep_x\left(a \right)\perp y  \right.\right\}
 $$
 left ideal is proper and maximal/

\end{proof}
\begin{lemma}\label{ctr_g_lem!}!!!
	If  $A$ is a {continuous-trace} $C^*$-{algebra} then the  bicommutant $\mathfrak{Gelfand}\left(A \right)''$
	 of Gelfand space (cf. Definition \ref{bicommutant_defn}) is equivalent to the  a set of pairs $\left(x_\xi, V_\xi \right)$ where $x_\xi \in \sX$ is an element of the spectrum $\sX$ of $A$ and $V$ is a one dimensional subspace of the corresponding to $x$ Hilbert space $\H_x$ of irreducible representation $\rep_x: A \to B\left(\H_x \right)$.
\end{lemma}
\begin{proof}
From the Theorem \ref{dauns_hofmann_thm} and the Lemma \ref{hausdorff_spectrum_lem} it follows that there is Hausdorff blowing-up (cf. Definition \ref{blowing_defn})
\be 
