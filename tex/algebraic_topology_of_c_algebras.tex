...
\end{proof}
	...
	\begin{proof}
		...
		\eean 
		are canonical resolutions (cf. \eqref{canonical_resolution_eqn}). 
		Using the above inclusion \eqref{nt_can_st_eqn} one cab construct commutative diagram
		\bean
		\begin{tikzcd}
			0 \arrow[r] & \mathscr F \arrow[d] \arrow[r, "\eps^{\T^n}"]	&\mathscr{C}^{0}\left(\T^n, \mathscr F \right)  \arrow[d]\arrow[r, "d_1^{\T^n}"] & \mathscr{C}^{1}\left(\T^n, \mathscr F \right) \arrow[d]\arrow[r, "d_2^{\T^n}"] & \mathscr{C}^{2}\left(\T^n, \mathscr F \right) \arrow[d] \arrow[r, "d_3^{\T^n}"] & \dots\\
			0 \arrow[r] & \mathscr F  \arrow[r, "\eps^{\sX}"]	&\mathscr{C}^{0}\left(\sX, \mathscr F \right)  \arrow[r, "d_1^{\sX}"] & \mathscr{C}^{1}\left(\sX, \mathscr F \right) \arrow[r, "d_2^{\sX}"] & \mathscr{C}^{2}\left(\sX, \mathscr F \right)  \arrow[r, "d_3^{\sX}"] & \dots
		\end{tikzcd}
		\eean
		The above diagram yields the diagram 
			\bean
		\begin{tikzcd}
			0 \arrow[r] & \Ga\left( \mathscr F \right) \arrow[d] \arrow[r, "\Ga\left( \eps^{\T^n}\right) "]	&\Ga\left( \mathscr{C}^{0}\left(\T^n, \mathscr F \right)\right)   \arrow[d]\arrow[r, "\Ga\left( d_1^{\T^n}\right) "] & \Ga\left( \mathscr{C}^{1}\left(\T^n, \mathscr F \right)\right)  \arrow[d]\arrow[r, "\Ga\left( d_2^{\T^n}\right)"] & \Ga\left( \mathscr{C}^{2}\left(\T^n, \mathscr F \right)\right)  \arrow[d] \\
...
\begin{definition}
Under the above hypothesis the space  $\mathfrak{Alg}\text{-}\mathfrak{Gelfand} \left(A \right)\bydef \varprojlim_{\varphi \in \Phi} \sX_\varphi$  is the {\it algebraic Gelfand space} of $A$. From the surjective continuous map 
\be
...
\begin{remark}
	If $A$ is a commutative $C^*$-algebra then the inclusion \eqref{dauns_hofmann_eqn} coincides with the isomorphism	\eqref{dauns_hofmann_eqn} given by the Dauns-Hoffman theorem.
\end{remark}
...
\end{remark}	
	...
	\paragraph{}
	Any projector $p \in \mathbb{M}_n\left(	C\left( M\left(\mathfrak{Gelfand}\left(A \right) \right) \right) \right) $ naturally yields the projector $\mathbb{M}_n\left( \varphi_{\text{DH}} \circ p\right)\in  \mathbb{M}_n\left(M\left(A \right)  \right)$. So there is the map
	\be
V^{\text{DH}}\left( A\right)  :	V\left( M\left(\mathfrak{Gelfand}\left(A \right) \right)\right) \hookto V\left(A \right) 	
\ee
...
\bean
K_0\left(C\left( M\left(\mathfrak{Gelfand}\left(A \right) \right)\right) \to K_0\left( M\left(A \right)  \right) \right),\\
K^0\left(M\left(\mathfrak{Gelfand}\left(A \right) \right) \to K_0\left( M\left(A \right)  \right) \right)
\eean
