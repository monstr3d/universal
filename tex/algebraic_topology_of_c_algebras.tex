...
\begin{lemma}\label{haudorff_lem}
	One has:
	\begin{enumerate}
		\item [(i)] for any topological space $\sX$ there is the natural, by $\mathfrak{Hausdorff}\left( \sX\right)$  initial object (cf. Definition \ref{initial_ob_defn}) of the  $\sX$-{Hausdorff category}, 
		\item [(ii)] every continuous map $\sX \to \sY$ yields the commutative diagram 
...
\begin{definition}\label{good_hom_defn}
	If both $A$ and $B$ are $C^*$-algebras  then a $*$-homomorphism $\varphi : M\left(  A\right)  \to M\left( B\right) $ is {\it open} if for any compact projection $q \in B^{**}$ (cf. Definition \ref{compact_p_defn}) there is a compact projection $p \in A^{**}$ such that
...
(a) If $\xi \in \mathfrak{Ultrafilters}\left(A \right) = \left\{ p_\la \right\}_{\la \in \La}$ then 
$$
...
\end{lemma}
\begin{proof}
Follows the Proposition the Proposition \ref{top_ultra_prop},
\end{proof}
