...
\begin{definition}\label{gelfand_space_defn}
	...
	Aa \in \xi.
	$$
	The \textit{(wild) Gelfand space} $\mathfrak{Gelfand}\left(A \right)$ of $C^*$-algebra $A$ is a space  of finite points with the final topology (cf. Definition \ref{top_final_defn}) given by the natural inclusion $\mathfrak{wild}-\mathfrak{Gelfand}\left(A \right) \hookto \mathfrak{Ultrafilters}\left(A \right)$.
\end{definition}
\begin{remark}\label{loc_compact_rem}
	From the natural inclusion $\mathfrak{wild}-\mathfrak{Gelfand}\left(A \right) \hookto \mathfrak{Ultrafilters}\left(A \right)$ and the Remark \ref{compact_rem} it follows that the  Gelfand  space $\mathfrak{Gelfand}\left(A \right)$ is locally compact and Hausdorff. The Gelfand  space $\mathfrak{Gelfand}\left(A \right)$ is compact if and only is the $C^*$-algebra $A$ is unital. 
\end{remark}
...
\begin{lemma}\label{lem_g_lem} (Generalized commutative Gelfand theorem).
	If $\sX$ is a locally compact (cf. Definition \ref{top_locally_compact_defn}), Hausdorff space then  is a natural  homeomorphism $ \mathfrak{wild}=\mathfrak{Gelfand}_\sX: \mathfrak{wild}-\mathfrak{Gelfand}\left(C_0\left( \sX\right) \right)\cong \sX$, where  $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right)$ is the Gelfand space.
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{exercise}
Prove that the wild Gelfand space of $C^*$-algebra $\K$ of compact operators is a complex projective spacer $\\C P^\infty$ with discrete topology.
\end{exercise}
\paragraph*{}
...
\end{proof}
	...
	\begin{definition}\label{gelfand_functor_defn}
		The Theorem \ref{good_thm} yields  a contravariant {\it wild Gelfand functor} 
	...
	\end{definition}
		 ...
		 \begin{empt}\label{alg_top_equivalence_empt}
		If $e \in \mathbb{M}_n\left(A^+ \right)$ is an idempotent then one has the natural disjoint union of open sets
		\be\label{matrppbu_eqn}
		\begin{split}
			\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A^+ \right)\right)=\\= \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A^+ \right)\right)_{\mathbb{M}_n\left(A \right)e}\bigsqcup \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right)_{\mathbb{M}_n\left(A^+ \right)\left(1_{\mathbb{M}_n\left(A^+ \right)} -e\right) }
		\end{split}
		\ee
		(cf. notation \ref{topology_eqn}). From the Shilov Idempotent Theorem (cf. Appendix \ref{commutative_k_empt})  there is $e_{\mathfrak{Gelfand}} \in  C_c\left( \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A^+ \right)\right), \Z\right)$  such that 
		\be\label{matrppbuy_eqn}
		\begin{split}
			\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A^+ \right)\right)_{\mathbb{M}_n\left(A \right)e}= \left\{\left. x\in \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A^+ \right)\right) \right|e_{\mathfrak{Gelfand}}\left( x\right)  = 1\right\},\\
			\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right)_{\mathbb{M}_n\left(A^+ \right)\left(1_{\mathbb{M}_n\left(A^+ \right)} -e\right) }=\\=\left\{\left. x\in \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A^+ \right)\right) \right|e_{\mathfrak{Gelfand}}\left( x\right)  = 0\right\}
		\end{split}
		\ee
		...
		\end{split}
		\eean
		where $L$ and $L'$ are corresponding to $\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right)_{\mathbb{M}_n\left(A \right)e}$ and\\ $\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right)_{\mathbb{M}_n\left(A \right)\left(1_{\mathbb{M}_n\left(A \right)} -e\right) }$ ideals. Conversely any  idempotent
		\bean
		\begin{split}
			\mathbb{M}_n\left(C\left(  \mathfrak{Gelfand}\left( A^+\right) \right) \right)= L \oplus L'
		\end{split}
		...
		Moreover $p$ is the image (cf. Definition \ref{image_defn}) of $p_{ \mathfrak{Gelfand}\left( A\right) }$, i.e.
		$$
		...
		If $S$ is a $C^*$-algebra then the group $\mathrm{Int}\left(A \right)$ of the internal automorphisms acts on  $\mathfrak{wild}$-$\mathfrak{Gelfand}\left(A^+ \right)$
	\end{empt}
	\begin{definition}
		The \textit{Gelfand space} of $A$ is the quotient space $\mathfrak{wild}$-$\mathfrak{Gelfand}\left(A \right)/\mathrm{Int}\left(A \right)$. 
	\end{definition}
...
\mathrm{where}~~\mathfrak{Gelfand}\left(A \right)_j\cong \mathfrak{Gelfand}\left(A \right)~~ \mathrm{as ~left~} A\mathrm{-module}\quad \forall j=1,..., n.
\end{split}
\ee
\end{lemma}
\begin{proof}
...
\begin{remark}\label{projection_equivalences_rem}\cite{wegge_olsen}
	In \cite{wegge_olsen} it is proven that 	the given by the Definitions \ref{projection_equivalences_defn}  and \ref{projection_equivalence_defn}equivalence relations of 
\end{remark}
...
%\end{definition}
	...
	\subsection{$K$-theory}\label{ca_k_sec}
...
\begin{definition}\label{proj_v_defn}\cite{blackadar:ko}
	%Definition 5.1.2. 
	$V(A)$ is the set of equivalence classes of idempotents
	in $\mathbb{M}_\infty\left(A \right)$ (cf. Definition \ref{projection_equivalence_defn}). 
\end{definition}
...
\end{definition}
\begin{remark}\label{projection_equivalences_rem}\cite{wegge_olsen}
In \cite{wegge_olsen} it is proven that 	the given by the Definitions \ref{projection_equivalences_defn}  and \ref{projection_equivalence_defn}equivalence relations of are equivalent.
\end{remark}
...
using Bott periodicity. There are two earlier related theorems, the Shilov Idempotent Theorem, which states that every idempotent in $C_0(\sX)$ is in the image of $\ga$, and hence that the group of integer
...
and the Arens-Royden Theorem, which states that  induces an isomorphism of
	$GL_1\left( A\right) / GL_1\left( A\right)_0$ with $GL_1\left(  C_0\left(\sX \right)\right) / GL_1\left( C_0\left(\sX \right)\right)_0\cong H^1_c\left( \sX, \Z\right)$.
