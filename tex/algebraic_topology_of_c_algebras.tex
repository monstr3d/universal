...
%A^\sim\bydef\begin{cases}
	...
	\paragraph{}
	If $p \in \mathbb{M}_n\left(A^+ \right)$ is a projection (resp. unitary $u \in \mathbb{M}_n\left(A^+ \right)$ ) then from the Lemma \ref{image_defn} it follows that there is the unique projector $p_{ \mathfrak{Gelfand}\left(A \right) }\in \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A^+ \right) \right)$ ((resp. unitary $u \in p_{ \mathfrak{Gelfand}\left(A \right) }\in \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A^+ \right) \right)$
	such that 
	\be
	\begin{split}
	...
	\end{split}
	\ee 
	Taking into account the equations \eqref{stab_k1_eqn} and \eqref{royden_eqn} one gas
	\be\label{k_1_h_eqn}
	\begin{split}
	...
	If $A$ is a (nonstable) $C^*$-algebra then any closed left ideal $L$ of $A$ yields a closed left ideal $\K_L$ generated by 
	\be\label{inf_mp_eqn}
	\begin{split}
		\begin{pmatrix}
			L& \ldots & 0 &  \dots \\
	...
	\end{split}
	\ee
	The map $L \mapsto \K_L$ yields an injective  continuous map
	\be\label{k_a_eqn}
	\varphi_{\K_A}: \mathfrak{Gelfand}\left(A \right)\hookto \mathfrak{Gelfand}\left(A \otimes \K\right)
	\ee
	In general the  initial  topology of $\mathfrak{Gelfand}\left(A \right)$ with respect to  $\varphi_{\K_A}$ (cf. Definition \ref{top_init_defn}) is finer that the topology of $\mathfrak{Gelfand}\left(A \right)$.
	
	
	\begin{definition}
		The \textit{stable Gelfand space} $\mathfrak{Gelfand}\left(A \right)^s$ of $C^*$-algebra $A$ is a set $\mathfrak{Gelfand}\left(A \right)$ supplied with the initial topology with respect to the map \eqref{k_a_eqn}. There is a \textit{stable group} $U^s\left(A^+ \right)$ acting on the set $\mathrm{Pr}\left( C\left(\mathfrak{Gelfand}\left(A \right)^s \right) \right)$ of projections which is the restriction of $U\left(\left(A \otimes K \right)^+ \right)$ on $\mathfrak{Gelfand}\left(\left(A \otimes K \right)^+\right)$
	\end{definition}
	From our construction it follows that there is a disconnected union
	\be\label{disconn_eqn}
	\begin{split}
		\mathfrak{Gelfand}\left(\left(A \otimes K \right)^+\right) = \coprod_{n =1}^\infty \mathfrak{Gelfand}\left(A^+ \right)^s_n,\\
				\forall n \in \N\quad  \mathfrak{Gelfand}\left(A^+ \right)^s_n\cong  \mathfrak{Gelfand}\left(A^+ \right)^s_n
	\end{split}
	\ee
	Taking into account that $U\left(\left(A \otimes K \right)^+ \right)$ contains all transpositions of the components of the union \eqref{disconn_eqn} from the equations \eqref{k_00_h_eqn} and \eqref{k_1_h_eqn} it follows that
	\be\label{k_00_1_eqn}
	\begin{split}
		K_{00}\left(A^+ \right) \cong H^0_c\left(\mathfrak{Gelfand}\left(A^+ \right)^s; \Z \right)/U^s\left(A^+ \right),\\
		K_{1}\left(A \right) \cong H^1_c\left(\mathfrak{Gelfand}\left(A^+ \right)^s; \Z \right).
	\end{split}
	\ee
	\begin{example}
		Let $u \in C\left( S^1\right)$ be an unitary  generator i.e. $K^1\left(S^1 \right)\cong K_1\left(C\left(S^1 \right)  \right)  \cong \Z \left[u\right]$. According to \cite{blackadar:ko} the matrix 
			\be
		\begin{split}
				u_{\mathbb{M}_2\left(S^1 \right) } \bydef \begin{pmatrix}
				u & 0\\
				0 & u^*
				\end{pmatrix}  
		\end{split}
		\ee
		is contractible in $U\left( \mathbb{M}_2\left(S^1 \right)\right)$. It follows that 	$u_{\mathbb{M}_2\left(S^1 \right) }$ can be extended up to $u_{\mathbb{M}_2\left(S^2 \right) }\in U\left( {\mathbb{M}_2\left(S^2 \right) }\right)$ where the equator inclusion $S^1 \hookto S^2$ is implied. 
		It is proven in \cite{blackadar:ko} that
			\be
	\begin{split}
	p_{\mathbb{M}_2\left(S^2 \right) }\bydef u_{\mathbb{M}_2\left(S^2 \right) }\begin{pmatrix}
		1_{C\left( S^2\right) } & 0\\
		0 & 0
	\end{pmatrix}   u^*_{\mathbb{M}_2\left(S^2 \right) }
	\end{split}
	\ee
	is a projector which is not injective. So there is a nonempty space 
			\be
\begin{split}
	E_p\bydef \left\{\left.\left( x, y\right)\in C\left(S^2 \right) \right| 	p_{\mathbb{M}_2\left(S^2 \right) }\left(x, y \right) = 0 \right\}
\end{split}
\ee
Thee is an ideal 
			...
			\be
\begin{split}
I_p\bydef \left\{\left.x \in C\left(S^2 \right)  \right| 	\left( x, y\right)\in E_p \right\}\subset C\left(S^1 \right) 
\end{split}
\ee
and  open sets 
...
\be
\begin{split}
	U^x_p \bydef \left\{\left.s \in  S^2  \right| \exists x \in I_p \quad x\left( s\right) \neq 0 \right\}\subset S^2,\\
		U^y_p \bydef \left\{\left.s \in  S^2  \right| \exists y \in I_p \quad y\left( s\right) \neq 0 \right\}\subset S^2
	\end{split}
\ee
From the Lemma \ref{lem_g_lem} it follows that $S^2\cong \mathfrak{Gelfand}\left( C\left( S^2\right) \right)$. So both $\sU^x_p$ and  $\sU^y_p$ is are open subsets of  $\mathfrak{Gelfand}\left( C\left( S^2\right) \right)$. However both sets   $S^2\setminus \sU^x_p$ and $S^2\setminus \sU^y_p$ is not open in $\mathfrak{Gelfand}\left( C\left( S^2\right) \right)$. But $S^2\setminus \sU^x_p$ ans $S^2\setminus \sU^y_p$ is open in $\mathfrak{Gelfand}\left( C\left( S^2\right) \right)^s$. If the projector $p_{\mathbb{M}_2\left(S^2 \right) }$ represents $\left[p_{\mathbb{M}_2\left(S^2 \right) }\right]\in K_0\left(C\left( S^2\right)  \right)$. There are two continuous maps $f_x, f_y : \mathfrak{Gelfand}\left( C\left( S^2\right) \right)^s \to \Z$
\be
\begin{split}
f_x \left( s\right) = \begin{cases}
1 & s \in \sU^x_p
\end{cases}
\end{split}
\ee
