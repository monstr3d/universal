...
Let $A$ be a {\bf stable} $C^*$-algebra and $\sX \bydef \mathfrak{Gelfand}_r\left(A\right)$. Since $A$ is stable for all $n', n'' \in \N$ there are $*$-isomorphisms 
\be\label{s_m_eqn}
\begin{split}
...
then there are unitary  $u', u'' \in U\left( \left( \mathbb{M}_{n'}\left( A\right)\otimes  \mathbb{M}_{n''}\left( A\right)\otimes C_0\left( \R^{r'}\right)\otimes C_0\left( \R^{r''}\right)^+\right) \right)$ such that
 ...
 \end{split}
 \ee
From the Lemma \ref{product_lem}, the isomorphisms  \eqref{is_m_eqn}  and the equation \eqref{dl_sh_eqn} the above unitary elements can be regarded as representatives of homology groups, i.e.
\be\label{h1is_m_eqn}
\begin{split}
	\left[v'\right]\in H^1_c\left(\mathfrak{Gelfand}_r\left(A\right) \times \R^{r'}; \Z \right),\\ 
	\left[v''\right]\in H^1_c\left(\mathfrak{Gelfand}_r\left(A\right)  \times \R^{r''}; \Z \right),\\ 
	\left[u'\right]\in H^1_c\left(\mathfrak{Gelfand}_r\left(A\right)  \times \R^{r'+ r''}; \Z \right),\\ 
	\left[u''\right]\in H^1_c\left(\mathfrak{Gelfand}_r\left(A\right)  \times \R^{r'+ r''}; \Z \right),\\ 
\end{split}
\ee
From the Theorem \ref{cup_sheaf_thm} there is a cup product
\be\label{h1isc_m_eqn}
\begin{split}
	\left[u'\right]\smile \left[u''\right]\in H^2_c\left(\mathfrak{Gelfand}_r\left(A\right) \times \R^{r'+ r''}; \Z\otimes \Z \right)\cong \\ \cong H^2_c\left(\mathfrak{Gelfand}_r\left(A\right) \times \R^{r'+ r''}; \Z \right).
\end{split}
\ee 
...
\begin{split}
	...
	\left[v''\right]_K \in K_{r''+1~ \mathrm{mod}~2 }\left(A \right),\\ 
		\left[\phi^2_1\left(	\left[u'\right]\smile \left[u''\right] \right) \right]_K  \in K_{r' + r'' +1 - 1~ \mathrm{mod}~2 }\left(A \right)
\end{split}
\ee
...
\left[v'\right]_K \smile	\left[v''\right]_K\bydef 	\left[\phi^2_1\left(	\left[u'\right]\smile \left[u''\right] \right) \right]_K
\ee
