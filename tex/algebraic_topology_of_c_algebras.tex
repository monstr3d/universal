...
C \mapsto C \cap ~_\sV A _\sV.
		\end{split}
		\ee
		and let $\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)$ be the \'espace etal\'e of the presheaf $\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)$ (cf. Exercise \ref{sheaf_etale_exer}) with the natural local homeomorphism $p_{\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)} : \mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right) \onto \sX$. Let $\mathfrak{Comm}\left( A\right)$ be a set of all commutative $C^*$-subalgebras of $A$.
...
\ee
where $C_x$ is a stalk of $C$ (cf. Definition \ref{sheaf_stalk_defn}), For any open subset $\mathscr U\subset \mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)$ we define a hereditary $C^*$-subalgebra $B_{\mathscr U } \subset A$ which contains the union
\be
\bigcup \left\{C \in \mathfrak{Comm}\left( A\right)\left| \forall x \in \sX  \quad \rep_x\left(C \right)\neq 0 \quad \Rightarrow \quad  C_x \in  \mathscr U\right.\right\}
\ee
\begin{lemma}
One has.
\begin{enumerate}
...
(a)
	...
	(b) If both  $\mathscr U', \mathscr U'' \subset \mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A}\right)$ are open subsets with $\mathscr U'\setminus \mathscr U''\neq \emptyset$ then since $\sX$ is Hausdorff and  the map $\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)\su \sX$ is local homeomorphism one has an open subset $\mathscr V \subset \mathscr U'$ with $\mathscr V \cap \mathscr U'' = \emptyset$. Since the set $\mathscr V$ is open there is $C \in \mathfrak{Comm}\left( A\right)$ with
	$$
	\forall x \in \sX  \quad \rep_x\left(C \right)\neq 0 \quad \Rightarrow \quad  C_x \in  \mathscr V.
	$$
	From our construction it turns out that $C \subset B'$ and $C \cap B'' = \{0\}$, i.e. the map \eqref{u_t_b_eqn} is injective.


(b)	If $B', B'' \subset A$ be $C^*$-subalgebras such that $B'\setminus B'' \neq \{0\}$ then there is $C\in \mathfrak{Comm}\left( A\right)$ such that $C \subset B"$ and $C\cap B'' = \{0\}$. So the map \eqref{b_t_u_eqn} is injective
\end{proof}
...
\end{lemma}	
\begin{proof}
 Let $\xi \in 	\mathfrak{Gelfand}\left( A\right)$,  $\quad x\bydef p_{\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A}  \right)}\left( \xi\right)  \in \sX$, and
\bean
...
\eean
where $C$ is a commutative $C^*$-subalgebra which represents $\zeta$ and $C_x$ is the germ of $C$ at $x$ (cf. Definition \ref{sheaf_stalk_defn}). According to the Definition \ref{gelfand_space_defn} there is  positive n element $a \in K\left( A\right)_+$ of the Pedersen's ideal such that there is  $aAa \in \xi$ (cf. notation \eqref{hered_ideal_eqn}). Let $\rep_x : A \to B\left( \sH_x\right)$ be the corresponding to  irreducible representation of $A$ (cf. \ref{irred_defn}) then from the equation  \eqref{peder_k_eqn} it follows that there is $n \in \N$ such that $\rep_x \left(aAa \right) \cong \mathbb{M}_n\left(\C\right)$. If 
\bean
\bigcap_{\substack{B \in \xi\\\rep_x\left( B\right)  \subset aAa}} B = \{0\}
\eean  
then there is a finite set $B_1, ..., B_m \in \xi$ with
$$
\rep_x\left(B_1\cap ...\cap B_m\right)= \{0\}
$$
Using the above equation one can find $B'_1, ..., B'_m \in \xi$ with $B'_1\cap ...\cap B'_m = \{0\}$. It is impossible since $\xi$ is a filter (cf. Definition \ref{filter_defn}).
It turns out that there is $k \in \N$ with
$$
\rep_x\left( \bigcap_{\substack{B \in \xi\\\rep_x\left( B\right)  \subset aAa}} B\right)  =\rep_x\left( \bigcap_{B \in \xi} B\right) = \mathbb{M}_k\left(\C\right)
$$
From the Proposition \ref{less_n_pi_prop} one can deduce that there is an open neighborhood $\sU$ of $x$ such that there is the natural inclusion $$C_0\left(\sU \right) \otimes \mathbb{M}_k\left(\C\right)\subset \bigcap_{B \in \xi} B.$$ There is a commutative $C^*$-subalgebra $C \subset  \bigcap_{B \in \xi} B$  given by a system inclusions
$$
C_0\left(\sU \right) \otimes\begin{pmatrix}
1& 0 & \ldots &0  \\
	0&  0 & \ldots& 0 \\
	\vdots& \vdots &  \ddots & \vdots\\
	0& 0 &\ldots &0 \\
\end{pmatrix}\subset C_0\left(\sU \right) \otimes \mathbb{M}_k\left(\C\right)\subset  \bigcap_{B \in \xi} B
$$
If a filter $\xi'$ is generated by the set
$$
 \left\{B \cap C | B \in \xi \right\}
$$
then $\xi \subsetneqq \xi'$. It is impossible since $\xi$ is an ultrafilter. It follows that $\rep_x\left( \bigcap_{B \in \xi} B\right) = \mathbb{M}_k\left(\C\right)= \C$. Let $\left\{\sU\right\}$ be a basis of neighborhoods of $x$. Let $S \subset \N$ be such that 
$$
\forall l \in S \quad \exists \sU \in \left\{\sU\right\}\quad \exists B \in \xi\quad  \forall x \in \sU \quad \quad \Rightarrow \quad \rep_x\left(B\right) \subset \mathbb{M}_l\left(\C\right)
$$
Let  $d \bydef \min_{l \in S}l > 1$ and let $\sU\in  \left\{\sU\right\}$, $~B \in \xi$ be such that
$$
\forall x \in \sU  \quad \rep_x\left(B \right)\subset  \mathbb{M}_d\left(\C\right)
$$
From the Proposition \ref{less_n_pi_prop} it follows that there is an open subset $\sV \subset \sU$ such that 
$$
\forall x \in \sV\quad  \rep_x\left(B\right) \cong  \mathbb{M}_d\left(\C\right).
$$
Moreover there is $\sU' \in \left\{\sU\right\}$ such that there is the the natural inclusion 
$$
C_0\left(\sU' \cap \sV \right) \otimes \mathbb{M}_d\left(\C\right)\subset B
$$
There is a commutative $C^*$-subalgebra $C' \subset  B$  given by a system inclusions
$$
C_0\left(\sU \right) \otimes\begin{pmatrix}
	1& 0 & \ldots &0  \\
	0&  0 & \ldots& 0 \\
	\vdots& \vdots &  \ddots & \vdots\\
	0& 0 &\ldots &0 \\
\end{pmatrix}\subset C_0\left(\sU' \cap \sV \right) \otimes \mathbb{M}_d\left(\C\right)\subset   B
$$
If   
$$
B' \bydef \left\{b \in B \left| \forall x \in \sU' \cap \sV  \quad \rep_x\left(b \right) \in \rep_x\left(C' \right) \right.\right\}
$$ 
then 
$$
\forall x \in \sU' \quad  \rep_x\left(B' \right)\subsetneqq \mathbb{M}_d\left(\C\right).
$$
If a filter $\xi'$ is generated by the set
$$
...
$$
and $C'' \bydef B'' \cap ~_{\sU''} A_{\sU''}\subset A$ is a commutative $C^*$-algebra.
...
!!!
 Let us prove that $\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)_\xi\neq \emptyset$. Since $p_{\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A}  \right)}$ is a local homeomorphism for any $\zeta \in mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A}  \right)$ there is neighborhood. 

If $\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)_\xi$ contains more than !!!! EXISTS  one element and $\zeta \in \mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)_\xi$ then the  filter of hereditary $C^*$-subalgebras
\bean
...
\forall x \in \sV \quad f\left(x \right) = \frac{a'}{c}
\eean 
 such that
 $\frac{a'}{c}$ means the division of collinear rank-one operators and
\bean
...
If $C_\sU \bydef C \cap~_\sU A _\sU$ and 
  $L_\sU\in \xi $ is a closed left ideal with $C_\sU= L_\sU \cap L^*_\sU$ (cf. Lemma \ref{hered_ideal_lem}), $a_\sU \bydef a'' + f_\sU c$ then
\bean
