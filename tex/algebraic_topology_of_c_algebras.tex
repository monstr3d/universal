...
\begin{empt}
Let $A$ be a $C^*$-algebra, and let $\sY= \hat A$ be the spectrum of $A$ (cf. Definition \ref{spectrum_prime_primtive_defn}). For any closed left ideal there is a set
\bean
\sY_L \bydef \left\{y \in \sY | \rho_y\left(L \right) \neq 0\right\}.
\eean
where $\rho_y$ is the corresponding to irreducible representation.
...
$$
where $\C P_{\mathfrak{Hausdorff}}\left(\H \right) $ is the projective  space with   well known Hausdorff  topology and  $\C P\left(\H \right)_{\mathfrak{Gelfand}}$ is the same set  with the given by Definition  \eqref{ultrafilters_space_defn} topology.
...
\end{example}
\begin{remark}
In the example \ref{foli_exm} the $C^*$-algebra the Gelfand space and the map $\mathfrak{Blowing}$ enable us define the $C^*$-algebra $C^*_r\left(M, \F \right)$.
\end{remark}
\begin{remark}
...
\end{lemma}	
\begin{proof} 
If $\xi \in 	\mathfrak{Gelfand}\left( A\right)$ and $x = \/ \phi\left( \xi\right)\in  \sX$ (cf. the equation \eqref{ctr_g_eqn}) then from the the Lemma  \ref{ctr_g_lem} it follows that there is a pair $\left(x_\xi, V_\xi \right)$ such that
$$
...
\eean  where $\C P\left(\E \right)_{\mathfrak{Hausdorff}}$ is the set $\C P\left(\E \right)$ having well known Hausdorff topology. Indeed the topology of $\C P\left(\E \right)_{\mathfrak{Gelfand}}$ is final (cf. Definition \ref{top_final_defn}) with respect to the surjective  map 
\bean
...
Under the hypothesis \ref{ctr_coh_empt} the homomorphism \eqref{h_ctr_eqn} is an isomorphism.
...
\end{proof}
\begin{remark}
Using the Theorem \ref{ctr_thm} one can calculate  the Dixmier Douady class of $A$ which defines $A$ up to isomorphism. So $A$ defines a Gelfand space and vice-versa.
\end{remark}
