...
subcategory of fibrations which assigns to 6 the principal fibration induced by $\th$. 
...
\bean
\th\left(f \right) : \left( A \times L \cup X \times \dot I, \sX \times 0 \right) \to \left(B', b'_0 \right)  
\eean
...
!!! %	i map  $g: A \times I \cup \left(I, \dot I \right)\to B'$  it follows from Theorem \ref{top_obstr_thm} that $g$ can be 
!!! %	 extended over $\sX \times I$ if and only if $\dl g^*\left( \iota\right)= 0$  in $H^{n+1}\left(\left(\sX ,A \right) \left(I \dot I \right) ; \pi \right)$ 
In particular, given a map pair $f: j \to p_\th$, there is a lifting of $f$ !!!%if and only if $\dl \th \left(f \right) \left( \.\iota\right)$ .
!!!
% \begin{defn}\cite{spanier:at}
	%  The obstruction to lifting f denoted by c(f) ? Hn(X,A; w), 
	%( is defined by 
	%| «?(/)*(0 = (-i)Mc(/)) 
	!!!%where t: Hn(X,A; 7r) ^ H«+1((X,A) X (IJ); w) is the map t(«) = u X I,  
	%defined in Sec. 8.1 [I ? W{lj; Z) is the generator such that if 0 ? H°({0}; Z) 
		%	and 1 ? H°({1); Z) are the respective unit integral cohomology classes, then, 
			!!!%		identifying H°(jf;Z) =; H°({0};Z) 0 JF({1};Z), we have Si = J = -SO]. 
			a!!!		
			%\end{defn}
