...
%A^\sim\bydef\begin{cases}
	...
	\begin{theorem}\label{good_thm}
		\begin{enumerate}
		If the  homomorphism $\varphi : M\left( A\right) \to M\left( \widetilde{A}\right)$ good 	  then the map \eqref{gelfand_map_eqn} is continuous (cf. Definition \ref{top_continuous_defn}).
		\end{enumerate}
		\end{theorem}
	\begin{proof}
	...
	\paragraph{} Here we find a bijective continuous map from the Gelfand space of noncommutative torus to the commutative one.
	\begin{definition}\label{connected_case}{\it Noncommutative torus}. 
		A noncommutative torus \cite{varilly:nc_intro} $C\left(\T^2 _{\theta}\right) $ is the universal  $C^*$-algebra generated by two unitary elements $u, v$ which satisfy following conditions
		\begin{equation}\nonumber
			uu^*=u^*u=vv^*=v^*v=1;\\
		\end{equation}
		\begin{equation}\nonumber
			uv=e^{2\pi i \theta}vu
		\end{equation}
		where $\theta \in \R\setminus\Q$.
		There is a trace $\tau_0$ on  $A_{\theta}$ such that $\tau_0 (\sum_{-\infty < i < \infty, -\infty < j<\infty}a_{ij}u^iv^j) = a_{00}$. $C^*$ - norm of $A_{\theta}$  is defined by following way $\|a\|=\sqrt{\tau_0 (a^*a)}$. 
	\end{definition}
