...
Let $L_1, L_2 \in A$ be two closed left ideals of $C^*$-algebra $A$ such that $L_1 \cap L^*_2 \neq \{0\}$. If $a\in L_1 \cap L^*_2 \setminus \{0\}$ then $a^*\in L_2 \cap L^*_1 \setminus \{0\}$. One has $aa^* > 0$ and $aa^*=\left( aa^*\right)^*\in L_2\cap L^*_1$
		\be\label{aa_eqn} 
\begin{split}
	aa^* \in L_1\cap L^*_1\setminus\{0\},\\
	aa^*= \left( aa^*\right)^* \in L_2\cap L^*_1\setminus\{0\},\\
...
\subsection{Standard Exact Sequence}
...
\begin{proof}
	...
	\end{tikzcd}
	\ee
	such that horizontal  arrows are grading preserved ring homomorphisms. Using the Remark \ref{cup_prod_suspernsion_rem} the diagram \eqref{thoh_xexs_eqn} can be replaced with the following one
...
If we consider the given by \eqref{nt_repr_eqn} representation 
\begin{equation}\nonumber
...
The maps 
\bean
\{0\}\hookto	K_0\left( C\left(\T^n_\th \right)\right) \xrightarrow{\iota_*} K_0\left( C\left(\T^n_\th \right)  \rtimes_\a \Z \right),  \\ 
\{0\}\hookto	K_1\left( C\left(\T^n_\th \right)\right) \xrightarrow{\iota_*} K_1\left( C\left(\T^n_\th \right)  \rtimes_\a \Z \right) 
\eean
yield the injective  ring homomorphism 
\be\label{tor_geyn_eqn}
\begin{split}
	K_*\left( C\left(\T^n_\th \right)\right)\hookto K_*\left( C\left(\T^{n}_\th \right)\right).
	\end{split}
\ee	
which comes from the inclusion $ C\left(\T^n_\th \right)\hookto  C\left(\T^{n}_\th \right)$. From \eqref{kkt_0_nt_eqn} and \eqref{kkt_1_nt_eqn} one can obtain following isomorphisms 
\be\label{tor_geyyn_eqn}
\begin{split}
 K_0\left( C\left(\T^{n}_\th \right)  \rtimes_\a \Z \right)/K_0\left( C\left(\T^{n}_\th \right)\right) \cong  K_1\left( C\left(\T^n_\th \right)\right),\\
  K_1\left( C\left(\T^{b + 1}_\th \right)  \rtimes_\a \Z \right)/K_1\left( C\left(\T^n_\th \right)\right) \cong  K_0\left( C\left(\T^n_\th \right)\right)
\end{split}
\ee
Using the Remark \ref{cup_prod_suspernsion_rem} one can proof that the equation \eqref{tor_geyyn_eqn} yields the ring homomorphism
\be\label{tor_geyynr_eqn}
\begin{split}
	K_*\left( C\left(\T^{n}_\th \right)  \rtimes_\a \Z \right)/K_0\left( C\left(\T^n_\th \right)\right) \cong  K_*\left(S C\left(\T^n_\th \right)\right),\\
\end{split}
\ee
