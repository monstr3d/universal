...
\begin{definition}\label{chern_defn}
	...
	The \textit{$K$-theoretical} (resp. \text{cohomological}) {\it Chern classes} $c^K_j\left(\sV \right) \in K\left(\sX^+ \right)$   (resp. $c^H_j\left(\sV \right)\in H^{2j}\left(\sX^+, \Z \right)$) of $\sV$ are {Chern classes} $c^K_j\left(\sV_\sX \right)$ (resp. $c^H_j\left(\sV_\sX \right)$) of $\sV_\sX$ (cf. Definition  \ref{chern_class_defn}), i.e.
...
\eean
	...
	\begin{proposition}\label{pop3_prop}
		...
		\begin{enumerate}
			...
			\ee
			o\`u $\K\left( \E^\sU_T \right)$ est l'alg\`ebre des opérateurs compacts de $\E^\sU_T$.
	...
	\begin{lemma}\label{l4_lem}
		...
		$\ell$, $C \cap \ell$. soit négligeable pour la classe de mesure de Lebesgue longitudinale.
		Alors, pour tout ouvert 0 de $M$ et toute sous-vari\^et\'e
		$\sV$ transverse à $\F$, l'application naturelle $\E^{0\setminus C}_\sU\to \E^0_\sU$ est un
		isomorphisme de $C^*$-modules hilbertiens,
	\end{lemma}
	\begin{remark}\label{scandalis_rem}\cite{hilsum_scandalis:stab}
		D'après le lemme \ref{l4_lem} 
		...
		et donc $\E^M_T\cong \K \otimes C^*_r\left( G^T_T \right)$  en vertu du théorème de stabilisation
...
\paragraph{} Here I follow to \cite{gelfand_manin}.
\subsection{Nerve of Covering}

Let $\sY$ be a topological space, $\sU \bydef \left\{\sU_\a\right\}_{\a \in \mathscr A}$ be its covering (either by open or by 
closed sets), a running over some set $\mathscr A$. Let 
\be
\begin{split}
	X_n \bydef \left\{\left({\a_0}, ..., {\a_n} \right)  \left|\sU_{\a_0}\cap, ...,\cap \sU_{\a_n} \neq \emptyset  \right.\right\}\\
	Xf\left({\a_0}, ..., {\a_n} \right)\bydef  \left(\a_{f(0)}, ...,\a_{f(n)} \right)\quad \text{for} \quad f : \left[m\right]\to \left[n\right]	
	\end{split}
		\ee
			This simplicial set reflects the combinatorial structure of a covering. One 
		can show that if the covering $\sU$ is locally finite and all nonempty finite intersections $ \sU_{\a_0}\cap, ...,\cap \sU_{\a_n}$ are contractible, then the geometric realization $\left|X\right|$
		of $X$ is homotopically equivalent to $Y$, so that the topology can be efficiently 
		encoded into combinatorial data. 
		The similar role is played by the following construction.
...
$$
A more canonical way to de¯ne the (equivalence class of the) above idempotent $e \in \Coo_c\left(r^{-1} \left( N\right), s^*\left( \Om^{1/2}\right) \right) $
