...
A \arrow[r, "-\otimes p_1"] 	& A \otimes \K \arrow[d, "\varphi^2"]\\
	C\left( \sX^+\right) \arrow[r, "\varphi^1"]	& M\left( A\right) \otimes \K \\
	 \mathrm{Proj}^{\mathrm{nat}}\left(A \right) \arrow[r, "\subset"] \arrow[d, "\varphi_*"]&\mathrm{Proj}\left(A \right) \arrow[r, "\otimes p_1"]& \mathrm{Proj}\left( A\otimes \K\right)   \arrow[d, "\varphi^2_*"]\\
...
where 
\bean
 \mathrm{Proj}^{\mathrm{nat}}\left(A \right)\bydef \left\{x \in  \mathrm{Proj}\left(A \right)\left|\exists y \in  	\mathrm{Proj}\left( C\left( \sX^+\right)\right)\right.\right\}
\eean
...
%Proposition 7.1.5.
\bean
K_1\left(A \right) ) = \varinjlim% \left(\mathrm{GL}^+_n\left(A \right)= \varprojlim \left(U^+_n\left(A \right)\\= \mathrm{GL}^+_n\left(A \right)_0 \right)=\left(\mathrm{GL}_n\left(A^+ \right) \mathrm{GL}_n\left(A^+ \right)_0 \right)=\varprojlim \left(U_n\left(A^+ \right)
\eean
...
is an $n \in \N$ and unitaries (invertibles) $u, v \in U^+_n\left(A \right)$ ($\mathrm{GL}^+_n\left(A \right)$) such that $u$ and $v$ are homotopic within $U^+_n\left(A \right)$, ($\mathrm{GL}^+_n\left(A \right)$).
\end{proposition}
\begin{corollary}\label{stab_k_0_cor}\cite{wegge_olsen}
	%			Corollary 6.2.11. Stability of Ko 
...
\begin{proof}
	...
	\begin{definition}\label{suspension_defn}\cite{blackadar:ko}
		...
		\cong 	\left(SA \right)^+ \cong \left\{\left.f:S^1\to A^+\right| f \text{ continuous }~, f(z)=f(1)=\la1 + x_z, ~x_z \in A, ~x_1 = 0 \in A\right\}
		\eean
	\end{definition}
	...
	\begin{proof} (Fragment)
		...
		(note that the path given in the Proposition \ref{inv_path_prop} lies in $\mathrm{GL}_{2n}\left(A \right)$. Set $e_t \bydef z_t p_n z^{-1}$.
