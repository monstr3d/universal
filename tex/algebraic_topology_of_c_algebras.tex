...
\end{split}
\ee
From \eqref{red_eqn} and \eqref{red_c_eqn} it follows that the product $f'_*\left( y'\right) \smile 	f''_*\left( y''\right)$ uniquely yields $y \in H^1_c\left( \sX\times \R^{p} \times \R^q\times \R ; \Z\right)$. But from \eqref{k0_s_eqn}, \eqref{k1_s_eqn} it follows that $y$ yiels and element $x \in K_{p + q~\mathrm{mod}~2}$. Thus one has a map
\be 
\begin{split}
\smile\\
