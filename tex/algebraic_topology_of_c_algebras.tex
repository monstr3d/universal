...
\paragraph{} Let $A$ be a ring. To define $K_0\left(A \right)$  we consider the following equivalence
relation. We say that two finitely projective $A$-modules $P$ and $Q$ are equivalent
if and only if they are isomorphic, i.e. if there is an isomorphism of
$A$-modules $P$ ?! $Q$ This is clearly an equivalence relation.
...
\begin{definition}\cite{isely:akt}
%Denition 2.1 
(Milnor) The projective module group $K_0\left(A \right)$ is the group
...
\eean
\end{definition}
\begin{proposition}\cite{isely:akt}
%Proposition 2.2 
Every element of $K_0\left(A \right)$  can be expressed by the formal
difference $\left[P\right]-\left[Q\right]$ of two generators.

\end{proposition}
\begin{definition}\cite{isely:akt}
%Denition 3.4
 Let $n \in \N$: A matrix in $GL_n\left(A \right) $ is called {\it elementary} if
...
matrices.
\end{definition}
\begin{remark}\cite{isely:akt}
Since $i_n : E_n\left( A\right) \subset E_{n+1}\left( A\right)$ we can embed $E_{n}\left( A\right)$ in $E_{n+1}\left( A\right)$ for all $n \in \N$.

\end{remark}

\begin{definition}\cite{isely:akt}
%Denition 3.5 
We define $E(A) \bydef \bigcup_{n \in \N} E_n\left( A\right)$ 
...
\bean
K_1\left( A\right)\bydef GL\left(A \right)/E\left(A \right)
\eean
\end{definition}
