...
\subsection{Pimsner-Voiculescu Exact Sequence in cohomology}
	\paragraph{} If $A$ is an unital $C^*$-algebra then  Theorems \ref{k_h_thm} and \ref{p_v_theorem} yield the following  a cyclic six-term exact sequence
	\be
	\begin{tikzcd}
	 H^0_c\left(\sX; \Z \right)/U \arrow[r, "1-\a_*"] &  H^0_c\left(\sX; \Z \right)/U\arrow[r, "\iota_*"] &H^0_c\left(\sX_\Z; \Z \right)/U_\Z \arrow[d]\\
		 H^1_c\left(\sX_\Z; \Z \right)\arrow[u] & \arrow[l, "\iota_*"]  H^1_c\left(\sX; \Z \right) & \arrow[l, "1-\a_*"]H^1_c\left(\sX; \Z \right)
	\end{tikzcd}
	\ee
	where 
	\bean
	\sX \bydef\mathfrak{Gelfand}_r\left(A \otimes \K\right),\\
	\sX_\Z \bydef\mathfrak{Gelfand}_r\left(\left( A\rtimes_\a \Z\right)  \otimes \K\right),\\
	U \bydef U\left(\left(A\otimes \K \right) ^+ \right),\\
	U_\Z \bydef U\left(\left(\left( A\rtimes_\a \Z\right)\otimes \K \right) ^+ \right)
	\eean
		\begin{theorem}\label{k_h_thm!}!!!
		If $A$ is an unital $C^*$-algebra then one has
		\bea\label{k_00_eqn}
		K_0\left( A\right) =H^0_c\left(\sX; \Z \right)/U\left(\left(A\otimes \K \right) ^+ \right)
		\eea
		There is the natural cup product on $H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)/U\left(\left(A\otimes \K \right) ^+ \right)$ which comes from the cup-product on  $H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)$.	If $A$ is any $C^*$-algebra then one has
		\bea
		\label{k_1_eqn}
