...
\begin{empt}
		...
		\end{itemize}
		where the notation \eqref{topology_eqn} is used. The set $\mathscr A$ of finite subsets  of $\la$ is a net. If for any $\a \in \mathscr A$ 
		\be
		L_\a \bydef \sum_{\la\in \a} L_\la
		\ee
		is the algebraic finite sum of closed left $A$-modules then $\left\{L_\a | \a \in \mathscr A\right\}$ is a net, so the union
		\be\label{aun_eqn} 
		\begin{split}
	...
	\begin{theorem}\label{gelfand_max_thm}
		For any unital $C^*$-algebra there is the natural continuous  map
...
\paragraph{} Let $O_N$ be the Cuntz $C^*$-algebra (cf. Definition \ref{cuntz_defn}), and let $\mathfrak{Gelfand}\left(O_N\right)$ be its Gelfand space (cf. Definition \ref{gelfand_space_defn}) . !!!
...
\begin{definition}
	%Definition 2.5.
	Fix an integer $H\ge 2$. The {\it Cantor group on N letters} is
	\be
