...
\paragraph*{}
From  the Theorem \ref{k_1_tor_thm} (resp \ref{k_1_tor_thm})  element of $K_0\left(C\left(\T^2_\th \right)  \right)$  (resp. $K_1\left(C\left(\T^2_\th \right)  \right)$) is represented by an idempotent (resp. invertible element) of $C\left(\T^2_\th \right)$ From this circumstance the given by the Theorem \ref{k_h_thm} equations 
\bean
...
It is known that $C\left(\T^2_\th \right)$ is an universal $C^*$-algebra generated by two unitaries $u, v \in C\left(\T^2_\th \right)$ such that $uv = e^{2\pi i \th}vu$. It is also known 
 that $C\left(\T^2_\th \right) \cong C\left(u \right)\rtimes_{\a_u} \Z$, where $u_{S^1} \in C\left( u_{S^1}\right) \cong C(S^1)$ is unitary,  !!!! $\a_{u_{S^1}}\in \Aut\left(C\left( S^1\right)  \right) \cong \Aut\left(C\left( u_{S^1}\right)  \right)$ is a $*$- automorphism given by $u \mapsto e^{2\pi i \th} u$.
The Theorem \ref{p_v_theorem} yields  a cyclic six-term exact sequence
	\bean
	\begin{tikzcd}
		K_0\left( C\left(u \right)\right)\arrow[r, "1-\a^u_*"] & K_0\left( C\left(u \right)\right)\arrow[r, "\iota^u_*"] & K_0\left( C\left(u \right) \rtimes_\a \Z \right) \arrow[d]\\
		K_1\left(  C\left(u \right)\rtimes \Z\right)\arrow[u] & \arrow[l, "\iota^u_*"] K_1\left( C\left(u \right) \right) & \arrow[l, "1-\a^u_*"]K_1\left( C\left(u \right)  \right)
	\end{tikzcd}
	\eean
	where the map $\iota^u_*$ corresponds to the injective $*$-homomorphism
	\be\label{iota_torus}
	\begin{split}
...
$$	
	 it follow that $\a^u_*$ is identical, $1=\a^u_*= 0$, so there are the exact sequences
		\bea\label{kk_0_nt_eqn}
\{0\}\hookto	K_0\left( C\left(u \right)\right) \xrightarrow{\iota^u_*} K_0\left( C\left(u \right)  \rtimes_\a \Z \right) \xrightarrow{p^u_*} K_1\left( C\left(u \right)\right) \onto \{0\} \\ \label{kk_1_nt_eqn}
\{0\}\hookto	K_1\left( C\left(u \right)\right) \xrightarrow{\iota^u_*} K_1\left( C\left(u \right)  \rtimes_\a \Z \right) \xrightarrow{p^u_*} K_0\left( C\left(u \right)\right) \onto \{0\}	\eea
Moreover one has
\be\label{tor_gen_eqn}
\begin{split}
	K_0\left( C\left(u \right)\right) = \Z \left[1_u\right],\\
		K_1\left( C\left(u \right)\right) = \Z \left[u\right],\\
		H^0\left( \mathfrak{Gelfand}_r\left( C\left(u \right)\right); \Z\right)\cong 	H^0\left( S^1; \Z\right)  = \Z \left[1_u\right],\\
		H^1\left(\mathfrak{Gelfand}_r\left(  C\left(u \right)\right);\Z\right)  \cong  H^1\left( S^1; \Z\right)= \Z \left[u\right],\\
	\end{split}
\ee	
If we consider the cup product (cf. Theorem \ref{cup_sheaf_thm}) then one has 
\be\label{prod_u_tor_eqn}
\left[1_u\right]\smile \left[u\right]=\left[u\right]\in 		H^1\left(\mathfrak{Gelfand}_r\left(  C\left(u \right)\right);\Z\right). 
\ee
From  the Appendix \ref{rieffel_sec} it turns out  that
\be\label{iprod_u_tor_eqn}
\iota_*\left( \left[1_u\right]\right) = \left[p_\th\right]\in K_0\left( C\left(u \right)  \rtimes_\a \Z \right)
\ee
...
\be
\left[\mathfrak{Image}\left( p_\th\right)\right]\smile \iota \left[u\right] = \iota \left[u\right]\in  H^1\left(\mathfrak{Gelfand}_r\left(  C\left(u \right)\rtimes_\a \Z\right);\Z \right).
\ee
Using the Theorem \ref{cup_prod_thm} and the Remark \ref{cup_prod_rem} we have 
\be
\left[ p_\th\right]\smile \left[u\right]= \left[u\right] \in  K_1\left( C\left(u \right)  \rtimes_\a \Z \right)=  K_1\left( C\left(\T^2_\th \right)\right)
\ee
where $\left[ p_\th\right]\in K_0\left( C\left(\T^2_\th \right)\right)$ and $\left[ u\right]\in K_1\left( C\left(\T^2_\th \right)\right)$. Using isomorphism \ref{k_0_nt_eqn} and \ref{k_1_nt_eqn} one can identify $\left[ p_\th\right]$ and $\left[ u\right]$ with their representatives in $H^0\left(\mathfrak{Gelfand}_r\left(C\left(\T^2_\th \right) \right); \Z \right)/U\left(C\left(\T^2_\th \right)  \right)$ and $H^1\left(\mathfrak{Gelfand}_r\left(C\left(\T^2_\th \right) \right); \Z \right)$.  We leave to the reader the proof of the following
