...
\begin{split}
S^2 \setminus \{x_0\}\sqcup \{x_1\} \cong SS^1 \cong\left(  0, 1 \right) \times S^1,\\
...
\bean
\psi_{\left( SS^1\right)^+}\left(\left( SS^1\right)^+ \right) \cong \Z\left[H_{\left( SS^1\right)^+ }\right] \oplus \Z\left[!_{\left( SS^1 \right)^+ }\right]  
\eean 
Using diagram chasing one can conclude that there is a rank-one bundle $H_{\left(SS^1 \right)^+ }$ on $\left(SS^1 \right)^+$ such that
\bea
...
\eea
From the inclusion $\C P^1 \subset \C P^\infty$ one can deduce that there is $\xi \in H^2\left( \C P^1 \right)\cong  H^2\left( \C P^\infty \right)$ such that  $H^2\left( \C P^\infty \right)$ is a generated by $\xi$ unital $\Z$-algebra. 



where $H$ is the canonical line bundle (cf. \ref{kt_kan_line_empt}). There is a contractible open subset $A \subset \sX$ with  $\{x_0\}\sqcup \{x_1\}\subset A$. If $B \bydef \phi_{S^2/\left( SS^1 \right)^+}\subset \left(SS \right)^+$ then $B$ is a contractible open subset.
One can prove that there are following isomorphisms 
\bea\label{cherh_h_11_eqn}
 \Z\cong H^1\left( \left(SS^1 \right)^+ \setminus \{y_0\}; \Z\right) \xrightarrow{\cong}H^1\left(B \setminus \{y_0\}; \Z \right) \cong \Z, \\\label{cherh_h_12_eqn}
 \Z^2\cong H^1\left( S^2 \setminus \{x_0\}\sqcup \{x_1\}; \Z\right) \xrightarrow{\cong}H^1\left(A \setminus \{x_0\}\sqcup \{x_1\}; \Z\right) \cong \Z^2,\\\label{cherh_h_13_eqn}
 H^2\left( B \setminus \{y_0\} \Z\right) \cong H^2\left(A \setminus \{x_0\}\sqcup \{x_1\}; \Z\right) \cong \{0\}.
\eea
where  both homomorphisms \ref{cherh_h_11_eqn} and \eqref{cherh_h_12_eqn} follow from { the exact sequence of cohomology groups} (cf. equation \eqref{exact_coh_eqn}). The following diagram 
\bean
\begin{tikzcd}
	0\arrow[r]&H^2\left(\left(SS^1 \right)^+\setminus\{y_0\}, B\setminus \{y_0\};\Z  \right) \arrow[r]\arrow[dd, "\cong"]&H^2\left(\left(SS^1 \right)^+\setminus\{y_0\};\Z  \right) 
	\arrow[r] \arrow[dd, "H^2\left( \Id_{\left( SS^1\right)^* }; \Z\right) "]& 0\\
	~&~&~&\\
	0\arrow[r]&H^2\left(S^2\setminus\{x_0\}\sqcup \{x_1\}, A\setminus \{x_0\}\sqcup \{x_1\};\Z  \right)\arrow[r] &\arrow[r]H^2\left(S^2\setminus\{x_0\}\sqcup \{x_1\};\Z  \right) 
	& 0
\end{tikzcd}
\eean
is a consequence of \eqref{cherh_h_11_eqn}-  \eqref{cherh_h_13_eqn} and { the exact sequence of cohomology groups} (cf. equation \eqref{exact_coh_eqn}). The homomorphism $H^2\left( \Id_{\left( SS^1\right)^* }; \Z\right)$ is isomorphism, so from the 5 - lemma it turns out that there is the natural  isomorphism 
\be\label{chern_sss_eqn}
H^2\left(\left(SS^1 \right)^+\setminus\{y_0\}, B\setminus \{y_0\};\Z  \right)\cong H^2\left(S^2\setminus\{x_0\}\sqcup \{x_1\}, A\setminus \{x_0\}\sqcup \{x_1\};\Z  \right).
\ee
An application of the excision  in cohomology (cf. equation \eqref{exc_coh_eqn})  yields following isomorphisms
\bean
H^2\left(\left(SS^1 \right)^+\setminus\{y_0\}, B\setminus \{y_0\};\Z  \right)\cong H^2\left(\left(SS^1 \right)^+, B;\Z  \right),\\
H^2\left(S^2\setminus\{x_0\}\sqcup \{x_1\}, A\setminus \{x_0\}\sqcup \{x_1\};\Z  \right)
 \cong H^2\left(S^2, A;\Z  \right)
\eean
and taking into account \eqref{chern_sss_eqn} one has the natural isomorphism
\be\label{chern_hhhh_eqn}
H^2\left(\left(SS^1 \right)^+, B;\Z  \right)\xrightarrow{\cong }H^2\left(S^2,A ;\Z   \right).
\ee
The exact sequence of cohomology groups (cf. equation \eqref{exact_coh_eqn}) yields the following diagram
\be\label{chern_dhhhh_eqn}
\begin{tikzcd}
H^1\left( B;\Z\right)\arrow[r] \arrow[dd]&H^2\left(\left(SS^1 \right)^+, B;\Z  \right)\arrow[r] \arrow[dd, "\cong"] &H^2\left( \left(SS^1 \right)^+ ;\Z  \right) \arrow[r] \arrow[dd, "H^2\left(  \phi_{S^2/\left( SS^1 \right)^+};~\Z\right)  "] &H^2\left( B; \Z\right)\arrow[dd]\\
&	~&~ &~ \\
H^1\left( A;\Z\right)\arrow[r]&H^2\left(S^2,A ;\Z   \right)\arrow[r]&H^2\left(S^2 ;\Z   \right) \arrow[r, "\phi_{S^2}"]  &H^2\left( A; \Z\right).
\end{tikzcd}
\ee
From $H^1\left( A; \Z\right)\cong H^1\left( B; \Z\right)\cong H^2\left( A; \Z\right)\cong H^2\left( B; \Z\right)\cong \{0\}$, the diagram \eqref{chern_dhhhh_eqn},
 the isomorphism \eqref{chern_hhhh_eqn} and the 5-lemma it follows that the map  $$H^2\left(  \phi_{S^2/\left( SS^1 \right)^+}\right): H^2\left(\left(SS^1 \right)^+;\Z  \right) \xrightarrow{\cong} H^2\left(\left(SS^1 \right)^+;\Z  \right)$$ is a group isomorphism. There is the unique $\xi_{\left( \left(SS^1 \right)^+ \right) }\in H^2\left(  \phi_{S^2/\left( SS^1 \right)^+};~\Z\right) \in H^2\left( \left(SS^1 \right)^+ ;\Z  \right)$ such that 
 \be\label{chern_xx_eqn}
  \xi = H^2\left(  \phi_{S^2/\left( SS^1 \right)^+};~\Z\right)\left(\xi_{\left( \left(SS^1 \right)^+ \right) } \right)
  \ee
...
From (i) of the Theorem \ref{chern_class_thm} it turns out that $c_1\left( H_{\left(SS^1 \right)^+ }\right) =  \xi_{\left(SS^1 \right)^+}$. 
