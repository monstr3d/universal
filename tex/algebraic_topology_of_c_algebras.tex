...
%A^\sim\bydef\begin{cases}
	...
	According to the  Shilov Idempotent Theorem any projector in $C_0\left(\mathfrak{Gelfand}\left(A \right) \right)$ corresponds to an element of the group of cohomology $H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)$ with compact support (cf. \ref{commutative_k_empt}).  Using the Definitions \ref{proj_v_defn} and \ref{k00_defn} and the Remark \ref{projection_equivalences_rem} one can deduce that 
	\be\label{k_00_jh_eqn}
	\begin{split}
		K_{00}\left(A \right) \cong H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)/\sim_h
	\end{split}
	\ee 
where the equivalence relation $\sim_h$ comes from the homotopy equivalence (cf. Definition \ref{projection_equivalences_defn}). Otherwise from the Remark \ref{ho_inv_rem} and the Theorem \ref{ho_inv_thm} it follows that 
	\be\label{k_00_h_eqn}
\begin{split}
	K_{00}\left(A \right) \cong H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)
\end{split}
\ee 
...
\be\label{k0_eq_eqn}
	\begin{split}
		REDUCED !!!!
	K_{0}\left(A \right) \cong H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)
\end{split}
\ee
...
\begin{definition}\cite{bredon:sheaf}
	...
	Since $C^0_\Phi\left( \bullet; \bullet \right)$ and $\mathscr Z^1\left( \sX ; \bullet \right)$ are exact functors, it follows that 
...
\be\label{mv_eqn}
\begin{split}
...\to H^p_\Psi\left(\sX; \mathscr A \right)\to  H^p_{\Psi\cap \sX_1}\left(\sX_1; \mathscr A \right)\oplus  H^p_{\Psi\cap \sX_2}\left(\sX_2; \mathscr A \right)\to  H^p_{\Psi\cap A}\left(A; \mathscr A \right)\to \\\to H^{p+1}_\Psi\left(\sX; \mathscr A \right)\to ...
\end{split}
\ee
\begin{example}
%13.1. Example. 
...
to show that $\tilde H^k (SX) \cong H^k\left( \Sigma\sX\right) \cong  H^-{k-1}(\sX)$ for all $k$. %Note that this doesnot hold in this generality for singular theory where one must assume thespace to be "well pointed," that is, that the base point is nondegenerate.
\end{example}
