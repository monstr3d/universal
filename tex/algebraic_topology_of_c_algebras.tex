...
\end{remark}
	...
	According to the  Shilov Idempotent Theorem any projector in $C_0\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right) \right)$ corresponds to an element of the group of cohomology $H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)$ with compact support (cf. \ref{commutative_k_empt}).  There is the action $U\left(\left(A\otimes \K \right) ^+ \right)\times \mathrm{Pr}\left( A\right)\to \mathrm{Pr}(A)$ of the unitary group on the set of projectors. So there is the natural action $U\left(\left(A\otimes \K \right) ^+ \right)\to H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)$. Using the Definitions \ref{proj_v_defn} and \ref{k00_defn} one can deduce that 
	\be\label{k_00_h_eqn}
	\begin{split}
...
From  \eqref{stab_k1_eqn} it follows that 
	...
	One has $\mathfrak{Gelfand}_r\left(C_0\left(\R \right)\otimes  A\right)^+\cong S^1`\times \mathfrak{Gelfand}_r\left(A \right)^+$. The natural projection  $S^1`\times \mathfrak{Gelfand}_r\left(A \right)^+\onto S^1$ and $u$ yield an unitary  $u_{\mathfrak{Gelfand}}\in U_1\left(C\left(  \mathfrak{Gelfand}_r\left(A^+ \right)\right) \times S^1 \right)$ and an element 
	$$
\left[	u_{\mathfrak{Gelfand}}\right]\in H^0_c\left(S^1\times \mathfrak{Gelfand}_r\left(A \right)^+; \sU\left(1 \right)  \right) 
	$$
	The projection 
	$
	S^1\times \mathfrak{Gelfand}_r\left(A \right)\onto \mathfrak{Gelfand}_r\left(A \right)
	$
	yields an injective homomorphism
	$$
\varphi:H^0_c\left(\mathfrak{Gelfand}_r\left(A \right); \Z \right)\to H^0_c\left(S^1\times \mathfrak{Gelfand}_r\left(A \right); \Z \right)	
	$$ 
Any  $x \in H^0_c\left(\mathfrak{Gelfand}_r\left(A \right); \Z \right)$ can be represented by the continuous  map $\phi_x \in C_c\left(\mathfrak{Gelfand}_r\left(A \right); \Z \right)$.
The given by the Theorem  \ref{cup_thm} cup product
$$
\left[	u_{\mathfrak{Gelfand}}\right]\smile\varphi\left(x \right)\in H^0_c\left(S^1\times \mathfrak{Gelfand}_r\left(A \right);~ \sU\left(1 \right)\otimes \Z  \right) \cong H^0_c\left(S^1\times \mathfrak{Gelfand}_r\left(A \right);~ \sU\left(1 \right)  \right) 
$$
can be represented by the map
\be\label{pr_f_eqn}
\begin{split}
S^1\times \mathfrak{Gelfand}_r\left(A \right)\to U\left( 1\right),\\
(s, y)\mapsto u^{p\left(y \right) }_{\mathfrak{Gelfand}}
\end{split}
\ee
The above formula yields an  element of $U_1\left( C\left(\left(\R \times \mathfrak{Gelfand}_r\left(A \right) \right)^+  \right)\right)$. So it defines an element of 
$$
U_1\left( C\left(\left(\R \times \mathfrak{Gelfand}_r\left(A \right) \right)^+  \right)\right)/ U_1\left( C\left(\left(\R \times \mathfrak{Gelfand}_r\left(A \right) \right)^+  \right)\right)_0.
$$
...
$\dl_S\left[	u_{\mathfrak{Gelfand}}\right]\smile \varphi\left(x \right)\in H^1_c\left(\R \times \mathfrak{Gelfand}_r\left(A \right); \Z \right)$ (cf. Appendix \ref{sheaf_k1_sec}). The equation \eqref{pr_f_eqn} complies with the map \eqref{idem_map_eqn} which yields the isomorphism $\bt_A : K_0\left( A\right) \cong K_1\left( SA\right)$. So the isomorphism $\bt_A$ corresponds to the homomorphism
 \be\label{hs_f_eqn}
 \begin{split}
 	H^0_c\left( \mathfrak{Gelfand}_r\left(A \right);\Z \right)\onto H^1_c\left(\R \times \mathfrak{Gelfand}_r\left(A \right); \Z \right),\\
...
\bean
\begin{split}
I_A \bydef \\ \left\{\left. x \in  H^0_c\left( \mathfrak{Gelfand}_r\left(A \right); \Z \right)\right|\dl_s\left[u_{\mathfrak{Gelfand}}\right]\smile \varphi\left(x \right) = \{0\} \in H^1_c\left(\R \times \mathfrak{Gelfand}_r\left(A \right); \Z \right)   \right\}
\end{split}
\eean
is a right ideal of the ring $H^0_c\left( \mathfrak{Gelfand}_r\left(A \right);\Z \right)$ (with respect to the cup product). One can prove that it is a left ideal, so the factor-group $H^0_c\left( \mathfrak{Gelfand}_r\left(A \right);\Z \right)/I_A$. has a natural structure of the ring.
 From the isomorphisms $\bt_A : K_0\left( A\right) \cong K_1\left( SA\right)$ and $K_1\left( SA\right)\cong  H^1_c\left(\mathfrak{Gelfand}_r\left(\R \times A \right); \Z \right)$ it follows that there is the isomorphism
 \be\label{k_0_iso_eqn}
 H^0_c\left( \mathfrak{Gelfand}_r\left(A \right);\Z \right)/I_A \cong K_0\left(A \right). 
 \ee
	
	\begin{theorem}\label{k_h_thm}
	If $A$ is an unital $C^*$-algebra then one has
	\bea\label{k_00_eqn}
	K_{0}\left(A \right) \cong H^0_c\left(\mathfrak{Gelfand}_r\left(A \otimes \K\right); \Z \right)/I_A.
	\eea
