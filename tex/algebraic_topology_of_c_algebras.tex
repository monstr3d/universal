...
\be\label{chern_eqn}
\begin{tikzcd}
	\mathbb{M}_n \left( A\right)  \ar[r,hook, "\phi"] & A \otimes \ar[rd,hook, "\phi_A"]  \K &\\
	& & M\left(A \otimes \K \right) \\
	& \mathbb{M}_n \left(C\left( \sX^+\right) \right) \ar[ru,hook, "\phi_\sX"]&
\end{tikzcd}
\ee
\begin{definition}\label{chern_defn}
	Let  $\sV\in \mathrm{Proj}\left(A \right)$ be an equivalence class of idempotents represented by $e_A\in 	\mathbb{M}_n \left( A\right)$ such that there is the unique class  $\sV_\sX\in \mathrm{Proj}\left(C\left(\sX^+ \right) \right) $ represented by $e_\sX\in 	\mathbb{M}_n \left(C\left( \sX^+\right) \right)$ with $\phi_A \circ \phi\left(e_A \right) = \phi_\sX\left( e_\sX\right)$. 
...
If both $u, v \in C\left(\G^N_N \right)$ are given by
  ...
  \end{pmatrix}\in U^+\left(C^*_r\left(\G^\N_\N \right)\otimes  \K\right)
  \eean 
  then since $u'_{\T^2}$ is homotopic to $u_{\T^2}$ one has $\left[ u_{C^*_r\left(\G^{\T^2}_{\T^2} \right)}\right]=\left[ u'_{C^*_r\left(\G^{\T^2}_{\T^2} \right)}\right]\in  K_1\left( {C^*_r\left(\G^{\T^2}_{\T^2} \right)}\right) \setminus \{0\}$
\paragraph{DEL}
...
define {\cite characteristic classes} $c_j\left(\sV \right)\in K\left( \sX\right) $, for $j=0, .. ., n$ such that $c_0\left(\sV \right)=1$ . 
...
\begin{enumerate}
 ...
 The $c_j\left( \sV\right)$  are "natural", i.e. 	$c_j\left( \sV\right)  = f^*\left(c_j\left( \sV\right)  \right)$  for any general morphism $\sV \xrightarrow{g}\sV'$ , which induces $f: \sX \to \sX"$ on the bases, $\sX$ and $\sX'$, of $\sV$ and $\sV'$ respectively; 
		...
		\eean
		\item[(iii)] If the rank of $V$ is one, then $c_1\left(]sV \right)= \chi\left(\sV \right) \bydef  1 -\left[\sV\right]$, and $c_i\left( \sV\right)  = 0$ for $i > 1$.
	\end{enumerate}
...
associate cohomology classes  $c_j\left(\sV \right)\in H^{2j}\left(\sX, \Z \right) $ called Chern classes of $\sV$ which satisfy the following axioms:
	\begin{enumerate}
		...
		\eean
		\item[(iii)] If the rank of $\sV$ is one, then $c_0\left(\sV \right)= \chi\left(\sV \right) \bydef  1 -\left[\sV\right]$, and $c_i\left( \sV\right)  = 0$ for $i > 1$.
	\end{enumerate}
\end{theorem}
