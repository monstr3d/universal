...
\begin{definition}\label{constant_presheaf_defn}\cite{johnstone:topos}
	...
	\sU \mapsto F.
	\eean
The $F$-\textit{constant sheaf} is the sheaf associated with the $F$-{constant presheaf}
\end{definition}
...
%\begin{theorem}\cite{bredon:sheaf}
	%10.6. 
%	Theorem. Let $\Phi$ be a family of supports on $\sX$ and let $A$ be a subspace	of $\sX$. Let  $\mathscr N$  be a collection of $\Phi$-taut subspaces of $\sX$ containing $A$ 	and directed downwards by inclusion. Assume that for each $K \in \Phi|K \setminus A$	there is an $N \in \mathscr N$ with $N \in \sX \setminus K$. Then $A$ is $\Phi$-taut $\Leftrightarrow$ the map
%	$$
%	\th: \lim_{N \in \mathscr N} H^*_{\Phi \cap N} \left( N, \mathscr A|N\right) \to H^*_{\Phi \cap A} \left( A, \mathscr A|A\right)
%	$$
	
%	induced by restriction, is an isomorphism for every sheaf $\mathscr A$ on $\sX$.
%\end{theorem}
%Let $A \subset \sX$, let $\Phi$ be a family of supports on $\sX$, let $\mathscr A$ be a sheaf on $\sX$,
%and let $f: \sX \to \sY$ be a map.
%If  $\dim \H = \infty$ and $\left\{L_0, L_1, ...\right\}$ is a set of mutually orthogonal codimension one projective subspaces of $\C P\left(\H \right)$ then
%$$
%\C P\left( \H\right) = \bigcup_{j=0}^\infty \left( \C P\left(\H \right) \setminus L_j\right) 
%$$
...
%\begin{theorem}\label{top_nerve_thm}\cite{bredon:sheaf}
	%PAGE 193 !!! DOWN !!!
	%4.13. Theorem. 
%	Let $\mathscr A$ be a sheaf of Abelian groups  on $\sX$ and let $\mathscr U = \left\{\sU_\a\right\}_{\a \in I}$ an open covering  of $\sX$ having the property that $H^p\left( \sU_{\sigma};\mathscr A \right)= 0$  for $p > 0$ and	all $\sigma \in K\left(\mathscr U \right)$ in the nerve of covering (cf. Definition \ref{nerve_defn}). Then there is a canonical isomorphism
%	\be
%	H^*\left(\sX, \mathscr A \right) \cong \check H^*\left( \mathscr U; \mathscr A\right). 
%	\ee
%\end{theorem}
...
%\end{split}
%\ee
%where $\sH'$ is a Hilbert space and the isomorphism of semi-lattices comes from the inclusion $\sU_{\sigma}\subset \sH'$.If  $\mathscr F$ is a corresponding to $F$ constant sheaf on $\C P\left(\H \right)_{\mathfrak{Hausdorff}}$  and $\mathscr W= \left\{ \C P\left(\H \right) \setminus L_j\right\}_{j = 0, 1, ... }$ or  $\mathscr W= \left\{ \C P^n\setminus L_j\right\}_{j = 0,..., n }$ then $H^p\left( \sU_{\sigma};\mathscr A \right)= 0$  for $p > 0$ and all $\sigma \in K\left(\mathscr W\right)$ where  $K\left(\mathscr W\right)$ is the nerve of $\mathscr W$ (cf. Definition \ref{nerve_defn}). From the Theorem \ref{top_nerve_thm} it turns out that
%$$
%H^*\left(\C P^n_{\mathfrak{Hausdorff}}, \mathscr A \right) \cong \check H^*\left( \mathscr W; \mathscr A\right)
%$$
%Taking into account that $\C P^n \setminus L_j$ is open in $\C P\left(\H \right)_{\mathfrak{Gelfand}}$ for any $j$ one has
The following theorem can be deduced from the \ref{leray_thm} one.
\begin{theorem}
	There is the natural isomorphism
	$$
	H^*_c\left(\C P\left(\H \right) _{\mathfrak{Hausdorff}}, \mathscr F_{\mathfrak{Hausdorff}} \right) \cong H^*_c\left(\C P\left(\H \right) _{\mathfrak{Gelfand}}, \mathscr F_{\mathfrak{Gelfand}}\right)
	$$
...
\begin{empt}\label{blowing_empt}
	...
	\eean
where $\rho_y: A \to B\left(\H_y \right)$ is the corresponding to $y$ irreducible representation. Let $\xi \in \mathfrak{Gelfand}\left(A \right)$, $x \bydef \phi_A\left( \xi\right)$ and $L_\xi \in \xi$ be a left ideal.  
...
\C P\left(\E \right)\cong \mathfrak{pre}- \mathfrak{Gelfand}\left(A \right)
\ee
There is the natural Gelfand  topology on $\C P\left(\E \right)$ such that the map \eqref{gtop_map_eqn} is continuous. 
...
\newline
\begin{tikzcd}
	\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)^{\text{ext}}	 \arrow[r, "\phi'"] \arrow[d, "\phi''"] &  \mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)_{\mathfrak{Etale}}\arrow[d]\\
	C P\left(\E \right)_{\mathfrak{Gelfand}} \arrow[r]  &  \sX	
\end{tikzcd}
\\
