...
\bean
\begin{split}
\smile : K^n\left( \sX \right)\times  K^p\left( \sY \right) \to K^{n+p}\left( \sX \times \sY\right)
\end{split}
\eean
(cf \eqref{mult_k_eqn}). The following Theorem yields a generalization of this product. 
\begin{theorem}\label{cup_prod_thm}
If $A$ is a $C^*$-algebra then there is the natural graded cup product
\be\label{cup_prodc_eqn}
\smile : K_n\left(A \right) \otimes K_p\left(A \right) \to K_{n+p}\left(A \right) 
\ee
\end{theorem}
\begin{proof}
