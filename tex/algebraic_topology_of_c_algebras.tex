...
The above equation means that the filter $\left\{L_\la\right\}$ could not be extended, i.e. it is an ultrafilter.	
	It corresponds to a point $\{+\}\in \mathfrak{Gelfand}\left(A^+ \right)\setminus \mathfrak{Gelfand}\left(A \right)$ such that $\mathfrak{Gelfand}\left(A^+ \right)\cong \mathfrak{Gelfand}\left(A \right)\cup \{+\}$.
\end{proof}
...
%A^\sim\bydef\begin{cases}
	...
	\end{lemma}
	\begin{proof}
		Let  $a \in M\left(A \right) $ is a normal element (cf. Definition \ref{normal_defn}). If $C_0\left(\sX \right)\subset K\left(A \right) $ is the generated by $a$ commutative $C^*$-subalgebra then the Theorem \ref{good_thm} yields a continuous map $\phi: \mathfrak{Gelfand}\left(A  \right)\onto \sX$. If $\overline a\in C_0\left( \sX\right)$ corresponds to $a$ then $a$ is the image of $\overline a\circ \varphi$.  
	\end{proof}
	\begin{theorem}
...
\end{proof}
	...
	\begin{proof}
		...
		\bean
		\begin{split}
			P \bydef A^n L \subset A^n = \bigoplus_{j=1}^n A_j
		\end{split}
		\eean
...
If $A$ is any $C^*$-algebra then one has
	...
	\end{theorem}
	\begin{proof}
		The $C^*$-algebra $A$ is strongly Morita equivalent to $ A\otimes \K$. So this lemma is the direct consequence of the equations \eqref{k_0_iso_eqn} and \eqref{k_1_h_eqn}.
	\end{proof}
	\begin{rem}
...
\begin{lemma}\label{final_blowing_lem}
	Let $A$ be a $C^*$-algebra. If  a category $\mathfrak{Hausdorff}\left(A\right)$ (cf. Definition \ref{category_defn}) such that 
	\begin{enumerate}
	...
	\end{enumerate}
	then there is the final object $\mathfrak{Hausdorff}\left(A\right)$ of (cf. Definition \ref{terminal_ob_defn}). 
\end{lemma}
\begin{proof}
