...
%A^\sim\bydef\begin{cases}
	...
	\end{split}
	\ee 
Any homotopy $\left[0, 1\right]\to U_1\left( \left( A \otimes \K\right)^+ \right)$ corresponds to the normal operator  $u_{\left[0, 1\right]}\in C\left(  \left[0, 1\right]\right) \otimes U_1\left( A ^+ \right)$. So from the Lemma \ref{image_lem} there is an unitary 
$$
...
\bean
\begin{split}
	K_1\left(A \right) \cong U_1\left( A ^+ \right) / U_1\left(  A ^+ \right)_0.
\end{split}
\eean	
...
\varphi_{H^0\to K_{00}}	: H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\onto  H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)/U\left(\left( A\otimes \K\right) ^+ \right)\cong \\K_{00}\left(A \right) 
	...
	\end{split}
	\eean
	The ring $\Z$ is a  principal ideal domain, so from the Theorem \ref{kunnet_sheaf_thm} it follows that there is the natural injective map
	\bean
	\begin{split}
H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\otimes H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\hookto\\ H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right)\times \mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)
	\end{split}
	\eean
	If $\Dl : \mathfrak{Gelfand}\left(A \otimes \K\right) \to \mathfrak{Gelfand}\left(A \otimes \K\right) \times \mathfrak{Gelfand}\left(A \otimes \K\right)$ is the diagonal map then from the equation \eqref{cohom_mor_eqn} it follows that
	there is the natural homomorphism
	\bean
\begin{split}
\Dl^* : H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right)\times \mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\to H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)
\end{split}
\eean
From the above equations it follows that there is the natural map
	\be\label{hh_p_eqn}
\begin{split}
H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right)\otimes \mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\to \H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right)\right), 
\end{split}
\ee
i.e. the Abelian group $H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right);\Z\right) $ has the natural ring structure. Using to pairing  \eqref{hh_p_eqn} as prototype we wold like the pairing 
	\be\label{hh_po_eqn}
\begin{split}
\varphi_{H^0\to K_{00}}\left( 	H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right)\right)\right)  \otimes \varphi_{H^0\to K_{00}}\left( 	H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right)\right)\right)\to\\ \to \varphi_{H^0\to K_{00}}\left( 	H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right)\right)\right), 
\end{split}
\ee
where 	the homomorphism 		
\bean
\begin{split}
	\varphi_{H^0\to K_{00}}	: H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\onto  H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)/U\left(\left( A\otimes \K\right) ^+ \right).
\end{split}
\eean
is given by \eqref{nh_eqn}. The group $H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)$ by the pairs $\left( \sU, n\right)$ where $\sU\subset \mathfrak{Gelfand}\left(A \otimes \K\right)$ is an open closed subset and $n \in \N$.
	One has
\bean
\begin{split}
...
\end{split}
\ee
If $n_1, n_2 \in \Z$  then under the hypothesis \eqref{int_c_eqn} we define
	\be\label{int_cm_eqn}
\begin{split}
	\varphi_{H^0\to K_{00}}\left( \sU_1, n_1\right) \smile 	\varphi_{H^0\to K_{00}}i \left( \sU_2, n_2\right)\bydef 	\varphi_{H^0\to K_{00}} \left( \sU'_1, n_1\cdot n_2\right).
\end{split}
\ee
...
\end{split}
\ee
such that the group $K_0\left( A\right)$ obtains the ring structure.
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{lemma}
If $\sX$ is a compact Hausdorff space and $A \cong C\left(\sX \right)$ then the given by the Lemma \ref{prod_lem} operation coincides with the operation explained in !!!
...
\end{proof}
\begin{empt}
According to the Bott periodicity  one has 
$$
K_0\left(A \right) \cong K_1\left(SA \cong A \times C_0\left( \R\right)  \right) \cong H^1_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right)\times \R; \Z \right)
$$
From the Theorem \ref{kunnet_sheaf_thm} there is the natural bilinear  map 
	\bean
\begin{split}
H^1_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\times H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\to H^1_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)
\end{split}
\eean 
