...
\dim C_0\left( \sX\right) / \mathfrak{Gelfand}\left(\lift \right)\left( \widetilde x\right) > 1.	
	...
	It is impossible so $\widetilde A\cong C_0\left(\mathfrak{Gelfand}\left(\widetilde A\right) \right)$ is a commutative $C^*$-algebra. Thus there is an 1-1 correspondence between topological and noncommutative coverings of $C_0\left(\sX \right)$. From the Definition \ref{fund_den} that the fundamental group of $C_0\left(\sX \right)$ if it exists is isomorphic to $\pi_1\left( \sX\right)$.
	 \subsection{Cohomology of commutative $C^*$-algebras}
	 \paragraph{}
	 If $\sX$ is a locally compact, Hausdorff  space then from 
	the Lemma \ref{lem_g_lem} it follows that there is a homeomorphism  $ \mathfrak{Gelfand}_\sX: \mathfrak{Gelfand}\left(C_0\left( \sX\right)  \right)_{\mathfrak{strong}}\cong \sX$, where $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right) _{\mathfrak{strong}}$. If we consider the weak topology (cf. Definition \ref{weak_defn}) then the corresponding space $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right) _{\mathfrak{weak}}$ is not in general  locally compact. If $\sX$ is compact then given the equation \eqref{strong_weak_map_eqn} map is a homeomorphism. However there is a set of relations between invariants $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right) _{\mathfrak{weak}}$ and $\sX$. Using results of 
\cite{bredon:sheaf} one can prove that $\sX$ is $n$-manifold then there is the natural isomorphism
$$
H_*\left(
\sX \right) \cong H_*\left(
\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right)_{\mathfrak{weak}}  \right)
$$
where $H_*$ is the group of singular homology;
...
$$
\mathfrak{Gelfand}\left(\K\left(\H \right) \right)\cong H^*\left(\C P\left(\H \right) _{\mathfrak{weak}}, \overline F \right) \cong H^*\left(\C P\left(\H \right) _{\mathfrak{Hausdorff}}, \overline F\right)
$$
...
\sU_\iota \right\}_{\iota \in \mathscr I}$ of $\sY$ is a covering of $\sX$. For any $\left[c_\sY\right] \in \check  H^p\left(\sY, F_\sY  \right)$ there is a covering $\mathscr U$ such that $c_y$ is represented by the cycle 
\bean
c_\sY =	\sum_{j = 1} ^n\a^j_{\iota_0,...,\iota_{p}}\in \check C^p\left( \mathscr U, F_\sY\right) 
\eean
This cycle can be regarded as the cycle in $\check c_\sX\in  C^p\left( \mathscr U, F_\sX\right)$. From the Theorem \ref{top_nerve_thm} it turns out that $\left[c_\sX\right]$ can be represented by the chain
\bean
\begin{split}
c'_\sX = \sum_{j = 0}^n	\left( \tau^r \a_{j_0,..., j_{n}} \right)\in \check C^p\left( \left\{\sX\right\}, F_\sX\right).
\end{split}
\eean
(cf. \eqref{tau_c_eqn}) where $\tau : \mathscr U \to \left\{\sX\right\}$ is the unique map onto the singleton (cf. \eqref{cech_tau_eqn}). It turns out that $\left[c_\sY\right]$ can be represented by the cycle
\bean
c'_\sY = \sum_{j = 0}^n	\left( \tau^r \a_{j_0,..., j_{n}} \right)\in \check C^p\left( \left\{\sY\right\}, F_\sY\right)
\eean  
From $\left[c'_\sX\right]= 0\in H^p\left(\sX, F_\sX  \right)$ it follows that there is $b_\sX \in  C^p\left( \left\{\sX\right\}, F_\sX\right)$ such that $c'_\sX = d_{p -1}b_\sX$. The chain $b_\sX$ uniquely yields the chain $b_\sY \in  C^p\left( \left\{\sY\right\}, F_\sY\right)$ with $c'_\sY = d_{p -1}b_\sY$. It turns out that $\left[c'_\sY\right]= \left[c_\sY\right]= 0$. So one has $ H^p\left(\sY, F_\sY  \right) = 0$.
\end{proof}
\begin{lemma}\label{iso_cech_lem}
	Let  $\sX \to \sY$ is a bijective continuous map and $F$ is an Abelian group. If $\mathscr U \bydef \left\{\sU_\iota\right\}_{\iota \in I}$ is a satisfying to the hypothesis of the Theorem \ref{top_nerve_thm} covering  of $\sY$ then there are following is
	\be
...
\begin{lemma}\label{zero_product_lem}
	...
	\bean
	H^p\left(\sX, F_\sX \right) \cong 	H^p\left(\sY, F_\sY \right) \cong  \begin{cases}
		F & p = 0\\
		\{0\} & p > 0
	\end{cases}
	\eean 
	then
	\bean
	H^p\left(\sX \times \sY, F_{\sX \times \sY} \right) \cong \begin{cases}
		F & p = 0\\
		\{0\} & p > 0
	\end{cases}
	\eean	
\end{lemma}
\begin{proof}
	If $c \in \check H^p\left(\sX \times \sY, F_{\sX \times \sY} \right)$ then there are coverings $\mathscr U \bydef \left\{\sU_\a\right\}_{\a \in \mathscr A}$ and  $\mathscr V \bydef \left\{\sV_\bt\right\}_{\bt \in \mathscr B}$, i.e. 
	\bean
	...
	with
	\bean
	\rho^\bullet: \check C^\bullet\left( \left\{\sX\right\} F_\sX\right)\to \check C^\bullet\left( \mathscr U, F_\sX\right),\\
	\left( \rho^{p_j} \ga^j_{\iota_0,...,\iota_{p_j}}\right) \bydef \a^j_{\iota_0,...,\iota_{p_j}},\\
	\sigma^\bullet: \check C^\bullet\left( \left\{\sY\right\} F_\sY\right)\to \check C^\bullet\left( \mathscr V, F_\sY\right),\\
...
\begin{lemma}
If the bundle $\C P\left(\E \right)\xrightarrow{\phi_\E} \sX$  is trivial and  $\C P\left(\H \right)_{\mathfrak{Gelfand}}$ is explained in the Section \ref{compact_sec} then the n   atural set theoretic map $\phi:\C P\left(\E \right)_{\mathfrak{Gelfand}}\cong \C P\left(\H \right)_{\mathfrak{Gelfand}}\times\sX$ is a homeomorphism.
\end{lemma}
\begin{proof}   	If $\xi'' \in \mathfrak{Gelfand  }\left(A \right)''$ then from the Lemma \ref{ctr_g_lem} it                                 torms                 
Let  both $L_\sX \subset C_0\left(\sX \right)$ and $L_{\C P\left(\H \right)}\subset \K\left( \H\right) $ be closed left ideals and let $L_{\sX\times \C P\left(\H \right)}$ be the $C^*$-norm closure of the $\C$-sp ace
	\bean
	\left\{a = b c\left| b \in L_\sX \quad c \i n L_{\C P\left(\H \right)} \right.\right\}.
	\eean
                          s the corresponding to the ideal ${L_{\sX\times \C P\left(\H        \right)}}$ closed subset of $\mathfrak{Gelfand}\left(A \right)''$
\end{proof}
...
\bean
H^*\left( \mathfrak{Gelfand}\left(A \right)'',  F_{\mathfrak{Gelfand}\left(A \right)''}\right) \cong    H^*\left( \C P\left(\E \right)_{\mathfrak{Gelfand}}, F_{\C P\left(\E \right)_{\mathfrak{Gerlfand}}} \right) \cong \\ \cong  H^*\left( \C P\left(\E \right)_{\mathfrak{Hausdorff}}, F_{\C P\left(\E \right)_{\mathfrak{Hausdorff}}} \right)
\eean
...
\begin{empt}
	For any Abelian group $F$ one can define a constant presheaf $F_\sX$ of Abelian groups (cf. Definition \ref{constant_presheaf_defn}). 
