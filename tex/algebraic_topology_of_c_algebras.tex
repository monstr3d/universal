...
!!!	If $A$ is a $C^*$-algebra then	an ultrafilter  $\xi \in \mathfrak{Ultrafilters}\left(A \right)$ is an $A$-\textit{finite point} if there is a nontrivial element $a\in K\left( A\right)\setminus\{0\}$ in Pedersen's ideal (cf. Definition \ref{pedersen_ideal_defn}) such that 
	...
	If $A$ is an unital  $C^*$-algebra then the space  $\mathfrak{Gelfand}\left(A \right)$ is locally compact.
\end{lemma}
\begin{proof}
...
\begin{remark}\label{hausdorff_sim_rem}
	...
	\end{remark}
\begin{definition}\label{gelfand_r_defn}  !!!! DELETE !!!!
	If $A$ is a $C^*$-algebra then the universal Hausdorff quotient space $$\mathfrak{Hausdorff}\left(  \mathfrak{Gelfand}\left(A \right)\right)$$ of the Gelfand space $\mathfrak{Gelfand}\left(A \right)$ denoted by  $\mathfrak{Gelfand}\left(A \right)$ is {\it the reduced Gelfand !!! DELETE !!! space of} $A$, i.e. $\mathfrak{Gelfand}\left(A \right)\bydef \mathfrak{Hausdorff}\left(  \mathfrak{Gelfand}\left(A \right)\right)$,
\end{definition}
...
%	\end{proof}
	...
	\begin{lemma}\label{one_point_lem}
		...
		\end{split}
		\eean
		where $\{x_0\}$ is the singleton, i.e. $\mathfrak{Gelfand}\left(A^+ \right)$ is the one-point compactification of $\mathfrak{Gelfand}\left(A\right)$ (cf. Definition \ref{one_point_compactification_defn}).
	\end{lemma}
	\begin{proof} The set $A$ is a maximal left ideal of $A^+$
		If  there are $x'_0, x''_0 \in \mathfrak{Gelfand}\left(A^+ \right)$ with $x'_0 \neq x''_0$ then there are left ideals $L', L'' \in A^+$ such that
		\bean
		x'_0 \in \mathfrak{Gelfand}\left(A^+ \right)_{L'},\\
		x''_0 \in \mathfrak{Gelfand}\left(A^+ \right)_{L''},
		L' \cap L'' = \{0\}.
		\eean
		It follows that
		\be 
		A \subsetneqq A + L' \subsetneqq  A + L' + L'' \subset A^+,
		\ee 
		i.e. $A$ is not a maximal proper ideal of $A$. The contradiction proves this lemma.
	\end{proof}
...
\end{remark}
	...
	For any $n \in \N$ there is an $*$-isomorphisms $\left( A\otimes \K \right)^+ \cong  \mathbb{M}_n\left( A\otimes \K\right)^+$ which yields a homeomorphism $$\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A\otimes \K \right)^+\right)\cong \mathfrak{Gelfand}\left(\left(A \otimes \K \right)^+  \right).$$ 
	So any projector in $\mathbb{M}_n\left(A \otimes \K \right)^+$  yield a projector in $\left( A\otimes \K\right)^+$ and vice versa.  On the other hand any projector in $\left(A \otimes \K \right)^+$ yields a projector in $C\left(\mathfrak{Gelfand}\left(\left(A \otimes \K \right)^+\right) \right)$ (cf. Lemma \ref{image_lem}).
	According to the  Shilov Idempotent Theorem any projector in\\ $C\left(\mathfrak{Gelfand}\left(A\otimes \K \right)^+ \right)$ corresponds to an element of the group of cohomology \\$H^0\left(\mathfrak{Gelfand}\left(\left( A\otimes \K \right)^+\right); \Z \right)\cong C\left(\mathfrak{Gelfand}\left(A\otimes \K \right); \Z \right)$ with compact support (cf. \ref{commutative_k_empt}).  There are natural actions

	\bea\label{g_act_eqn}
	U\left(\left(A\otimes \K \right) ^+ \right)\times \mathfrak{Gelfand}\left(A\otimes \K \right)\to\mathfrak{Gelfand}\left(A\otimes \K \right),\\
	\label{pr_act_eqn}
	U\left(\left(A\otimes \K \right) ^+ \right)\times \mathrm{Pr}\left( A\otimes \K\right)\to \mathrm{Pr}(A\otimes \K)
	\eea
	...
	\end{split}
	\ee 
	From the Definition \ref{k0_defn} one can prove that 	$K_{0}\left(A \otimes \K\right)$ correspond to the set
	\be\label{k_0set}
	\left\{\left.f \in  C\left(\mathfrak{Gelfand}\left(\left(A \otimes \K \right)^+\right), \Z \right)\right| f\left(x_0 \right) = 0\right\}\subset C\left(\mathfrak{Gelfand}\left(\left(A \otimes \K \right)^+\right), \Z \right)
	\ee 
 where $\{x_0\}\bydef \mathfrak{Gelfand}\left(\left(A \otimes \K \right)^+\right)\setminus \mathfrak{Gelfand}\left(A \otimes \K\right)$ (cf. Lemma \ref{one_point_lem}). Since $\Z$ is the principal !!!ideal domain the set \eqref{k_0set} is isomorphic to the reduced cohomology $\tilde H^0\left(\mathfrak{Gelfand}\left(\left(A \otimes \K \right)^+\right), \Z \right)$ (cf. Definition \ref{red_defn}). On the other hand from  \eqref{red_eqn} it turns out that 
 \bean
 \tilde H^0\left(\mathfrak{Gelfand}\left(\left(A \otimes \K \right)^+\right), \Z \right)\cong H^0_c\left(\mathfrak{Gelfand}\left(\left(A \otimes \K \right)^+\right), \Z \right)
 \eean 
In result one has
	\be\label{k_0_h_eqn}
\begin{split}
...
If $A$ be an unital $C^*$-algebra then any element of $x' \in K_0\left( A\right)$ can be represented by $y' \in H^0_c\left( \mathfrak{Gelfand}\left(A \otimes \K\right)\right)$. If $z \in H^1_c\left( \mathfrak{Gelfand}\left(A \otimes \K\right)\right)$ then from the Remark \ref{inv_h1_rem}  it follows that
\be
...
\ee
where $p_\th$ is  the Powers - Rieffel projector. If $\mathfrak{Image}\left( p_\th\right)\in C\left( \mathfrak{Gelfand}\left(  C\left(u \right)\rtimes_\a \Z\right)\right) $ is the image of $p_\th$ (cf. Definition \ref{image_defn}) then $\mathfrak{Image}\left( p_\th\right)$ is a projector represents 
\bean
...
%\be
%\varphi_r : \mathfrak{Gelfand}\left( C\left(\T^n _{\theta}\right) \right)\onto \T^n
%\ee
%where $\mathfrak{Gelfand}\left( C\left(\T^n _{\theta}\right) \right)$ is the reduced Gelfand !!! DELETE !!! space (cf. Definition \ref{gelfand_r_defn}  !!!! DELETE !!!!).
%\begin{exercise}
...
\ee
where $f_{O_N}$ is the given by \eqref{cuntz_repres_eqn} representation. From \eqref{cuntz_max_eqn} it turns out that the ideal is maximal, so taking into account  the Theorem \ref{gelfand_max_thm} one concludes that $L_{\mathfrak{F}}$ corresponds to the unique point in  	$\mathfrak{Gelfand}\left(O_N\right)$. This point is $x$ so $\mathfrak{F}$ is an ultrafilter. So there is the bijective map
