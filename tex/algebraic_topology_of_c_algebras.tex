...
is a right ideal of the ring $H^0_c\left( \mathfrak{Gelfand}\left(A \right);\Z \right)$ (with respect to the cup product). One can prove that it is a left ideal, so the factor-group $H^0_c\left( \mathfrak{Gelfand}\left(A \right);\Z \right)/I_A$. has a natural structure of the ring.
 ...
 \ee
	...
	\end{proof}
	\begin{rem}
	From \eqref{k_1_eqn} and the Corollary \ref{bott_kk_cor} it follows that for any $r \ge 0$ one has
	\bea\label{k0_s_eqn}
...
\begin{split}
\smile : K^n\left( \sX \right)\times  K^p\left( \sY \right) \to K^{n+p~ \mathrm{mod}~2}\left( \sX \times \sY\right)
\end{split}
\ee
(cf \eqref{kfn_prod_eqn}). Here we find a generalization 

\be\label{cup_prodcc_eqn}
\smile : K_n\left(A \right) \otimes K_p\left(A \right) \to K_{n+p~ \mathrm{mod}~2}\left(A \right) \quad n, p \in \{0, 1\}.
\ee
of the product \eqref{k_top_p_eqn}

 The following Theorem yields a generalization of this product. !!! From the Theorem \ref{k_h_thm} it turns out that $K_0\left( A\right) =H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)/I$ so we can define $\smile : K_0\left(A \right) \otimes K_0\left(A \right) \to K_0\left(A \right)$ which comes from the given by the Theorem \ref{cup_thm} cup product $$\smile :H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\otimes H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\to H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right).$$
...
\eean
If $x', x'' \in  K_1\left(A \right)$ are elements represented !!!! 
Let $A$ be a \textbf{stable} $C^*$-algebra. If $\sX \bydef \mathfrak{Gelfand}\left(A\otimes \K \right)$ then  diagram of topological spaces and continuous  maps
\be 
\begin{tikzcd}
& \sX\times \R^{p} \times \R^q \arrow[dl, "f'"]\arrow[dr, "f''"]&\\
\sX\times \R^{p} & & \sX\times \R^{q}
\end{tikzcd}
\ee
yields the diagram 
\be\label{hc_d_eqn} 
\begin{tikzcd}
	& H^1_c\left( \sX\times \R^{p} \times \R^q ; \Z\right) &\\
 H^1_c\left( \sX\times \R^{p} ; \Z \right) \arrow[ur, "f'_*"]& &  H^1_c\left( \sX\times \R^q : \Z \right)\arrow[ul, "f''_*"]
\end{tikzcd}
\ee
From the equations \eqref{k0_s_eqn} and \eqref{k1_s_eqn} it turns out that the diagram \eqref{hc_d_eqn} is equivalent to following one 
\be\label{k_d_eqn} 
\begin{tikzcd}
	&K_{p + q + 1 ~ \mathrm{mod}~2} \left( A\right) &\\
	K_{p + 1 ~ \mathrm{mod}~2} \left( A\right) \arrow[ur, "K_*\left( f'\right) "]& & K_{ q + 1 ~ \mathrm{mod}~2} \left( A\right)\arrow[ul, "K_*\left( f''\right) "]
\end{tikzcd}
\ee
If  $x' \in K_{p + 1 ~ \mathrm{mod}~2} \left( A\right)$ (resp.  $x'' \in K_{p + 1 ~ \mathrm{mod}~2} \left( A\right)$ ) is represented by $y' \in  H^1_c\left( \sX\times \R^{p} ; \Z \right) $ and $y'' \in  H^1_c\left( \sX\times \R^{q} ; \Z \right)$ then the Theorem  \ref{cup_thm} cup product yields the cup product
\be 
\begin{split}
	f'_*\left( y'\right) \smile 	f''_*\left( y''\right)\in H^2_c\left( \sX\times \R^{p} \times \R^q ; \Z\right)
\end{split}
\ee
From \eqref{red_eqn} and \eqref{red_c_eqn} it follows that the product $f'_*\left( y'\right) \smile 	f''_*\left( y''\right)$ uniquely yields $y \in H^1_c\left( \sX\times \R^{p} \times \R^q\times \R ; \Z\right)$. But from \eqref{k0_s_eqn}, \eqref{k1_s_eqn} it follows that $y$ yiels and element $x \in K_{p + q~\mathrm{mod}~2}$.  Since $p + 1 + g + 1 \equiv p + q ~\mathrm{mod}~2$  one has a map
\be 
\begin{split}
\smile:	K_{p + 1 ~ \mathrm{mod}~2} \left( A\right)\times 	K_{q + 1 ~ \mathrm{mod}~2} \left( A\right)\to 	K_{p + 1 + q + 1 ~ \mathrm{mod}~2} \left( A\right)\\
\left(x', x'' \right) \mapsto x
\end{split}
\ee
...
\end{proof}
\begin{theorem}
If $\sX$ and $A \bydef C\left(\sX \right)$  is a compact space then the given by the Theorem \ref{cup_prod_thm} cup product coincides with \eqref{kfn_prod_eqn} one.
\end{theorem}
