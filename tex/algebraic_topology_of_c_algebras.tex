...
One has a homeomorphism $\T^2 \cong S^1 \times S^1$ and a projector $p_{\T^{2}/S^1}: \T^2\to S^1$ such that
\be\label{folipu_eqn}
u_{\T^2} = u_{S^1}\circ p_{\T^{2}/S^1}
\ee
where $u_{S^1}\in U\left(C\left(S^1 \right) \right)
$ such that $K_1\left(C\left( S^1\right)  \right)= \Z\left[u_{S^1}\right]$. From the Theorem \ref{suspension_thm} it follows that  there is a commutative diagram
\be\label{foli_sus_eqn}
\begin{tikzcd}
	K_1\left(C\left( S^1\right)  \right) \arrow[rrr, "K_1\left( C\left( p_{\T^{2}/S^1} \right) \right)" ]  \arrow[d, "\th_{C\left( S^1\right)}" ]&&& K_1\left(C\left( \T^2\right)  \right)\arrow[d, "\th_{C\left(\T^2 \right) }" ]\\ 
	K_0\left(S(C\left( S^1\right) \right) \arrow[rrr, "K_0\left( SC\left( p_{\T^{2}/S^1} \right)\right)" ] &&& K_0\left(SC\left(\T^2 \right)  \right)
\end{tikzcd}
\ee
such that
$$
\left[u_{\T^2} \right] = K_1\left( C\left( p_{\T^{2}/S^1} \right) \right)\left[ u_{S^1}\right].
$$
...
$$
If $1_{\left(SS^1 \right)^+ }$ is a trivial rank-one bundle on $\left( SS^1 \right)^{+}$ then one has $\phi^*_{S^2/\left( SS^1 \right)^+}1_{\left(SS^1 \right)^+ }\cong  1_{S^2}$. From the Theorem \ref{serre_swan_thm} the projective $C\left( \left( SS^1\right)^+ \right)$-module $p_{S_1} C\left( \left( SS^1\right)^+ \right)^2\subset C\left( \left( SS^1\right)^+ \right)^2$ yields line bundle $H_{\left( SS^1\right)^+ }$ on $\left( SS^1\right)^+$ such that
\bean
\psi_{\left( SS^1\right)^+}\left(K\left( \left( SS^1\right)^+\right)  \right) \cong \Z\left[H_{\left( SS^1\right)^+ }\right] \oplus \Z\left[1_{\left( SS^1 \right)^+ }\right]  
\eean 
Using diagram chasing one has
\bea
\psi_{S^2}\left( \left(0, 1 \right) \times S^1\right)\cong \Z\left[H \right] \oplus \Z  \left[1_{S^2}\right],\\ 
\label{chern_pp_eqn}
\phi^*_{S^2/\left( SS^1 \right)^+}\left( H_{\left(SS^1 \right)^+ }\right) \cong  H.
\eea
From the inclusion $\C P^1 \subset \C P^\infty$ one can deduce that there is $\xi \in H^2\left( \C P^1 \right)\cong  H^2\left( \C P^\infty \right)$ such that  $H^2\left( \C P^\infty \right)$ is a generated by $\xi$ unital $\Z$-algebra. From the { the exact sequence of cohomology groups} (cf. equation \eqref{exact_coh_eqn}) one has
\be
\begin{split}
...
$ and taking into account \eqref{chern_ss_eqn} one concludes that these isomorphisms are equivalent to the following one
  $$
  H^2\left(  \phi_{S^2/\left( SS^1 \right)^+}\right): H^2\left(\left(SS^1 \right)^+;~\Z  \right) \xrightarrow{\cong} H^2\left(\left(SS^1 \right)^+;~\Z  \right).$$ 
There is the unique $\xi_{ \left(SS^1 \right)^+  }\in H^2\left(  \phi_{S^2/\left( SS^1 \right)^+};~\Z\right) \in H^2\left( \left(SS^1 \right)^+ ;\Z  \right)$ such that 
 \be\label{chern_xx_eqn}
  \xi = H^2\left(  \phi_{S^2/\left( SS^1 \right)^+};~\Z\right)\left(\xi_{ \left(SS^1 \right)^+  } \right)
  \ee
...
From (i) of the Theorem \ref{chern_class_thm}, and the equations \eqref{chern_pp_eqn}, \eqref{chern_xx_eqn} it turns out that $c_1\left( H_{\left(SS^1 \right)^+ }\right) =  \xi_{\left(SS^1 \right)^+}$. 
...
$$
where $\xi_{ \left(SS^1 \right)^+  }$ satisfies to \eqref{chern_xx_eqn} and $1_{H^0\left(S^1; \Z \right) }\in H^0\left(S^1; \Z \right) $ is the generator of the group $H^0\left(S^1; \Z \right)$, It is easy to prove that
\be
H^2\left(\phi_{SS^1\times S^1 / SS^1}, \Z \right)\left( \xi_{ \left(SS^1 \right)^+} \right) = \xi_{ \left(SS^1 \right)^+  } \otimes 1_{H^0\left(S^1; \Z \right)}. 
\ee
...
One can prove that
\be
\xi_{\left(S\T^2 \right)^+ }= H^2 \left( ; \Z\right) \xi_{\left( SS^1\right)^+ }
\ee
