...
This contradiction proves this lemma.
	...
	\end{proof}
		...
		\end{remark}
\begin{empt}
If  $\xi \in \mathfrak{Gelfand}\left(A \right)$ then $\ker \{\xi\}$ is the closed left ideal. If $L \subset A$ is a closed left ideal then one has
			\be
\begin{split}
...
or
			\be
\begin{split}
	L \not\subset \ker \{\xi\} \quad \Leftrightarrow \quad 	\mathrm{hull}\left( \ker \{\xi\} \cup L\right) = \mathfrak{Gelfand}\left(A \right)
\end{split}
\ee
and from 
...
\end{proof}
	\subsection{The bicommutant and the  map onto the spectrum}
\paragraph{}
For any element $\xi\in \mathfrak{Gelfand}\left(A \right)$ any $L\in \Xi$ yields an open neighborhood $\mathfrak{Gelfand}\left(A \right)_L$ of $\xi$. The closed left ideal  
\be
L^\perp\bydef \left\{ a \in A \left| \forall a' \in L \quad a^*a' = 0\right.\right\}
\ee
corresponds  to the open set $\mathfrak{Gelfand}\left(A \right) \setminus \overline{\mathfrak{Gelfand}\left(A \right)_L}$ where $\overline{\mathfrak{Gelfand}\left(A \right)_L}$ is the closure of ${\mathfrak{Gelfand}\left(A \right)_L}$. From 
\be
\bigcap_{L \in \xi }  \overline{\mathfrak{Gelfand}\left(A \right)_L} = \{\xi\}.
\ee
it follows that the $C^*$-norm closure  of the union \eqref{uni_eqn}
\be\label{uni_eqn}
\bigcup_{L \in \xi } L^\perp
\ee
is the maximal left ideal $\xi^\perp \subset A$ which corresponds to the set $\mathfrak{Gelfand}\left(A \right)\setminus \{\xi\}$. 

\begin{definition}\label{bicommutant_defn}
Under the above hypothesis the natural bijective  map 
\be\label{bicommutant_eqn}
\begin{split}
(\cdot)^\perp:	\mathfrak{Gelfand}\left(A \right)\cong \mathfrak{LeftMax}\left(A \right),\\
\xi \mapsto \xi^\perp
\end{split}
\ee
from the Gelfand space $\mathfrak{Gelfand}\left(A \right)$ onto the set $\mathfrak{LeftMax}\left(A \right)$ of maximal left ideals is the {\it bicommutant map}.
The final topology on $\mathfrak{LeftMax}\left(A \right)$ with respect to the map $\left( \cdot\right)^\perp$ (cf. Definition \ref{top_final_defn}) is the {\it natural topology of} $\mathfrak{LeftMax}\left(A \right)$.
\end{definition}

\begin{remark}\label{bicommutant_rem}
	From the construction \ref{topology_empt} it follows then 
for any closed left ideal $L \in A$ the set
\be\label{bicommutantl_eqn}
\begin{split}
\mathfrak{LeftMax}\left(A \right)_L \bydef \left\{\left.\xi^\perp \in \mathfrak{LeftMax}\left(A \right)\right| L \subset \xi^\perp \right\}\subset \mathfrak{LeftMax}\left(A \right)
\end{split}
\ee
is open in $\mathfrak{LeftMax}\left(A \right)$. Moreover the set of given by \eqref{bicommutantl_eqn} sets is a subbasis (cf. Definition \ref{top_subbasis_defn}) of the natural topology of $\mathfrak{LeftMax}\left(A \right)$.
\end{remark}
\begin{definition}\label{belongs_defn}
	Let $A$ be a $C^*$ -algebra and let $\hat A$ be the spectrum of $A$ (cf. Definition \ref{spectrum_prime_primtive_defn}). An element $\xi^\perp \in \mathfrak{LeftMax}\left(A \right)$  \textit{belongs to} $x \in \hat A$ if one has
	\be\label{belongs_eqn}
	\xi^\perp \subset \ker \rep_x
	\ee
	where $\rep_x : A \to B\left(\H_x \right)$ is an irreducible representation (cf. Definition  \ref{irred_defn}). 
\end{definition}
\begin{defn}\label{gelfand_spectal_map_defn}
	Under the hypothesis of the Definition \ref{belongs_defn}  if $x', x'' \in \hat A$ with $x' \neq x''$ then 
	and $\xi\in \mathfrak{Gelfand}\left(A \right)$ belongs to both $x'$, and $x''$ then the generated by the set $\left\{ L \cap \ker_{x'}\right\}$ filter exceeds $\xi$. It is impossible since $\xi$ is an ultrafilter. Moreover
	\be\label{not_bel}
	\begin{split}
	...
	So from (c) of the Theorem \ref{top_continuous_thm} one has the natural continuous surjective \textit{Gelfand spectral map}:
	\be\label{to_spec_eqn} 
	\begin{split}
		\mathfrak{Gelfand}\left(A \right)\cong \mathfrak{LeftMax}\left(A \right) \xrightarrow{\hat \phi} \hat A
	\end{split}
	\ee
\end{defn}
