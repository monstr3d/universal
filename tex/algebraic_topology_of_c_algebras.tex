...
\end{proof}
	...
	\bean
				\mathfrak{Gelfand}\left(SA \right)\onto \mathfrak{Gelfand}\left(A  \right),\\
								\mathfrak{Gelfand}\left(SA \right)\onto \mathfrak{Gelfand}\left(C_0\left(\R \right).   \right)
	\eean
...
them 
	\bean
\begin{split}
	L_\la \bydef\widetilde L^\R_\la \bigcap \widetilde L^A_\la
\end{split}
\eean
is a closed left ideal of $A\otimes_\C C_0\left(\R \right) $. There is a filter $\left\{L_\la\right\}$. If the filter $\left\{L_\la\right\}$ is not an ultrafilter then there are $y', y'' \in \mathfrak{Gelfand}\left(SA  \right)$ such that $y'\neq y''$ ans corresponding to $y'$ and $y''$ ultrafilters exceed $\left\{L_\la\right\}$. !!!!

 there is the nontrivial left ideal 
	\bean
\begin{split}
	L\subset \bigcap_{\la \in \La} L_\la \neq \{0\}
\end{split}
\eean
but it is impossible. The ultrafilter  $\left\{L_\la\right\}$ yields the unique $y \in \mathfrak{Gelfand}\left(SA  \right)$ so one has a map
