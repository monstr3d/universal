...
\end{definition}	
	...
	\paragraph{}
	The Gelfand space yields a new conception of the $K$-theory. If $A$ is an unital $C^*$-algebra then the group $K_0\left(A \right)$ is given by projectors of $\mathbb{M}_n\left(A \right)$. On the other hand  if $p \in  \mathbb{M}_n\left(A \right)$ is projector then one has a direct sum of left ideals
	\bean
	\mathbb{M}_n\left(A \right) = p\mathbb{M}_n\left(A \right) \oplus \left( 1_{\mathbb{M}_n\left(A \right)}-p\right) \mathbb{M}_n\left(A \right) 
	\eean
	It follows that
	\bean
	\mathbb{M}_n\left(A \right)  = \mathbb{M}_n\left(A \right)_{p}\bigsqcup \mathbb{M}_n\left(A \right)_{1_{\mathbb{M}_n\left(A \right)}-p},
	\eean 
	i.e. the group $K_0\left(A \right)$  can be explained in terms of quasicomponents of the Gelfand space.
\paragraph{}	
	If $A$ is an unital $C^*$-algebra then projector $p \in \mathbb{M}_n\left(	C\left( \mathfrak{Alg}\text{-}\mathfrak{Gelfand}\left(A \right) \right) \right) $ naturally yields the projector $\mathbb{M}_n\left( \varphi_{\text{DH}} \circ p\right)\in  \mathbb{M}_n\left(M\left(A \right)  \right)$. So there is the map
	\be
V^{\text{DH}}\left( A\right)  :	V\left( C\left( \mathfrak{Alg}\text{-}\mathfrak{Gelfand}\left(A \right) \right) \right) \hookto V\left(A \right) 	
\ee
where $V$ means the set of algebraic equivalence classes of idempotents (cf. Definition \ref{proj_v_defn}). Using it one can define homomorphisms
\bean
K_0\left(C\left( \mathfrak{Alg}\text{-}\mathfrak{Gelfand}\left(A \right)  \right) \to K_0\left( M\left(A \right)  \right) \right),\\
K^0\left(\mathfrak{Alg}\text{-}\mathfrak{Gelfand}\left(A \right) \right)  \to K_0\left( M\left(A \right)  \right) 
\eean
\bean
\begin{tikzcd}
	 A \arrow[r] \arrow[rd]& \mathfrak{Alg}\text{-}\mathfrak{Gelfand}\left(A \right)\arrow[r]  & K^0\left(\mathfrak{Alg}\text{-}\mathfrak{Gelfand}\left(A \right) \right)\arrow[ld]\arrow[d]\\
&K_0\left( M\left( A\right) \right) \arrow[r, dotted ]  &H^{\text{even}}\left(\mathfrak{Alg}\text{-}\mathfrak{Gelfand}\left(A \right) , \Z\right)
\end{tikzcd}
\eean
