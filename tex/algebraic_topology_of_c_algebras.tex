...
\end{proof}
\begin{defn}\label{strong_defn}
 If $\mathfrak{Ultrafilters}\left(A \right)$ 
  is a set of ultrafilters of the meet-semilattice $\mathfrak{Lattice}\left( A\right)$  then the \textit{strong topology} on $\mathfrak{Ultrafilters}\left(A \right)$ is the minimal among topologies such that for any closed left ideal $L \subset A$ the set
...
\begin{defn}\label{weak_defn}
	If $\mathfrak{Ultrafilters}\left(A \right)$ 
	is a set of ultrafilters of the meet-semilattice $\mathfrak{Lattice}\left( A\right)$  then the \textit{weak topology} on $\mathfrak{Ultrafilters}\left(A \right)$ is the minimal among topologies such that for any element $a \in \K\left( A\right)$ of Pedersen's ideal (cf. Definition \ref{pedersen_ideal_defn}) the set
		\be\label{weak_eqn}
	\mathfrak{Ultrafilters}\left(A \right)_a	\bydef \left\{\left. \xi \in \mathfrak{Ultrafilters}\left(A \right) \right| \exists L \in \xi\quad L \cap Aa =\{0\} \right\}
	\ee
is open.
...
\begin{definition}\label{open_ideal_defn}
	If both \bean
	\sU_{\mathfrak{strong}} \bydef \mathfrak{Gelfand}\left( A\right) \bigcap \left(  \bigcap_{j = 1}^n \mathfrak{Ultrafilters}\left(A \right)_{L_j}\right) \subset \mathfrak{Gelfand}\left(A\right)_{\mathfrak{strong}},\\ \sU_{\mathfrak{weak}} \bydef \mathfrak{Gelfand}\left( A\right) \bigcap\left(  \bigcap_{j = 1}^n \mathfrak{Ultrafilters}\left(A \right)_{a_j}\right) \subset \mathfrak{Gelfand}\left(A\right)_{\mathfrak{weak}}\eean are open subsets then the closures of the unions 
	\bean
	\bigcup \left\{ L \text{ is a left ideal of } A  \left| L \bigcap  \left( \bigcup_{j= 1}^n L_j \right) = \{0\}\right.\right\},\\
		\bigcup \left\{ L \text{ is a left ideal of } A  \left| L \bigcap  \left( \bigcup_{j= 1}^n L_j \right) = \{0\}\right.\right\}
		\eean 
		are $\sU_{\mathfrak{strong}}$-\textit{ideal} and $\sU_{\mathfrak{waek}}$-\textit{ideal} respectively.
\end{definition}
...
If $A$ is a $C^*$-algebra then	an ultrafilter  $\xi \in \mathfrak{Ultrafilters}\left(A \right)$ is a \textit{finite point} if there is a nontrivial element $a\in K\left( A\right)\setminus\{0\}$ such that 
  ...
  Aa \in \xi.
	$$
	The \textit{Gelfand space} $\mathfrak{Gelfand}\left(A \right)$ of $C^*$-algebra $A$ is a topological subspace  of the {space of} $A$-{ultrafilters} 
	$$
	\mathfrak{Gelfand}\left(A \right)\bydef \left\{\left. \xi \in \mathfrak{Ultrafilters}\left(A \right)\right| \exists L \in \xi \quad L \text{ is a finite point}\right\}.
	$$
	supplied with the strong or the weak topology (cf.Definitions 
	\ref{strong_defn} and
\ref{weak_defn}).
\end{definition}
\begin{remark}\label{strong_weak_map_rem}
For any $C^*$-algebra there is the natural bijective continuous map 
\be\label{strong_weak_map_eqn}
\mathfrak{Gelfand}\left(A\right)_{\mathfrak{strong}}   \to \mathfrak{Gelfand}\left(A\right)_{\mathfrak{weak}}
\ee
between Gelfand spaces supplied with strong and weak topology.
\end{remark}
...
\begin{lemma}\label{lem_g_lem} (Generalized commutative Gelfand theorem).
	If $\sX$ is a locally compact, Hausdorff space then  is a natural  homeomorphism $ \mathfrak{Gelfand}_\sX: \mathfrak{Gelfand}\left(C_0\left( \sX\right) \right)\cong \sX$, where  $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right)$ is the Gelfand space supplied with the strong or weak  topology (cf. Definitions \ref{strong_defn} and \ref{weak_defn}).
\end{lemma}
\begin{proof}
...
\xi_a \bydef 	\left\{\left.\sU \cap \supp a \right| \sU \in\in \xi \right\}\in \mathfrak{Ultrafilters}\left( {\supp a}\right) 
	$$
	then  from the Proposition \ref{top_ultra_thm} it turns out that $\xi_a$ has the unique limit. This limit corresponds to $\mathfrak{Gelfand}_\sX\left( \xi\right)= \mathfrak{Gelfand}_{\supp a}\left( \xi_a\right)$. So we have the map is the set theoretic bijection. From the equation \ref{strong_eqn} one can deduce that the any open subset of $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right)_{\mathfrak{strong}}$ with respect to the strong topology correspond to an open subset of $\sX$ and vice versa.  From the equations \eqref{weak_eqn} \eqref{peder_c0_eqn} it follows that for any compact set $K 
	\subset \sX$ the difference $\sV \bydef \sX \setminus K$ is an open subset of $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right)_{\mathfrak{weak}}$. So any compact set $K 
	\subset \sX$ is closed in $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right)_{\mathfrak{weak}}$. Similarly if $\partial \sV$ is a boundary of the closure of $\sV$ then  $\partial \sV$ is closed. If a set  $\sU \bydef 
	\sX \setminus \left( \sV \cup \partial \sV\right) $ is closed in $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right)_{\mathfrak{weak}}$ then there is $x \in \sX$ and $\mathcal W \cap \left( \sV \cup \partial \sV\right) \neq \emptyset$ for every open with respect to the weak topology neighborhood $\mathcal W$ of $x$. On the hand $\mathcal W$ is open with respect to the topology of $\sX$ since the topology of $\sX$ is finer then the weak topology. It turns out that $\mathcal W \cap \sV\neq \emptyset$. The family $\left\{\mathcal W \cap \sV\right\}$ is an ultrafilter which exceeds the ultrafilter of all neighborhoods of $x$. It is possible if and only of the family convergent to $x$. It is possible only if $x 	\in \sV \cup \partial \sV$. This fact contradicts with
	$$
	x \in \sU = 	\sX \setminus \left( \sV \cup \partial \sV\right) 
	$$
	From this fact it follows that the set $\sU$ is open with respect to the weak topology. Using this fact one can prove that the weak topology of $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right)_{\mathfrak{weak}}$ contains all open subsets of compact sets of $\sX$. Since the $\sX$ is locally compact the weak topology of $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right)_{\mathfrak{weak}}$ equals to $\sX$ one.
	\end{proof}
...
Let $A$ be a $C^*$-algebra,  let both $\mathfrak{Gelfand}\left(A\right)_{\mathfrak{strong}}$ and  $\mathfrak{Gelfand}\left(A\right)_{\mathfrak{weak}}$ are Gelfand spaces supplied with strong and weak topology. Consider the equivalence relations 
\bean
\forall \xi',\xi''\in\mathfrak{Gelfand}\left(A\right)_{\mathfrak{strong}} \quad   \xi'\sim_{\mathfrak{strong}}\xi''\quad \Leftrightarrow \quad\\ \left(
\forall \text{ open set }\quad  \sU_{\mathfrak{strong}} \subset  \mathfrak{Gelfand}\left(A\right)_{\mathfrak{strong}}\quad 
\xi' \in \sU_{\mathfrak{strong}}
\quad \Leftrightarrow \quad \xi'' \in \sU_{\mathfrak{strong}}  \right),\\\forall \xi',\xi''\in\mathfrak{Gelfand}\left(A\right)_{\mathfrak{weak}} \quad   \xi'\sim_{\mathfrak{weak}}\xi''\quad \Leftrightarrow \quad\\ \left(
\forall \text{ open set }\quad  \sU_{\mathfrak{weak}} \subset  \mathfrak{Gelfand}\left(A\right)_{\mathfrak{weak}}\quad 
\xi' \in \sU_{\mathfrak{weak}}
\quad \Leftrightarrow \quad \xi'' \in \sU_{\mathfrak{weak}}  \right).
\eean 
The quotient spaces 
\bean
...
If the  homomorphism \eqref{lat_mor_eqn} is good and we consider strong or weak topology on both $\mathfrak{Gelfand}\left(\widetilde A \right)$ and $\mathfrak{Gelfand}\left( A \right)$ (cf. Definitions 
\ref{weak_defn} and \ref{strong_defn})	  then the map \eqref{gelfand_map_eqn} is continuous.
\end{lemma}
\begin{proof}
	From the equations \eqref{strong_eqn} and \eqref{weak_defn} one can deduce any closed  set of  $\mathfrak{Gelfand}\left(A\right)$  with respect to strong or weak topology corresponds to a closed set of $\mathfrak{Gelfand}\left(\widetilde A\right)$ with respect to strong or weak topology.
\end{proof}.
...
\end{definition}
\subsection{Inversion of geometric quotient}
