...
For any $m = 1,..., n/2$ there is an injective $*$-homomorphism 
\be\label{tor_2_n_eqn}
\begin{split}
\phi_k : C\left(\T^2_\th \right) \hookto C\left(\T^n_\th \right),\\
U_1 \mapsto u_{2k - 1},\\
U_2 \mapsto u_{2k},\\
\end{split}
\ee
where $u_1, ..., u_n\subset  C\left(\T^n_\th \right)$ are {generators} of $C\left(\mathbb{T}^n_{\th}\right)$ (cf. Definition \ref{nt_uni_defn})
From the Theorem \ref{good_thm} it follows that it turns out that for all $k = 1,..., n/2$ there is the natural surjective continuous map
	\be\label{tor_gelfand_map_eqn}
\mathfrak{Gelfand}\left(\C\left(\mathbb{T}^n_{\th}\right) \right)\xrightarrow{\mathfrak{Gelfand}\left(\phi_k \right)}\mathfrak{Gelfand}\left(C\left(\mathbb{T}^2_{\th}\right) \right).
\ee
From the equation \eqref{l12_eqn} ot follows that 
\be\label{tor_ints_n_eqn}
\begin{split}
\forall x \in\left\{1,2\right\}^k \quad \sX_x\bydef  \bigcap_{j = 1}^k \mathfrak{Gelfand}\left(\phi_k \right)^{-1}\left( \sX_{x[j]}\right) \neq \emptyset
\end{split}
\ee
Since both sets $\sX_1$ and $\sX_2$ are open and closed the set $\sX_x$ is open and closed for every $x \in \sX$. Moreover one has
\be\label{tor_intss_n_eqn}
\begin{split}
\mathfrak{Gelfand}\left(\C\left(\mathbb{T}^n_{\th}\right)\right) = \bigsqcup_{x \in \left\{1,2\right\}^k }\sX_x\\
	\forall x', x' \in \left\{1,2\right\}^k \quad x' \neq x'' \quad \Rightarrow \quad \sX_{x'}\cap \sX_{x''}= \emptyset.
\end{split}
\ee
Any $\sX_{x}$ yields a projector $p_x \in C\left(\mathbb{T}^n_{\th}\right)$ and using the the idempotent loop \eqref{idem_map_eqn} one can prove that
\be\label{tor_intsts_n_eqn}
\begin{split}
	\forall x', x' \in \left\{1,2\right\}^k \quad x' \neq x'' \quad \Rightarrow \quad \bt_A\left( \left[p_{x'}\right]\right) \neq \bt_A\left( \left[p_{x''}\right]\right)
	\end{split}
\ee
where $\bt_A$ is the Bott map for $\C\left(\mathbb{T}^n_{\th}\right)$ (cf. Definition \ref{bott_map_defn}). From the Theorem \ref{suspension_thm} it follows that 
\be\label{tor_inttss_n_eqn}
\begin{split}
	\forall x', x' \in \left\{1,2\right\}^k \quad x' \neq x'' \quad \Rightarrow \quad  \left[p_{x'}\right] \neq \left[p_{x''}\right]
\end{split}
\ee
It turns out that
\be\label{tor_inttss_nh_eqn}
\begin{split}
	\forall g \in C\left(\mathbb{T}^2_{\th}\right)\quad \forall x \in \left\{1,2\right\}^k\quad \Rightarrow \quad g \left[p_{x}\right]= \left[p_{x}\right]
\end{split}
\ee
