...
\begin{definition}\label{chern_defn}
	...
	The  {\it Chern classes} $c_j\left(\sV \right)\in H^{2j}\left(\sX^+, \Z \right)$ of $\sV$ as {Chern classes} $c_j\left(\sV_\sX \right)$ of $\sV_\sX$ (cf. Theorem \ref{k:char_thm})
\end{definition}
\paragraph{title}
...
\begin{definition}\label{chern_char_defn}
	...
	we define  {\it Chern classes} $c_j\left(\sV \right)\in H^{2j}\left(\sX^+, \Z \right)$ of $\sV$ as {Chern classes} of $\sV'\in  \mathrm{Proj}\left( C\left( \sX^+\right)\right)$ (cf. Theorem \ref{k:char_thm}).
\end{definition}
\begin{remark}
...
\eean
and the natural projection $\R^2\to \T^2 \cong \R^2/\Z^2$ yield two unitary elements $u_{\T^2}, v_{\T^2}\in C\left( \T^2\right)$.
...
\Psi:	\mathbb{M}_\infty \left(SC_r^*\left(  \G^{N}_{N}\right) \right)  \ar[r,hook, "\phi"] & SC^*_r\left(\G^{\T^2}_{\T^2} \right)  \ar[rd,hook, "\phi_A"]  &\\
	...
	& \mathbb{M}_\infty \left(C\left( \left( S\T^2\right) ^+\right) \right) \ar[ru,hook, "\phi_\sX"]&
\end{tikzcd}
\ee
...
\ee
where the relation $\sim$ is given by the Definition \ref{projection_equivalence_defn}. From the Definition \ref{chern_defn} it follows that
\be
c_j\left(\left[p_{G^N_N}\right] \right) = c_j\left(\left[p_{\T^2}\right] \right) \in H^{2j}\left(\left( S\T^2\right)^+, \Z \right).
\ee
One has a homeomorphism $\T^2 \cong S^1 \times S^1$ and a projector $p_{\T/S^1}: \T^2\to S^1$ such that $u_{\T^2} = u_{S^1}\circ p_{\T/S^1}$ where $u_{S^1}\in U\left(C\left(S^1 \right) \right)$ such that $K_1\left(C\left( S^1\right)  \right)= \Z\left[u_{S^1}\right]$. If $\sX$ is a compact space then from the equation \eqref{suspension_s_eqn} it follows that
\bean
	\left(SC\left(\sX \right)  \right)^+ \cong\\ \left\{\left.f:S^1\to C\left(\sX \right) \right| f \text{ continuous}, ~f(z)=\la1 + x_z, ~x_z \in C\left(\sX \right) , ~x_1 = 0 \in C\left(\sX \right) \right\}.
\eean
If $\sY$ is a compact space with $C\left( \sY\right)\cong SC\left(\sX \right)$ then there is a point $y_0\in \sY$ which corresponds to the maximal ideal
\bean
 \left\{\left.f:S^1\to C\left(\sX \right) \right| f \text{ continuous}, ~f(z)= x_z, ~x_z \in C\left(\sX \right) , ~x_1 = 0 \in C\left(\sX \right) \right\} \subset 	\left(SC\left(\sX \right)  \right)^+
\eean
\begin{exercise}
Under the above hypothesis prove that if $\Sigma \sX$ is the (topological) unreduced suspension (cf. Definition \ref{top_sus_defn}) then there is a continuous surjective map  $f:\Sigma \sX\to \sY$ such that $f\left( x_0\right) = f\left( x_1\right) = y$ where both $x_0$ and $x_1$ are images of $\sX \times \{0\}$ and  $x_0$ and  $\sX \times \{1\}$ respectively.
...
\begin{definition}\label{suspension_defn!}\cite{blackadar:ko}
	...
	\left(SA \right)^+ \cong \left\{\left.f:S^1\to A^+\right| f \text{ continuous }~, ~f(z)=f(1)=\la1 + x_z, ~x_z \in A, ~x_1 = 0 \in A\right\}
	\eean
\end{definition}
...
\end{remark} 
\begin{theorem}\cite{karoubi:k} DELETE !!!
%3.3. Theorem (Thom).
 (Thom) Let us assume that the base $\sX$ is compact. Then there exists a 
unique cohomology class $U_V \in  H^n(V)$, such that the restriction of $U_V$ to each group 
$H^n(V_x)$ is the generator $\om_x$ Moreover, the cup-product with Uv defines an  
isomorphism 
$$
\varphi_V : H^j\left(\sX \right) \xrightarrow{\cong}  H^{n+j}\left(\sX \right)
$$
($U_V$ is called the Thom class in cohomology and ($\varphi_V$ the {\it Thorn isomorphism}). 
\end{theorem}
\begin{definition} DELETE !!!!\label{euler_class_defn}\cite{karoubi:k}
The restriction $\chi\left(V \right)$ of the Thom class $U_V$ to $H^n\left( V\right) \cong H^n\left( \sX\right)$
called the {\it Euler class} of the oriented vector bundle V (more precisely, it is the 
{\it cohomological Euler class}
\end{definition}
\begin{proposition}\cite{karoubi:k}\label{k:char_prop} DELETE !!!
%2.17. Proposition. 
For each vector bundle $\sV$ of rank $n$, with compact base $\sX$, we 
define {\it characteristic classes} $c_j\left(\sV \right)\in K\left( \sX\right) $, for $j=0, .. ., n$ such that $c_0\left(\sV \right)=1$ . 
These characteristic classes satisfy the following axioms:
\begin{enumerate}
	\item [(i)]
 The $c_j\left( \sV\right)$  are "natural", i.e. 	$c_j\left( \sV\right)  = f^*\left(c_j\left( \sV\right)  \right)$  for any general morphism $\sV \xrightarrow{g}\sV'$ , which induces $f: \sX \to \sX"$ on the bases, $\sX$ and $\sX'$, of $\sV$ and $\sV'$ respectively; 
		...
		\bean
		c_k\left(V_1 \oplus V_2 \right) = \sum_{i + j = k} c_i\left( V_1\right) c_j\left( V_2\right) 
		\eean
		\item[(iii)] If the rank of $\sV$ is one, then $c_1\left(\sV \right)= \chi\left(\sV \right) \bydef  1 -\left[\sV\right]$, and $c_i\left( \sV\right)  = 0$ for $i > 1$.
	\end{enumerate}
			Moreover, the characteristic classes $c_j\left( \sV\right)$  are uniquely determined by these axioms. 
\end{proposition}
\begin{theorem}\label{k:char_thm}\cite{hatcher:at}
%Theorem 3.2. 
...
\item[(ii)] $c\left( \sV_1 \oplus \sV_2\right) = c\left(\sV_1 \right) \smile  c\left(\sV_1 \right)$ for $c \bydef 1 + c_1 + c+2 + ... \in  H^*\left(\sX, \Z \right)$.
\item[(iiic)] $cj(\sV) = 0$ if $j > \dim \sV$.
...
\begin{theorem}\cite{karoubi:k}
	%3.15. THM. 
To each complex vector bundle $\sV$ of rank $n$, with compact base $\sX$, we uniquely 
associate cohomology classes  $c_j\left(\sV \right)\in H^{2j}\left(\sX, \Z \right) $ called Chern classes of $\sV$ which satisfy the following axioms:
	\begin{enumerate}
		\item [(i)]
		The $c_j\left( \sV\right)$  are "natural", i.e. 	$c_j\left( \sV\right)  = f^*\left(c_j\left( \sV\right)  \right)$  for any general morphism $\sV \xrightarrow{g}\sV'$ , which induces $f: \sX \to \sX"$ on the bases
		\item[(ii)] If $\sV_1$ and $\sV_2$ are vector bundles on $\sX$, then 
		\bean
		c_k\left(\sV_1 \oplus \sV_2 \right) = \sum_{i + j = k} c_i\left( \sV_1\right) c_j\left( \sV_2\right) 
		\eean
		\item[(iii)] If the rank of $\sV$ is one, then $c_0\left(\sV \right)= \chi\left(\sV \right) \bydef  1 -\left[\sV\right]$ (cf. Definition \ref{euler_class_defn}), and $c_i\left( \sV\right)  = 0$ for $i > 1$.
	\end{enumerate}
\end{theorem}

The proof of the Theorem \ref{k:char_thm} is analogous to the proof of \ref{k:char_prop}. Moreover, 
this proof gives us the following proposition: 
