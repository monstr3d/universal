...
\end{proof}
	...
	\begin{definition}\label{gelfand_functor_defn}
		...
		Any functor  $\mathfrak{Hausdorff}\to \mathscr C$ naturally yields the functor $ C^*\text{-}\mathfrak{Gelfand}\to
...
\Phi_\varphi\left(C_0\left(\sX_\varphi \right) \right)\hookto M\left( A\right)  
\eean
If a  space  $M\left(\mathfrak{Gelfand}\left(A \right) \right)$ such that $C_0\left( M\left(\mathfrak{Gelfand}\left(A \right) \right)\right)$ is generated by the union
$$
\bigcup_{\varphi \in \mathfrak{GoodComm}\left( A\right)} \Phi_\varphi\left(C_0\left(\sX_\varphi \right) \right)
$$
subalgebra of $C_b\left( \mathfrak{Gelfand}\left(A \right)\right)$ and $\bt~	\mathfrak{Gelfand}\left(A \right)$ is the Stone-\v{C}ech compactification of $\mathfrak{Gelfand}\left(A \right)$ (cf. Definition \ref{top_stone_cech_defn}) then there are following surjective continuous maps
\bea
...
\eea
	...
	$$
	C_0\left( M\left(\mathfrak{Gelfand}\left(A \right) \right)\right) \hookto M\left(A \right) 
	$$
...
\begin{lemma}\label{nt_lem}
There is the natural bijective continuous map $\varphi \mathfrak{Gelfand}\left(C\left(\T^n _{\theta}\right) \right) \onto \T^n$.
\end{lemma}
\begin{proof}
...
$$
\mathfrak{Gelfand}\left(\widetilde C\left(\T^n _{\theta}\right) \right)\xrightarrow{\mathfrak{Gelfand}\left(\phi_j \right)}\mathfrak{Gelfand}\left(C\left( U_j\right)  \right)
$$
...
$$
From (ii) of the Theorem \ref{good_hom_defn} it follows that the map $\varphi_j$ is surjective for any $j = 1,..,n$. We leave to the reader the proof of that the map $\varphi$ is surjective.
!!! INJECTIVE
\end{proof}
