...
\begin{definition}
If $A$ is a $C^*$-algebra then the universal Hausdorff quotient space of the Gelfand space $\mathfrak{Gelfand}\left(A \right)$ denoted by  $\mathfrak{Gelfand}_r\left(A \right)$ is {\it the reduced Gelfand space of} $A$.
\end{definition}
...
\subsection{Gelfand spaces of unital $C^*$-algebras}
\paragraph{}
...
\begin{empt} Let $A$ be an unital $C^*$-algebra.
	...
	\end{split}
	\ee
	From \eqref{max_y_eqn} it turns out that $\ker{\xi}$ is the maximal proper ideal. 
\end{empt}
\begin{theorem}\label{gelfand_max_thm}
For any unital $C^*$-algebra there is the natural bijective map
	\be
\begin{split}
\mathfrak{Max}_A: \mathfrak{Gelfand}\left(A \right)\cong \mathfrak{LeftMax}\left(A \right),\\
\xi \mapsto \ker\{\xi\}
\end{split}
\ee
from the Gelfand space to the set of maximal proper left ideals.
For any closed ideal $L \subset A$ an open set $\mathfrak{Gelfand}\left(A \right)_L \subset \mathfrak{Gelfand}\left(A \right)$ (cf. equation \eqref{topology_eqn}) corresponds to the set
	\be\label{gelfand_max_eqn}
\begin{split}
\mathrm{hull}\left(L \right) = \left\{t \in \mathfrak{LeftMax}\left(A \right) | L \subset t\right\}\subset  \mathfrak{LeftMax}\left(A \right)
\end{split}
\ee
\end{theorem}	
\begin{proof}
