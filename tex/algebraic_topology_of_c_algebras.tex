...
\end{split}
\ee
and taking into account \eqref{h1is_m_eqn} we have
...
\begin{remark}\label{cup_prod_suspernsion_rem}
	...
	\end{split}
	\ee
!!!	Using the equations \eqref{h1isc_m_eqn} \eqref{red_eqn}  \eqref{red_c_eqn} one can prove that $S$ is the ring isomorphism.
\end{remark}
...
\subsection{Pimsner-Voiculescu Exact Sequence in cohomology}
	\paragraph{} 	The left shift of $\Z$ yields the natural $*$-automorphism $\ga \in \Aut\left(\K \right) \cong \Aut\left(\K \right)\cong \Aut\left( \K\left( \ell^2\left( \Z\right) \right) \right)$. Similarly to the Proposition \ref{k_aut_prop} we consider the action $\Z \times A\otimes \K A\otimes \K$ generated by $\left(\a\otimes\ga \right)$. i.e. we imply $\a^\K \bydef \a\otimes\ga$. 
...
\be 
\begin{tikzcd}
	K_0\left( S\left( A \otimes \K\right)\right)\arrow[r]&K_0\left( B\rtimes_\bt \R \right)\arrow[r]& K_0\left( A \otimes \K\right)\arrow[d, "\partial"] \\
	K_1\left(A \otimes \K\right)\arrow[u, "\partial"]&\arrow[l]K_1\left( B\rtimes_\bt \R \right)& \arrow[l]K_1\left(S\left( A \otimes \K\right)\right)
\end{tikzcd}
\ee
