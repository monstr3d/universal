...
\end{split}
	...
	where $\mathrm{GL}_n\left(A \right) \bydef \left\{\left.x \in \mathrm{GL}_n\left(A^+ \right)\right| x\equiv 1_n \mod  \mathbb{M}_n\left( A\right) \right\}$ (cf. the equation \eqref{gl_eqn})
			...
			One has the quotient map  $\pi: A \to A^+/A$.  Similarly to the Appendix \ref{k0_empt} suppose than both  $e, f$ are idempotents in $\mathbb{M}_n\left( A^+\right)\subset \mathbb{M}_\infty\left( A^+\right)$, and $z$ is an invertible element of $\mathbb{M}_n\left( A/J\right)$ and 
			with $z \pi\left(e\right)z^{-1} = \pi\left(f\right)$.
			\be\label{k0_1_eqn!1}!!!
			\begin{split}
...
The $C^*$-algebra $A\otimes \K$ is a two-sided ideal of $\left( A\otimes \K\right)^+$.	
	, so from the Corollary \ref{one_point_cor} it follows that
	\bean
...
\end{definition}
Suppose that the noncommutative torus $C\left(\T^n_\th \right)$ is {good}. The Lemma  \ref{suspension_lem} it follows that 
		\be 
\begin{split}
