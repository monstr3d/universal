...
\end{defn}
\begin{definition}
	A closed right ideal $I\subset A$ of $C^*$ algebra is \textit{compact} if there is $a \in K\left(A \right)$ such that $I \subset Aa$.
\end{definition}
...
\end{remark}
\begin{empt}
	If $A$ is a $C^*$-algebra then there is a meet-semilattice of left compact ideals. If $\mathfrak{Gelfand}\left(A \right)$ is a set of ultrafilters of this meet-semilattice then for any closed left ideal $I \subset A$ denote by
	\be\label{gelfand_eqn}
\mathfrak{Gelfand}\left(A \right)_I	\bydef \left\{\left. x \in \mathfrak{Gelfand}\left(A \right) \right| x \notin I\right\}
	\ee
\end{empt}
\begin{definition}\label{gelfand_defn}
	The \textit{Gelfand space} of $C^*$-algebra $A$ is a set of $\mathfrak{Gelfand}\left(A \right)$ with the minimal topology such that any given by \eqref{gelfand_eqn} set is minimal.
\end{definition}
...
\begin{lemma} (Generalized commutative Gelfand theorem).
	If $\sX$ is a locally compact, Hausdorff space then  is a natural  homeomorphism $\mathfrak{Gelfand}\left(C\left( \sX\right)  \right)\cong \sX$.
\end{lemma}
\begin{proof}
The set of left ideals of $C_0(\mathcal{X})$ naturally coincides with the set of two-sided ones. Any two sided ideal !!!
\end{proof}
...
\eean
of $\C$-algebras is \textit{good} if for any  ideal $\widetilde I \subsetneqq \widetilde A$ there is a  ideal $I \subsetneqq A$ such that
$ \widetilde I\subset \widetilde A \varphi_R\left(I \right)$. 	
\end{defn}
...
\end{lemma}
\begin{proof}
	For any $\widetilde x \in  \mathfrak{Gelfand}\left(\widetilde A \right)$ we define a filter $x$ of compact ideals $I\subset A$ such that
	\bean
	x = \left\{ I \subset A \left| \exists \widetilde I \in \widetilde x \quad  \widetilde I\subset \widetilde A \varphi_R\left(I \right)\right.\right\}
	\eean 
	If $I' \notin x$ then from the Definition \ref{lolale_defn} one can deduce 
	$$
	\forall I \in x \quad \varphi_R\left(I' \cap I \right) = \{0\}\quad \Rightarrow \quad I' \cap I' = \{0\}.
	$$
	So $x$ is an ultrafilter and the mapping \eqref{gelfand_map_eqn} is defined as $\widetilde x \mapsto x$.
From the equation 
	\eqref{gelfand_space_egn} it turns out that the mapping \eqref{gelfand_map_eqn} is continuous.
\end{proof}
\begin{remark}
