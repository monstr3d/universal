...
If $A$ is a $C^*$-algebra and  $C_0\left( \sX\right)\subset M\left(A \right)$ is natural Hausdorff blowing-up (cf. Definition \ref{blowing_natural_defn}) then thee are pullback
diagrams
\bean
\begin{tikzcd}
~
	& A  \arrow[d, "\varphi^2"] & & V^{\mathrm{nat}}\left(A \right) \arrow[r, "\subset"] \arrow[d, "\varphi_*"]& V\left( A\right)   \arrow[d, "\varphi^2_*"]\\
	C_0\left( \sX\right) \arrow[r, "\varphi^1"]	& M\left( A\right)& & 	V\left( C_0\left( \sX\right)\right)  \arrow[r, "\varphi^1_*"]	& V\left( M\left( A\right)\right) 
\end{tikzcd}
\eean
(cf. Definition \ref{proj_v_defn}).
\begin{definition}
	Under the above hypothesis for any $\sV\in V^{\mathrm{nat}}\left(A \right)$ we define  {\it Chern classes} $c_j\left(\sV \right)\in H^{2j}\left(\sX, \Z \right)$ of $\sV$ as {Chern classes} of $\varphi_*\left( \sV\right) \in V\left( C_0\left( \sX\right)\right)$ (cf. Theorem \ref{k:char_thm})
 \end{definition}
...
\end{definition}
For any $\sV \in V^{\mathrm{nat}}\left(A \right)$ the Chen classes 

If $\sV \in V\left( A\right)$ is such that there is a unique 
in 
...
\begin{lemma}
If $\sX$ is a Hausdorff locally compact space and $A\bydef C_0\left(\sX \right) \otimes \K$ then one has
\begin{enumerate}
	\item [(i)] There is the 
	\item[(ii)]
\end{enumerate}
