...
\mathbb{M}_n\left(A \right) \cong \prod_{j',j'', k', k''\in \left\{1, ..., n\right\}}\mathbb{M}_n\left(A \right)A_{j',j'', k', k''}
\ee
From the equation \eqref{mat_dec_eqn} one can deduce that there is an open continuous map
	\be\label{matrpp_eqn}
\begin{split}
\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right) \right)	\xrightarrow{\cong}\coprod_{j',j'', k', k''\in \left\{1, ..., n\right\}}\mathfrak{Gelfand}\left( A_{j',j'', k', k''} \right) 
\end{split}
\ee
...
The given by \eqref{matrpp_eqn} map is a homeomorphism.
\end{lemma}
\begin{proof}
	The topology of $ \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right) \right)$ is finer then the topology of $$\coprod_{j',j'', k', k''\in \left\{1, ..., n\right\}}\mathfrak{Gelfand}\left( A_{j',j'', k', k''} \right)$$ 	
	If $L \subset \mathbb{M}_n\left(A \right)$ is a closed left ideal then it corresponds to an element of the basis of the topology of $\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right) $. 
$$
L \neq \{0\} \quad \Leftrightarrow \quad \exists j',j'', k', k'' \in \left\{1, ..., n\right\}\quad  L \cap A_{j',j'', k', k''} \neq \{0\}.
$$
So if $L \neq \{0\}$ then there is   $a \in  L \cap A_{j',j'', k', k''} \setminus \{0\}$ which yields an ideal
$$
  L_{j',j'', k', k''}\bydef \mathbb{M}_n\left(A \right) a 
$$
The ideal $L_{j, k}$ corresponds to an open set of 
$$
\mathfrak{Gelfand}\left( A_{j',j'', k', k''} \right) \subset \coprod_{j',j'', k', k''\in \left\{1, ..., n\right\}}\mathfrak{Gelfand}\left( A_{j',j'', k', k''} \right) 
$$
From $L_{j',j'', k', k''}$ subset if follows that this open set is a subset of the given by $L$ open subset of $\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right) \right)$ i.e. the topology of 	$$\coprod_{j',j'', k', k''\in \left\{1, ..., n\right\}}\mathfrak{Gelfand}\left( A_{j',j'', k', k''} \right)$$ is finer than the topology of $\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right) \right)$.
\end{proof}
\begin{empt}
If $p \in \mathbb{M}_n\left(A \right) $ is a projector then $p \mathbb{M}_n\left(A \right)\mathbb{M}_n\left(A \right)$ is a left ideal. It corresponds to an open subset of $$
\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right) \right)_L\subset \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right) \right).
$$
From the Lemma \eqref{proj_id_lem} it follows that 
$\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right) \right)_L$ corresponds to an open subset 
$$
\sU \subset \mathfrak{Gelfand}\left(\mathbb{M}_n\left(C\left(  \mathfrak{Gelfand}\left( A\right) \right)  \right) \right)
$$ 
Using $p + \left( 1_{\mathbb{M}_n\left(A\right) }-p\right)= 1_{\mathbb{M}_n\left(A\right)}$ one cap prove that there is an open subset% $\sU' \subset \mathfrak{Gelfand}\left(\mathbb{M}_n\left(C\left(  \mathfrak{Gelfand}\left( A\right) \right)  \right)$ such that 
$$
\mathfrak{Gelfand}\left(\mathbb{M}_n\left(C\left(  \mathfrak{Gelfand}\left( A\right) \right) \right)  \right) = \sU \sqcup \sU'
$$
where $\sqcup$ is the disconnected topological union. One can prove that $\sU$ and $\sU'$ correspond to the left ideals $L_{\sU}, L_{\sU'}\subset \mathbb{M}_n\left(A \right)$ with  $ \mathbb{M}_n\left(A \right)= L_{\sU}\oplus L_{\sU'}$. There a map $p_{\sU}\in C\left(\mathfrak{Gelfand}\left(\mathbb{M}_n\left((C\left(  \mathfrak{Gelfand}\left( A\right) \right) \right) \right)  \right)$ such that
\be
p_{\sU}\left(x \right) = \begin{cases}
	1 & x \in \sU\\
	0 & x \notin \sU
\end{cases}
\ee 
...
\end{definition}
\ref{projection_equivalences_defn}
\end{empt}
