...
\begin{theorem}
If $F$ is an Abelian group and both $\mathscr F_{\mathfrak{Hausdorff}}$ and $\mathscr F_{\mathfrak{Gelfand}}$ are corresponding to $F$ constant sheaves on $\C P\left(\H \right) _{\mathfrak{Hausdorff}}$ and $\C P\left(\H \right) _{\mathfrak{Hausdorff}}$ respectively then the continuous map yields the natural isomorphism
\be\label{proj_h_eqn}
...
\begin{definition}
	...
	\begin{itemize}
		...
		\eean 
	\end{itemize} 
	The space of generated by such ideals principal filters is the \textit{pre}-\textit{Gelfand space of} $A$ denoted by $\mathfrak{Pre-Gelfand} \left(A \right)$. 
\end{definition}
...
$$
\mathfrak{Gelfand}\left(A \right)\to \mathfrak{Pre-Gelfand}\left(A \right)
$$
...
\end{proposition}
Suppose that $A$ is the given by the Proposition \ref{ctr_bundle_prop} a locally   fibre bundle $\F$ with fibre $\K = \K\left(\H \right)$. There is a subbundle $\E \subset \F$ such that fibers of $\E$ are  rank-one positive operators. It gives a locally trivial bundle $\C P\left(\E \right)\to \sX$ and the Cartesian square
\\
\newline
\begin{tikzcd}
	\sY'	 \arrow[r, "\phi'"] \arrow[d, "\phi''"] &  \mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)\arrow[d]\\
	C P\left(\E \right) \arrow[r]  &  \sX	
\end{tikzcd}
\\

	On the other hand there is the natural map $\phi''': \mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)\to C P\left(\E \right)$. If
	\be
	\sY \bydef \left\{y \in \sY' | \phi''\left( y\right) = \phi''\circ \phi\left(y \right)  \right\}
	\ee
