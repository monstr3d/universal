...
\begin{definition}\label{gelfand_category_defn}
	The {\it principal $C^*$ category} $C^*$-$\mathfrak{Principal}$ is such that:
	\begin{itemize}
		\item objects of $C^*$-$\mathfrak{Principal}$ are $C^*$-algebras,
		\item arrows of of $C^*$-$\mathfrak{Principal}$ are principal $*$-homomorphisms in the sense of the Definition \ref{nonunital_principal_defn}
	\end{itemize}
...
\subsection{Unital $*$-homomorphisms and Gelfand functor}
\paragraph*{}
...
%A^\sim\bydef\begin{cases}
	...
	\begin{definition}\label{gelfand_functor_defn}
		...
		C^*\text{-}\mathfrak{Gelfand}\xrightarrow{\mathfrak{Gelfand}}\mathfrak{Top},\\\label{gelfand_h_functor_eqn}
			C^*\text{-}\mathfrak{Gelfand}\xrightarrow{\mathfrak{Gelfand}_r \bydef \mathfrak{Hausdorff}~\circ~\mathfrak{Gelfand}}\mathfrak{Hausdorff}
		\eea
...
\begin{definition}\label{open_close_ideal_defn}\cite{akerman_stone}
	%DEFINITION 11.1. 
	A projection $p \in M$ (cf. Definition \ref{env_alg_defn}) is {\it open} if there exists a
