...
If both $A$ and $B$ are $C^*$-algebras a homomorphism $\phi: K_*\left(A \right) \to K_*\left(B \right)$ \textit{has degree 1} if $\phi\left(K_j\left( A\right)  \right) \subset K_{1=j}\left( B\right)$, $j = 0,1$. We say that $\phi$ is {\it of degree 1}.
\end{definition}
...
A homomorphism $\phi: K_*\left(A \right) \to K_*\left(B \right)$ \textit{has degree 1} is a \text{ring homomorphism} if the composition
\be
K_*\left(A \right) \xrightarrow{\phi} K_*\left(B \right)\cong  K_*\left(SB \right)
\ee
is the ring homomorphisms, where $K_*\left(B \right)\cong  K_*\left(SB \right)$ is given by the Example \ref{degr_1_exanple}.  Both given by the Example \ref{degr_1_exanple} homomorphisms are ring homomorphisms.
\end{definition}
...
The Abelian group homomorphisms $\partial$ in \eqref{sisf_ext_eqn} yield a  degree 1 ring homomorphisms $K_*\left( A/J\right) \to K_*\left( J\right)$.
\end{theorem}
\begin{proof}
...
\end{tikzcd}
\ee
comes from  \eqref{p_v_eqn} then all Abelian group homomorphism $\iota_*$ and $\partial$ of \eqref{pimsner_voiculesky_eqn} are  ring homomorphisms. Both $1-\a_*$ and $\iota_*$ are grade, $\partial$ has degree 1.
\end{theorem}	
\begin{proof}
...
\{0\}\hookto	K_0\left( C\left({u_{S^1}} \right)\right) \xrightarrow{\iota^u_*} K_0\left( C\left(u \right)  \rtimes_\a \Z \right) \xrightarrow{p^{u_{S^1}}_*} K_1\left( C\left({u_{S^1}} \right)\right) \onto \{0\} \\ \label{kk_1_nt_eqn}
\{0\}\hookto	K_1\left( C\left(u \right)\right) \xrightarrow{\iota^{u_{S^1}}_*} K_1\left( C\left({u_{S^1}} \right)  \rtimes_\a \Z \right) \xrightarrow{p^{u_{S^1}}_*} K_0\left( C\left({u_{S^1}} \right)\right) \onto \{0\}	\eea
Moreover one has
...
\be
\left[ p_\th\right]\smile \left[{u}\right] \in  K_1\left( C\left({u_{S^1}} \right)  \rtimes_\a \Z \right)=  K_1\left( C\left(\T^2_\th \right)\right)
\ee
where $\left[ p_\th\right]\in K_0\left( C\left(\T^2_\th \right)\right)$ and $\left[ {u_{S^1}}\right]\in K_1\left( C\left(\T^2_\th \right)\right)$. Using isomorphism \ref{k_0_nt_eqn} and \ref{k_1_nt_eqn} one can identify $\left[ p_\th\right]$ and $\left[ u\right]$ with their representatives in $H^0\left(\mathfrak{Gelfand}_r\left(C\left(\T^2_\th \right) \right); \Z \right)/U\left(C\left(\T^2_\th \right)  \right)$ and $H^1\left(\mathfrak{Gelfand}_r\left(C\left(\T^2_\th \right) \right); \Z \right)$.  We leave to the reader the proof of the following
...
\end{split}
\ee
where one implies the sheaf-theoretic  cup product, given by the Theorem \ref{cup_sheaf_thm}. The equations \eqref{tor_prod_eqn} can be regarded as $K$-theoretic ones where we consider the given by the  Theorem  \ref{cup_prod_thm}  $K$-theoretic cup product.
The left ideal $C\left(\T^2_\th \right)\mathfrak{Image}\left( p_\th\right)$ yields an open closed subset 
\be\label{x_u_eqn}
\sX_u \bydef \mathfrak{Gelfand}_r\left( C\left(\T^2_\th \right)\right)_{C\left(\T^2_\th \right)\mathfrak{Image}\left( p_\th\right)}\subset \mathfrak{Gelfand}_r\left( C\left(\T^2_\th \right)\right)
\ee
If $\sX_v \bydef \mathfrak{Gelfand}_r\left( C\left(\T^2_\th \right)\right)\setminus \sX_v$, then there is a disconnected union
\be\label{tor_dis_eqn}
 \mathfrak{Gelfand}_r\left( C\left(\T^2_\th \right)\right)= \sX_u \bigsqcup \sX_v.
\ee
and one has
\be
 H^0\left(\mathfrak{Gelfand}_r\left(  C\left(u \right)\rtimes_\a \Z\right);\Z \right)\cong \Z\left[\sX_u\right]\oplus \Z\left[\sX_v\right]
\ee
where we imply that  $\mathfrak{Gelfand}_r\left( C\left(\T^2_\th \right)\right)$  is a free Abelian group generated by quasicomponents of $\mathfrak{Gelfand}_r\left( C\left(\T^2_\th \right)\right)$ (cf. proof of the Lemma \ref{ring_auto_lem})


Below we will use alternative notation given by

\be\label{tor_not_eqn}
\begin{split}
	U_1 \bydef u, \quad 	U_2 \bydef v \quad \text{where} \quad u, v \quad \text{are generators of }C\left(\T^2_\th \right),\\
	\sX_1 \bydef \sX_u \quad \sX_2 \bydef \mathfrak{Gelfand}_r\left( C\left(\T^2_\th \right)\right)\setminus  \sX_u\quad \text{(cf. \eqref{x_u_eqn})},\\
	\mathfrak{Gelfand}_r\left( C\left(\T^2_\th \right)\right)= \sX_1 \bigsqcup \sX_2 \quad \text{(cf. \eqref{tor_dis_eqn})},\\
	P_1 \in   C\left(\T^2_\th \right) \quad \text{is  the Powers - Rieffel projector (cf. Appendix \ref{rieffel_sec})}\\
	 P_2 \bydef 1- P_1.
	\end{split}
\ee
From the above construction it follows that 
\be\label{tor_nott_eqn}
\begin{split}
K_0\left( C\left(\T^2_\th \right)\right) \cong \Z\left[P_1\right] \oplus \Z\left[P_2\right] \cong\\ \cong H^0\left(\mathfrak{Gelfand}_r\left(C\left(\T^2_\th \right) \right); \Z \right)/U\left(C\left(\T^2_\th \right)  \right)\cong  H^0\left(\mathfrak{Gelfand}_r\left(C\left(\T^2_\th \right) \right); \Z \right),\\
K_1\left( C\left(\T^2_\th \right)\right) \cong \Z\left[U_1\right] \oplus \Z\left[U_2\right] \cong\\ \cong H^1\left(\mathfrak{Gelfand}_r\left(C\left(\T^2_\th \right) \right); \Z \right).
\end{split}
\ee
Using the above Abelian group isomorphisms one can set 
\be\label{tor_iso_eqn}
\begin{split}
	\left[ P_1\right]\smile \left[u\right]
\left[P_1\right], \left[P_2\right]\in H^0\left(\mathfrak{Gelfand}_r\left(C\left(\T^2_\th \right) \right); \Z \right),\\
\left[U_1\right], \left[U_2\right]\in H^1\left(\mathfrak{Gelfand}_r\left(C\left(\T^2_\th \right) \right); \Z \right)
\end{split}
\ee
and from \eqref{tor_prodU_eqn} it follows that 
\be\label{tor_prodU_eqn}
\begin{split}
	\left[ P_1\right]\smile \left[P_2\right] = 0,\\
\left[P_1\right]\smile \left[P_1\right] = \left[ P_1\right],\\
\left[P_2\right] \smile \left[P_2\right]= \left[P_2\right],\\
\left[ P_1\right]\smile \left[U_1\right]= \left[U_1\right],\\
\left[P_1\right]\smile \left[U_2\right]=0,\\
\left[P_2\right]\smile \left[U_1\right]=0,\\
\left[P_2\right]\smile \left[U_2\right]=\left[U_2\right].
\end{split}
\ee
The equations \eqref{tor_prodU_eqn} can be regarded as both $K$-theoretic and sheaf-theoretic where the cup product is given by the Theorems \ref{cup_prod_thm} and  \ref{cup_sheaf_thm} respectively. Besides \eqref{tor_prodU_eqn} there are $K$-theoretic equations 
\be\label{tor_prodUK_eqn}
\begin{split}
	\left[ U_1\right]\smile \left[U_2\right] = 0,\\
	\left[U_1\right]\smile \left[U_1\right] = \left[ P_1\right],\\
	\left[U_2\right] \smile \left[U_2\right]= \left[P_2\right],\\
\end{split}
\ee
The Remark \ref{cup_prod_suspernsion_rem} yields the following products
\be\label{tor_dprodU_eqn}
\begin{split}
	\left[ S P_1\right]\smile \left[SP_2\right] = 0,\\
	\left[ SP_1\right]\smile \left[ SP_1\right] = \left[ S P_1\right],\\
	\left[ SP_2\right] \smile \left[ SP_2\right]= \left[ SP_2\right],\\
	\left[ S P_1\right]\smile \left[ SU_1\right]= \left[ SU_1\right],\\
	\left[ SP_1\right]\smile \left[ SU_2\right]=0,\\
	\left[ SP_2\right]\smile \left[ SU_1\right]=0,\\
	\left[ SP_2\right]\smile \left[ SU_2\right]=\left[ SU_2\right],\\
		\left[ S U_1\right]\smile \left[ SU_2\right] = 0,\\
	\left[ SU_1\right]\smile \left[ SU_1\right] = \left[ S P_1\right],\\
	\left[ SU_2\right] \smile \left[ SU_2\right]= \left[S P_2\right]
\end{split}
\ee
...
\begin{definition}
	...
	\begin{enumerate}
		\item [(a)] $$H^*\left(\mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right); \Z \right)\cong \left(  \bigoplus_{j = 1}^{2^{n-1}}\Z t_j\right)\bigoplus  \left(  \bigoplus_{j = 1}^{2^{n-1}}\Z t_j \smile \left[U\right]\right) $$ where $\left\{t_1, ...,t_{2^{n-1}} \right\}\subset H^0\left(\mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right); \Z \right)$ and $\left[U\right]\in .H^1\left(\mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right); \Z \right)$.
 		\item[(b)] For all $x', x'' \in \left\{t_1, ...,t_{2^{n-1}} \right\}$ one has 
	\be 
	\begin{split}
x' \smile x'' = \begin{cases}
	x'  & x' = x''\\
	0 & x'\neq x''
	\end{cases}\in .H^0\left(\mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right); \Z \right),\\
		x' = x'' \quad \Leftrightarrow \quad x'  \smile \left[U\right] = x''  \smile \left[U\right]
\end{split}	
\ee
		\item[(c)] The action  $$U_1\left(C\left(\T^n_\th \right) \right)  \times H^0\left(\mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right); \Z \right)\to  H^0\left(\mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right); \Z \right)$$ is trivial, so there is the natural isomorphism  $$\varphi: H^0\left(\mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right); \Z \right)\cong K_0\left((C\left(\T^n_\th \right) \right)$$ is an isomorphism.
		\item[(d)] There is the natural ring isomorphism
		\be 
	\begin{split}
		H^*\left(\mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right); \Z \right)\cong K_*\left( C\left(\T^n_\th \right)\right). 
\end{split}
\ee
...
\end{definition}
Suppose that the noncommutative torus $C\left(\T^n_\th \right)$ is {good}. The Lemma  \ref{product_lem} it follows that 
		\be 
\begin{split}
...
\section{$C^*$-algebras of groupoids}
	...
	\begin{lemma}\cite{Rieffel:stable_irrat}
	%THEOREM 8. 
