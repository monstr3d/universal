...
It is known that $C\left(\T^2_\th \right)$ is an universal $C^*$-algebra generated by two unitaries $u, v \in C\left(\T^2_\th \right)$ such that $uv = e^{2\pi i \th}vu$. It is also known 
 that $C\left(\T^2_\th \right) \cong C\left(u \right)\rtimes_{\a_u} \Z$, where $u_{S^1} \in C\left( u_{S^1}\right) \cong C(S^1)$ is unitary,  !!! $\a_u\in \Aut\left(C\left( S^1\right)  \right) \cong \Aut\left(C\left( u_{S^1}\right)  \right)$ is a $*$- automorphism given by $u \mapsto e^{2\pi i \th} u$.
...
H^1_c\left(S^1 \times \mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right), \{x_0\}\times \mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right)\right) ;\Z \right)\to \\ \to H^1_c\left(S^1 \times \mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right);\Z \right) \to\\ \to  H^1_{c\cap \{x_0\}\times \mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right)\right)}\left(\{x_0\}\times \mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right)\right);\Z \right)\onto \{0\}.
\end{split}
\ee
and taking into account \eqref{setmunus_h_eqn} one has
\be
\begin{split}
	0 \hookto  H^0_c\left(\R \times \mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right) ;\Z \right)\to  H^0_c\left(S^1 \times \mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right);\Z \right) \to\\ \to  H^0_c\left( \mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right)\right);\Z \right)\to \\ \to 
H^1_c\left(\R \times \mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right) ;\Z \right)\to  H^1_c\left(S^1 \times \mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right);\Z \right) \to\\ \to  H^1_c\left( \mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right)\right);\Z \right)\onto \{0\}
\end{split}
\ee
...
%	of (complex-valued) functions on $\sY$ is the commutative algebra $\C \bigoplus 	\C$ of $2 \times 2$ diagonal
%	matrices. There are two ways of declaring that the two points a and b of Y are identical, 	i.e. of quotienting $\sY$ by the equivalence relation $a \sim b$.
%	\begin{enumerate}
...
%		\eean
%	\end{enumerate}
%	The relation between the two algebras is the notion of strong Morita equivalence of  $C^*$-algebras. This relation preserves many 	invariants of $C^*$-algebras, such as $K$-theory and the topology of the space of irreducible	representations. Thus, in our example the pure states $\om_a$ and $\om_b$ of $B$ which come	directly from the points $a$ and $b$ of $\sY$ by the formula
%	\bean
