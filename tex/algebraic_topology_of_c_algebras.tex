...
\end{remark}
	\subsection{Sheaf-theoretic interpretation of $K$-theory}
	\paragraph{} Here we prove that $K$-theory of $C^*$-algebras can be formulated in terms of the sheaf cohomology.	
	Suppose that $A$ is a stable
	$C^*$-algebra. Then for any $n \in \N$ there is an $*$-isomorphisms $A\otimes \K \cong \mathbb{M}_n\left( A\otimes \K\right)$ which yields a homeomorphism $\mathfrak{Gelfand}_r\left(\mathbb{M}_n\left(A \right)\right)\cong \mathfrak{Gelfand}_r\left(A \right)$. So any projector in $\mathbb{M}_n\left(A\right)$  yield a projector in $A$ and vice versa.  On the other hand any projector in $A$ yields a projector in $C_0\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right) \right)$.
	According to the  Shilov Idempotent Theorem any projector in $C_0\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right) \right)$ corresponds to an element of the group of cohomology $H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)\cong C_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)$ with compact support (cf. \ref{commutative_k_empt}).  There are natural actions 
	\bea\label{g_act_eqn}
	...
	\eea
	 of the unitary group on the reduced Gelfand space $\mathfrak{Gelfand}_e\left(A \right)$ and the set of projectors $\mathrm{Pr}(A\otimes \K)$. So there is the natural action \be\label{u_1_act_eqn}
	 U\left(\left(A\otimes \K \right) ^+ \right)\times H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)\to H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right).\ee Using the Definitions \ref{proj_v_defn} and \ref{k00_defn} one can deduce that 
	\be\label{k_00_h_eqn}
	\begin{split}
		K_{00}\left(A \right) \cong H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)/U\left(\left(A\otimes \K \right) ^+ \right). 
	\end{split}
	\ee 
%		\begin{empt}
...
There is the natural cup product on $H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)/U\left(\left(A\otimes \K \right) ^+ \right)$ which comes from the cup-product on  $H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)$.	If $A$ is any $C^*$-algebra then one has
	...
	\end{theorem}
	\begin{proof}
		The $C^*$-algebra $A$ is strongly Morita equivalent to $ A\otimes \K$. So this lemma is the direct consequence of the equations \eqref{k_00_h_eqn} \eqref{k_1_h_eqn} and the Lemma \ref{ring_auto_lem}.
	\end{proof}
