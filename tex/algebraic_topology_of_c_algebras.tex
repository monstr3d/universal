...
\paragraph{Cobordism category.} Any $\C$-linear map $\C^{m+1}\otimes_\C \C^2 \to \C^{n+1}$ yields a continuous map $\C P^m  \times \C P^1  \xrightarrow{f} \C P^n$. The map $f$ naturally yields a map $ \C P^m \xrightarrow{\mathrm{ex}\left( f\right)} \left( \C P^1\right) ^{\C P^n}$ 
Let $n \in \N$, and let $\mathscr{L}\left(n \right) $ be a category such that
\begin{itemize}
	\item any  $\mathscr{L}\left(n \right)$-object is a described above map $\C P^m  \times \C P^1  \xrightarrow{f} \C P^n$ such  that
	\bean
	m < n,\\
	\forall x', x''\in \C P^m \quad x' \neq x'' \quad  \exists y \in \C P^1\quad f\left( x',y\right) \neq f\left( x'',y\right),
	\eean 
	\item an arrow from $\C P^{m'}  \times \C P^1  \xrightarrow{f'} \C P^n$ to $\C P^{m''}  \times \C P^1  \xrightarrow{f''} \C P^n$ is a map $\C P^{m'}  \times \C P^1  \xrightarrow{\phi } \C P^{m''}  \times \C P^1  \xrightarrow{f'}$ such than $m'\le m''$ and the diagram 
	\bean
\begin{tikzcd}
\C P^{m'}  \times \C P^1 \arrow[dd, "\phi"] \arrow[rd, "f'"]& ~\\
~ & \C P^n\\
\C P^{m''}  \times \C P^1 \arrow[ru, "f''"]& ~
\end{tikzcd}
	\eean
commutative.
\end{itemize}
Let $\mathscr{C}\left(n \right)$ be a category such that
...
\begin{itemize}
	...
	\item if $f_1, ..., f_{m'}, g_1, ..., g_{m''}$ are $\mathscr{L}\left(n \right)$-objects then there exists the unique  $\mathscr{C}\left(n \right)$ arrow from $\sum_{j = 1}^{m'}c_j f_j$ to $\sum_{j = 1}^{m''}d_j g_j$ if and only if for any $j' \in 1,..., m'$ there is  $j'' \in 1,..., m''$ and $\mathscr{L}\left(n \right)$-arrow $f_{j'}\xrightarrow{\phi} g_{j''}$.
\end{itemize}
So the set of $\mathscr{C}\left(n \right)$ objects is the Grothendieck group of the category $\mathscr{L}\left(n \right)$.
We suppose that there is the natural subset $\left\{0, \infty\right\}\subset \C P^1$. For any $\mathscr{L}\left(n \right)$-object  map $\C P^m  \times \C P^1  \xrightarrow{f} \C P^n$ we define  $\C P^m  \times \C P^1  \xrightarrow{f_0} \C P^n$ and $\C P^m  \times \C P^1  \xrightarrow{f_1} \C P^n$ such that
\be\label{co_dd_eqn}
f_0\left( x,y\right) \bydef f\left(x, 0 \right),\\ 
f_1\left( x,y\right) \bydef f\left(x, 1 \right).
\ee
If $1_\Z \cdot f$ is $\mathscr{C}\left(n \right)$-object then we define
\be\label{co_d_eqn}
\partial \left( 1_\Z \cdot f\right)  \bydef 1_\Z \cdot f_0 - 1_\Z \cdot f_1
\ee 
...
\paragraph{}(b) If we consider \eqref{co_d_eqn} then from
\bean
\forall x \in \C P^m \quad  \quad f_0\left(x, 0  \right) = f_0\left(x, \infty \right); \quad f_1\left(x, 0  \right) = f_1\left(x, \infty \right);
\eean 
if follows that $\partial \left( 1_\Z \cdot f_0\right)= \partial \left( 1_\Z \cdot f_1\right)= 0_{\mathscr{C}\left(n \right)}$ so one has $\partial \partial\left( 1_\Z \cdot f\right)= 0_{\mathscr{C}\left(n \right)}$. From this fact it turns out that for each object $X$ of $\mathscr C\left( n\right) $ one has $~\partial \partial \left( X\right)= 0_{\mathscr{C}\left(n \right)}$.
\paragraph{}(c) From the equations \eqref{co_dd_eqn} it turns out that there are $\mathscr{L}\left(n \right)$-arrows $f_0 \to f$ and $f_1 \to f$, so there is the natural arrow $\mathscr{C}\left(n \right)$
$$
j_{1_\Z \cdot f}:  \partial \left( 1_\Z \cdot f\right) \to 1_\Z \cdot f.
$$ 
...
\break
\begin {table}[H]
\caption {Mapping !!!} \label{main_mapping_table} 
\begin{center}
	\begin{tabular}{|c|c|c|}
		\hline
...
Cycles 	&	Linear subspace  & Algebraic subvariety \\
	...
	&& \\
Chow ring	& $\Z\left[\left[\C P^1\right]\right] /\left( \left[\C P^1\right] \right)^{n+1}$	& $\Z\left[\left[\C P^1\right]\right] /\left( \left[\C P^1\right] \right)^{n+1}$\\
&& \\
		\hline
	\end{tabular}
\end{center}
...
called the divisor class group of $\sX$, and is denoted by  $\mathrm {Cl}\sX$ 
\end{definition}
...
f\left( \sY\right)\bydef \left[K\left(\sY \right):K\left(f\left( \sY\right)  \right)\right]\cdot \overline{f\left(\sY \right) } 
	...
	to the group of cycles on $\sX'$.	  
			...
			$\sV$ of $\sX$, let $f: \widetilde \sV\to \sV$ be the normalization of $\sV$. Then $\sV$ satisfies the condition (*)!!! of (II, ¤6), so we can talk about Weil divisors and linear equivalence 
			...
			For each $r$ we let $A^r\left(\sX \right)$  be the group of cycles of codimension r on X 
			modulo rational equivalence. We denote by $A\left(\sX\right) \bigoplus_{r= 0}^n A^r\left( \sX\right)$  the graded group  where $n = \dim \sX$. Note that $A^0\left( \sX\right)= \Z$ , and that $A^r\left(\sX \right)= 0$  for $r > \dim \sX$. Note also that if X is complete there is a natural group homomorphism, the {\it degree}, from $A^n\left( \sX\right)$  to $\Z$, defined by $\deg \left(\sum n_j P_j \right) \bydef \sum n_j$ , where 
			the $P_j$ are points. This is well-defined on rational equivalence classes because 
			of (II, 6.10). !!!
\end{empt}
...
\end{definition}
\begin{theorem}\cite{pjective_reometty}
	2.2.2 Theorem. Let V be a vector space over the division ring F. Then the theorem
...
\end{theorem}
\begin{theorem}\cite{pjective_reometty}
%2.2.3 Hessenberg's theorem. Let P be an arbitrary projective space. If the theorem
	of Pappus holds in P then the theorem of Desargues is also true in P.\end{theorem}

\begin{definition}\cite{pjective_reometty}
	Definition. Let $P$ be a projective space of finite dimension $d$, The subspaces of
	dimension 2 are called {\it planes}, and the subspaces of dimension $d - 1$ are called
	{\it hyperplanes} of $P$.
	\end{definition}
