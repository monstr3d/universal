...
$$
\left[	u_{\mathfrak{Gelfand}}\right]\cup \varphi\left(x \right)\in H^0_c\left(S^1\times \mathfrak{Gelfand}\left(A \right);~ \sU\left(1 \right)\otimes \Z  \right) \cong H^0_c\left(S^1\times \mathfrak{Gelfand}\left(A \right);~ \sU\left(1 \right)  \right) 
$$
...
$\dl_S\left[	u_{\mathfrak{Gelfand}}\right]\cup \varphi\left(x \right)\in H^1_c\left(\R \times \mathfrak{Gelfand}\left(A \right); \Z \right)$ (cf. Appendix \ref{sheaf_k1_sec}). The equation \eqref{pr_f_eqn} complies with the map \eqref{idem_map_eqn} which yields the isomorphism $\bt_A : K_0\left( A\right) \cong K_1\left( SA\right)$. So the isomorphism $\bt_A$ corresponds to the homomorphism
 \be\label{hs_f_eqn}
 \begin{split}
 	H^0_c\left( \mathfrak{Gelfand}\left(A \right);\Z \right)\onto H^1_c\left(\R \times \mathfrak{Gelfand}\left(A \right); \Z \right),\\
 	x \mapsto \left[	u_{\mathfrak{Gelfand}}\right]\cup \varphi\left(x \right) 
  \end{split}
 \ee
 The set 
\be
\begin{split}
I_A \bydef \\ \left\{\left. x \in  H^0_c\left( \mathfrak{Gelfand}\left(A \right); \Z \right)\right|\dl_s\left[u_{\mathfrak{Gelfand}}\right]\cup \varphi\left(x \right) = \{0\} \in H^1_c\left(\R \times \mathfrak{Gelfand}\left(A \right); \Z \right)   \right\}
\end{split}
\ee 
is a right ideal of the ring $H^0_c\left( \mathfrak{Gelfand}\left(A \right);\Z \right)$ (with respect to the cup product). One can prove that it is a left ideal, so the factor-group $H^0_c\left( \mathfrak{Gelfand}\left(A \right);\Z \right)/I_A$. has a natural structure of the ring.
 ...
 \ee
	...
	\bea
K_{0}\left(A \right) \cong H^1_c\left(S^{2k +1}\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\cong H^1_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right)\times \R^{2k + 1}; \Z \right),\\
...
\ee
!!! REDUCED !!!!(b) Since H.0(X; L) H¡(X+; L), it suffices to consider reduced cohomology
of a compact space, and so we shall assume X to be compact.


In this section we take cohomology with coefficients in the constant sheaf
L, where L is a principal ideal domain. There is the canonical inclusion
L I,(L) = H¡(X; L), and the cokernel is called the reduced cohomology
%group H¡(X; L) = H¡(X; L)/L. For p # 0 we let Hp(X; L) = H9(X; L).
...
to show that $\tilde H^k (\sX) \cong \tilde H^k\left( \Sigma\sX\right) \cong \tilde H^{k-1}(\sX)$ for all $k$. %Note that this doesnot hold in this generality for singular theory where one must assume thespace to be "well pointed," that is, that the base point is nondegenerate.
\end{example}
