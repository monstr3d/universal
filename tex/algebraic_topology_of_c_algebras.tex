...
\begin{remark}
	If $A$ is a $C^*$ algebra with Hausdorff spectrum  then the inclusion \eqref{dauns_hofmann_eqn} coincides with the isomorphism	\eqref{dauns_hofmann_eqn} given by the Dauns-Hoffman theorem.
\end{remark}
\begin{remark} If $\Phi$ is given by \eqref{Phi_eqn} and
	\be
	M_0\left( \mathfrak{Gelfand}\left(A \right)\right)  \bydef \Phi\left(\mathfrak{Gelfand}\left(A \right) \right) 
	\ee 
	then there is restriction of the homomorphism \eqref{dauns_hofmann_eqn} yields an injective $*$-homomorphism
	\be
	C^{\text{DH}}_0\left(A \right) : C_0\left( M_0\left( \mathfrak{Gelfand}\left(A \right)\right)\right)   \hookto M\left( A\right) 
	\ee
	For any $x_0\in 	M_0\left( \mathfrak{Gelfand}\left(A \right)\right)$ there is $x \in \mathfrak{Gelfand}\left(A \right)$ with $\Phi\left(x \right) = x_0$. !!!
	From the Definition \ref{gelfand_space_defn} it follows that if $\Phi$ is given by \eqref{Phi_eqn} and
	\be
	M_0\left( \mathfrak{Gelfand}\left(A \right)\right)  \bydef \Phi^{-1}\left(\mathfrak{Gelfand}\left(A \right) \right) 
	\ee 
	then the restriction of the homomorphism \eqref{dauns_hofmann_eqn} yields a good injective $*$-homomorphism
	\be
	C^{\text{DH}}_0\left(A \right) : C_0\left( M_0\left( \mathfrak{Gelfand}\left(A \right)\right)\right)   \hookto M\left( A\right) 
	\ee 
\end{remark}	
