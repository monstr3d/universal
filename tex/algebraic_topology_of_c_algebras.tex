...
\begin{defn}\label{weak_defn}
	If $\mathfrak{Ultrafilters}\left(A \right)$ 
	is a set of ultrafilters of the meet-semilattice $\mathfrak{Lattice}\left( A\right)$  then the \textit{strong topology} on $\mathfrak{Ultrafilters}\left(A \right)$ is the minimal among topologies such that for any element $a \in \K\left( A\right)$ of Pedersen's ideal the set
		\be\label{weak_eqn}
	\mathfrak{Ultrafilters}\left(A \right)_L	\bydef \left\{\left. \xi \in \mathfrak{Ultrafilters}\left(A \right) \right| \exists L \in \xi\quad L \cap Aa =\{0\} \right\}
	\ee
is open.
...
\begin{lemma}\label{lem_g_lem} (Generalized commutative Gelfand theorem).
	If $\sX$ is a locally compact, Hausdorff space then  is a natural  homeomorphism $ \mathfrak{Gelfand}_\sX: \mathfrak{Gelfand}\left(C_0\left( \sX\right)_{\mathfrak{strong}}  \right)\cong \sX$, where $\mathfrak{Gelfand}\left(C_0\left( \sX\right)\right) _{\mathfrak{strong}}$ is the Gelfand space supplied with the strong topology (cf. Definition \ref{strong_defn}).
\end{lemma}
\begin{proof}
...
The equation \eqref{fil_mor_eqn}  induces a continuous map with respect to the strong topology (cf. Definition \ref{strong_defn}).

\end{lemma}
\begin{proof}
 From the equation \eqref{strong_eqn} one can deduce any closed set on  that $\mathfrak{Ultrafilters}\left(A\right)$ corresponds to a closed set on $\mathfrak{Ultrafilters}\left(\widetilde A\right)$ 
\end{proof}.
...
\begin{theorem}\label{top_nerve_thm}\cite{bredon:sheaf}
	%PAGE 193 !!! DOWN !!!
	%4.13. Theorem. 
	Let $\mathscr A$ be a sheaf of Abelian groups  on $\sX$ and let $\mathscr U = \left\{\s\sU_\a\right\}_{\a \in I}$ an open covering  of $\sX$ having the property that $H^p\left( \s\sU_{\sigma};\mathscr A \right)= 0$  for $p > 0$ and
