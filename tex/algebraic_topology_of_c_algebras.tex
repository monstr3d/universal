...
\subsection{Example of the Chern character}
\paragraph{}
...
\end{pmatrix}\left( \Phi_{\T^2}\left( t\right)\right)^{-1};\\
  \eean
  From the proof of the Theorem \ref{suspension_thm} it follows that the maps $\Psi_{G^N_N}$ and $\Psi_{\T^2}$ yield the projections  $p_{\G^N_N} \in \mathbb{M}_2 \left(SC_r^*\left(  \G^{N}_{N}\right)^+ \right)\subset  \mathbb{M}_\infty \left(SC_r^*\left(  \G^{N}_{N}\right)^+ \right)$ and  $p_{S\T^2} \in \mathbb{M}_2 \left(SC\left(  \T^2 \right)^+\right)\subset \mathbb{M}_\infty \left(SC\left(  \T^2 \right)^+\right)$. 
...
\end{tikzcd}
\ee
Taking into account the equations \eqref{filinf_m_eqn} and \eqref{tori_lit_eqn} one can deduce that
\be\label{foli_p_eqn}
\phi_A \circ \phi \left(p_{G^N_N}\right)\sim  \phi_\sX \left( p_{S\T^2}\right) 
\ee
where the relation $\sim$ is given by the Definition \ref{projection_equivalence_defn}. If
...
c_1\left( \sV_{G^N_N} \right) = c_1\left( \sV_{S\T^2 }\right)\in H^2\left(\left(S \T^2\right)^+  ; \Z  \right)  
\ee
...
\eean
From the proof of the Theorem \ref{suspension_thm} it follows that the map $\Psi_{S^1}$ the  projection  $p_{S^1} \in \mathbb{M}_2\left( C\left( \left( SS^1\right)^+ \right)  \right)\subset \mathbb{M}_\infty\left( C\left( \left( SS^1\right)^+ \right)  \right)$. If we consider a diagram \ref{foli_sus_eqn} then from the Theorem \ref{suspension_thm} if turns out that $\th_{C\left( S^1\right)}$ is an isomorphism, so from 	$K_1\left(C\left( S^1\right)  \right) \cong \Z\left[u_{S^1}\right]$ it turns out that 	$K_0\left(\left( S(C\left( S^1\right) \right)^+ \right)\cong \Z\left[p_{S^1}\right]$. Moreover one has $\left[p_{\T^2}\right] = K_0\left( C\left(\phi_{S\T^{2}/SS^1} \right)\right)\left( \left[p_{S^1}\right]\right)$ and $p_{\T^2}
= p_{S^1}\circ \phi_{\T^{2}/S^1}$. If 
\be
\sV_{SS^1}\bydef p_{S^1}\left(\left( SS^1 \right)^+  \right)^2 
\ee
then
\be
\sV_{\T^2}= \phi^*_{\T^{2}/S^1}\sV_{SS^1}
\ee
where  the projective finitely generated modules are identified with vector bundles (cf Theorem \ref{serre_swan_thm}) and $\phi^*_{\T^{2}/S^1}\sV_{SS^1}$ means the inverse image (cf. Definition \ref{vb_inv_img_funct_defn}).
...
\phi_{S^2/\left( SS^1 \right)^+} : S^2\onto \left( SS^1 \right)^+.
\end{split}
\ee
...
\be 
\begin{tikzcd}
K\left( \left(0, 1 \right) \times S^1\right) \cong K\left( \left(SS^1 \right)^+ \setminus \{y_0\}\right)\arrow[r, "\psi_{\left(SS^1 \right)^+ }"] \arrow[dd, "\cong"] &K\left( \left(SS^1 \right)^+   \right) \arrow[r, "\phi_{\left(SS^1 \right)^+ }"] \arrow[dd, "K\left(  \phi_{S^2/\left( SS^1 \right)^+}\right)  "] &K\left( \{y_0\} \right)\arrow[dd]\\
~&~ &~ \\
...
\end{tikzcd}
\ee
From \ref{kt_kan_line_empt} it follows that $K\left(S^2 \right) \cong \Z \left[H\right] \oplus \Z \left[1_{S^2}\right]$ where $H$ is the canonical line bundle and $1_{S^2}$ is the trivial rank-one bundle.  From \eqref{foli_sus_eqn} and $K_1\left( C\left( S^1\right) \right)\cong \Z\left[u_{s^1}\right]$ it follows that $K_0\left( \left( \left(0, 1 \right) \times S^1\right) \right)  \Z \th_{C\left( S^1\right)}\left(  \left[u_{S^1}\right]\right)$.
On the other hand $
\ker \phi_2 = \Z \left(\left[H\right] -\left[1\right] \right) 
$ so one has
$$
\forall n \in \Z \quad \psi_{S^2}\left(n \th_{C\left( S^1\right)}\left(  \left[u_{S^1}\right]\right)\right) = n\left(1- H \right) 
$$
...
\bean
\psi_{\left( SS^1\right)^+}\left(K\left( \left( SS^1\right)^+\right)  \right) \cong \Z\left[\sV_{SS^1}\right] \oplus \Z\left[1_{\left( SS^1 \right)^+ }\right]. 
\eean 
Using diagram chasing one has
\bea
\psi_{S^2}\left(K\left(  \left(0, 1 \right) \times S^1\right)\right) \cong \Z\left[H \right] \oplus \Z  \left[1_{S^2}\right],\\ 
\label{chern_pp_eqn}
\phi^*_{S^2/\left( SS^1 \right)^+}\left( \sV_{SS^1}\right) \cong  H.
\eea
From the inclusion $S^2 \cong \C P^1 \subset \C P^\infty$ one can deduce that there is $\xi \in H^2\left( \C P^1 \right)\cong  H^2\left( \C P^\infty \right)$ such that  $H^2\left( \C P^\infty \right)$ is a generated by $\xi$ unital $\Z$-algebra. From the { the exact sequence of cohomology groups} (cf. equation \eqref{exact_coh_eqn}) one has
\be
\begin{split}
...
$ and taking into account \eqref{chern_ss_eqn} one concludes that these isomorphisms are equivalent to the following one
  $$
  H^2\left(  \phi_{S^2/\left( SS^1 \right)^+}\right): H^2\left(\left(SS^1 \right)^+;~\Z  \right) \xrightarrow{\cong} H^2\left(S^2;~\Z  \right).$$ 
There is the unique $\xi_{ SS^1  }\in H^2\left(  \phi_{S^2/\left( SS^1 \right)^+};~\Z\right) \in H^2\left( \left(SS^1 \right)^+ ;\Z  \right)$ such that 
 \be\label{chern_xx_eqn}
  \xi = H^2\left(  \phi_{S^2/\left( SS^1 \right)^+};~\Z\right)\left(\xi_{ SS^1  } \right)
  \ee
From (iv) of the Theorem \ref{chern_class_thm} the equations \eqref{chern_pp_eqn}  and \eqref{chern_xx_eqn}  it follows that $c_1\left( H\right) = \xi$ where $c_1\left(H \right)$ is the Chern character (cf. Definition \ref{chern_class_defn}). 
From (i) of the Theorem \ref{chern_class_thm}, and the equations \eqref{chern_pp_eqn}, \eqref{chern_xx_eqn} it turns out that $c_1\left( \sV_{SS^1 }\right) =  \xi_{SS^1}$. There is the natural surjective continuous map
\be
\begin{split}
\phi_{SS^1/S\T^2}: \left( SS^1 \right)^+ \times S^1 \onto \left( S\T^2\right)^+\\
\phi_{SS^1/S\T^2}\left(\{y_0\} \times S^1\right) = \{z_0\},\\
...
\bean
H^2\left(\phi_{SS^1/S\T^2}, \Z \right) : H^2\left(\left( \left( SS^1 \right)^+ \times S^1\right) /S^1~;\Z   \right) \xrightarrow{\cong } H^2\left( \left( SS^1 \right)^+ \times S^1 ~;\Z  \right)
\eean 
...
$$
where $\xi_{SS^1   }$ satisfies to \eqref{chern_xx_eqn} and $1_{H^0\left(S^1; \Z \right) }\in H^0\left(S^1; \Z \right) $ is the generator of the group $H^0\left(S^1; \Z \right)$, It is easy to prove that
\be
H^2\left(\phi_{SS^1\times S^1 / SS^1}, \Z \right)\left( \xi_{ \left(SS^1 \right)^+} \right) = \xi_{ SS^1  } \otimes 1_{H^0\left(S^1; \Z \right)}. 
\ee
There is the unique $\xi_{\left(S\T^2 \right)^+ }\in H^2\left(\left(S\T^2 \right)^+, \Z \right)$ such that 
$$
H^2\left(\phi_{SS^1/S\T^2}, \Z \right)  \left(\xi_{\left(S\T^2 \right)^+ } \right) = \xi_{\left(S\T^2 \right)^+ }=\xi_{ \left(SS^1 \right)^+  } \otimes 1_{H^0\left(S^1; \Z \right)}.
$$ 
One can prove that
\be
\xi_{\left(S\T^2 \right)^+ }= H^2 \left( ; \Z\right) \xi_{SS^1 }
\ee
\begin{lemma}
	Under the above hypothesis there is the nontrivial Chern character $c_1\left(\sV_{S\G^N_N} \right) \in H^2\left(\left( S\T^2 \right)^+; \Z\right) $ (cf. Definition \ref{chern_defn}) of the given by \eqref{chern_nt_eqn} element $\sV_{\G^N_N} \in V\left( \left( SC\left(\T^2_\th \right)\right)^+\right)  \cong V\left( \left(S C^*_r\left( \G^N_N\right)\right)^+ \right) $. 
\end{lemma}
\begin{proof}
...
\begin{exercise}
	Under the above hypothesis prove that if $\Sigma \sX$ is the (topological) unreduced suspension (cf. Definition \ref{top_sus_defn}) then there is a continuous surjective map  $f:\Sigma \sX\to \sY$ such that $f\left( x_0\right) = f\left( x_1\right) = y$ where both $x_0$ and $x_1$ are images of $\sX \times \{0\}$ and  $\sX \times \{1\}$ respectively.
	Prove that there is the natural homeomorphism
	\be
	\sY = \Sigma \sX/ \{x_0\}\cup \{x_1\}.
	\ee
\end{exercise}
If $\sY$ is a compact Hausdorff space such that
\be
\begin{split}
\sY \bydef \Sigma \T^2/\{x_0\}\cup \{x_1\},\\
C\left(\sY  \right) \cong S C\left( \T^2\right) ^+.
\end{split}
\ee
From the Definition \ref{chern_defn} it follows that
\be
c_j\left(\left[p_{G^N_N}\right] \right) = c_j\left(\left[p_{\T^2}\right] \right) \in H^{2j}\left(\sY, \Z \right).
\ee
From the Theorem \ref{chern_class_thm}.
...
\begin{empt}\cite{hatcher:at}
	...
	...\to H^n\left(\sX, A , G \right) \to \left( \sX , ;G\right)\to H^n\left(\sX; G \right) \xrightarrow{\dl} H^{n +1}\left(\sX, A , G \right)\to
	\ee
\end{empt}
...
\
\
\begin{theorem}\label{k:char_thm}!!!!\cite{hatcher:kt}
	%Theorem 3.2. 
	...
	\begin{enumerate}
		...
		\item[(iv)]For the canonical line bundle $\sV \to \C P^\infty$  is a generator of $H^2\left(\C P^\infty \right)$  specified
		in advance.
	\end{enumerate}
\end{theorem}
\
...
\begin{remark}\label{k_exact_rem!}!!!\cite{karoubi:k}
	...
	$$
\end{remark}
\begin{corollary}\label{k_exc_cor1}\cite{karoubi:k}
	%5.20. Corollary. 
