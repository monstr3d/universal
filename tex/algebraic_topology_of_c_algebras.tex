...
\end{definition}
\begin{remark}
	The given by  	\eqref{topology_eqn} sets yield a subbase (cf. Definition \ref{top_subbasis_defn})  of the topology of the Gelfand space  $\mathfrak{Gelfand}\left(A \right)$. The base (cf. Definition \ref{top_base_defn}) of $\mathfrak{Gelfand}\left(A \right)$ is given by the sets
...
\end{lemma}
\begin{proof}
	From the equation \eqref{topology_eqn} one can deduce any  set of subbasis (cf. Definition \ref{top_subbasis_defn}) of the topology of  $\mathfrak{Gelfand}\left(A\right)$  corresponds to a set of subbasis of the topology of $\mathfrak{Gelfand}\left(\widetilde A\right)$. This lemma follows from (c) of the Theorem \ref{top_continuous_thm}.
\end{proof}
\begin{lemma}
...
\begin{remark}
	There is a basis for the topology (cf. Definition \ref{top_base_defn}) of $\mathbb{P}\left(\H \right)_{\mathfrak{Gelfand}}$ containing all open subsets of $\sU_{\mathfrak{Gelfand}} \subset \mathbb{P}\left(\H \right)_{\mathfrak{Gelfand}}$ given by
	\be\label{open_g_eqn}
...
\subsection{Noncommutative torus versus commutative one}
!!!!
\begin{appendices}
...
\eean
	...
	\subsection{Noncommutative torus}
	Here I follow to \cite{varilly:nc_intro}.
	\begin{definition}
For a fixed irrational real number~$\th\in \R\setminus \Q$, let
$C\left( \T^2_\th\right) $ be the unital $C^*$-algebra generated by two elements $u$, $v$
subject only to the relations $uu^* = u^*u = 1$, $vv^* = v^*v = 1$,
and
\be\label{eq_43_eqn}
vu = \la\,uv  \sepword{where} \la := e^{2\pi i\th}.
%\eqno (4.3)
\ee
Let $\SS(\Z^2)$ denote the double sequences $\ul{a} = \{a_{rs}\}$ that
are \textit{rapidly decreasing} in the sense that
$$
\sup_{r,s\in\Z} (1 + r^2 + s^2)^k \,|a_{rs}|^2 < \infty
\sepword{for all} k \in \N.
$$
The \textit{irrational rotation algebra} $\Coo\left( \T^2_\th\right)$ is defined as
\be\label{eq_44_eqn}
\Coo\left( \T^2_\th\right) \bydef \bigl\{\, a = \sum_{r,s\in \Z} a_{rs}\,u^r v^s
: \ul{a} \in \SS(\Z^2) \bigr\}.
%\eqno (4.4)
\ee 
It is a \textit{pre-$C^*$-algebra} that is dense in~$C\left( \T^2_\th\right)$.


The product and involution in $\Coo\left( \T^2_\th\right)$ are computable from \eqref{eq_43_eqn}
\be\label{eq_45_eqn}
ab = \sum_{r,s\in \Z} a_{r-n,m}\,\la^{mn}\,b_{n,s-m}\, u^r v^s,  \qquad
a^* = \sum_{r,s} \la^{rs}\,\bar a_{-r,-s}\, u^r v^s.
%\eqno (4.5)
\ee	
\end{definition}
...
\begin{definition}
%Definition. 
!!! Let Q be a point of P. The rank 2 geometry P IQ whose points are
the lines of P through Q, whose lines are the planes of P through Q, and whose
incidence is the incidence induced by P is called the quotient geometry of Q.
We also speak of 'P modulo the point Q'.
\end{definition}
\begin{theorem}
!!! 1.4.1 Theorem. Let P be a d-dimensional projective space, and let Q be a
point of P. Then the quotient geometry P IQ of P modulo Q is a projective
space of dimension d - 1.
\end{theorem}
\begin{definition}
Definition. Let V be a vector space of dimension d + I Z 3 over a division ring
F. We define the rank 2 geometry P(V) as follows:
the points of P(V) are the I-dimensional subspaces of V,
- the lines of P(V) are the 2-dimensional subspaces of V,
- the incidence of P(V) is set-theoretical containment.
\end{definition}
\begin{theorem}
2.1.1 Theorem. Let V be a vector space of dimension d + I Z 3 over a division
ring F. Then P(V) is a projective space. We call P(V) the projective space
coordinatized by F
\end{theorem}
\begin{empt}\label{proj_axiomatic_empt}
There is the {\it axiomatic projective geometry} which roughly speaking operates with pairs such that $X$ is a set and $L \subset 2^X$ is a subset of the set of subsets of $X$. Any element of $X$ is a {\it point}, an element of $L$ is a {\it line}. The pair satisfies to the explained in \cite{projective_geometry} axioms. There is a set of versions of systems of axiom. One of the axioms comes from the  theorem of Desargues. There is an axiom which comes from the theorem Pappus.
\end{empt}

\section{Abstract projective space}
\begin{definition}
	An {\it abstract projective space} $\mathbb{P} \bydef \left( X, L\right) $  is a pair such that $X$ is a set and $L \subset 2^X$ is a subset of the set of subsets of $X$. Any element of $X$ is a {\it point}, an element of $L$ is a {\it line}. We assume that the pair  $\left( X, L\right)$ satisfies to the following axiom theorem of Desargues and there is a 
\end{definition}
