...
\end{proof}
	...
	\paragraph{} If $A$ is a $C^*$- algebra and $\varphi : M\left(C_0\left(\sX \right)  \right) \hookto M\left( A\right)$ is a good injective $*$-homomorphism from the Theorem \ref{good_thm} is the natural surjective continuous map
	\be\label{cont_eqn}
	\psi_\varphi:	\mathfrak{Gelfand}: \left(A \right) \onto \sX_{\varphi}
\ee
	this map yields an injective  $*$-homomorphism
	$$
	\phi_\varphi:	C_0\left( \sX_{\varphi}\right)\hookto C_b\left(\mathfrak{Gelfand}\left(A \right) \right)  
	$$
	Since  $\varphi$ is good one has $\phi	\left( C_0\left( \sX\right)\right) \subset C_0\left(\mathfrak{Gelfand}\left(A \right) \right)$. Let $\Phi$ be a set of all given by the equation \eqref{cont_eqn} conditions maps. There the partial order on $\Phi$ such that for any $\phi', \phi''\in \Phi$ the condition $\varphi' \ge \varphi''$ is equivalent to the following commutative diagram
	\bean
	\begin{tikzcd}
	...
	\begin{definition}\label{gelfand_alg_functor_defn}
		...
		\be
		C^*\text{-}\mathfrak{Gelfand}\xrightarrow{\mathfrak{Alg}\text{-}\mathfrak{Gelfand}} \mathfrak{Hausdorff}
		\ee
