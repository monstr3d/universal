...
For any section $s \in \mathscr{C}^{p}\left(\T^n, \mathscr F \right)$ there is a section !!!
	\end{proof}
	\subsection{$K$-theory functor. Chern class}
	\paragraph{}
	The Gelfand space yields a new conception of the $K$-theory. If $A$ is a $C^*$-algebra then any element $a \in \mathbb{M}_n\left(A \right) $ corresponds to a matrix
	\be\label{matr_eqn}
a\in 	\begin{pmatrix}
		a_{1,1}& \ldots & a_{n,1}  \\
		\vdots& \ddots & \vdots\\
	...
	\end{cases}
\ee
		Denote by $\mathfrak{e}_{j,k}$ the $j,k$-{elementary} matrix. 
	\end{definition}
...
A_{j',j'', k', k''} \cong A
\ee
From \eqref{matr_eqn} it turns out that there is a set theoretic bijection
\be\label{mat_dec_eqn}
\mathbb{M}_n\left(A \right) \cong \prod_{j',j'', k', k''\in \left\{1, ..., n\right\}}A_{j',j'', k', k''}
\ee
If we consider $C^*$-norm topology on $\mathbb{M}_n\left(A \right)$ and the topology on $A_{j',j'', k', k''}$ which comes from the map \eqref{mat_jjkk_eqn} then one can prove that the bijection \eqref{mat_dec_eqn} is a homeomorphism.
	If $L \subset \mathbb{M}_n\left(A \right)$ is a closed left ideal then 
	$$
	L \neq \{0\} \quad \Leftrightarrow \quad \exists j',j'', k', k'' \in \left\{1, ..., n\right\}\quad  L \cap A_{j',j'', k', k''} \neq \{0\}
	$$
