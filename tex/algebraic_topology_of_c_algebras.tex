...
\subsubsection{Example of Chern character}
\paragraph{}
...
and $\overline I\in H^1\left(I, \dot I, \Z \right)$ is the generator such that if $\overline 0 \in H^0\left( \{0\}, \Z\right)$ $\overline 1 \in H^0\left( \{1\}, \Z\right)$ and $\overline 1 \in H^0\left( \{0\}, \Z\right)$ are respective unit integral cohomology classes then identifying $H^0\left(\dot I, \Z \right) \cong H^0\left(\{0\} ; \Z\right)\oplus  H^0\left(\{1\} ; \Z\right)$, we have $\dl \overline 1 = \dot I = - \dl \overline 0$. 
		...
		\end{definition}
\subsubsection{Elements of $K$-theory}
\paragraph*{}
...
\end{lemma} 
	...
	\subsection{$K$-theory and $K$-homology}
...
$\sV$ transverse ˆ $\F$, l'application naturelle $\E^{0\setminus C}\to \E^0_\sU$ est un
isomorphisme de $C^*$-modules hilbertiens,
\end{lemma}
\begin{remark}\cite{hilsum_scandalis:stab}
D'aprs le lemme \ref{l4_lem} 
et la proposition \ref{pop3_prop} , $\E^M_T = \E^\Om \oplus \E^{M \setminus \Om}\cong \K \otimes C^*_r\left(\G^T_T \right) \oplus \E^{M \setminus \overline{\Om}}$ 
...
N_{p,q} = \left\{(ps, qs) | s\in \R/\Z\right\}
\ee	The graph $\G$ reduced by $N = N_{p,q}$, i.e. $\G^N_N \bydef \left\{\left.\ga \in \G \right| r(\ga)\in N, \quad s(\ga)\in N\right\}$
	the manifold $\G^N_N=\T\times \Z$ with range and source maps given by:
	\bean
	r(x, n) = x + n\th' ;\\
	s(x, n) = x
