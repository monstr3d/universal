...
The Definition \ref{classic_quant_rem}  is motivated by the Connes quantization (cf. Section \ref{connes_q_sec}).
\end{remark}
...
\end{enumerate}
	...
	\begin{theorem}
	%Theorem 1.0.3
	 The categories $\sS$, $\sS$, $\A$, $\mathcal{T}op$, $\mathcal{T}op_*$ and $\sS pt$ ÒareÓ model
	categories.
	\end{theorem}		
			\begin{definition}\cite{motivic}
			%	Definition 1.2.1 
...
many times: that the Òbuilding blocksÓ for (trivial) cofibrations were easy to
cope with.
		\begin{definition}\cite{motivic}.
%Definition 3.0.1 
