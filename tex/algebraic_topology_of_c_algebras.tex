...
\end{remark}
	...
	\eea
	 of the unitary group on the reduced Gelfand space $\mathfrak{Gelfand}_e\left(A \right)$ and the set of projectors $\mathrm{Pr}(A\otimes \K)$. So there is the natural action $$U\left(\left(A\otimes \K \right) ^+ \right)\times H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right)\to H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right); \Z \right).$$ Using the Definitions \ref{proj_v_defn} and \ref{k00_defn} one can deduce that 
	\be\label{k_00_h_eqn}
	\begin{split}
...
\paragraph*{}
It is known that $C\left(\T^n_\th \right) \cong C\left(S^1 \right)\rtimes_\a \Z$, where $\a\in \Aut\left(C\left( S^1\right)  \right) \cong \Aut\left(C\left( u\right)  \right)$ is a $*$- automorphism given by $u \mapsto e^{2\pi i \th} u$.
The Theorem \ref{p_v_theorem} yields  a cyclic six-term exact sequence
	\bean
	\begin{tikzcd}
		K_0\left( C\left(S^1 \right)\right)\arrow[r, "1-\a_*"] & K_0\left( C\left(S^1 \right)\right)\arrow[r, "\iota_*"] & K_0\left( C\left(S^1 \right) \rtimes_\a \Z \right) \arrow[d]\\
		K_1\left(  C\left(S^1 \right)\rtimes \Z\right)\arrow[u] & \arrow[l, "\iota_*"] K_1\left( C\left(S^1 \right) \right) & \arrow[l, "1-\a_*"]K_1\left( C\left(S^1 \right) \rtimes_\a \Z \right)
	\end{tikzcd}
	\eean
	where the map $\iota_*$ corresponds to 
	From the isomorphisms $	K_0\left( C\left(S^1 \right)\right)\cong 	K_1\left( C\left(S^1 \right)\right)\cong \Z$ it follow that $\a_*$ is identical, $1=\a_*= 0$, so there are the exact sequences
		\bea\label{kk_0_nt_eqn}
\{0\}\hookto	K_0\left( (C\left(S^1 \right)\right) \xrightarrow{\iota_*} K_0\left( C\left(S^1 \right)  \rtimes_\a \Z \right) \onto K_1\left( C\left(S^1 \right)\right) \onto \{0\} \\ \label{kk_1_nt_eqn}
	\eea
	
From  the Theorem \ref{k_1_tor_thm} (resp \ref{k_1_tor_thm})  element of $K_0\left(C\left(\T^n_\th \right)  \right)$  (resp. $K_1\left(C\left(\T^n_\th \right)  \right)$) is represented by an idempotent (resp. invertible element) of $C\left(\T^n_\th \right)$ From this circumstance the given by the Theorem \ref{k_h_thm} equations 
	\bean
K_0\left( (C\left(\T^n_\th \right) \right) =H^0_c\left(\mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right)\otimes \K \right); \Z \right)/U\left(\left((C\left(\T^n_\th \right) \otimes \K \right) ^+ \right),\\
K_{1}\left((C\left(\T^n_\th \right) \right) \cong H^1_c\left(\mathfrak{Gelfand}_r\left((C\left(\T^n_\th \right) \otimes \K\right); \Z \right). 
\eean
can be replaced with following ones,
	\bea\label{k_0_nt_eqn}
K_0\left( (C\left(\T^n_\th \right) \right) =H^0_c\left(\mathfrak{Gelfand}_r\left(C\left(\T^n_\th \right) \right); \Z \right)/U\left(C\left(\T^n_\th \right)  \right),\\ \label{k_1_nt_eqn}
K_{1}\left((C\left(\T^n_\th \right) \right) \cong H^1_c\left(\mathfrak{Gelfand}_r\left((C\left(\T^n_\th \right)\right); \Z \right). 
\eea
