...
\begin{definition}\label{hausdorff_funct_defn}
	The map $\sX \mapsto \mathfrak{Hausdorff}\left(\sX \right)$ yields the {\it Hausdorff functor}
...
\ee
for the basis of the topology of the Gelfand space.
\end{notation}
	\begin{lemma}\label{gelfand_compact_lem}
	If $A$ is an unital  $C^*$-algebra then the space  $\mathfrak{Gelfand}\left(A \right)$ is  compact.
\end{lemma}
\begin{proof}
...
\end{proof}
	...
	If $A$ is a nonunital $C^*$ algebra and $A^+\bydef A \oplus \C$ is a unitization 
	then the difference $\mathfrak{Gelfand}_r\left(A^+ \right)\setminus \mathfrak{Gelfand}_r\left(A \right)$ contains the single point.
\end{lemma}
\begin{proof}!!!
	$A \subset A^+$ is the maximal left ideal, it correspond to an open projection of $p_A \in \left( A^+\right)^{**}$ (cf. Definition \ref{open_close_ideal_defn}). From the Proposition \ref{open_close_ideal_prop} it turns out that $p'_A \bydef 1-p_A$ is the minimal projection in $\left( A^+\right)^{**}$.  If $p_1, p_2 \in \left(  A^+\right)^{**}$ are two open projections with $p_1 > p'_A$ and $p_2 > p'_A$ ans $L_1, L_2 \subset A^+$ are corresponding to $p_1$ and $p_2$  closed ideals then $L_1\cap L_2 \neq \{0\}$. So the set $\left\{L_\la \right\}_{\la \in \La}$ of all closed ideals which correspond to open projection which exceed $p'$ is a filter. There are two types of ultrafilter 
	\begin{itemize}
...
%A^\sim\bydef\begin{cases}
	...
	\end{split}
	\ee
	Since $\varphi$ is good the map \eqref{ultra_mor_eqn} naturally yields the natural  map 
		\be\label{gelfand_map_eqn}
	\mathfrak{PreGelfand}\left(B \right)\xrightarrow{\mathfrak{PreGelfand}\left(\varphi \right)}\mathfrak{PreGelfand}\left(A \right).
	\ee
	\begin{theorem}\label{good_thm}
	One has
			\begin{enumerate}
				\item [(i)] 	If the  homomorphism $\varphi : M\left( A\right) \to M\left( B\right)$ good 	  then the map \eqref{gelfand_map_eqn} is continuous (cf. Definition \ref{top_continuous_defn}).
	...
	\begin{proof}(i) 		If $p \in  A^{**}$ is an open projection (cf. Definition \ref{open_close_ideal_defn}) and $q = \varphi^{**}\left(p \right)\in B^{**}$ then $q$ is an open projection (cf. Lemma \ref{proj_lem})	
		\bean
		\mathfrak{Gelfand}\left(\varphi \right)^{-1}\left( \mathfrak{Gelfand}\left(A \right)_p\right) = \mathfrak{Gelfand}\left(\widetilde A \right)_{q}
		\eean 
		So any  set of basis (cf. Definition \ref{top_base_defn}) of the topology of  $\mathfrak{Gelfand}\left(A\right)$  corresponds to a set of subbasis of the topology of $\mathfrak{Gelfand}\left(B\right)$. From (c) of the Theorem \ref{top_continuous_thm} it turns out turns out that the map \ref{gelfand_map_eqn} is continuous. 
...
\begin{conjecture}\cite{akerman_gelfand}
%Conjecture ?.2. 
A regular  projection $p \in M$ is {\it compact}
if for every family $\left\{p_\a\right\}$ of closed projections such that the
family $\left\{p_\a \wedge p\right\}$ has the finite intersection property, then $p \wedge \bigwedge_\a p_\a\neq 0$.
\end{conjecture}
!!! REGULAR EXPL
