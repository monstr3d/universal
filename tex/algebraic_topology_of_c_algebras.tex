...
\begin{empt}\label{bicommutant_empt}
	Let $\varphi_M: A^\sim \hookto M\left(\widetilde A\right)$ be a good $*$-homomorphism (cf. Definition \ref{good_defn}). If $\widetilde \xi \in \mathfrak{Gelfand}\left(\widetilde A \right)$ and 
...
\begin{definition}\label{fin_pre_defn}
	Let   $A$ be an unital connected $C^*$-algebra  and let  $\widetilde{A}$ be  connected $C^*$-algebra (cf. Definition \ref{connected_c_a_defn}), and let $\lift: A^\sim  \hookto M\left( \widetilde{A}\right) $ be a good  $*$-homomorphism (cf. Definition 
	\ref{good_defn}) of  connected
	...
	\begin{enumerate}
		...
		\item[(b)] 	$A^\sim = M\left( \widetilde{A}\right) ^G\stackrel{\text{def}}{=}\left\{\left.a\in \widetilde{A}~~\right|\forall g \in G\quad  a = g a\right\}$.
	\end{enumerate}
...
\begin{definition}\label{cov_defn} 
	A  noncommutative  pre-covering $\left(A, \widetilde{A}, G, \lift \right)$ (cf. Definition \ref{fin_pre_defn})  is a {\it noncommutative covering} if one has

\begin{enumerate}
	\item[(a)] for any $\xi \in \mathfrak{Gelfand}\left(A \right)$ there is a hereditary connected $C^*$-subalgebra of $B$ {evenly covered by} $\left(A, \widetilde{A}, G, \lift \right)$ with $B \in \xi$ and  $*$-isomorphism $\lift^{\widetilde B}: B \cong \widetilde B$ (cf. Definition \ref{evenly_defn})
\item[(b)] $\widetilde{A}$ is the $C^*$-norm completion of $*$-subalgebra of $A$ generated by the union of its described in (a)  subalgebras $\widetilde B$.
\end{enumerate}	
...
\paragraph*{}	
In general the quantization yields a passage from the classical physics to the quantum one. In particular there is a quantization which yields an the transposition from commutative $C^*$-algebra $B$ to noncommutative ones. In particular a commutative  $C^*$-algebra $B$ with the product $\times$ is a depending on real parameter $\hslash$  limit of continuous fields  of noncommutative $C^*$-algebras $\overline B_\hslash$ with product $\times_\hslash$ such that
\begin{itemize}
	...
	\item For any $f, g \in B$ we have $\left\|\left(f\times_\hslash g - g\times_\hslash f \right) /\hslash - i\left\{f, g\right\}_J \right\|_{\hslash}$ AS $\hslash\to 0$, where $\left\{ ,\right\}_J$ is the Poisson bracket defined by $J$ (with a factor of $\pi$).
\end{itemize}
...
\ee
contains both a commutative $C^*$-algebra $C_0\left( \sX\right)$ and  a noncommutative one $A$. Elements of  $C_0\left( \sX\right)$  are limits of elements of $A$ with respect to the strict topology of $M\left(A \right)$. This duality can have rich structure. Physical fields correspond to vector bundles, or equivalently projective modules. The equation \eqref{chern_eqn} yields a bridge between projective $C\left(\sX^+ \right)$-modules and  $A^+$-modules. Physically it means that there is a set of common fields belonging to classical geometry and noncommutative one. The Rieffel projection on the noncommutative torus  correspond to a projective module which does not have a classical geometry analog. This circumstance can have following physical interpretation. Classical fields correspond  to the "experimentally  proven" phenomena.  Rieffel projection is responsible for phenomena which cannot be easily detected, e.g. vanishing of the divergences.  This analogy can be extends. For example the Dirac operator of noncommutative torus coincides with commutative one.
...
\paragraph{} If $\sX$ is a connected, locally compact, Hausdorff space then there is a homeomorphism
 $\mathfrak{Gelfand}\left(C_0\left(\sX \right)  \right)\cong \sX$. If  $\left(C_0\left(\sX \right), \widetilde{A}, G, \lift \right)$ is a noncommutative
 covering then there is the given by  \ref{gelfand_map_eqn} continuous map
$$
\mathfrak{Gelfand}\left(\lift \right): \mathfrak{Gelfand}\left(\widetilde A \right)\to \sX.
$$
Any $C^*$-subalgebra of $C_0\left( \sX\right) $ is commutative. From (b) of the Definition \ref{cov_defn} it follows that $\widetilde A$ is the  $C^*$-norm completion of $*$-algebra generated by its commutative $C^*$-subalgebras. So $\widetilde{A}$ is commutative and there is locally compact Hausdorff space % $\widetilde \sX$ with $\widetilde{A}\cong C_0\left( \widetilde\sX\right)$ (cf. Theorem \ref{gelfand-naimark_thm}). The Lemma \ref{lem_g_lem} yields  a natural  homeomorphism $ \mathfrak{Gelfand} {\widetilde _\sX}: \mathfrak{Gelfand}\left(C_0\left( \widetilde\sX\right) \right)\cong  \widetilde\sX$. If a map
$$
\phi_\sX : \widetilde \sX \to \sX
$$
if a specialization of \eqref{gelfand_map_eqn} one then from the Lemma \ref{good_lem} it follows that $\phi_\sX$ is continuous. 	If $B\subset A$  is a hereditary $C^*$-subalgebra evenly covered by $\left(C_0\left(\sX \right), \widetilde{A}, G, \lift \right)$ (cf. Definition \ref{evenly_defn}) then there is a connected open subset $\sU \subset \sX$ with $B \cong C_0\left( \sU\right) $. Using the Definition \eqref{evenly_defn} one can prove that $\sU$ is evenly covered by $\phi_\sX$ in the sense of the Definition \ref{top_covering_defn}. Thus there is an 1-1 correspondence between topological and noncommutative coverings of $C_0\left(\sX \right)$. If there is the universal covering of $\sX$ then from the Definition \ref{fund_den} that the fundamental group of $C_0\left(\sX \right)$ if it exists is isomorphic to $\pi_1\left( \sX\right)$ (cf. the Definition \ref{top_universal_covering_defn} and the Lemma \ref{top_simply_con_cov_lem}).
...
from the  Corollary  \ref{ctr_bic_cor} it follows that there is a pair $\left(x_\xi, y_\xi \right)$ such that for any $B \in \xi$ the one-dimensional subspace
$$
