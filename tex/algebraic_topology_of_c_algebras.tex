...
\begin{split}
	\sX \times \R^{r'+ r''} = \left(\sX \times \R^{r'+ r'' - 1}\right) \times \R,\\
	H^p_c\left(\R; \Z \right)= \begin{cases}
		0 & p \neq 1,\\
		1 & p = 1
	\end{cases}
	\end{split}
	\ee
so from  Theorem \ref{kunnet_sheaf_thm} it follows that
\be\label{kunnet_sheafr_eqn}
\begin{split}
	\bigoplus_{p+q = n} H^p_c\left(\sX \times \R^{r'+ r'' - 1}; \Z \right) \otimes H^q_c\left(\R; \Z \right)\hookto H^n_c\left(\sX \times \R^{r'+ r''}; \Z \otimes \Z \right)\onto\\ \onto  \bigoplus_{p+q = n+1} \mathrm{Tor}\left( H^p_c\left(\sX \times \R^{r'+ r'' - 1}; \Z \right),  H^p_c\left(\R; \Z \right) \right). 
\end{split}
\ee
Both \eqref{kunnet_sheafrr_eqn} and \eqref{kunnet_sheafr_eqn} yield the following group isomorphism $H^2_c\left(\sX \times \R^{r'+ r''}; \Z \right)\cong H^1_c\left(\sX \times \R^{r'+ r''-1}; \Z \right)$.
From \eqref{k01_s_eqn} it follows that  $\left[v'\right]$, $\left[v'\right]$ and $\bt$ can be regarded as elements of $K$-groups, i.e.
...
\begin{theorem}\label{bott_map_thm}\cite{blackadar:ko} $\bt_A$ is an isomorphism.
