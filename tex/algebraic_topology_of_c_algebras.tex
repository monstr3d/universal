...
\end{proof}
	...
	\begin{lemma}
		...
		\be
		\begin{split}
			\mathfrak{Gelfand}\left(A\otimes_\C C_0\left(\R \right)  \right) \cong 	\mathfrak{Gelfand}\left(A  \right)\times \R
		\end{split}
		\ee
	\end{lemma}
	\begin{proof}
...
\end{theorem}
	...
	\be\label{rp_eqn}
	\begin{split}
		p \in \Coo\left( \T^2_\th\right) \subset C\left( \T^2_\th\right)
	\end{split} 
	\ee
	 has the
	...
	\begin{theorem}\label{rp_nc_thm}\cite{Rieffel:irrat}
			...
			$p \in C\left(\T^2_\th \right)$  such that $\tau\left( p\right) =\bt$.
		\end{theorem}
	...
	obtained by pulling back the Bott projector from $K_0(\Coo(\Sf^2))$.]
