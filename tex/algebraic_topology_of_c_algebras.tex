...
\begin{remark}\label{good_ultrafilter_rem}
	From the Proposition \ref{top_ultra_prop} it follows that any locally compact Hausdorff  space is a set of its finite points,
\end{remark}
...
\end{lemma}
\begin{proof}
		$\Rightarrow$ If $b \in C_c\left(\sY \right)$ then the support $\supp b$ is compact. So the image $f\left(\supp b \right) \subset \sX$ is compact.  From the Tietze theorem  is $a\in C_c \left(\sX\right)$ with $a\left( f\left(\supp b \right)\right)  = \{1\}$, and $b = b\phi\left( a\right)$.
\\$\Rightarrow$ If $y\in \sY$ then there is   $b \in C_c\left(\sY \right)$ with $b\left( y\right) \neq 0$. If $a \in C_c\left(\sX \right)$ is such that $b = c \phi\left(a \right)$ with $c \in C_0\left( \sY\right)$ then $f\left(y \right) \in \supp\left(a \right) \subset \sX$.
\end{proof}
...
\begin{empt}\label{topology_empt} 
	...
	is a set of $A$-ultrafilters (cf. Definition \ref{lattice_a_defn}) then foe any for any closed left ideal $L \subset A$ we define a set
	\be\label{topology_eqn}
	\mathfrak{Ultrafilters}\left(A \right)_L	\bydef \left\{ \xi \in \mathfrak{Ultrafilters}\left(A \right) \left| L \in \xi\right.\right\}\subset 	\mathfrak{Ultrafilters}\left(A \right)
	\ee
\end{empt}
...
(a) If $\xi \in \mathfrak{Ultrafilters}\left(A \right) = \left\{ L_\la \right\}_{\la \in \La}$ then 
$$
\forall \la \in \La\quad  \xi \in 	\mathfrak{Ultrafilters}\left(A \right)_{L_\la}.
$$
(b) 
\bean
\forall \la', \la'' \in \La\quad \exists \la \in \La\quad L_\la = L_{\la'}\cap L_{\la''},\\
\mathfrak{Ultrafilters}\left(A \right)_{L_{\la'}}\cap\mathfrak{Ultrafilters}\left(A \right)_{L_{\la''}}=\mathfrak{Ultrafilters}\left(A \right)_{L_{\la}}.
\eean
\end{proof}
...
\begin{definition}\label{gelfand_space_defn}
	...
	Aa \in \xi.
	$$
	The \textit{Gelfand space} $\mathfrak{Gelfand}\left(A \right)$ of $C^*$-algebra $A$ is a space  of finite points with the final topology (cf. Definition \ref{top_final_defn}) given by the natural inclusion $\mathfrak{Gelfand}\left(A \right) \hookto \mathfrak{Ultrafilters}\left(A \right)$, where the topology on $\mathfrak{Ultrafilters}\left(A \right)$ is the   Gelfand topology (cf. Definition \ref{topology_defn} 
	\end{definition}
...
\end{lemma}
\begin{proof}
	From $1_A \in K\left(A \right)$ it follows that  $\mathfrak{Gelfand}\left(A \right)= \mathfrak{Ultrafilters}\left(A \right)$.  However the space $\mathfrak{Ultrafilters}\left(A \right)$ is compact (cf. Lemma \ref{gelfand_u_compact_lem}).
 \end{proof}

\begin{lemma}\label{ineq_lem}!!!
If $A$ is an unital $C^*$-algebra and $L', L''$ are two closed left ideals then one has
\bean 
L' \subseteqq L'' \quad \Rightarrow \quad \mathfrak{Ultrafilters}\left(A \right)_{L'}\subsetneqq \mathfrak{Ultrafilters}\left(A \right)_{L''}\quad \Leftrightarrow\\ \Leftrightarrow \mathfrak{Gelfand}\left(A \right)_{L'}\subsetneqq \mathfrak{Gelfand}\left(A \right)_{L''}.
\eean
\end{lemma}
\begin{proof}
If $a \in L'' \setminus L'$ then
\bean
\mathfrak{Ultrafilters}\left(A \right)_{Aa}\neq \emptyset,\\
\mathfrak{Ultrafilters}\left(A \right)_{Aa} \subset  \mathfrak{Ultrafilters}\left(A \right)_{L''}\setminus \mathfrak{Ultrafilters}\left(A \right)_{L'}.
\eean 
\end{proof}
%\begin{empt}
...
\end{definition}
\begin{remark}\label{hausdorff_sim_rem}!!!
	One has an alternative construction of the the space $\mathfrak{Hausdorff}\left(\sX \right)$. We define the minimal  equivalence relation $\sim_{\mathfrak{Hausdorff}}$ on $\sX$ such  that for all $x', x''\in \sX$ the condition    $x'\sim_{\mathfrak{Hausdorff}}x''$ holds if and only if for any neighborhoods $\sU'$ and $\sU''$ of $x'$ and $x''$ one has $\sU'\cap \sU'' \neq \emptyset$. In this case
	\be\label{hausdorff_sim_eqn}
	\mathfrak{Hausdorff}\left(\sX \right) \cong \sX / \sim_{\mathfrak{Hausdorff}}
	\ee
	We say that $\sim_{\mathfrak{Hausdorff}}$ is the {\it Hausdorff equivalence relation}.
	\end{remark}
	\begin{definition}\label{gelfand_r_defn}  
		If $A$ is a $C^*$-algebra then the universal Hausdorff quotient space $$\mathfrak{Hausdorff}\left(  \mathfrak{Gelfand}\left(A \right)\right)$$ of the Gelfand space $\mathfrak{Gelfand}\left(A \right)$ denoted by  $\mathfrak{Gelfand}_r\left(A \right)$ is {\it the reduced Gelfand  space of} $A$, i.e. $\mathfrak{Gelfand}_r\left(A \right)\bydef \mathfrak{Hausdorff}\left(  \mathfrak{Gelfand}\left(A \right)\right)$.
	\end{definition}
	
\begin{remark}\label{topology_h_rem}
	The basis of topology of the reduced Gelfand space are the sets
	\be\label{topology_h_eqn}
	\mathfrak{Gelfand}_r\left(A \right)_L \bydef \left\{\left.\xi \in 	\mathfrak{Gelfand}_r\left(A \right)\right| \exists \eta \in \mathfrak{Gelfand}\left(A \right)_L ~~ \xi \mathrm{~is~represented~by~}\eta\right\} 
	\ee
	where $L$ is a closed left ideal and $\mathfrak{Gelfand}\left(A \right)_L\subset \mathfrak{Gelfand}\left(A \right)$ is given by the equation \eqref{topology_eqn}.
\end{remark}
...
\begin{definition}\label{open_close_ideal_defn}\cite{akerman_stone}
	%DEFINITION 11.1. 
	A projection $p \in A''= A^{**}$ (cf. Definition \ref{env_alg_defn}) is {\it open} if there exists a
...
\begin{proposition}\label{open_close_ideal_prop}\cite{akerman_stone}
	%PROPOSITION 11.4. 
	The minimal closed projections in  $A''= A^{**}$ are the
	minimal projections.
\end{proposition}

\begin{empt} Here I follow to \cite{akerman_gelfand}.
Consider the $W^*$-algebra $A^{**}$, the second
Banach space dual of $A$. There exists a central projection
$z \in A^{**}$ which is the supremum of all the minimal projections in $A^{**}$. Set $M \bydef z A^{**}$. The minimal projections of M are in
...
\end{empt}
\begin{definition}\cite{akerman_gelfand}
%DEFINITION ?.l.
 A projection $p\in M$ is {compact} if $p$ is {\it closed}
and there exists $b \in A_+$ with $bp = p$.
\end{definition}

\begin{conjecture}\cite{akerman_gelfand}
%Conjecture ?.2. 
A regular projection $p \in M$ is {compact}
