...
If both $u, v \in C\left(\G^N_N \right)$ are given by
  ...
  From the Proposition \ref{inv_path_prop} it follows that there are continuous maps 
  $
...
\ee
One has a homeomorphism $\T^2 \cong S^1 \times S^1$ and a projector $p_{\T/S^1}: \T^2\to S^1$ such that $u_{\T^2} = u_{S^1}\circ p_{\T/S^1}$ where $u_{S^1}\in U\left(C\left(S^1 \right) \right)$ such that $K_1\left(C\left( S^1\right)  \right)= \Z\left[u_{S^1}\right]$  
\paragraph{DEL}
\begin{definition}\label{chern_defn!}
	Let  $\sV\in V\left(A \right)$ be an equivalence class of projections represented by $e_A\in 	\mathbb{M}_\infty \left( A\right)$ (cf. Definition \ref{projection_equivalence_defn}) such that there is the unique class  $\sV_\sX\in V\left(C\left(\sX^+ \right) \right) $ represented by $e_\sX\in 	\mathbb{M}_\infty \left(C\left( \sX^+\right) \right)$ with $\phi_A \circ \phi\left(e_A \right)\sim\phi_\sX\left( e_\sX\right)$ where $\sim$ is given by the Definition \ref{projection_equivalence_defn}. 
	The  {\it Chern classes} $c_j\left(\sV \right)\in H^{2j}\left(\sX^+, \Z \right)$ of $\sV$ as {Chern classes} $c_j\left(\sV_\sX \right)$ of $\sV_\sX$ (cf. Theorem \ref{k:char_thm})
