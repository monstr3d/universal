...
\end{pmatrix}\left( \Phi_{\T^2}\left( t\right)\right)^{-1};\\
  \eean
  From the proof of the Theorem \ref{suspension_thm} it follows that the maps $\Psi_{G^N_N}$ and $\Psi_{\T^2}$ yield the projections  $p_{G^N_N} \in \mathbb{M}_2 \left(SC_r^*\left(  \G^{N}_{N}\right)^+ \right)\subset  \mathbb{M}_\infty \left(SC_r^*\left(  \G^{N}_{N}\right)^+ \right)$ and  $p_{\T^2} \in \mathbb{M}_2 \left(SC\left(  \T^2 \right)^+\right)\subset \mathbb{M}_\infty \left(SC\left(  \T^2 \right)^+\right)$. 
...
\be\label{foli_p_eqn}
\phi_A \circ \phi \left(p_{G^N_N}\right)\sim  \phi_\sX \left( p_{\T^2}\right) 
\ee
where the relation $\sim$ is given by the Definition \ref{projection_equivalence_defn}. If
\be
\begin{split}
	\sV_{G^N_N} \bydef p_{G^N_N}\left(SC_r^*\left(  \G^{N}_{N}\right)^+ \right)^2,  \\
	\sV_{\T^2 }\bydef p_{\T^2}\left( \left(SC\left(  \T^2\right)  \right)^+\right)^2 
\end{split} 
\ee
then from the equation \eqref{chern_defn_eqn}
it follows that
\be
c_1\left( \sV_{G^N_N} \right) = c_1\left( \sV_{\T^2 }\right)\in H^2\left(\left(S \T^2\right)^+  ; \Z  \right)  
\ee
...
\bean
\sY \bydef \Sigma \T^2/\{x_0\}\cup \{x_1\} \onto \sY' \bydef \Sigma S^1/\{x_0\}\cup \{x_1\} \cong S^2 /\{x_0\}\cup \{x_1\}.
\eean 
Using \cite{karoubi:k} one can prove that $K_1\left(C\left( S^1\right)  \right)\cong \Z\left[u_{S^1}\right]$ and $K_1\left(C\left( \T^2\right)  \right)\cong \Z\left[u_{\T^2}\right]\oplus \Z\left[v_{\T^2}\right]$ and f
\paragraph{RRRRR}
