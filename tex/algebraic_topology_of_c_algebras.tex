...
If $\la =  \hat \phi\left(\xi' \right) =  \hat \phi\left(\xi' \right) \in M/\sim$ then from \eqref{not_bel} and the $C^*$-norm \eqref{groupoid_red_norm} it follows that the equations \eqref{foli_topology_base_herd_eqn} are equivalent to the following ones
 ...
 There is an open neighborhood  $\mathcal W \subset M$ of $y$ and elements $a'\in B'$, $~a'' \in B''$ such that $\mathcal W\subset \supp a'\cap \supp a''$ where $\supp$ means the support  (cf. Definition \ref{blowing_support_defn}). The equation $\sV'\cap \sV'' = \emptyset$ means that there is no a nontrivial closed left ideal $L$ such that]
$$
