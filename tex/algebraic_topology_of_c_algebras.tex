...
\begin{remark}\label{cup_prod_suspernsion_rem}
	If $A$ is an unital $C^*$-algebra then from the Theorems \ref{suspension_thm} and \ref{bott_map_thm} it turns out that there is the $\Z_2$  isomorphism of Abelian groups.
	\be\label{cup_prod_suspernsion_eqn}
	\begin{split}
		S : K_*\left( A\right) \cong K_*\left( SA\right) 
	\end{split}
	\ee
	where $S$ is the  suspension (cf. Definition \ref{suspension_defn}). The isomorphism inverses grading (is degree 1), i.e. it is given by maps
	\be\label{cup_prod_suspernsiong_eqn}
	\begin{split}
		\forall j = 0, 1 \quad	S : K_{j}\left( A\right) \cong K_{1 - j}\left( SA\right) 
	\end{split}
	\ee
Using the equations \eqref{cup_product_eqn} \eqref{red_eqn}  \eqref{red_c_eqn} one can prove that $S$ is the ring isomorphism.
\end{remark}
...
\end{split}
\ee
Let us prove that $\dl$ in \eqref{coh_x_eqn} yields the homomorphism $\partial K_0\left( A/J\right) \to K_1\left( J\right)$. Any $x \in H^0_{c}\left(\mathfrak{Gelfand}_r\left(A/J\right);\Z \right)$ can represented by  $f_x \in C_{c}\left(\mathfrak{Gelfand}_r\left(A/J\right);\Z \right)$. The support of $f_x$ is compact so from the Theorem \ref{tietze_ext_thm} there is real valued $y \in C_c\left( \mathfrak{Gelfand}_r\left(A\right)\right)$ such that 
\be
\forall \xi \in \mathfrak{Gelfand}_r\left(A/J\right)\quad f_x\left( \xi\right) = y\left( \xi\right) 
\ee 
If  
\be\label{u_x_eqn}
u_x \bydef e^{2\pi i y}
\ee
then $u_x$ can be regarded  an element of both  $U\left(C_0\left( \mathfrak{Gelfand}_r\left(A/J\right)^+\right)  \right)$, $H^0_{c}\left(\mathfrak{Gelfand}_r\left(J\right);U\left( 1\right)\right)$ and $\left[u_x\right]=\partial x \in K_1\left( J\right)$ (cf. equations \eqref{u_x_eqn} and \eqref{exp_map_eqn}).

\begin{theorem}\label{six_term_thm}
The Abelian group homomorphisms $\partial$ in \eqref{sisf_ext_eqn} are degree 1 ring homomorphisms.
\end{theorem}
\begin{proof}
	!!!!
\end{proof}
...
\bean
	K_*\left(A \right) \cong K_*\left(A \rtimes_\a \R \right). 
\eean
From the Remark \ref{c_t_rem} it turns out that this isomorphism coincides with \eqref{c_t_eqn} one.
\end{proof}	
...
\end{tikzcd}
\ee
comes from  \eqref{p_v_eqn} then both Abelian group homomorphism $\iota_*$ and $\partial$ of \eqref{pimsner_voiculesky_eqn} are  ring homomorphisms having degree 0 and 1 respectively.
\end{theorem}	
\begin{proof}
...
The Theorem \ref{p_v_theorem} yields  a cyclic six-term exact sequence
	\bean
	\begin{tikzcd}
		K_0\left( C\left(u_{S^1} \right)\right)\arrow[r, "1-\a^{u_{S^1}}_*"] & K_0\left( C\left(u \right)\right)\arrow[r, "\iota^{u_{S^1}}_*"] & K_0\left( C\left({u_{S^1}} \right) \rtimes_\a \Z \right) \arrow[d]\\
		K_1\left(  C\left(u \right)\rtimes \Z\right)\arrow[u] & \arrow[l, "\iota^{u_{S^1}}_*"] K_1\left( C\left(u \right) \right) & \arrow[l, "1-\a^{u_{S^1}}_*"]K_1\left( C\left(u \right)  \right)
	\end{tikzcd}
	\eean
...
\begin{empt}\cite{blackadar:ko}
	...
	\end{proof}
\end{empt}
\begin{remark} It is proven in \cite{blackadar:ko} that
