...
\end{proof}
\begin{empt}\label{ultrafilters_empt}
...
\mathfrak{Ultrafilters}\left(A \right)_L	\bydef \left\{\left. x \in \mathfrak{Ultrafilters}\left(A \right) \right| \exists L' \in x\quad L'\cap L = \{0\} \right\}
	\ee
	Consider the smallest topology on $\mathfrak{Ultrafilters}\left(A \right)$ such that all sets $\mathfrak{Ultrafilters}\left(A \right)_L$ are open.
...
\begin{definition}\label{gelfand_space_defn}
  ...
  Aa \in x.
	$$
	The \textit{Gelfand space} $\mathfrak{Gelfand}\left(A \right)$ of $C^*$-algebra $A$ is a topological subspace  of the described in \ref{ultrafilters_empt} space $\mathfrak{Ultrafilters}\left(A \right)$ of ultrafilters such that 
	$$
...
\begin{remark}\label{hered_ideal_rem}
	Using the Lemma \ref{hered_ideal_lem} one can replace the closed left ideals in the Definition \ref{gelfand_space_defn} with right ones or hereditary $C^*$-subalgebras. So we use following notation
	\be\label{hered_ideal_eqn}
	L \in x \quad \Leftrightarrow \quad B \bydef L \cap L^* \in x.
	\ee
\end{remark}
...
\begin{split}
	\mathfrak{Lattice}\left(\widetilde  A\right) \xrightarrow{\mathfrak{Lattice}\left(\varphi_R \right)}\mathfrak{Lattice} \left( A\right),\\
		\widetilde{I} \mapsto \bigcap \left\{L \subset A \left| \widetilde{L}\subset \widetilde{A}\varphi_R\left( L\right) \right.\right\}.
	\end{split}
	\ee
...
\bean 
\widetilde J \notin \widetilde x,\\
\forall \widetilde L \in \widetilde x \quad \widetilde J \cap \widetilde L \neq \{0\}.
\eean
...
The injective homomorphism \eqref{lat_mor_eqn} is \textit{good} if the given by \eqref{ultra_mor_eqn} mapping $\mathfrak{Ultrafilters}\left(\varphi_R \right)$ yields the natural  map
...
\paragraph{} if $\H$ is a Hilbert space and $A = \K\left(\H \right)$ then there is a set theoretic bijection $\mathfrak{Gelfand}\left(\K\left(\H \right) \right)\cong  \C P\left(\H \right)$. The subbase of $\mathfrak{Gelfand}\left(\K\left(\H \right)\right) $ contains all sets $\C P\left(\H \right) \setminus L$ where $L$ is a linear subspace of  $\C P\left(\H \right)$. Similarly to \eqref{pe_c_eqn} there is a continuous map 
$$
...
\subsection{Gelfand space continuous trace $C^*$-algebra}
\paragraph{}
...
\end{exercise}
\begin{lemma}
	If $A$ is a {continuous-trace} $C^*$-{algebra} then there is the natural homeomorphism $\mathfrak{Gelfand}\left( A\right) \cong \mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)$  with the following commutative diagram
	\newline
	\begin{tikzcd}
