...
\begin{empt}\label{topology_empt} 
	...
	is a set of $A$-ultrafilters (cf. Definition \ref{lattice_a_defn}) then foe any for any closed left ideal $L \subset A$ ww define a set
...
\end{lemma}
\begin{proof}
!!!! From the construction \ref{topology_empt} !!! \ref{topology_defn} and the Proposition \ref{top_ultra_prop} it follows that the space of $A$-ultrafilters is compact.  
\end{proof}
\begin{lemma}\label{one_point_s_lem}
If $A$ is a nonunital $C^*$ algebra and $A^+\bydef A \oplus \C$ is a unitization 
then the difference $\mathfrak{Gelfand}\left(A^+ \right)\setminus \mathfrak{Gelfand}\left(A \right)$ contains the single point.
\end{lemma}

\begin{proof}
$A \subset A^+$ is the maximal ideal, it correspond to an open projection of $p \in \left( A^+\right)^{**}$ (cf. Definition \ref{open_close_ideal_defn}). From the Proposition \ref{open_close_ideal_prop} it turns out that $p' \bydef 1-p$ is the minimal projection in $\left( A^+\right)^{**}$ the set of ideal which correspond to the projections which exceeds $p'$ is an ultrafilter. !!!!
\end{proof}
\begin{lemma}
...
%A^\sim\bydef\begin{cases}
	...
	\begin{definition}\label{gelfand_functor_defn}
		...
		C^*\text{-}\mathfrak{Gelfand}\xrightarrow{\mathfrak{Gelfand}}\mathfrak{Top},\\\label{gelfand_h_functor_eqn}
			C^*\text{-}\mathfrak{Gelfand}\xrightarrow{\mathfrak{Hausdorff}~\circ~\mathfrak{Gelfand}}\mathfrak{Hausdorff}
		\eea
...
\section{Lattices and filters}
	...
	\end{proposition}
	\begin{remark}
		From (b) of the Proposition \ref{top_ultra_prop} it follows that the notion of ultrafilter is not adequate analog of the point of Hausdorff , locally compact space. One needs the finer notion.
	\end{remark} 
...
\end{thm}
\subsection{Open and closed projections}
