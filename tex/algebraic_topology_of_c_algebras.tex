...
\subsection{The Gelfand space of $C^*$-algebra}
	...
	Let $A$ be a partially ordered set (poset), and let $S$ be a subset of $A$. We say an element $a \in A$
...
\paragraph{} If $\H$ is a Hilbert space and $A = \K\left(\H \right)$ then for any  left ideal $L\subset $ there is a closed $\C$-linear subspace $V_L \subset \K\left(\H \right)$ such that
\bean
...
\bean H^0\left( \sU^{\mathfrak{Hausdorff}}_{\sigma};\mathscr F_{\mathfrak{Hausdorff}} \right)= F, \\H^p\left( \sU^{\mathfrak{Hausdorff}}_{\sigma};\mathscr F_{\mathfrak{Hausdorff}} \right)= 0
\eean
From the Theorem \ref{top_nerve_thm} it turns out that
$$
H^*\left(\C P^n_{\mathfrak{Hausdorff}}, \mathscr F_{\mathfrak{Hausdorff}} \right) \cong \check H^*\left( \mathscr W; \mathscr F_{\mathfrak{Hausdorff}}\right)
$$
...
\end{definition}
\begin{definition}\label{pap_dern}
	If we consider the equation \eqref{phi_a_eqn} then the map
	$$
\sU \mapsto \phi^{-1}\left(\sU \right) 	
	$$
yields the $A$-$\sX$-\textit{presheaf} denoted by $\mathscr P_{A-\sX}$,
\end{definition}
...
\begin{empt}\label{sheaf_empt}
	...
	\end{itemize}
\end{empt}
\begin{definition}\label{pas_defn}
	If $\mathscr P_{A-\sX}$ is a $A$-$\sX$-{presheaf} (cf. Definition \ref{pap_dern})  then $A$-$\sX$-\textit{sheaf} is associated to $\mathscr P_{A-\sX}$. We denote  by $\mathscr S_{A-\sX}$ the $A$-$\sX$-{sheaf}.
\end{definition}
...
\subsubsection{Basic constriction}
...
If $A$  is a continuous trace  $C^*$-algebra then the spectrum $\sX$ of $A$ is a Hausdorff, locally compact space  $\sX$. There is Hausdorff blowing-up $C_0\left(\sX \right) \to M\left( A\right)$ and the given by \eqref{phi_a_eqn} natural continuous map  	\bean
\phi_A : \mathfrak{Gelfand}\left(A \right) \to \sX.
\eean

For any pair  $\sU\subset\sV$ of open subsets of $\sX$ one has a presheaf
\bean
\mathscr{P}\left( \sU\right) \bydef \text{ a set of all commutative hereditary subalgebras of } ~_\sU A_{\sU};\\
\rho_{\sU \sV}: \mathscr{P} \left(\sU\right)\to  \mathscr{P} \left(\sV\right), \quad C \mapsto C \cap ~_\sV A_{\sV}.
\eean
where both $_\sU A_{\sU}$ and $_\sV A_{\sV}$ are given by \eqref{blowing_compact_b_eqn}.
\begin{exercise}\label{sheaf_etale_exer}\cite{hartshorne:ag}
	...
	follows. As a set, $\mathrm{Sp\acute{e}}\left(\mathscr F \right)= \bigcup_{x\in\sX} \mathscr F_x$. We define a projection map 
	\be\label{pf_eqn}
...
\begin{lemma}\label{hausdorff_spectrum_lem}\cite{rae:ctr_morita}
	...
	\end{itemize}
\end{lemma}
Suppose that $A$ is given by a locally trivial  fibre bundle $\F$ with fibre $\K = \K\left(\H \right)$. There is a subbundle $\E \subset \F$ such that fibers of $\E$ are  rank-one positive operators. It gives a locally trivial bundle $\C P\left(\E \right)\to \sX$. On the other hand any  $\xi \in 	\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)$ can be represented by a commutative $C^*$-subalgebra $C \subset A$. On the the other hand if 
	\bean
y \bydef p_{\mathrm{Sp\acute{e}}\left(\mathscr F \right)}\left(\xi \right) 
\eean
(cf. \eqref{pf_eqn}) and $\rho_y : A \to B\left(\H_y \right)$ is the corresponding to $y$ irreducible representation then $\rho_y\left(C \right)$ yields an element of $\C P\left(\H_y \right)\subset  \C P\left(\E \right)$. In result there is a commutative diagram\\
    	\newline
  \begin{tikzcd}
 \mathfrak{Gelfand}\left( A\right)\cong  	\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)	 \arrow[rr]\arrow[rd]  & ~ & \C P\left(\E \right) \arrow[ld]\\
  	& \sX &
  \end{tikzcd}



 with fibre $\C P\left( \H\right)$. From the Lemma \ref{ctr_gelfand_lem}  it turns out that there is the natural set theoretic bijective map $\mathfrak{Gelfand}\left(A \right)\cong  \C P\left(\E \right)$.
