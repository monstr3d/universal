...
\end{pmatrix}\left( \Phi_{\T^2}\left( t\right)\right)^{-1};\\
  \eean
  From the proof of the Theorem \ref{suspension_thm} it follows that the maps $\Psi_{G^N_N}$ and $\Psi_{\T^2}$ yield the projections  $p_{G^N_N} \in \mathbb{M}_2 \left(SC_r^*\left(  \G^{N}_{N}\right) \right)\subset  \mathbb{M}_\infty \left(SC_r^*\left(  \G^{N}_{N}\right) \right)$ and  $p_{\T^2} \in \mathbb{M}_\infty \left(C\left(  \T^2 \right)\right)\subset \mathbb{M}_\infty \left(C\left(  \T^2 \right)\right)$.  Similarly to \eqref{chern_eqn} there is a diagram
\be\label{tori_chern_eqn}
\begin{tikzcd}
\Psi:	\mathbb{M}_\infty \left(SC_r^*\left(  \G^{N}_{N}\right) \right)  \ar[r,hook, "\phi"] & SC^*_r\left(\G^{\T^2}_{\T^2} \right)  \ar[rd,hook, "\phi_A"]  &\\
	& & 	 M\left( SC^*_r\left(\G^{\T^2}_{\T^2}   \right)\right)  \\
...
There  is a map 
\bean
\Psi_{S^1}:[0, 1] \to \mathbb{M}_2\left( C\left( \T^2\right)
\right),\\
...
\be 
\begin{tikzcd}
K\left(0, 1 \right) \times S^1\cong K\left( S^2 \setminus \{x_0\}\sqcup \{x_1\}\right)\arrow[r] \arrow[d, "\cong"] &K\left( S^2  \right) \arrow[r, "\phi_{S^2}"] \arrow[d, "\phi_{S^2/\left( SS^1 \right)^+} "] &K\left( \{x_0\}\sqcup \{x_1\} \right)\arrow[d]\\
K\left(0, 1 \right) \times S^1\cong K\left(\left(SS^1 \right)^+  \setminus \{y_0\}\right)\arrow[r]  &K\left(\left(SS^1 \right)^+     \right) \arrow[r, "\phi_{\left( SS^1\right)^+ }"]  &K\left( \{y_0\} \right)\end{tikzcd}
\ee
One can proof that
$$
\ker \phi_2 = \Z \left(\left[H\right] -\left[1\right] \right) 
$$
where $H$ is the canonical line bundle (cf. \ref{kt_kan_line_empt})
