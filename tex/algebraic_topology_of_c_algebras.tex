...
If both $A$ and $B$ are $C^*$-algebras  then a $*$-homomorphism $\phi : A \to M\left( B\right) $ is {\it good} if for any element $b \in K\left(B\right)$ of the Pedersen's ideal (cf. Definition \ref{pedersen_ideal_defn})  there is $a \in K\left(A\right)$ such that $b 
...
\begin{theorem}\label{good_thm}
	If the  homomorphism \eqref{lat_mor_eqn} is good 	  then the map \eqref{gelfand_map_eqn} is continuous and !!! open (cf. Definitions \ref{top_continuous_defn} and \ref{top_onen_map_defn}).
\end{theorem}
\begin{proof}
...
If $\widetilde A$ is a $C^*$-algebra and $\mathfrak{Gelfand}\left(\widetilde A \right)$ is its Gelfand space then an open continuous map $\phi: \mathfrak{Gelfand}\left(\widetilde A \right)\to \sX$ is {\it good} is there is a good $*$-homomorphism $\varphi: A \hookto \widetilde{A}$ (cf. Definition \ref{good_defn}) such that the map $\phi$ is equivalent to the given by the equation \eqref{gelfand_map_eqn} map  $
\mathfrak{Gelfand}\left(\widetilde A \right)\xrightarrow{\mathfrak{Gelfand}\left(\varphi_M \right)}\mathfrak{Gelfand}\left(A \right)$.
