...
\begin{definition}\label{blowing_uni_defn}
	...
	\end{enumerate}
		The final object (cf. Definition \ref{terminal_ob_defn})of this category $C_0\left( \sX\right) \hookto M\left(A \right)$ is  \textit{universal Blowing-up of $A$}.  Hausdorff blowing-up $C\left(\sX \right)   \to M\left( A\right)$ is {\it maximal}  if for any Hausdorff blowing-up $C_0\left(\sX' \right)   \to M\left( A\right)$ with the commutative diagram
			\newline
		\begin{tikzcd}
			&M\left(A \right)   & \\
...
\end{definition}
\begin{lemma}
If $A$ is a $C^*$-algebra and $C_0\left( \sX\right) \hookto M\left(A \right)$ is maximal Blowing-up (cf. Definition \ref{blowing_uni_defn}) then $\sX$ is naturally isomorphic to the universal Hausdorff quotient space $\sY$ (cf. Definition \ref{blowing_uni_h_defn}) of the Gelfand space $\mathfrak{Gelfand}\left( \sX\right)$ (cf. Definition \ref{gelfand_space_defn}) of $A$.
\end{lemma}
\begin{proof}
	According to the Definition \ref{blowing_uni_h_defn} and the equation \ref{phi_a_eqn} it follows that there is a continuous surjective map $\phi : \sY \onto \sX$. If phi is mot homeomorphism then there are tho points $y', y'' \in \sY$ such that $y' \neq y''$ and $\phi\left(y' \right) = \phi\left(y'' \right)$. There are open neighborhoods  $\sU$, $\sV'$ and $\sV''$ of $x$, $y'$ and $y''$ such that $\sV'\cap \sV'' = \emptyset$ and $\sU \subset \phi\left(\sV' \right)$,  $~~\sU \subset \phi\left(\sV'' \right)$. If both $L'
	\subset A$ and $L''\subset A$ are $\sV'$- and $\sV''$- ideal (cf. Definition \ref{open_ideal_defn} respectively then from $\sV'\cap \sV'' = \emptyset$ it follows that $L' \cap L'' = \{0\}$. Any $f\in C_0\left(\sU \right)$ 
	
\end{proof}
\subsubsection{Algebraic counterpart of geometric quotient and its inversion}
\paragraph*{} If $\sX$ is a Hausdorff, locally compact space and $\sim$ is an equivalence equation on $\sX$  then there is the described by algebraic counterpart of geometric quotient (cf. Appendix \ref{quotions_a_sec}). Suppose that $A$ is a $C^*$-algebra such that the spectrum $
\sY$ of $A$ is homeomorphic to the quotient i.e. there is a map 
\be\label{sp_homeo_eqn} 
\begin{split}
\phi_{\sX\text{-} A} : \sX \to \sY,\\
\forall x', x'' \quad  x' 
\sim x'' \quad \Leftrightarrow  \quad \phi_{\sX\text{-} A}\left( x'\right) = \phi_{\sX\text{-} A}\left( x'\right) 
\end{split}
\ee
In several cases this it satisfies to the following definition.
\begin{definition}
 If $\sX$ is a Hausdorff, locally compact space and $\sim$ is an equivalence equation on $\sX$ then  $C^*$-
{\it algebraic representative of} $\sX/\sim$ is a $C^*$-algebra $A$ with  Hausdorff bowing-up $C_0\left(\sX \right)\to M\left( A\right) $ such that if $\sY$ is the spectrum of $A$ then there is a given by \eqref{sp_homeo_eqn} map.
\end{definition}
...
\begin{lemma}
	...
	If $A$ is a $C^*$-algebra and  $\mathfrak {Blowing}_{\sX-A }:C_0\left( \sX\right) \hookto M\left(A \right)$  Hausdorff bowing-up then $\sX$ is the $\mathfrak{Gelfand}\left(A \right)$-universal Hausdorff apace.
\end{lemma}
\begin{proof}
	!!!!
\end{proof}
