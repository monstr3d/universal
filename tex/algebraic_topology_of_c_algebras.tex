...
so taking into account  \eqref{royden_eqn} one has
	...
	\bean
	\begin{split}
		\{0\}  \to  H^0_c\left(\sX, \Z\right)  \to  H^0_c\left(\sX, \sR \right)\to  H^0_c\left(\sX, \sS \right)\xrightarrow{\dl_\SS }  H^1_c\left(\sX, \Z \right)\to  H^1_c\left(\sX, \sR \right)\to ... 
	\end{split}
	\eean
		...
		\be\label{dl_sh_eqn}
	\dl_\SS	: H^0_c\left(\sX, \sS \right)\onto H^1_c\left(\sX, \Z \right)
		\ee
	is surjective.
	...
	\begin{split}
		K_{0}\left(A^+ \right) \cong H^0\left(\mathfrak{Gelfand}\left(A \right)^+; \Z \right)/U\left(A^+ \right),\\
				K_{0}\left(A^+ \otimes C\left(S^1 \right) \right) \cong H^0\left(\mathfrak{Gelfand}\left(A \right)^+\times S^1; \Z \right)/U\left(A^+\times C\left(S^1 \right)  \right). 
	\end{split}
	\ee 
	The identical map $u: S^1 \cong S^1$ yields an element $\left[u\right] \in H^0\left(S^1; \sS \right)$ such that 
	 \be
	 	H^1_c\left(S^1 ;\Z\right)= \Z \left(\dl_\SS \left[u\right] 
	 	\right) 
	 \ee 
	 If $e \in C\left( \mathfrak{Gelfand}\left(A \right)^+; \Z\right)\cong H^0\left( \mathfrak{Gelfand}\left(A \right)^+; \Z\right)$ is an idempotent 
	 then there is u unitary element
	 \be\label{e_u_eqn}
	 \begin{split}
u_{\mathfrak{Gelfand}}\bydef e\otimes u + \left( 1 -e\right)\otimes 1_{C\left(S^1 \right) } \in C\left( U\left( \mathfrak{Gelfand}\left(A \right)^+\times S^1\right)\right) 	 
\end{split}
\ee
 yields elements
	 \be 
	 \begin{split}
	 \left[u_{\mathfrak{Gelfand}}\right]\in H^0\left(\mathfrak{Gelfand}\left(A \right)^+\times S^1; \SS \right),\\
	 \dl_\SS  \left[u_{\mathfrak{Gelfand}}\right]\in H^1\left(\mathfrak{Gelfand}\left(A \right)^+\times S^1; \Z \right)
	 \end{split}
	 \ee
	 Comparison of the  equations  \eqref{e_u_eqn} and \eqref{idem_map_eqn} enables us to state that the map
	  \be 
	 \begin{split}
	\Phi : H^0\left( \mathfrak{Gelfand}\left(A \right)^+, \Z\right) \to H^1\left(\mathfrak{Gelfand}\left(A \right)^+\times S^1 \right) ,\\
	\left[e\right]\mapsto  \dl_\SS  \left[u_{\mathfrak{Gelfand}}\right]
	 \end{split}
	 \ee
yields the isomorphism 
	  \be 
\begin{split}
\bt_{ \mathfrak{Gelfand}\left(A \right)^+}: K_0\left(  C\left( \mathfrak{Gelfand}\left(A \right)^+\right) \right) \cong !!!
\end{split}
\ee
From \eqref{e_u_eqn} ir follows that the map $\Phi$ is an isomorphism of rinds so since $\bt_{ \mathfrak{Gelfand}\left(A \right)^+}$ the
\bean
I \bydef \ker \left(  H^0\left( \mathfrak{Gelfand}\left(A \right)^+, \Z\right)\to K_0\left(C\left( \mathfrak{Gelfand}\left(A \right)^+ \right)\right)  \right) 
\eean
