...
\begin{remark}\label{cup_prod_suspernsion_rem}
	...
	\end{split}
	\ee
	where $S$ is the  suspension (cf. Definition \ref{suspension_defn}). The isomorphism inverses grading, i.e. it is given by maps
...
\end{proof}	
	...
	The left shift of $\Z$ yields the natural $*$-automorphism $\ga \in \Aut\left(\K \right) \cong \Aut\left(\K \right)\cong \Aut\left( \K\left( \ell^2\left( \Z\right) \right) \right)$. Similarly to the Proposition \ref{k_aut_prop} we consider the action $\Z \times A\otimes \K A\otimes \K$ generated by $\left(\a\otimes\ga \right)$. i.e. we imply $\a^\K \bydef \a\otimes\ga$. 
		If $A$ is an unital $C^*$-algebra then  Theorems \ref{k_h_thm} and \ref{p_v_theorem} yield the following  a cyclic six-term exact sequence
	\be\label{k_hom_seq_eqn}
	\begin{tikzcd}
	 H^0_c\left(\sX; \Z \right)/U \arrow[r, "1-\a^\K_*"] &  H^0_c\left(\sX; \Z \right)/U\arrow[r, "\iota_*"] &H^0_c\left(\sX_\Z; \Z \right)/U_\Z \arrow[d]\\
		 H^1_c\left(\sX_\Z; \Z \right)\arrow[u] & \arrow[l, "\iota_*"]  H^1_c\left(\sX; \Z \right) & \arrow[l, "1-\a^\K_*"]H^1_c\left(\sX; \Z \right)
	\end{tikzcd}
	\ee
	where 
	\bean
	\sX \bydef\mathfrak{Gelfand}_r\left(A \otimes \K\right),\\
	\sX_\Z \bydef\mathfrak{Gelfand}_r\left(\left( A\rtimes_{\a^\K} \Z\right)  \otimes \K\right),\\
	U \bydef U\left(\left(A\otimes \K \right) ^+ \right),\\
	U_\Z \bydef U\left(\left(\left( A\rtimes_{\a^\K} \Z\right)\otimes \K \right) ^+ \right)
	\eean
From the Theorems \ref{suspension_thm} and \ref{bott_map_thm} the sequence \eqref{k_hom_seq_eqn} is equivalent to the following one.
	\be\label{k_hom_sequ_eqn}
\begin{tikzcd}
	H^0_c\left(\sX; \Z \right)/U \arrow[r, "1-\a^\K_*"] &  H^0_c\left(\sX; \Z \right)/U\arrow[r, "\iota_*"] &H^0_c\left(\sX_\Z; \Z \right)/U_\Z \arrow[d]\\
	H^0_c\left(\sX_S; \Z \right)\arrow[u] & \arrow[l, "\iota_*"]  H^0_c\left(\sX_S; \Z \right) & \arrow[l, "1-\a^\K_*"]H^0_c\left(\sX_S; \Z \right)
\end{tikzcd}
\ee
	
	\begin{definition}\label{mapping_torus_defn!}
		%Definition 10.3.1. 
