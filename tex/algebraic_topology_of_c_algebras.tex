...
%A^\sim\bydef\begin{cases}
	...
	\end{lemma}
	\begin{proof}
		Let  $a \in K\left(A \right) $ is a normal element (cf. Definition \ref{normal_defn}). If $C_0\left(\sX \right)\subset K\left(A \right) $ is the generated by $a$ commutative $C^*$-subalgebra then the Theorem \ref{good_thm} yields a continuous map $\phi: \mathfrak{Gelfand}\left(A  \right)\onto \mathfrak{Gelfand}\left(A  \right)\onto \sX$. If $f\in C_0\left( \sX\right)$ corresponds to $a$ then $a$ is the image of $f\circ \varphi$ where $\varphi: \mathfrak{Gelfand}\left(A  \right)\onto \sX$ comes from $\phi$.  
	\end{proof}
