...
%A^\sim\bydef\begin{cases}
	...
	\end{lemma}
	\begin{proof}
		Let  $a \in K\left(A \right) $ is a normal element (cf. Definition \ref{normal_defn}). If $C_0\left(\sX \right)\subset K\left(A \right) $ is the generated by $a$ commutative $C^*$-subalgebra then the Theorem \ref{good_thm} yields a continuous map $\phi: \mathfrak{Gelfand}\left(A  \right)\onto \mathfrak{Gelfand}_r\left(A  \right)\onto \sX$. If $f\in C_0\left( \sX\right)$ corresponds to $a$ then $a$ is the image of $f\circ \varphi$ where $\varphi: \mathfrak{Gelfand}_r\left(A  \right)\onto \sX$ comes from $\phi$.  
	\end{proof}
...
\begin{lemma}\label{one_point_lem}
	...
	\be 
	\begin{split}
		\mathfrak{Gelfand}\left(A^+ \right)\cong \mathfrak{Gelfand}\left(A \right)\cup\{+\}
	\end{split}
	\ee
...
\begin{lemma}\label{ring_auto_lem}
	Any $g \in U\left(\left(A\otimes \K \right) ^+ \right)$ yields a ring automorphism of $H^0_c\left(\mathfrak{Gelfand}_r\left(A\otimes \K \right)\right) $ with respect to the cup-product (cf. Definition \ref{cup_thm}).
\end{lemma}	
\begin{proof}
...
\begin{remark}\label{blow_rem}
	...
	\be\label{phi_a_eqn}
	\phi_A : \mathfrak{Gelfand}_r\left(A \right) \onto \sX.
	\ee
\end{remark}
...
\begin{empt}\label{blowing_empt}
	...
	\ee
	If for all  $\xi \in \mathfrak{Gelfand}_r\left(A \right)$  such that there is a bicommutant (cf, Definition \ref{bicommutant_defn}) $\xi'' \subset A$ then there is the unique $y \in \sY$ such that $\rho_y\left( L\right) \H_y \neq \H_y$. It turns out that there ia a following commutative diagram
	\be\label{gg_m_eqn}
	\begin{tikzcd}
		\mathfrak{Gelfand}_r\left( A\right)	 \arrow[rr, "~"]\arrow[rd]  & ~ & \mathfrak{Gelfand}_r\left( A\right)'' \arrow[ld, , "~"]\\
		& \sY &
	\end{tikzcd}.
	\ee
	...
	\bean
	\begin{tikzcd}
		\mathfrak{Gelfand}_r\left( A\right)	 \arrow[rr, "\phi_A"]\arrow[rd]  & ~ & \sX \arrow[ld, , "\mathfrak{Blowing}"]\\
		& \sY &
	\end{tikzcd}
	\eean
...
It is known that $C\left(\T^2_\th \right)$ is an universal $C^*$-algebra generated by two unitaries $u, v \in C\left(\T^2_\th \right)$ such that $uv = e^{2\pi i \th}vu$. It is also known 
 that $C\left(\T^2_\th \right) \cong C\left(u \right)\rtimes_{\a_u} \Z$, where $\a_u\in \Aut\left(C\left( S^1\right)  \right) \cong \Aut\left(C\left( u\right)  \right)$ is a $*$- automorphism given by $u \mapsto e^{2\pi i \th} u$.
The Theorem \ref{p_v_theorem} yields  a cyclic six-term exact sequence
	...
	\be\label{iota_torus}
\iota:	C\left(u \right) \hookto C\left(u \right) \rtimes_\a \Z
	\ee
...
\begin{split}
	...
	\end{split}
\ee	
If we consider the cup product (cf. Theorem \ref{cup_thm}) then one has 
...
\ee
it the Powers - Rieffel projector. If $\mathfrak{Image}\left( p_\th\right)\in C\left( \mathfrak{Gelfand}_r\left(  C\left(u \right)\rtimes_\a \Z\right)\right) $ is the image of $p_\th$ (cf. Definition \ref{image_defn}) then $\mathfrak{Image}\left( p_\th\right)$ is a projector represents 
\bean
...
\ee
where $\left[ p_\th\right]\in K_0\left( C\left(\T^2_\th \right)\right)$ and $\left[ u\right]\in K_1\left( C\left(\T^2_\th \right)\right)$. We leave to the reader the proof of the following
\be
\begin{split}
...
From the equation \eqref{k_1_eqn} it follows that $K_1\left( C\left(\T^n _{\theta}\right) \right) \cong H^1_c\left(\mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\otimes \K\right) ; \Z \right)$.  The following exact sequence 
\be\label{sxx_shhnt_eqn}
\begin{split}
...\to  H^0\left(\mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\right); \sU\left( 1\right)  \right)\xrightarrow{\dl }  H^1\left(\mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\right); \Z \right)\to\\ \to  H^1\left(\mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\right); \sR \right)\to ... 
\end{split}
\ee
is a specialization of \eqref{sxx_shh_eqn} one. Similarly to the Appendix \ref{sheaf_k1_sec} the homomorphism $\dl$ is surjective. From  the Theorem \ref{k_1_tor_thm}  element of $K_1\left(C\left(\T^n_\th \right)  \right)$ is represented by an invertible element $u$ of $C\left(\T^n_\th \right)$ and $u$ naturally yields an element of $H^0\left(\mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\right); \sU\left( 1\right)\right) $ According to the The Arens-Royden Theorem (cf. \cite{royden:fa}) the the element of $K_1\left( C\left(\T^n _{\theta}\right) \right)$ uniquely depends on $\dl u\in H^1\left(\mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\right); \Z \right)$ (cf. equation \eqref{royden_eqn}). From these circumstances the equation
$$
K_1\left( C\left(\T^n _{\theta}\right) \right) \cong H^1_c\left(\mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\otimes \K\right) ; \Z \right)
$$
can be replaced with the following one
\be
K_1\left( C\left(\T^n _{\theta}\right) \right) \cong H^1\left(\mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\right) ; \Z \right)
\ee
...
one has $$K_1\left( C\left(\T^n _{\theta}\right) \right) \cong H^1\left(\mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\right) ; \Z \right).$$
Really from the Theorem \ref{k_1_tor_thm} ir follows that any $x \in K_1\left( C\left(\T^n _{\theta}\right) \right)$ can be represented  by unitary $u \in  C\left(\T^n _{\theta}\right)$.  From the Lemma \ref{image_lem} it follows that there is $u_f \in \mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\right)$ such that $u = \mathfrak{Image}\left( u_f\right)$. Similarly to \eqref{sxx_shh_eqn} there is a long exact sequence
\bean
\mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\right)
\eean 

For any $j = 1,..., n$ the inclusion $C\left( U_j\right) \subset C\left(\T^n _{\theta}\right)$ yields the following commutative diagram
\be 
\begin{tikzcd}
\mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\right) \arrow[d, "\varphi_T"]\arrow[rd , "\varphi_S"] & \\
 \T^n \arrow[r]& S^1
\end{tikzcd}
\ee
which yields a diagram of the sheaf cohomology rings
\be 
\begin{tikzcd}
	H^*\left( \mathfrak{Gelfand}_r\left(  C\left(\T^n _{\theta}\right)\right); \Z \right) & \\
	H^*\left(	\T^n ; \Z \right) \arrow[u, "	H^*\left(\varphi_T\right) "] & 	H^*\left( S^1; \Z\right) \arrow[l] \arrow[lu , "	H^*\left(\varphi_S\right) "]
\end{tikzcd}
\ee
...
\section{Examples of classical and quantum projective modules}
...
\eean 
The map $\a$ has degree 0 and $\bt$ has degree 1. The sequence is natural in each
variable, and splits unnaturally. So if $K_*\left(A \right)$ or $K_*\left(A \right)$ is torsion-free, $\a$ is an
isomorphism.
\end{theorem} 
...
\begin{tikzcd}
	K_0\left( A\right)\arrow[r, "1-\a_*"] & K_0\left(A \right)\arrow[r, "\iota_*"] & K_0\left(A \rtimes_\a \Z \right) \arrow[d]\\
		K_1\left( A\rtimes \Z\right)\arrow[u] & \arrow[l, "\iota_*"] K_1\left(A \right) & \arrow[l, "1-\a_*"]K_1\left(A \rtimes_\a \Z \right)
\end{tikzcd}
\eean
\end{theorem}
...
We need the following structure theorem for crossed products. If $\bt$ is an
action of $\R$ on a $C^*$-algebra $B$, and $\bt$ is trivial on $\Z$, then $\bt$ drops to an action
...
If $\tau$ is a tracial state on $A$ which is invariant under $\a$ then there is a canonical extension of $\tau$ to a tracial
state on $A \rtimes_\a\Z$. Conversely, of course, any tracial state on the
crossed product restricts to an invariant tracial state on $A$. Any trace on $A$
extends uniquely to $\mathbb{M}_n\left( A\right)$  (the extended trace has norm $n$). We will use the
same symbol $\tau$ to denote the original trace and its canonical extensions, and also
to denote the corresponding state of $K_0$.
...
\eean 
($u \in U_\infty\left(A \right), ~ u \a \left(u^{-1} \right)\in U_\infty\left( A\right)_0$  ) is a well defined group homomorphism, and
the following sequence is exact:
...
%Theorem 10.10.5. 
Under the conditions of \ref{trace_k_thm}, suppose $A \bydef C(\sX)$ is commutative. Then $\rho \left( \tau \left( K_0\left( A\rtimes_\a \Z\right) \right) \right)$  is equal to the image of $U_1\left( A\right)$ in $\ker\left(1-\a_* \right)$ under $\underline{\Dl}^\a_\tau$. 
So to compute the range of $\tau$ on $K_0\left(A \rtimes_\a \Z \right) $, we need only compute
$\tau \left(K_0\left(A \right)  \right)$  and $\underline{\Dl}_\tau\left(u \a \left(u^{-1} \right)  \right)$  for one $u$ in each component of $U_1(A)$ which is left fixed by 
$\a$, i. .e. for each element of $H^1\left( \sX; \Z\right)$ left fixed $\a^*$.
\end{theorem}
...
\end{theorem}
	...
	\subsubsection{$K$-theory and Poincar\'e duality}\label{rieffel_sec}\cite{varilly:nc_intro}
