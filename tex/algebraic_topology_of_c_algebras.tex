...
\begin{theorem}\label{cup_thm!}!!!\cite{bredon:sheaf}
	...
	\be\label{cup_p_eqn}
	\begin{split}
		\smile : H^p_\Phi\left(\sX, \mathscr A \right) \otimes  H^p_\Psi\left(\sY, \mathscr B \right)\to H^p_{\Phi\cap\Psi}\left(\sX, \mathscr A \otimes \mathscr B\right)
	\end{split}
	\ee
...
\paragraph{title}
	...
	\end{theorem}
	\begin{proof}
		The $C^*$-algebra $A$ is strongly Morita equivalent to $ A\otimes \K$. So this lemma is the direct consequence of the equations \eqref{k_00_h_eqn} and \eqref{k_1_h_eqn}.
	\end{proof}
	\begin{remark}
	From the equation \eqref{k_00_eqn} it turns out that there is the natural surjective homomorphism 
		\be\label{nh_eqn}
	\begin{split}
\varphi_{H^0\to K_{00}}	: H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\onto  H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)/U\left(\left( A\otimes \K\right) ^+ \right)\cong \\K_{00}\left(A \right) 
	 	\end{split}
	\ee 
	of Abelian groups.
	\end{remark}
