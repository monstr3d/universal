...
\begin{definition}\label{chern_defn}
	...
	The  {\it Chern classes} $c_j\left(\sV \right)\in H^{2j}\left(\sX^+, \Z \right)$ of $\sV$ as {Chern classes} $c_j\left(\sV_\sX \right)$ of $\sV_\sX$ (cf. Theorem \ref{chern_class_thm})
\end{definition}
\paragraph{title}
...
\end{pmatrix}\left( \Phi_{\T^2}\left( t\right)\right)^{-1};\\
  \eean
  From the proof of the Theorem \ref{suspension_thm} it follows that the maps $\Psi_{G^N_N}$ and $\Psi_{\T^2}$ yield the projections  $p_{G^N_N} \in \mathbb{M}_2 \left(SC_r^*\left(  \G^{N}_{N}\right) \right)^+\subset  \mathbb{M}_\infty \left(SC_r^*\left(  \G^{N}_{N}\right)^+ \right)$ and  $p_{\T^2} \in \mathbb{M}_2 \left(SC\left(  \T^2 \right)^+\right)\subset \mathbb{M}_\infty \left(SC\left(  \T^2 \right)^+\right)$.  Similarly to \eqref{chern_eqn} there is a diagram
...
\ee
where the relation $\sim$ is given by the Definition \ref{projection_equivalence_defn}.
...
The map $p_{\T^{2}/S^1}$ yields a surjective continuous map
