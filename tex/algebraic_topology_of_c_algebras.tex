...
\begin{tikzcd}
	...
	& & 	 M\left(A \otimes \K \right) \\
	& \mathbb{M}_\infty \left(C_0\left( \sX\right) \right) \ar[ru,hook, "\phi_\sX"]&
\end{tikzcd}
\ee
\begin{definition}\label{chern_defn}
	Let  $\sV\in V\left(A \right)$ be an equivalence class of projections represented by $e_A\in 	\mathbb{M}_\infty \left( A\right)$ (cf. Definition \ref{projection_equivalence_defn}) such that there is the unique class  $\sV_\sX\in V\left(C_0\left(\sX \right) \right) $ represented by $e_\sX\in 	\mathbb{M}_\infty \left(C_0\left( \sX\right) \right)$ with $\phi_A \circ \phi\left(e_A \right)\sim\phi_\sX\left( e_\sX\right)$ where $\sim$ is given by the Definition \ref{projection_equivalence_defn}. 
...
\\
\\
where $K_j$ and $K^J$ meant $K$-functors of $C^*$-algebras and topological spaces (cf. \cite{blackadar:ko,karoubi:k}). The groups  $K^{\mathrm{nat}}_j \left( A\right)$ are invariants of $A$. Suppose that there is a finitely generated $A^+$-module $P = e \ell^2\left( A^+\right)$ where $e \in \mathbb{M}_\infty \left(A^+ \right) $
