...
\begin{proof}
	The net of all  closed left ideals  of $L\subset A^+$ with $L \not\subset A$ is an ultrafilter. It corresponds to a point $\{+\}\in \mathfrak{Gelfand}\left(A^+ \right)\setminus \mathfrak{Gelfand}\left(A \right)$ such that $\mathfrak{Gelfand}\left(A^+ \right)\cong \mathfrak{Gelfand}\left(A \right)\cup \{+\}$.
\end{proof}
...
%A^\sim\bydef\begin{cases}
	...
	\begin{lemma}\label{image_lem}
		Any normal element $a$ (cf. Definition \ref{normal_defn}) of $C^*$-algebra  $A$ it the image (cf. Definition \ref{image_defn}) of an element $f \in \mathfrak{Gelfand}\left(A \right)$
	\end{lemma}
	\begin{proof}
		Let $A$ be a $C^*$-algebra and $a \in A$ is a normal element (cf. Definition \ref{normal_defn}). If $C\left(\sX \right)\subset A$ is the generated by $a$ commutative $C^*$-subalgebra then the Theorem \ref{good_thm} yields a continuous map $\phi: \mathfrak{Gelfand}\left(A  \right)\to \sX$. If $\overline a\in C\left( \sX\right)$ corresponds to $a$ then $a$ is the image of $\overline a\circ \varphi$.  
	\end{proof}
	...
	\paragraph{}
	If $p \in \mathbb{M}_n\left(A \right)$ is a projection (resp. unitary $u \in \mathbb{M}_n\left(A^+ \right)$ ) then from the Lemma \ref{image_lem} it follows that there is the unique projector $p_{ \mathfrak{Gelfand}\left(A \right) }\in \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right) \right)$ ((resp. unitary $u_{ \mathfrak{Gelfand}\left(A \right) }\in \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A^+ \right) \right)$
	such that 
	\be
	\begin{split}
		p =  \mathfrak{Image}\left( p_{ \mathfrak{Gelfand}\left(A \right) }\right),\\
		u =  \mathfrak{Image}\left( u_{ \mathfrak{Gelfand}\left(A \right) }\right) .
	\end{split}
	\ee
	Suppose that $A$ is a \textbf{stable} 
	 $C^*$-algebra. Then for any $n \in \N$ there is an $*$-isomorphisms $A \cong \mathbb{M}_n\left( A\right)$ which yields a homeomorphism $\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right)\cong \mathfrak{Gelfand}\left(A \right)$. So any projector in $\mathbb{M}_n\left(A\right)$  yield a projector in $C_0\left(\mathfrak{Gelfand}\left(A \right) \right)$ and vice versa.
	According to the  Shilov Idempotent Theorem any projector in $C_0\left(\mathfrak{Gelfand}\left(A \right) \right)$ corresponds to an element of the group of cohomology $H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)$ with compact support (cf. \ref{commutative_k_empt}).  There is the action $U\left(A^+ \right)\times \mathrm{Pr}\left( A\right)\to \mathrm{Pr}(A)$ of the unitary group on the set of projectors. So there is the natural action $U\left(A^+ \right)\times H^0_c\left(\mathfrak{Gelfand}\left(A\right); \Z \right)\to H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)$. Using the Definitions \ref{proj_v_defn} and \ref{k00_defn} one can deduce that 
	\be\label{k_00_h_eqn}
	\begin{split}
...
\end{split}
\eean	
so taking into account  \eqref{royden_eqn} one has
	\be\label{k_1_h_eqn}
	\begin{split}
	...
	Let $\sX$ be a locally compact Hausdorff  space. Let us consider there sheaves $\sR$, $\sS$ and $\sZ$ on $\R$ which corresponds to sheaves of continuous maps from $\R^+\cong S^1$ to $\R$, $S^1$ and $\Z$  respectively.  There is an exact sequence of sheaves
	
	\bean
	\begin{split}
		\{0\}  \hookto \sZ \hookto \sR \onto \sS \onto \{0\}
	\end{split}
	\eean
	which yield the following long exact sequence
	\bean
	\begin{split}
		\{0\}  \to  H^0_c\left(\sX, \Z\right)  \to  H^0_c\left(\sX, \sR \right)\to  H^0_c\left(\sX, \sS \right)\xrightarrow{\dl }  H^1_c\left(\sX, \Z \right)\to  H^1_c\left(\sX, \sR \right)\to ... 
	\end{split}
	\eean
		...
		\be\label{dl_sh_eqn}
		H^0_c\left(\sX, \sS \right)\xrightarrow{\dl }  H^1_c\left(\sX, \Z \right)
		\ee
	is surjective.
	If $A^+\bydef A \oplus \C$ is its minimal unitization then from the Lemma \ref{one_point_lem} it follows that 	$\mathfrak{Gelfand}\left(A^+ \right)\cong \mathfrak{Gelfand}\left(A \right)^+$. From \eqref{k_00_h_eqn} and the Proposition \ref{stably_unital_prop}  it turns out that 	\be\label{k_0_h_eqn}
	\begin{split}
		K_{0}\left(A^+ \right) \cong H^0\left(\mathfrak{Gelfand}\left(A \right)^+; \Z \right)/U\left(A^+ \right). 
	\end{split}
	\ee 

	Any $e \in 	V\left(A^+ \right)$ corresponds to an element $f \in C\left(\mathfrak{Gelfand}\left(A \right)^+ \right)$. If $n \bydef f\left( +\right)$ then from the Standard picture (cf. Remark \ref{standard_picture_rem}) any $x \in K_0\left( A\right)$ is given by $\left[e\right]-  \left[p_n\right]$ From this circumstance one can deduce 
\be\label{k0_eq_eqn}
	\begin{split}
	K_{0}\left(A \right) \cong H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)/U\left(A^+ \right) 
\end{split}
\ee
and there is the natural homomorphism surjective homomorphism
\be\label{k_0_hh_eqn}
\begin{split}
	\varphi_{H-K}:H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)\onto  K_0\left(A \right). 
\end{split}
\ee 
From \ref{dl_sh_eqn} it follows that the homomorphism  $H^0_c\left(\R, \sS \right)\onto   H^1_c\left(\R, \Z \right)\cong \Z$ is surjective. Let  $v^1_\Z \in H^1_c\left(\R, \Z \right)$ be a generator and let $v^0_\sS\in H^0_c\left(\R, \sS \right)$ be such that $\dl_\R v^0_\sS= v^1_\Z$.
If   $f:\mathfrak{Gelfand}\left(A \right) \times  \R\to \R$ from the equation \eqref{cohom_mor_eqn} one has a diagram
\be\label{dia_mult_dia}
\begin{tikzcd}
  H^0_c\left(\R, \sS \right) \arrow[rr, "\dl_\R"]  \arrow[d, "H^0_c\left( f\right)" ]&&  H^1_c\left(\R, \Z \right)   \arrow[d, "H^1_c\left( f\right)"] \\
  H^0_c\left(\mathfrak{Gelfand}\left(A \right) \times\R, f^*\sS \right) \arrow[rr, "\dl_{\mathfrak{Gelfand}}"] &&  H^1_c\left(\mathfrak{Gelfand}\left(A \right) \times\R, \Z \right) 
	\end{tikzcd}
\ee
 
If $g :\mathfrak{Gelfand}\left(A \right)\times \R\to \mathfrak{Gelfand}\left(A \right)$ is a projector then there is a homomorphism
\be\label{k_eqn}
\begin{split}
	H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)\xrightarrow{H^0_c\left( g\right) } 	H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)\times \R
\end{split}
\ee
Let $x \in   K_0\left(A \right)$, and let $x_H\in H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)$ be a representative of $x$, i.e. $x = \varphi_{H-K}\left( x_H\right)$.
From the Theorem \ref{cup_thm} it follows that there is the natural product
\be\label{k_u_eqn}
\begin{split}
\left( 	H^0_c\left(g\right)x_H\right) \smile \left( 	H^0_c\left(g\right)v^0_\sS\right)\in   H^0_c\left(\mathfrak{Gelfand}\left(A \right) \times\R, \otimes \Z\otimes f^*\sS \cong f^*\SS\right) 
\end{split}
\ee
The product reflects the formula \eqref{idem_map_eqn} for the Bott periodicity. From (c) of the Theorem \ref{cup_thm}
it follows that
\be\label{k_uu_eqn}
\begin{split}
u\bydef \dl_{\mathfrak{Gelfand}}\left(  \left( 	H^0_c\left(g\right)x_H\right) \smile \left( 	H^0_c\left(g\right)v^0_\sS\right)\right) = \left( 	H^0_c\left(g\right)x_H\right) \smile  H^1_c\left(f \right) \dl_\R v^0_\sS \in 
\\ \in H^1_c\left(\mathfrak{Gelfand}\left(A \right) \times\R, \Z\otimes \Z\cong \Z\right)
\end{split}
\ee
represents $\bt_A\left( x\right) \in K_1\left(SA \right)$ (cf. Theorem \ref{suspension_defn}). Since $\bt_A$ is the isomorphism one has
\be\label{k_utu_eqn}
\begin{split}
	\forall x \in H^0_c\left(\mathfrak{Gelfand}\left(A \right);\Z\right) \quad \varphi_{H-K}\left( x\right) = 0 \quad \Leftrightarrow\quad  \bt_A \circ \varphi_{H-K}\left( x\right) = 0\quad \Leftrightarrow\\
	\Leftrightarrow \quad  H^1_c\left(f \right) \dl_\R v^0_\sS  = 0
\end{split}
\ee
From \eqref{k_utu_eqn} it turns out that $\ker \varphi_{H-K}$ is a left ideal of the ring $	H^0_c\left(\mathfrak{Gelfand}\left(A \right); \Z \right)$. Similarly one can prove that $\ker \varphi_{H-K}$ the right ideal.
	...
	\bea\label{k_00_eqn}
	K_{0}\left(A \right) \cong H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)/I,\\
	\label{k_1_eqn}
		K_{1}\left(A \right) \cong H^1_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right). 
	\eea
	where $I$ is two sided ideal with respect to given ny the Theorem \ref{cup_thm} cup product.
	\end{theorem}
	\begin{proof}
		The $C^*$-algebra $A$ is strongly Morita equivalent to $ A\otimes \K$. So this lemma is the direct consequence of the equations \eqref{k_0_hh_eqn} and \eqref{k_1_h_eqn}.
	\end{proof}
\subsubsection{Multiplicative structures}
\begin{theorem}\label{cup_prod_thm}
If $A$ is a $C^*$-algebra then there is the natural graded cup product
\be\label{cup_prodc_eqn}
\smile : K_*\left(A \right) \otimes K_*\left(A \right) \to K_*\left(A \right) 
\ee
\end{theorem}
\begin{proof}
From the Theorem \ref{k_h_thm} it turns out that $K_0\left( A\right) =H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)/I$ so we can define $\smile : K_0\left(A \right) \otimes K_0\left(A \right) \to K_0\left(A \right)$ which comes from the given by the Theorem \ref{cup_thm} cup product $$\smile :H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\otimes H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\to H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right).$$
Similarly the the given by the Theorem \ref{cup_thm} cup products 
\bean
\smile :H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\otimes H^1_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\to H^1_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right),\\
\smile :H^1_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\otimes H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\to H^1_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)
\eean
naturally yield following products
\bean
\smile :K_0\left(A \right) \otimes K_1\left(A \right) \to K_1\left(A \right) ,\\
\smile :K_1\left(A \right) \otimes K_0\left(A \right) \to K_1\left(A \right).
\eean
If $x', x'' \in  K_1\left(A \right)$ are elements represented by $y', y'' \in H^1_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)$ ans  $f: A \times \R \to A$ ia a projection then the equation 
\end{proof}
		\paragraph{} If $A$ is a stably unital $C^*$-algebra (cf. Definition \ref{stably_unital_defn}) then from  the Proposition	
	\ref{stably_unital_prop} and the equation \eqref{k_00_eqn} it follows that there is the natural isomorphism
	\bean
	\begin{split}
		K_0\left(A \right) \cong H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)/U\left(\left( A\otimes \K\right) ^+ \right).
	\end{split}
	\eean
	Since $\Z$ is a ring the group  $H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)$ has the natural ring structure with the $\smile$ product (cf, Theorem \ref{cup_thm}). Here we will supply the natural ring structure on the factor-group $$H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)/U\left(\left( A\otimes \K\right) ^+ \right),$$ The group $H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)$ by the pairs $\left( \sU, n\right)$ where $\sU\subset \mathfrak{Gelfand}\left(A \otimes \K\right)$ is an open closed subset and $n \in \N$.
	One has
	\bean
	\begin{split}
		\left( \sU, n'\right)\smile \left( \sU, n''\right)= \left( \sU, n'\cdot n''\right),\\
		\sU' \cap \sU'' = \emptyset \quad \Rightarrow\quad \left( \sU', n'\right)\smile \left( \sU'', n''\right)= 0.
	\end{split}
	\eean
	If both $\sU_1, \sU_2 \subset H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)$ then thee is the following decomposition	
	
	\be\label{int_c_eqn}
	\begin{split}
		\sU_1 = 	\sU'_1 \sqcup 	\sU''_1,\\
		\sU_2 = 	\sU'_2 \sqcup 	\sU''_2, \\
		\exists g \in U\left(\left( A\otimes \K\right) ^+ \right) \quad \sU'_1 = g\left(\sU'_2 \right),\\
		\forall g \in U\left(\left( A\otimes \K\right) ^+ \right) \quad \sU''_1\cap  g\left(\sU''_2 \right) = \emptyset.
	\end{split}
	\ee
	If $n_1, n_2 \in \Z$ and
	$$
	\varphi : H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)\onto H^0_c\left(\mathfrak{Gelfand}\left(A \otimes \K\right); \Z \right)/U\left(\left( A\otimes \K\right) ^+ \right)\cong K_0\left( A\right) 
	$$
	is the natural surjective map then under the hypothesis \eqref{int_c_eqn} we define
	\be\label{int_cm_eqn}
	\begin{split}
		\varphi \left( \sU_1, n_1\right) \smile \varphi \left( \sU_2, n_2\right)\bydef \varphi \left( \sU'_1, n_1\cdot n_2\right).
	\end{split}
	\ee
	Using linearity one can define the cup product for any two elements of $K_0\left( A\right)$.
	\begin{lemma}\label{prod_lem}
		If $A$ is a stably unital $C^*$-algebra then 
		the equations \eqref{int_c_eqn} and \eqref{int_cm_eqn} yield a binary operation 
		\be\label{cup_eqn}
		\begin{split}
			\smile : K_*\left( A\right)  \times K_*\left( A\right) \to K_*\left( A\right)  
		\end{split}
		\ee
		such that the group $K_*\left( A\right)$ obtains the ring structure.
	\end{lemma}
	\begin{proof}
		Firstly we define that there is the natural operation 
		\bean
		\begin{split}
			\smile : K_0\left( A\right)  \times K_0\left( A\right) \to K_0\left( A\right)  
		\end{split}
		\eean
		
		One needs proof the following.
		\begin{itemize}
			\item The equations  \eqref{int_c_eqn} and \eqref{int_cm_eqn} uniquely defines the operation $\smile$.
			\item The  operation $\smile$ is associative and distributive.
		\end{itemize}
		We leave to the reader the details of this proof.
	\end{proof}
	\begin{lem}
		If $\sX$ is a compact space then 
	\end{lem}
	
	\be\label{int_cmu-eqn}
	\begin{split}
		\varphi \left( \sU_1, n_1\right) \smile \varphi \left( \sU_2, n_2\right)\bydef \varphi \left( \sU'_1, n_1\cdot n_2\right).
	\end{split}
	\ee
...
\section{Lattices and filters}
	...
	\begin{lem}\cite{bryl:loop}\label{gr_ring_lem}
...
f^*: H^*_c\left(\sY;\mathscr B  \right) \to  H^*_c\left(\sX;f^*\mathscr B  \right).
\ee
...
... \to H^p_c\left( \sX; \mathscr A'\right) \to H^p_c\left( \sX; \mathscr A\right) \to H^p_c\left( \sX; \mathscr A''\right) \xrightarrow{\dl} H^{p+1}_c\left( \sX; \mathscr A'\right)\to ... 
\ee
...
\end{remark}
	\begin{corollary}\cite{bredon:sheaf}
		%Corollary. 
...
\end{theorem}	
	...
	\begin{empt}\cite{bredon:sheaf}
		%2.6. p 11
		Let $A$ be a locally closed subspace of $\sX$ and let $\mathscr B$ be a sheaf on $A$. It is easily seen (since $A$ is locally closed) that there is a unique topology on the
		point set $\mathscr B\cup \left( \sX \times \{0\}\right) $ such that $\sX$ is a subspace and the projection onto $\sX$ is a local homeomorphism (we identify $A \times \{0\}$ with the zero section of  $\mathscr B$.
		...
		\end{split}
		\ee
		Thus $\mathscr B^\sX$ is the unique sheaf on $\sX$ inducing  $\mathscr B$ on $A$ and $0$ on $\sX \setminus A$. Clearly, $\mathscr B\mapsto \mathscr B^\sX$ is an exact functor. The sheaf $\mathscr B^\sX$ is called the {\it extension of $\mathscr B$
			by zero}. If $\sX^+$ is a one point compactification then there is the isomorphism 
		
		\be\label{sheaf_h_comp_eqn}
		\begin{split}
			H^*_c\left(\sX; \mathscr A \right) \cong H^*\left(\sX^+; \mathscr A^{\sX^+} \right)
		\end{split}
		\ee
	\end{empt}
...
\begin{theorem}
%7.6. Theorem. P 231
  Let $A \subset \sX$, where $\sX$ is any space, and let the base ring $L$
be a principal ideal domain. Let $\mathscr A$ be a sheaf of $L$-modules on $\sX$ and let be any family of supports on $\sX$. Let $\sY$ be a locally compact Hausdorff
and $clc_L^\infty$ space. Then there is a natural exact sequence
		\be
\begin{split}
\bigoplus_{p+q = n}	H^*_{\Phi}\left(\sX, A; \mathscr A \right) \hookto H^*_{\Phi \times c}\left(\sX,  \mathscr A^{\sX^+} \right)
\end{split}
\ee
...
The Arens-Royden Theorem, which states that  induces an isomorphism of
