...
There is the natural equivalence relation $\sim_{\mathfrak{Lattice}}$ on the $\mathfrak{Lattice}\left( A\right)$  such that
\be\label{lat_eqn}
L' \sim_{\mathfrak{Lattice}} L''\quad \Leftrightarrow \quad  \sX_{L'} = \sX_{L''}.
\ee
\end{empt}
\begin{definition}
...
\eean
where $\rho_y : A \to B\left(\H_y \right)$ is an irreducible representation (cf. Definition  \ref{irred_defn}).  Similarly the bicommutant $\xi''$ \textit{belongs to} $y$ if $\rho_y\left(\xi'' \right) \neq \rho_y\left(A \right)$.
...
\subsection{Hausdorff blowing-up}
	...
	If $A$ is a $C^*$-algebra then an inclusion $\mathfrak {Blowing}_{\sY-A }:C_0\left( \sY\right) \hookto M\left(A \right)$  (or equivalently $C_0\left( \sY\right) \subset M\left(A \right)$ ) is \textit{Hausdorff blowing-up} of $A$ if  both sets
...
If $C^*$-algebra $A$ satisfies to \eqref{norm_eqn} and $S\subsetneqq A$ is a closed subset then there is $a \in A\setminus S$ and $\eps > 0$ such that
$$
...
\begin{lemma}\label{ctr_g_lem}
	If  $A$ is a {continuous-trace} $C^*$-{algebra} then the  bicommutant of Gelfand space (cf. Definition \ref{bicommutant_defn}) is equivalent to the  a set of pairs $\left(x_\xi, V_\xi \right)$ where is an element of the spectrum of $A$ and $V$ is a one dimensional subspace of the corresponding to $x$ Hilbert space $\H_x$ of irreducible representation $\rho_x: A \to B\left(\H_x \right)$.
\end{lemma}
\begin{proof}
...
\end{split}
\ee
If $\xi \in \mathfrak{Gelfand}\left(A \right)$,  $\xi'' \bydef \phi\left(\xi \right)  \in \mathfrak{Gelfand}\left(A \right)''$ and $x_\xi = \phi''\left( \xi''\right)$ then for any $x \neq x_\xi$ there and an open neighborhood  $\sU$ of $x$ such that $f \in C_0\left(\sX \right)$ with $ f\left(\sU \right)= {0}$ and $L \bydef f A \in \xi$. If $L^\perp$ is a commutant of $L$ (cf. Definition \ref{bicommutant_defn}) then $\rho_{x'} \left(L^\perp \right) = \rho_{x'}\left( A\right)$ where $\rho_{x}: A \to B\left( H_{x'}\right) $ is the corresponding to $x$ irreducible representation. It follows that $x_\xi$ is the unique point such that $\rho_{x_\xi} \left( \xi''\right) \neq \rho_x \left( A\right)$. Since $\rho_{x_\xi} \left( \xi''\right)$ is a left ideal there is a vector subspace $V_\xi\subset \H$ such that $\rho_{x_\xi} \left( \xi''\right)V_\xi = \{0\}$. If dimension of $V_\xi$ exceeds 1 and $\eta \in V_\xi$ then the set
$$
\left\{ L \in \xi | \rho_x \left(L \right)\eta = \{0\} \right\} \subset \mathfrak{Lattuce}\left(A \right)
$$
...
\end{lemma}	
\begin{proof} 
If $\xi \in 	\mathfrak{Gelfand}\left( A\right)$ and $x = \/ \phi\left( \xi\right) \sX$ (cf. the equation \eqref{ctr_g_eqn}) then from the the Lemma  \ref{ctr_g_lem} it follows that there is a pair $\left(x_\xi, V_\xi \right)$ such that
$$
\xi'' = \left\{ b \in A \left| \rho_{x_\xi} \left(b^* \right)V_\xi = \{0\} \right.\right\}
$$ 
It follows that for any $L \in \xi$  one has $\rho_{x_\xi}\left(L \right) \H_{x_\xi}= V_\xi$. Let 
\bean
...
\eean
where $C$ is a commutative $C^*$-subalgebra which represents $\zeta$. If $\mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)_\xi$ contains more than one element and $\zeta \in \mathrm{Sp\acute{e}}\left(\mathscr P_{\sX\text{-}A} \right)_\xi$ then the  filter
\bean
...
Suppose that $A$ is the given by the Proposition \ref{ctr_bundle_prop} a locally   fibre bundle $\F$ with fibre $\K = \K\left(\H \right)$. There is a subbundle $\E \subset \F$ such that fibers of $\E$ are  rank-one positive operators. It gives a locally trivial bundle $\C P\left(\E \right)\xrightarrow{\phi_\E} \sX$ such that any fibre of $\phi_\E$ is a complex projective space $\C P\left(\E \right)$. From the Lemma \ref{ctr_g_lem} it follows that there is a set theoretic bijective map
\be\label{gtop_map_eqn}
\phi_C: \C P\left(\E \right)\cong  \mathfrak{Gelfand}\left(A \right)
\ee
There is the initial with respect to $\phi_C$ on  topology on $\C P\left(\E \right)$ (cf, Definition \ref{top_init_defn}), we denote corresponding space by $\C P\left(\E \right)_{\mathfrak{Gelfand}}$. There is a continuous map $F_{\C P}:\C P\left(\E \right)_{\mathfrak{Hausdorff}}\to \C P\left(\E \right)_{\mathfrak{Gelfand}}$ where $\C P\left(\E \right)_{\mathfrak{Hausdorff}}$ is the set $\C P\left(\E \right)$ having well known Hausdorff topology. If $F$ is an Abelian group then from the Theorem  


such that the bijective map \eqref{gtop_map_eqn} is a homeomorphism, we denote by $\C P\left(\E \right)_{\mathfrak{Gelfand}}$  the corresponding topological space.
...
\eean
There is the well known Hausdorff topology on  $\C P\left(\E \right)$. denote by  $\C P\left(\E \right)_{\mathfrak{Hausdorff}}$ the corresponding topological space. If we consider well known Hausdorff topology on $\C P\left(\E \right)$ the  map \eqref{gtop_map_eqn}  yields a continuous bijective map
\bean
...
\be\label{h_ctr_eqn}
H^*\left( C P \right) :  H^*\left( \C P\left(\E \right)_{\mathfrak{Hausdorff}}, \overline F \right) \to H^* \left( \C P\left(\E \right)_{\mathfrak{Gelfand}}, \overline F\right).
\ee
...
\begin{theorem}
If $A$
is a stable separable continuous-trace $C^*$-algebra such that the spectrum $\sX$ of $A$ is a $CW$-complex then the homomorphism \ref{h_ctr_eqn} is an isomorphism.
\end{theorem}
\begin{proof}
From the Proposition \ref{cw_contract_prop} it follows that there is a family $\left\{\sU_\la\in \sX\right\}_{\la \in \La}$ of open contractible sets, such that
\begin{itemize}
	\item 
\end{itemize}
\end{proof}
