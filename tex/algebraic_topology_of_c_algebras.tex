...
\end{proof}
	...
	\end{lemma}
	\begin{proof}
		If $\sX \bydef  \mathfrak{Gelfand}\left( C\left(\T^n _{\theta}\right)\right) $ and $\mathscr F' \bydef \varphi^{-1}\mathscr F$ then from the 
		...
		\bean
		\mathscr F_{\varphi\left(x \right) }\subset \mathscr F'_{x}.
		\eean 
		Let both !!!
		\bean
		...
		where both $\mathscr F_{\varphi\left(\xi \right) }$ and $\varphi^{-1}\mathscr F_{\xi}$ are stalks (cf. Definition \ref{sheaf_stalk_defn}). Let 
		\bean
		ff
		\eean
		
		Using the above inclusion one cab construct commutative diagram
		\bean
		\begin{tikzcd}
			0 \arrow[r] & \mathscr F  \arrow[r, "\eps^{\T^n}"]	&\mathscr{C}^{0}\left(\T^n, \mathscr F \right)  \arrow[r, "d_1^{\T^n}"] & \mathscr{C}^{1}\left(\T^n, \mathscr F \right) \arrow[r, "d_2^{\T^n}"] & \mathscr{C}^{2}\left(\T^n, \mathscr F \right)  \arrow[r, "d_3^{\T^n}"] & \dots\\
			0 \arrow[r] & \mathscr F  \arrow[r, "\eps^{\T^n}"]	&\mathscr{C}^{0}\left(\T^n, \mathscr F \right)  \arrow[r, "d_1^{\T^n}"] & \mathscr{C}^{1}\left(\T^n, \mathscr F \right) \arrow[r, "d_2^{\T^n}"] & \mathscr{C}^{2}\left(\T^n, \mathscr F \right)  \arrow[r, "d_3^{\T^n}"] & \dots
		\end{tikzcd}
		\eean
	\end{proof}
	...
	\begin{empt}  !!! INDUCTIVE LIMIT
If   $A$ is a $C^*$-algebra and 
\be
\mathfrak{GoodComm}\left( A\right)  \bydef \left\{\left.\varphi : M\left( C_0\left(\sX_\varphi \right)\right)   \to  M\left(A \right) \right| \varphi \quad \text{is good and injective} \right\}
\ee 
then there for any $\varphi \in \mathfrak{GoodComm}\left( A\right)$ there is a surjective continuous map 
$$ 
\mathfrak{Gelfand}\left(A \right) \onto \sX_\varphi
$$ 
and injective $*$-homomorphisms
\bean
\Phi_\varphi: C_0\left(\sX_\varphi \right)\hookto C_b\left( \mathfrak{Gelfand}\left(A \right)\right),\\
\Phi_\varphi\left(C_0\left(\sX_\varphi \right) \right)\hookto M\left( A\right) \subset C_b\left( \mathfrak{Gelfand}\left(A \right)\right) 
\eean
If a  space  $M\left(\mathfrak{Gelfand}\left(A \right) \right)$ it such that $C\left( M\left(\mathfrak{Gelfand}\left(A \right) \right)\right)$ is  the  completion of the union
\be\label{ph_union_eqn}
\bigcup_{\varphi \in \mathfrak{GoodComm}\left( A\right)} \Phi_\varphi\left(C_0\left(\sX_\varphi \right) \right)\subset C_b\left( \mathfrak{Gelfand}\left(A \right)\right)
\ee
 with respect to strict topology (cf. Definition \ref{strict_topology_defn}) of $C_b\left( \mathfrak{Gelfand}\left(A \right)\right)$ and $\bt~	\mathfrak{Gelfand}\left(A \right)$ is the Stone-\v{C}ech compactification of $\mathfrak{Gelfand}\left(A \right)$ (cf. Definition \ref{top_stone_cech_defn}) then the Theorem \ref{good_thm} yield following surjective continuous maps
\bea\label{Phi_eqn}
\Phi : \bt~	\mathfrak{Gelfand}\left(A \right)\onto M\left(\mathfrak{Gelfand}\left(A \right)\right),\\
\forall \varphi \in \mathfrak{GoodComm}\left( A\right)  \quad  \Phi^{-1}\left( \mathfrak{Gelfand}\left(A \right)\right) \onto \sX_\varphi
\eea
	\end{empt}
	\begin{definition}
	Under the above hypothesis the space $M\left(\mathfrak{Gelfand}\left(A \right) \right)$ is the {\it multiplier of Gelfand space}. The natural injective $*$-homomorphism 
	\be\label{dauns_hofmann_eqn}
C^{\text{DH}} \left(A \right) :	C\left( M\left(\mathfrak{Gelfand}\left(A \right) \right)\right) \hookto M\left(A \right) 
\ee
	is {\it  Dauns-Hoffman homomorphism}.
	\end{definition}
\begin{remark}
