...
\begin{empt}\label{topology_empt}
	...
	is a set of $A$-ultrafilters (cf. Definition \ref{lattice_a_defn}) of the meet-semilattice $\mathfrak{Lattice}\left( A\right)$  then there is a {\it Gelfand topology} on $\mathfrak{Ultrafilters}\left(A \right)$ is the minimal among topologies such that for any closed left ideal $L \subset A$ the set
...
\end{lemma}
\begin{proof}
From the construction \ref{topology_empt} and the Proposition \ref{top_ultra_prop} it follows that the space of $A$-ultrafilters is compact and Hausdorff. The Gelfand space $\mathfrak{Gelfand}\left(A \right)$ is locally compact and Hausdorff since it is a subspace of the space of  $A$-ultrafilters. If $A$ is unital then  $A= K\left( A\right)$, it turns out that $\mathfrak{Gelfand}\left(A \right) = \mathfrak{Ultrafilters}\left(A \right)$. 
\end{proof}
%\begin{empt}
...
\begin{empt}!!!
		Let $F\subset \mathfrak{Gelfand}\left(A \right)$ be a closed subset. For any $\xi \in \mathfrak{Gelfand}\left(A \right)\setminus F$ we define a set $\left\{L_\la\right\}_{\la \in \La_\xi}$ such that for any $\la \in \La_\xi$ one has 
		\begin{itemize}
			\item $  \mathfrak{Gelfand}\left(A \right)_{L_\la}\subset \mathfrak{Gelfand}\left(A \right)\setminus F$
			\item $\mathfrak{Gelfand}\left(A \right)_{L_\la}$ yields an open neighborhood of $\xi$
		\end{itemize}
		where the notation \eqref{topology_eqn} is used. The set $\mathscr A$ of finite subsets  of $\La$ is a directed (cf. Definition \ref{dir_map_eqn}). If for any $\a \in \mathscr A$ 
		\be
		...
		\end{split}
		\ee
		is a left $A$-module. We define 
		\be\label{kf_eqn} 
	\begin{split}
		\ker F \bydef \mathfrak{Gelfand}\left(A \right)\setminus \mathfrak{Gelfand}\left(A \right)_{L_F}
	\end{split}
	\ee
	where $L_F$ is the $C^*$-norm closure of the left ideal \eqref{aun_eqn}.	
		For each subset $I$ of $A$ define a set
		\be\label{hull_g_eqn}
		\mathrm{hull}\left( I\right)\bydef \bigcap_{I \subset L}\mathfrak{Gelfand}\left(A \right)_L
		\ee 
		where $L$ is a closed ideal and $\mathfrak{Gelfand}\left(A \right)_L$ is given by \eqref{topology_eqn}.		
	\end{empt}
	\begin{remark}!!!
		The equations \eqref{kf_eqn} and \eqref{hull_g_eqn} can be regarded as analogs of \eqref{ker_eqn} and \eqref{hull_eqn} ones.
	\end{remark}
	\begin{empt}!!!  Let $A$ be a $C^*$-algebra.
		If  $\xi \in \mathfrak{Gelfand}\left(A \right)$ then $\ker \{\xi\}$ corresponds to the closed left ideal $L_{\{\xi\}}\subset A$ such that
		\be 
		\mathrm{hull}\left( L_\xi\right) = \mathfrak{Gelfand}\left(A \right)\setminus \{\xi\}.!!!
		\ee
		If $L \subset A$ is a closed left ideal then one has
		\be
		\begin{split}
			L \not\subset L_\xi \quad \Leftrightarrow \quad \xi \in \mathfrak{Gelfand}\left(A \right)_L,
		\end{split}
		\ee
		or
		\be\label{max_y_eqn}
		\begin{split}
			L \not\subset L_\xi \quad \Leftrightarrow \quad 	\mathrm{hull}\left( \ker \{\xi\} \cup L\right) = \mathfrak{Gelfand}\left(A \right)\quad \Rightarrow \quad L_\xi \cup L = A.
		\end{split}
		\ee
		From \eqref{max_y_eqn} it turns out that $L_\xi$ is the maximal proper left ideal. In result one has the following theorem. 
	\end{empt}
	\begin{empt}\label{max_map_empt}
	Let $\xi \in \mathfrak{Gelfand}\left(A \right)$ and let $\left\{L_\la\right\}_{\la \in \La_\xi}$ be a set of closed left ideals of $A$ such that for any $\la \in \La_\xi$ one has  $  \mathfrak{Gelfand}\left(A \right)_{L_\la}\subset \mathfrak{Gelfand}\left(A \right)\setminus \{\xi\}$. The set $\mathscr A$ of finite subsets  of $\La$ is a directed (cf. Definition \ref{dir_map_eqn}). If for any $\a \in \mathscr A$ 
	\be
	L_\a \bydef \sum_{\la\in \a} L_\la
	\ee
	is the algebraic finite sum of closed left $A$-modules then $\left\{L_\a | \a \in \mathscr A\right\}$ is directed (cf. Definition \ref{directed_set_defn}), so the union
...
\ee		\end{definition} 
	...
	\ee
	The family $\left\{L_\la \cap  Aa'\right\}_{\la\in \La}$ exceeds $\left\{L_\la \right\}_{\la\in \La}$, so $\left\{L_\la \right\}_{\la\in \La}$ is not an ultrafilter. From this contradiction one concludes that the ideal $L_{\{\xi\}}$ is maximal. For any $L \in \mathfrak{LeftMax}\left(A \right)$ we define the set $\left\{L_\la\right\}$ if all closed left ideal with $L \cap L_\la \neq L_\la$. The set $\left\{L_\la\right\}$ is a filter. If $\xi \in  \mathfrak{Gelfand}\left(A \right)$ corresponds to an ultrafilter which exceeds $\left\{L_\la\right\}$ then 	$\mathfrak{Max}_A\left( \xi\right) = L$, i.e, the map 	$\mathfrak{Max}_A$ is surjective. The continuity of $\mathfrak{Max}_A$ follows from the Gelfand and Jacobson topology (cf. \ref{topology_empt} and the Definitions \ref{jacobson_defn}).
	\end{proof}
	\begin{remark}
