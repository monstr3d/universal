...
\end{proof}
	...
	\be
	\begin{split}
		\mathfrak{Gelfand}\left(SA  \right) \cong 	S\mathfrak{Gelfand}
	\end{split}
	\ee
	\end{lemma}
	...
	\bean
	\phi_A : A \hookto M\left(A\otimes_\C C_0\left(\R \right) \right),\\ 
		\phi_\R : C_0\left( \R\right)  \hookto M\left(A\otimes_\C C_0\left(\R \right) \right) 
	\eean
	so the Theorem \ref{good_thm}  yields surjective continuous maps
	\bean
				\mathfrak{Gelfand}\left(A\otimes_\C C_0\left(\R \right)  \right)\onto \mathfrak{Gelfand}\left(A  \right),\\
								\mathfrak{Gelfand}\left(A\otimes_\C C_0\left(\R \right)  \right)\onto \mathfrak{Gelfand}\left(C_0\left(\R \right).   \right)
	\eean
	These maps naturally yield a continuous map  
		\bean
	\begin{split}
		\mathfrak{Gelfand}\left(A\otimes_\C C_0\left(\R \right)  \right) \to 	\mathfrak{Gelfand}\left(A  \right)\times \R
	\end{split}
	\eean
	Let us construct the inverse continuous map 
	\bean
\begin{split}
	\mathfrak{Gelfand}\left(A  \right)\times \R\to 	\mathfrak{Gelfand}\left(A\otimes_\C C_0\left(\R \right)  \right).
\end{split}
\eean
	If $\left(x, r \right)\in \mathfrak{Gelfand}\left(A  \right)\times \R$, both $\left\{\sU_\la\right\}_{\la\in \La}$ and $\left\{\sV_\la\right\}_{\la\in \La}$ ares bases on neighborhoods of $x$ and $r$ then $\left\{\sU_\la\times \sV_\la\right\}_{\la\in \La}$ is a  basis of neighborhoods of $\left(x, r \right)$. If $\sU_\la$ (resp. $\sV_\la$) corresponds to the closed left ideal $L^A_\la \subset A$ (resp. $L^\R_\la \subset C_0\left(\R \right)$) then
	\bean
\begin{split}
	L_\la \bydef\left( \left(  L^A_\la \otimes 1_{C_b\left(\R\right)  }\right) \left(A\otimes_\C C_0\left(\R \right)  \right)\right) \bigcap \left( \left( 1_{M\left( A\right) } \otimes {C_0\left(\R\right)  }\right) \left(A\otimes_\C C_0\left(\R \right)  \right)\right) 
\end{split}
\eean
is a closed left ideal of $A\otimes_\C C_0\left(\R \right) $. There is a filter $\left\{L_\la\right\}$. If the filter $\left\{L_\la\right\}$ is not an ultrafilter then there is the nontrivial left ideal 
	\bean
\begin{split}
	L\subset \bigcap_{\la \in \La} L_\la \neq \{0\}
\end{split}
\eean
but it is impossible. The ultrafilter  $\left\{L_\la\right\}$ yields the unique $y \in \mathfrak{Gelfand}\left(A\otimes_\C C_0\left(\R \right)  \right)$ so one has a map
	\be\label{sus_inv_map_eqn}
\begin{split}
	\mathfrak{Gelfand}\left(A  \right)\times \R\to 	\mathfrak{Gelfand}\left(A\otimes_\C C_0\left(\R \right)  \right),\\
	\left(r, s \right) \mapsto y.
\end{split}
\ee
The set $\left\{\sU_\la\times \sV_\la\right\}$ is a basis of neighborhoods of $\left(r, s \right)$ so from (c) of  the Theorem \ref{top_continuous_thm} it follows that the map \eqref{sus_inv_map_eqn} is continuous.
	\end{proof}
	\begin{definition}\label{image_defn}
	For any  $f \in C_0\left( \mathfrak{Gelfand}\left(A \right) \right)$ an element $a \in M\left( A\right)$ is the \textit{image} of $f$ if for any $x \in  \mathfrak{Gelfand}\left(A \right)$ with $f\left(x \right)\neq 0$  and any $\eps > 0$ there is an ideal $L \subset A$ such that 
	\begin{enumerate}
...
If $A$ is a $C^*$-algebra then  according to the Proposition \ref{k_1_prop} any $[u]\in K_1\left(A \right)$ is represented by an element $u$ of $U_n\left(A^+ \right)\cong U\left( \mathbb{M}_n\left( A^+\right) \right)$. If $u_{S^1}\in C\left(S^1 \right)$ is an unitary generator of $C\left(S^1 \right)$ then there is the natural $*$-homomorphism $C\left(S^1 \right)\to \mathbb{M}_n\left( A^+\right)$. From the Theorem \ref{good_thm} it follows that there is the natural continuous map 
	\bean
\begin{split}
