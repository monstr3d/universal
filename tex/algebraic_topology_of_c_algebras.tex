...
\paragraph*{}	
If $A$ is a $C^*$-algebra and  $C_0\left( \sX\right)\subset M\left(A \right)$ is natural Hausdorff blowing-up (cf. Definition \ref{blowing_natural_defn}) then there is a given by \eqref{blowing_p_eqn} homomorphism 	$\mathfrak{Blowing}^+_{\sX-A\otimes \K }:C\left( \sX^+\right) \hookto M\left(A\otimes \K \right)$ and there is a diagram

\be\label{chern_eqn}
\begin{tikzcd}
	\mathbb{M}_\infty \left( A^+\right)  \ar[r,hook, "\phi"] & A \otimes \ar[rd,hook, "\phi_A"]  \K &\\
	& & 	 M\left(A\right)\otimes \K \\
	& \mathbb{M}_\infty \left(C\left( \sX^+\right)\cong C_0\left( \sX\right)^+ \right) \ar[ru,hook, "\phi_\sX"]&
\end{tikzcd}
\ee
...
\eean 
For any $x_0, x_1 \in S^2$ with $x_0\neq x_1$ there is a homeomorphisms
\bean
...
\be
K\left( S^2 \setminus \{x_0\}\sqcup \{x_1\}\right)\xrightarrow{\phi_1} K\left( S^2 \right) \xrightarrow{\phi_2} K\left( \{x_0\}\sqcup \{x_1\} \right).
\ee
One can proof that
$$
