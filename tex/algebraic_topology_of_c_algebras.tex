...
\begin{definition}
	Under the above hypothesis for any $\sV\in \mathrm{Proj}^{\mathrm{nat}}\left(A \right)$ 
	 we define  {\it Chern classes} $c_j\left(\sV \right)\in H^{2j}\left(\sX^+, \Z \right)$ of $\sV$ as {Chern classes} of $\varphi_*\left( \sV\right) \in \mathrm{Proj}\left( C\left( \sX^+\right)\right)$ (cf. Theorem \ref{k:char_thm})
 \end{definition}
\begin{empt}
...
\eean
correspond to the given by \eqref{open_g_eqn} subsets of $\mathbb{P}\left(\mathbb{P}\left(\H \right) \right)$. Using the Remark \ref{compact_nonempt_rem} one can prove that $\sV'\cap \sV'' \neq \emptyset$, i.e. there is a contradiction with our initial hypothesis.
