...
\end{theorem}
\section{Order Structure on $K_0$}
\section{Theory of AF Algebras}
\begin{definition}\cite{blackadar:ko}
%Definition 7.1.1.
 A $C^*$-algebra $A$ is an {\it AF algebra} if $A$ is an inductive limit
of a sequence of finite-dimensional $C^*$-algebras.
\end{definition}
\begin{example}\cite{blackadar:ko}
%Examples 7.1.2. 
The CAR algebra; more generally, UHF and matroid $C^*$-
algebras; $\K$; $C_0\left( \sX\right)$, when $\sX$  is locally compact, totally disconnected, and second
countable
\end{example}
