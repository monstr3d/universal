...
\paragraph*{}
If both  $A$ and $\widetilde{A}$ are $C^*$-algebras and $\varphi :M\left(  A\right) \to M\left( \widetilde{A}\right)$ is a good $*$-homomorphism (cf. Definition \ref{good_hom_defn}). Any maximal left ideal $\xi^\perp \in \mathfrak{LeftMax}\left(A \right)$ yields a generated by $\widetilde{A}\varphi\left(\xi \right)$ closed left ideal $\widetilde I_\xi$. If 
\be\label{phM_eqm}
\mathfrak{LeftMax}\left(\varphi  \right)^{-1}\left(\left.\xi^\perp \right)\bydef  \left\{ \widetilde \xi^\perp \in \mathfrak{LeftMax}\left(\widetilde A\right)\right| \widetilde I_\xi\subset \xi^\perp  \right\}.
\ee

	Using the above map one can construct the natural map
	\be\label{ultra_mor_eqn}
	\begin{split}
			\mathfrak{LeftMax}\left(\widetilde  A\right) \xrightarrow{\mathfrak{LeftMax}\left(\varphi \right)}\mathfrak{LeftMax} \left( A\right),\\
		\mathfrak{LeftMax}\left(\varphi \right)\left( \widetilde \xi^\perp\right) = \xi^\perp \quad \Leftrightarrow \quad \widetilde \xi^\perp \in \mathfrak{LeftMax}\left(\varphi_M \right)^{-1}\left(\xi^\perp \right).
	\end{split}
	\ee
From the Remark \ref{bicommutant_rem} it follows that the map \eqref{ultra_mor_eqn} is continuous. Using the bicommutant map \eqref{bicommutant_eqn} one can construct the continuous map.
	...
	\begin{theorem}\label{good_thm}
	 One has
			\begin{enumerate}
				\item [(i)] 	If the  homomorphism $\varphi : M\left( A\right) \to M\left( \widetilde{A}\right)$ good 	  then both maps \eqref{ultra_mor_eqn} and \eqref{gelfand_map_eqn} are continuous (cf. Definition \ref{top_continuous_defn}).
				\item[(ii)] If $\varphi$ is injective then the maps \eqref{ultra_mor_eqn} and \eqref{gelfand_map_eqn} are surjective.
			\end{enumerate}
	...
	\begin{proof}(i) 		If $L \subset A$ is a closed left ideal and 
		...
		(ii) If $\varphi$ is injective   then for any $\xi \in \mathfrak{Gelfand}\left(A\right)$   the given by \eqref{phM_eqm} set  is not empty. From \eqref{ultra_mor_eqn} it follows that $\xi \in  \mathfrak{Gelfand}\left(\varphi\right)\left( \mathfrak{Gelfand}\left(A\right) \right)$.
