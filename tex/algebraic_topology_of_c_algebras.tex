...
\end{proof}
	...
	\end{definition}
	\begin{lemma}\label{nt_lem} If $C\left(\T^n _{\theta}\right)$ is a noncommutative torus (cf. Definition \ref{nt_defn}) then there is a bijective continuous map 
	...
	\begin{proof}
		...
		and for any $x_j \in \{x_1,...,x_n\}$ we select a basis $\left\{\sU^k_{x_j}\right\}_{k\in \N}$ of connected open neighborhoods of $x_j$. Then the net of products $\mathfrak{U}_x \bydef \left\{\sU^k_x \bydef\sU^k_{x_1}\times ...\times \sU^k_{x_n}\right\}_{k\in N}$ is a basis of neighborhoods of $x$. The net $\mathfrak{U}_x$ is an ultrafilter of $\T^n$. There is a filter $\varphi^{-1} \left( \mathfrak{U}_x\right)  \bydef \left\{\varphi^{-1}\left( \sU^k_x \right) \right\}_{k\in N}$ on $\mathfrak{Gelfand}\left( C\left(\T^n _{\theta}\right) \right)$ which yields a filter of closed left ideals $\left\{L_k\subset  C\left(\T^n _{\theta}\right) \right\}$. If $\left\{L_k \right\}$ is not an ultrafilter then there is a nontrivial  closed ideal $L \subset  C\left(\T^n _{\theta}\right)$ such that $L \subsetneqq L_k$. It is impossible since $\bigcap L_k = \{0\}$. So $\varphi^{-1} \left( \mathfrak{U}_x\right)$ is an ultrafilter which yields the unique point which is mapped onto $x$, i.e. the map $\varphi$ is surjective.
		
	\end{proof}
	
	\begin{lemma}
		Under the hypothesis of the Lemma \ref{nt_lem} 
		if $\mathscr F$ is a sheaf of Abelian groups on $\T^n$ and $\varphi^{-1}\mathscr F$ is its inverse image (cf. Definition \ref{sheaf_inv_im_defn}) then for any $p \in \N$ there is the natural inclusion
		\bean
		H^p\left( \T^n, \mathscr F \right) \subset  H^p\left( \mathfrak{Gelfand}\left( C\left(\T^n _{\theta}\right)\right), \varphi^{-1}\mathscr F \right)
		\eean
	\end{lemma}
	\begin{proof}
		If $\sX \bydef  \mathfrak{Gelfand}\left( C\left(\T^n _{\theta}\right)\right) $ and $\mathscr G \bydef \varphi^{-1}\mathscr F$ then from the 
		the Lemma \ref{nt_lem} it follows that there is a bijective continuous map $\sX \onto \T^n$.
		For any $x \in \sX$ there is the natural inclusion 
		\be\label{nt_can_st_eqn}
		\mathscr F_{\varphi\left(x \right) }\subset \mathscr G_{x}
		\ee 
		since the topology of $\mathfrak{Gelfand}\left( C\left(\T^n _{\theta}\right)\right)$ is finer then the topology of $\T^2$
		Let both 
		\bean
		0 \to \mathscr F \xrightarrow{\eps^{\T^2}} 	\mathscr{C}^{0}\left(\T^2, \mathscr F \right) \xrightarrow{d^{\T^2}_1 } \mathscr{C}^{1}\left(\T^2, \mathscr F \right)\xrightarrow{d^{\T^2}_2 } \mathscr{C}^{2}\left(\T^2, \mathscr F \right)\xrightarrow{d^{\T^2}_3}\dots\\
		0 \to \mathscr G \xrightarrow{\eps^{\sX}} 	\mathscr{C}^{0}\left(\sX, \mathscr G \right) \xrightarrow{d^\sX_1 } \mathscr{C}^{1}\left(\sX, \mathscr F \right)\xrightarrow{d^\sX_2 } \mathscr{C}^{2}\left(\sX, \mathscr G \right)\xrightarrow{d^\sX_3}\dots\\
		\eean 
		are canonical resolutions (cf. Appendix \eqref{canonical_resolution_eqn}). Using the explained in the   Appendix \eqref{canonical_resolution_eqn} construction  one can prove that for any $p$ there is the natural inclusion
		\be\label{nt_cany_st_eqn}
		\forall x \in \sX \quad \mathscr{C}^{p}\left(\T^n, \mathscr F \right)_x\hookto \mathscr{C}^{p}\left(\sX, \mathscr G \right)_{\varphi\left(x \right) }
		\ee 
		Using the  inclusion \eqref{nt_cany_st_eqn} one can construct commutative diagram
		\bean
		\begin{tikzcd}
			0 \arrow[r] & \mathscr F \arrow[d, "\phi_0"] \arrow[r, "\eps^{\T^n}"]	&\mathscr{C}^{0}\left(\T^n, \mathscr F \right)  \arrow[d, "\phi_1"]\arrow[r, "d_1^{\T^n}"] & \mathscr{C}^{1}\left(\T^n, \mathscr F \right) \arrow[d, "\phi_2"]\arrow[r, "d_2^{\T^n}"] & \mathscr{C}^{2}\left(\T^n, \mathscr F \right) \arrow[d, "\phi_3"] \arrow[r, "d_3^{\T^n}"] & \dots\\
			0 \arrow[r] & \mathscr G \arrow[r, "\eps^{\sX}"]	&\mathscr{C}^{0}\left(\sX, \mathscr G \right)  \arrow[r, "d_1^{\sX}"] & \mathscr{C}^{1}\left(\sX, \mathscr F \right) \arrow[r, "d_2^{\sX}"] & \mathscr{C}^{2}\left(\sX, \mathscr G \right)  \arrow[r, "d_3^{\sX}"] & \dots
		\end{tikzcd}
		\eean
		The above diagram yields the diagram 
			\bean
		\begin{tikzcd}
			0 \arrow[r] & \Ga\left( \mathscr F \right) \arrow[d, "\Ga\left(\phi_0 \right)" ] \arrow[r, "\Ga\left( \eps^{\T^n}\right) "]	&\Ga\left( \mathscr{C}^{0}\left(\T^n, \mathscr F \right)\right)   \arrow[d, "\Ga\left(\phi_1 \right)"]\arrow[r, "\Ga\left( d_1^{\T^n}\right) "] & \Ga\left( \mathscr{C}^{1}\left(\T^n, \mathscr F \right)\right)  \arrow[d, "\Ga\left(\phi_2 \right)"]\arrow[r, "\Ga\left( d_2^{\T^n}\right)"] & \Ga\left( \mathscr{C}^{2}\left(\T^n, \mathscr F \right)\right)  \arrow[d, "\Ga\left(\phi_3 \right)"] \\
			0 \arrow[r] & \Ga\left( \mathscr G\right)   \arrow[r, "\Ga\left( \eps^{\sX}\right) "]	&\Ga\left( \mathscr{C}^{0}\left(\sX, \mathscr G \right)\right)   \arrow[r, "\Ga\left( d_1^{\sX}\right) "] & \Ga\left( \mathscr{C}^{1}\left(\sX, \mathscr G \right)\right)  \arrow[r, "\Ga\left( d_2^{\sX}\right) "] &\Ga\left(  \mathscr{C}^{2}\left(\sX, \mathscr G \right)\right)   
		\end{tikzcd}
		\eean
For any section $s \in \mathscr{C}^{p}\left(\T^n, \mathscr F \right)$ there is a section !!!
	\end{proof}
	\subsection{$K$-theory functor}
	\paragraph{}
If $A$ is a $C^*$-algebra then any element $a \in \mathbb{M}_n\left(A \right) $ corresponds to a matrix
\be\label{matr_eqn}
a= 	\begin{pmatrix}
	a_{1,1}& \ldots & a_{n,1}  \\
	\vdots& \ddots & \vdots\\
...
\begin{definition}\label{elementary_defn}
	If $A$ is an unital  $C^*$-algebra then a given by \eqref{matr_eqn} element $a\in 	 \mathbb{M}_n\left(A \right)$ is  $j,k$-\textit{elementary} if 
	...
	\ee
\end{definition}
If $j',j'', k', k'' \in \left\{1, ..., n\right\}$ and
\be\label{mat_dec_teqn}
A_{j',j'', k', k''}\bydef \mathfrak{e}_{j',k'}\mathbb{M}_n\left(A \right) \mathfrak{e}_{j'',k''}
\ee
then there is the natural $A$-$A$ bimodule isomorphism 
\be\label{mat_jjkk_eqn} 
A_{j',j'', k', k''} \cong A
\ee
From \eqref{mat_dec_teqn} for any $j',j'', k', k''\in \left\{1, ..., n\right\}$ there is the natural inclusion
\be\label{mat_dec_eqn}
\mathbb{M}_n\left(A \right)A_{j',j'', k', k''}\subset \mathbb{M}_n\left(A \right) 
\ee
From the equation \eqref{mat_dec_eqn} one can deduce that there is an open continuous map
\be\label{matrpp_eqn}
\begin{split}
	\mathfrak{Gelfand}\left( A_{j',j'', k', k''} \right) \subset \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right) 
\end{split}
\ee
where $\mathfrak{Gelfand}\left( A_{j',j'', k', k''} \right)$ is homeomorphic to $\mathfrak{Gelfand}\left( A \right)$ for all $j',j'', k', k''\in \left\{1, ..., n\right\}$.
\begin{lemma}\label{proj_id_lem}
	The given by \eqref{matrpp_eqn} map yields the natural  homeomorphism
	\be\label{matrppb_eqn}
	\begin{split}
		\coprod_{j',j'', k', k''\in \left\{1, ..., n\right\}}	\mathfrak{Gelfand}\left( A_{j',j'', k', k''} \right) \cong \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right) 
	\end{split}
	\ee
\end{lemma}
\begin{proof}
	Let $\xi \in \mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right) $ is represented by an ultrafilter $\left\{\sU_\la\right\}_{\la\in \La}$. If  both  $\left(  j'_1,j''_1, k'_1, k''_1\right) $  and $\left(  j'_2,j''_2, k'_2, k''_2\right) $  are different quadruples with
	\bean
	\begin{split}
	\forall \la\in \La \quad \sU_\la \cap \mathfrak{Gelfand}\left( A_{ j'_1,j''_1, k'_1, k''_1} \right)\neq \{0\}, \quad \sU_\la \cap \mathfrak{Gelfand}\left( A_{   j'_2,j''_2, k'_2, k''_2} \right)\neq \{0\
		\end{split}
		\eean
		then the filter 
		$$
	\left\{\sU_\la \cap \mathfrak{Gelfand}\left( A_{ j'_1,j''_1, k'_1, k''_1} \right)\right\}_{\la\in \La}	
		$$
		exceeds $\left\{\sU_\la\right\}$. If is impossible since   $\left\{\sU_\la\right\}$ is an ultrafilter. So there is the unique $j',j'', k', k'' \in \left\{1, ..., n\right\}$ and $\la_0 \in \La$ with 
			\bean
		\begin{split}
			\forall \la\in \La \quad \la \ge \la_0 \quad  \sU_\la \subset \mathfrak{Gelfand}\left( A_{  j',j'', k', k'' } \right)
		\end{split}
		\eean
		It turns out that the set $\mathfrak{Gelfand}\left( A_{  j',j'', k', k'' } \right)$ contains a basis of neighborhoods of $\xi$, From this circumstance it turns out that there is the homeomorphism \eqref{matrppb_eqn}.
	\end{proof}
	From the Lemma \ref{proj_id_lem} it follows that there is the natural isomorphism 
			\bean
	\begin{split}
\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right)\cong \mathfrak{Gelfand}\left(\mathbb{M}_n\left(C\left( \mathfrak{Gelfand}\left(A \right) \right)  \right)\right)
	\end{split}
	\eean
...
\end{split}
\ee
(cf. notation \ref{topology_eqn}). From \eqref{matrppbu_eqn} it turns out that 
	\bean
\begin{split}
	\mathbb{M}_n\left(C\left(  \mathfrak{Gelfand}\left( A\right) \right) \right)= L \oplus L'
\end{split}
\eean
where $L$ and $L'$ are corresponding to $\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right)_{\mathbb{M}_n\left(A \right)p}$ and\\ $\mathfrak{Gelfand}\left(\mathbb{M}_n\left(A \right)\right)_{\mathbb{M}_n\left(A \right)\left(1_{\mathbb{M}_n\left(A \right)} -p\right) }$ ideals.
So one has
	\be\label{matrppbup_eqn}
\begin{split}
	1_{\mathbb{M}_n\left(C\left(  \mathfrak{Gelfand}\left( A\right) \right) \right)}= p_{ \mathfrak{Gelfand}\left( A\right) }+\left( 1_{1_{\mathbb{M}_n\left(C\left(  \mathfrak{Gelfand}\left( A\right) \right) \right)}}-p_{ \mathfrak{Gelfand}\left( A\right) }\right) 
\end{split}
\ee
such that 
	\bean
\begin{split}
	1_{\mathbb{M}_n\left(C\left(  \mathfrak{Gelfand}\left( A\right) \right) \right)}= p_{ \mathfrak{Gelfand}\left( A\right) }+\left( 1_{\mathbb{M}_n\left(C\left(  \mathfrak{Gelfand}\left( A\right) \right) \right)}-p_{ \mathfrak{Gelfand}\left( A\right) }\right) 
\end{split}
\eean
where
\bean
p_{ \mathfrak{Gelfand}\left( A\right)} \in L,\\
1_{\mathbb{M}_n\left(C\left(  \mathfrak{Gelfand}\left( A\right) \right) \right)}-p_{ \mathfrak{Gelfand}\left( A\right) }\in L'	
\eean 
\begin{theorem}\label{alg_top_equivalence_thm}
There is the commutative diagram 
\be\label{alg_top_equivalence_eqn}
\begin{tikzcd}
	 C^*\text{-}\mathfrak{Gelfand}\arrow[dd, "\mathfrak{Gelfand}"] \arrow[rrdd, "K_*"]& & \\
	 & & \\
	 \mathfrak{Hausdorff}\arrow[r, "K^*"] & \mathfrak{Ab} \arrow[r, "\cong"] & \mathfrak{Ab}
\end{tikzcd}
\ee
where $\mathfrak{Gelfand}$ is the Gelfand functor (cf. Definitions \ref{gelfand_functor_defn}), $K_*$ and $K^*$ topological and algebraic $K$-theory functor, $\cong$ the natural equivalence between $K_*$ and $K^* \circ \mathfrak{Gelfand}$.
\end{theorem}
\begin{proof}
	From the Lemma \ref{proj_id_lem} it follows that any projector $p \in \mathbb{M}_n\left( A\right)$ naturally yields a projector   $p_{ \mathfrak{Gelfand}\left( A\right) }\in \mathbb{M}_n\left(C\left( \mathfrak{Gelfand}\left(A \right) \right)  \right)$ and vice versa. 
\end{proof}
...
If we consider $C^*$-norm topology on $\mathbb{M}_n\left(A \right)$ and the topology on $A_{j',j'', k', k''}$ which comes from the map \eqref{mat_jjkk_eqn} then one can prove that the bijection \eqref{mat_dec_eqn} is a homeomorphism.
From \eqref{matr_eqn} it follows that there is a continuous map 
\be 
\begin{split}
	\bigoplus_{j',j'', k', k''\in \left\{1, ..., n\right\}} 	\mathfrak{Gelfand}\left( A_{j',j'', k', k''} \right) 
\end{split}
\ee
From $L_{j, k}\subset L$ it follow
So if $L \neq \{0\}$ then there is   $a' \in  L \cap A_{j',j'', k', k''} \setminus \{0\}$ such that
$$
a' \mathbb{M}_n\left(A \right) \subset L
$$
Otherwise from the $A$-$A$ bimodule isomorphism 
$$
\mathfrak{e}_{j',k'}\mathbb{M}_n\left(A \right) \mathfrak{e}_{j'',k''}\cong A
$$
it follows that there is $a \in A \setminus \{0\}$ which corresponds to $a'$. 
For any $j,k \in \left\{1, ..., n\right\}$ and any closed left ideal $L \subset A$ there is a closed left ideal $L_{j,k}\bydef L \mathfrak{e}_{j,k} \subset \mathbb{M}_n\left(A \right)$. From this circumstance there is an inclusion
\bean
\mathfrak{inc}_{j,k}:\mathfrak{Gelfand} \left(A \right)\hookto \mathfrak{Gelfand} \left(\mathbb{M}_n\left(A \right) \right)
\eean
...
\be\label{matrp_eqn}
\begin{split}
	\mathbb{M}_n\left(A \right)  \to \mathfrak{e}^*_{j,k} \mathbb{M}_n\left(A \right)\mathfrak{e}_{j,k}\\
	a \mapsto  \mathfrak{e}^*_{j,k} a\mathfrak{e}_{j,k}
\end{split}
\ee
Since $\mathfrak{e}^*_{j,k} \mathbb{M}_n\left(A \right)\mathfrak{e}_{j,k}\cong A$ the homomorphism \eqref{matrp_eqn} yields a homomorphism 
\be\label{matr_pjk_eqn}
\begin{split}
	\mathfrak{pr}_{j,k}: \mathbb{M}_n\left(A \right)\to A.
\end{split}
\ee
From the Theorem !!! it follows that there is a continuous map
\be
\mathfrak{Gelfand} \left(A \right)\xrightarrow{\mathfrak{Gelfand}\left( \mathfrak{pr}_{j,k}\right)  }\mathfrak{Gelfand} \left(\mathbb{M}_n\left(A \right) \right)
\ee


	\subsection{Algebraic Gelfand space}
\paragraph{} If $A$ is a $C^*$- algebra and $\varphi : M\left(C_0\left(\sX \right)  \right) \to M\left( A\right)$ is a good injective $*$-homomorphism from the Theorem \ref{good_thm} is the natural surjective continuous map
$$
\psi_\varphi:	\mathfrak{Gelfand}: \left(A \right) \onto \sX_{\varphi}
$$
this map yields ab injective  $*$-homomorphism
$$
\phi_\varphi:	C_0\left( \sX_{\varphi}\right)\to C_b\left(\mathfrak{Gelfand}\left(A \right) \right)  
$$
Since  $\varphi$ is good one has $\phi	\left( C_0\left( \sX\right)\right) \subset C_0\left(\mathfrak{Gelfand}\left(A \right) \right)$. Let $\Phi$ be a set of all satisfying to the described above conditions maps. There the partial order on $\Phi$ such that for any $\phi', \phi''$ the condition $\varphi' \ge \varphi''$ is equivalent to the following commutative diagram
\bean
\begin{tikzcd}
	& \mathfrak{Gelfand} \left(A \right) \arrow[ld, "\psi_{\varphi'}" ] \arrow[rd, "\psi_{\varphi''}" ] &\\
	\sX_{\varphi'} \arrow[rr] && \sX_{\varphi''}	
\end{tikzcd}
\eean
If $\varprojlim_{\varphi \in \Phi} \sX_\varphi$ is the inverse limit of topological spaces then 
$$
C_0\left(\varprojlim_{\varphi \in \Phi}\sX_\varphi \right) \cong 	C^*\text{-}\varinjlim_{\varphi \in \Phi} C_0\left(\sX_\varphi \right) 
$$
where 	$C^*\text{-}\varinjlim$ means the inductive limit (cf. \cite{matro:hcm})
\begin{definition}
	Under the above hypothesis the space  \bean \mathfrak{Alg}\text{-}\mathfrak{Gelfand} \left(A \right)\bydef \varprojlim_{\varphi \in \Phi} \sX_\varphi\eean  
	is the {\it algebraic Gelfand space} of $A$. From the surjective continuous map 
	\be
	\mathfrak{Gelfand} \left(A \right)\onto \mathfrak{Alg}\text{-}\mathfrak{Gelfand} \left(A \right)
	\ee
	it turns out that there is the natural good injective {\it  Dauns-Hoffman $*$-homomorphism}
	\be\label{dauns_hofmann_eqn}
	C^{\text{DH}}_0 \left(A \right) :	C_0\left( \mathfrak{Alg}\text{-}\mathfrak{Gelfand} \left(A \right)\right) \hookto M\left(A \right)
	\ee
\end{definition}

\begin{definition}\label{gelfand_alg_functor_defn}
	There is a contravariant {\it algebraic Gelfand functor} $\mathfrak{Alg}\text{-}\mathfrak{Gelfand}$
	\be
	C^*\text{-}\mathfrak{Gelfand}\to \mathfrak{Hausdorff}
	\ee
	from Gelfand category (cf. Definition \ref{gelfand_category_defn}) to the category  $\mathfrak{Hausdorff}$ of locally compact Hausdorff topological spaces and continuous maps. For any $C^*$-algebra $A$ the functor   $\mathfrak{Alg}\text{-}\mathfrak{Gelfand}$ sets its algebraic Gelfand space.
\end{definition}	
...
\section{Gelfand spaces and good $*$-homomorphisms}
...
Let $A$ be a $C^*$-algebra with connected, locally connected Gelfand space $\mathfrak{Gelfand}\left(A \right)$. If  a connected open subset $\sU\subset \mathfrak{Gelfand}\left(A \right)$ is \textit{algebraically semilocally proper} if for any {noncommutative covering} $\mathfrak{Gelfand}\left(\widetilde A \right)\xrightarrow{\mathfrak{Gelfand}\left(\varphi \right)}\mathfrak{Gelfand}\left(A \right)$  (cf. Definition \ref{covering_defn}) the set $\sU$ is evenly covered by $\mathfrak{Gelfand}\left(\varphi \right)$ (cf. Definition \ref{top_covering_defn}). $A$ $\sX$ is said to be \textit{semilocally 1-connected} if for any $x\in \mathfrak{Gelfand}\left( A \right)$  there is a semilocally proper neighborhood.
\end{definition} 

 \begin{definition}\label{alg_contracible_defn}
Let $A$ be a  \textit{semilocally 1-connected} $C^*$-algebra.	A closed {weak path  $\mathfrak p\bydef\left(\sU_1, ..., \sU_n\right)$ on} $\mathfrak{Gelfand}\left(A \right)$  is \textit{algebraically contractible} for any   for any {noncommutative covering} $$\mathfrak{Gelfand}\left(\widetilde A \right)\xrightarrow{\mathfrak{Gelfand}\left(\varphi \right)}\mathfrak{Gelfand}\left(A \right)$$ the a $\mathfrak{Gelfand}\left(\varphi \right)$-{lift} of $\mathfrak p$ is closed (cf. Definitions \ref{top_closed_path_defn} and \ref{top_path_lift_defn}).
\end{definition}
...
Moreover one can construct the natural commutative diagram
		\be\label{cov_et_dia}
\begin{tikzcd}
\mathrm{Sp\acute{e}}\left(\widetilde{\mathscr P}\right) \arrow[r] \arrow[d]	&\mathfrak{Gelfand}\left(\widetilde A \right)\arrow[r, "\widetilde f"] \arrow[d, "\mathfrak{Gelfand}\left(\varphi \right)"] & \widetilde \sX \arrow[d, "\varphi"] \\
...
\begin{theorem}
%3.25. Theorem. 
The Chem character has the following properties:
\begin{enumerate}
	\item [(i)]  If $\sX$ is the sphere $S^{2n}$, then $Ch$ is injective, and its image is $H^*(S^{2n}, \Z)\subset H^*\left( S^{2n}, \Q\right)$,
		\item [(ii)] b) For any compact space $\sX$, Ch induces an isomorphism 
	$$K\left(\sX \right) \otimes \Q \xrightarrow{\cong} H^{\mathrm{even}} \left(\sX, \Q\right)$$
		(the cohomology used here is \v{C}ech cohomology).
\end{enumerate} 
 
\end{theorem}
\begin{remark}
!!!\cite{a_thomas_kt_hom}. 
...
\begin{itemize}
	...
	a partial isometry; 
	\item{\it unitarily equivalent}, denoted $p\sim_uq$, when $p = u^*qu$, $u \in A^+$
	being unitary; 
...
\begin{theorem}\label{rosen_ak2_thm}
%Theorem 3.3. 
...
\subsection{Hermitian connections}
...
\begin{theorem}\label{derived_finctor_thm}\cite{hartshorne:ag}
	...
	\end{enumerate}
\end{theorem}
\begin{definition}\label{cohom_defn}\cite{hartshorne:ag}
Let $\sX$ be a topological space. Let $\Ga\left( \sX, -\right)$  be the global section 
functor from the category of sheaves on $\sX$ to the category of Abelian groups. We define the cohomology functors $H^p\left(\sX,- \right)$
	to be the right derived functors of $\Ga\left( \sX, -\right)$. 
	\end{definition}
...
\end{remark}
\subsection{Canonical resolution}\label{canonical_resolution_sec}
...
0 \to \mathscr F \xrightarrow{\eps} 	\mathscr{C}^{0}\left(\sX, \mathscr F \right) \xrightarrow{d_1 } \mathscr{C}^{1}\left(\sX, \mathscr F \right)\xrightarrow{d_2 } \mathscr{C}^{2}\left(\sX, \mathscr F \right)\xrightarrow{d_3}\dots
\ee
is exact. That is, $\mathscr{C}^{\bullet}\left(\sX, \mathscr F \right)$ is an injective resolution of $\mathscr F$. It is called the \textit{canonical
	resolution} of $\mathscr F$.
If 
\be\label{canonical_resolution_cohomolgy_sheaf_eqn}
\mathscr H^p\left(\sX, \mathscr F \right) \cong \begin{cases}
	\ker d_1 & p = 0\\
	\ker d_{p+1}/ d_p\left(\mathscr{C}^{p-1}\left(\sX, \mathscr F \right) \right) & p > 0
\end{cases}
\ee
then from the Definition \ref{cohom_defn} it follows that
\be\label{canonical_resolution_cohomolgy_eqn}
\forall p > 0 \quad H^p\left(\sX,  \mathscr F\right) = \Ga\left(\sX, \mathscr H^p\left(\sX, \mathscr F \right) \right) 
\ee
