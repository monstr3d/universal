...
where $f_* \mathscr F$ is the direct image of $\mathscr F$.
 \end{theorem}
 The above cohomology are not integrable. It would be good find integral cohomology. The Zariski topology is such that any open subset is dense. However there is the \alert{Chow ring} which can be regarded as integral cohomology. The {Chow ring} is an algebraic geometry analog of the representation of homology by smooth manifolds. Here we consider an analog of the {Chow ring} for the Gelfand space $\C P^n_{\mathfrak{Gelfand}}$.
 \end{frame}
 \section{Dimension and codimension}
 \begin{frame}
 	{\Large Dimension and codimension} 
...
\alert{linearly equivalent} if there are an algebraic maps $f_\iota : \sX \to \mathbb P^1$ to one dimensional projective space
\be\label{projective_sum_eqn}
\sum n_\iota \left(\left[f^{-1}\iota\left(0 \right) \right]- \left[f^{-1}_\iota\left(\infty \right) \right] \right) 
\ee 
The linear equivalence is an algebraic geometry analog of the algebraic topology notion of homotopy.
  \end{frame} 
  \begin{frame}
 ...
 modulo rational equivalence. We denote by $A\left(\sX\right)= \bigoplus_{r= 0}^n A^r\left( \sX\right)$  the graded group  where $n = \dim \sX$. Note that $A^0\left( \sX\right)= \mathbb Z$ , and that $A^r\left(\sX \right)= 0$  for $r > \dim \sX$.
  \end{frame}
  \begin{frame}
  An \alert{ intersection theory} on a class of varieties $\mathfrak{B}$ consists of giving a 
...
\end{frame}
  ...
  The Chow ring does not depend on the Zariski topology. The Chow ring of  $\C P^n_{\mathfrak{Gelfand}}$ have much more easy structure which uniquely depends of the topology of Gelfand space.
  \end{frame}
  ...
  \end{frame}
  \section{Category of projective spaces}
  \begin{frame}
  	{\Large  Surjective projective maps}
  	According to our construction if the division ring is the field $\C$ then synthetic, analytic and topological  projective geometries are equivalent. We define a one-dimensional projective space a a line of any $\C P^n$.
  	\begin{definition}
  		%Definition. 
  ...
  \end{frame}
  \begin{frame}
  	{\Large  Complex projective category}
  	Using the above construction  if $m 
  	\le n$ then one can construct a surjective maps $\C P^n\onto \C P^m$. Also there are inclusions  $\C P^m\hookto\C P^n$.
  	\begin{definition}
  		There is the \alert{ complex projective category} $\C P$ such that 
  		\begin{itemize}
  			\item objects are the spaces $\C P^n$,
  			\item any morphism is a map $f: \C P^n\to \C P^m$ such that there is surjective maps $g:\C P^n\onto \C P^k$ and an inclusion $h:\C P^k\onto \C P^m$ with $f = h \circ g$,
  		\end{itemize}
  	\end{definition}
  	From the topological projective geometry it follows that the complex projective category is a subcategory of the category of topological spaces and continuous maps. The synthetic, analytic and topological structure are equivalent. The complex projective category can be regarded as a category of Gelfand spaces $\C P^n_{\mathfrak{Gelfand}}$ and continuous maps between them.
  ...
  \end{frame}
   ...
   \begin{frame}
   	{\Large Chow ring  of $\C P^n_{\mathfrak{Gelfand}}$}
   Similarly to the algebraic geometry define a \alert{prime divisor}  as an irreducible closed subset  of $\C P^n_{\mathfrak{Gelfand}}$ having codimension 1 . One can define the group Abelian group $\mathrm{Div}~\C P^n$ which is an analog of $\mathrm{Div}~\sX$ and any divisor correspond to a injective linear map $\C P^{n - 1} \hookto \C P^n$.
     There is the natural correspondence between the  set of continuous maps  $\C P^n_{\mathfrak{Gelfand}}\to \C P^1_{\mathfrak{Gelfand}}\cong \C P^n_{\mathfrak{Zariski}}$ and the set of linear in sense of analytical geometry  maps $\C P^n\to \C P^1$. Any linear map $f: \C P^n\to \C P^1$ one can define an analog of the \alert{ principal divisor} given by 
   	$$
   	f^{-1}\left( 0\right) - f^{-1}\left(\infty \right). 
   	$$
      There is a notion of \alert{ linearly equivalent} divisors  For any two different linear maps $\phi' :\C P^{n - 1} \hookto \C P^n$ and $\phi'': \C P^{n - 1} \hookto \C P^n$ there is a linear map $f: \C P^n\to \C P^1$ such that 
   \bean
   f\left(0 \right) = \phi'\left(\C P^{n - 1} \right),\\ 
   f\left(\infty \right) = \phi''\left(\C P^{n - 1} \right),
   \eean 
   i.e. any prime divisors are linearly equivalent.
\end{frame}
\begin{frame}
Using the linear equivalence one can define the \alert{group of classes of divisors}
  \bean
   \mathrm {Cl}~\C P^n_{\mathfrak{Gelfand}}\cong \Z\left[\C P^{n-1}\right]
  \eean
   where $\left[\C P^{n-1}\right]\in \mathrm {Cl}~\C P^n_{\mathfrak{Gelfand}}$ is an analog an element of divisor class group. One can define \alert{ cycle of codimension} $r$ on $\C P^n$ is an element 
...
\begin{enumerate}
	...
	\be\label{lc_eq_eqn}
	\sY' = f^{-1}\left(0 \right),\quad 
	\sY'' = f^{-1}\left(\infty \right)
	\ee 
	\end{enumerate}
i.e. cycles are \alert{ rationally equivalent}.\end{frame}
\begin{frame}

There is the group $A^r\left(\C P^n_{\mathfrak{Gelfand}} \right)$ of rationally equivalent  classes of $r$-cycles. The cycles $\sY'$ and $\sY''$ are rationally equivalent, i.e.
\be\label{ar_c_eqn}
...
\begin{frame}
	{\Large Cobordism category}
\begin{definition}\label{cobordism_category_defn}
	A \alert{cobordism category} $\left(\mathscr C, \partial, j\right)$ is a triple in which
	\begin{enumerate}
...
\section{Cobordism interpretation}
\begin{frame}
	{\Large Cobordism interpretation}
There is an interpretation of the Chow ring it terms of the cobordism category.
...
\begin{definition}\label{blowing_ideals_au_ua_defn}
	Let  $ C_0\left( \sY\right)\subset  M\left( A\right) $ be  Hausdorff blowing-up of $A$, and let $\sU \subset \sY$ be an open subset. Both   left and right  closed ideals $A_\sU$  and $_\sU A$ of $A$ generated by sets 	$AC_0\left( \sU\right)$ and $C_0\left( \sU\right)A$ are the \alert{left} $\sU$-\alert{ideal} and the \alert{right} $\sU$-\alert{ideal} respectively. A hereditary $C^*$-subalgebra of $A$
...
\begin{frame}
	{\Large Algebraic counterpart of geometric quotient}
...
Sometimes this duality comes from the given by Alain Connes  algebraic counterpart of geometric quotient  which makes a non Hausdorff spectrum from a Hausdorff space. In this case we say that there is  \alert{ Connes quantization}. Roughly speaking physics of noncommutative torus contains all elementary particles of commutative one. Besides common "elementary particles" noncommutative torus contains very massive particles which could not be easy detected.
