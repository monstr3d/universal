...
\end{exercise}
From the  \ref{ctr_rep_eq_lem} it follows that for any $a \in CT\left(\sX, \dl \right)$ a subset 
$$
...
\end{definition}
From the Theorem \ref{dauns_hofmann_thm} it follows that there is the natural inclusion $C\left( \sX \right) \subset M\left( CT\left(\sX, \dl \right)\right)$. From this fact one can deduce that is $B$ is a hereditary $C^*$-subalgebra and $S'(B)$ is a group of continuous maps from $\cosupp B \to \T$ then there is a natural injective homomorphism of groups $S'\left(B\right) \to U^A\left( B\right)$. One has a presheaf $B \mapsto S'\left( B\right)$. If
\be\label{ctr_sa_eqn}
...
From the *-homomorphism $\varphi : C\left( \sX \right) \hookto M\left( CT\left(\sX, \dl \right)\right)$ and the Lemma \ref{geometric_morphism_thm} it follows that there a  morphism of sites 
 \be\label{ctr_site_morphism_eqn} 
 ...
 \be\label{ctr_geometric_morphism_eqn} 
 ...
 Similarly to \ref{ctr_cycle_map_empt}  and the Lemma \ref{ctr_cycle_map_lem} one can construct a map
 $$
 \check H^1\left( \mathfrak{Site}\left(CT\left(\sX, \dl \right)\right) A^A\right)\to  \check H^2\left(\mathfrak{Site}\left(CT\left(\sX, \dl \right)\right), S^A \right). 
 $$
 
 From the Lemma \ref{cech_geom_lem} it follows that there is a natural homomorphism
 $$
 \check H^2\left(\mathbf{Sh}\left(\mathfrak{Closed}\left(\sX\right)\right)  , S \right) \xrightarrow{\check{H}^2} \check H^2\left(\mathbf{Sh}\left( \mathfrak{Site}\left( CT\left(\sX, \dl \right)\right)\right) , S^A \right)
 $$
 If $CT\left(\sX, \dl \right)=\Ga\left(E \right) $ is a  set of sections a locally
 trivial bundle with fibre $\K(\H)$ and structure group $\Aut~ \K(\H)$ over the   space $\sX$ with transition functions $\a_{\iota j}$ then 	$\dl\left(\left[\a_{\iota j}\right] \right) \in H^2\left( \sX, S\right) \cong H^3\left( \sX,\Z \right)$ (cf. Proposition \ref{ctr_dd_prop}) represents $\delta \in H^3\left( \sX,\Z \right)$.

\paragraph{DELETE}
If $U \bydef U\left( M\left(CT\left(\sX, \dl \right)\right)\right) $ is a group of unitary elements of multiplier $C^*$-algebra and $B$ is a hereditary subalgebra of $CT\left(\sX, \dl \right)$ then from the equation \eqref{ctr_pb_c_eqn} it follows that  there is homomorphism of groups
$
U'^A_B : U \to \Aut ~A^A\left( B\right) 
$. Moreover if $U^A\left(B \right)\bydef \im   U'^A_B$ then one has a surjective homomorphism of groups
$
U^A_B : U \to U^A\left(B \right)
$.
The map 
\be\label{ctr_ua_eqn}
B \mapsto U^A\left(B \right)
\ee 
is a $\mathbf{Hered}/CT\left(\sX, \dl \right)$-presheaf of groups (cf. Definitions \ref{presheaf_defn} and \ref{hered_cat_defn}). Similarly to the Lemma \ref{ctr_psh_sh_defn} one can proof that a  presheaf \eqref{ctr_ua_eqn} is a sheaf with respect to the Gelfand $CT\left(\sX, \dl \right)$-topology (cf. Definition \ref{gelfand-naimark_thm}). 
From the Theorem \ref{dauns_hofmann_thm} it follows that there is the natural inclusion
$$
dd
$$


  If $B \subset CT\left(\sX, \dl \right)$ is a hereditary $C^*$-subalgebra  and  $L\left( B\right)$ is a given by the equation  \eqref{left_ideal_eqn} left ideal then From $M\left(CT\left(\sX, \dl \right) \right) L\left( B\right)= L\left( B\right)$ it follows that there is a natural surjective group homomorphism
$$
...
\ee
When  the map $B \mapsto  U^A\left( B \right)$ is a presheaf of groups on the category $\mathbf{Hered}/A$ (cf. Definitions \ref{hered_cat_defn} and  \ref{presheaf_defn}).From the  \ref{ctr_rep_eq_lem} it follows that for any $a \in CT\left(\sX, \dl \right)$ a subset 
$$
\sX_a \bydef \left\{x \in \sX | \left\| \mathfrak{rep}_x\left(a \right) \right\| > 0\right\}\subset \sX
$$
is open (where $\mathfrak{rep}_x$ is given by \eqref{rep_x_eqn})

There is a presheaf $S^A$ of groups $S$ !!! 
...
\end{remark}
\begin{empt}
	The sheaf $U^A$ of $\mathfrak{Site\sqrt{}}\left(A \right)$ is a counterpart of the explained in \ref{relating_empt} sheaf $U$ on $\sX$.
...
\end{definition}
Note that if $f,g \in  K^*$, then $(f/g) = (f) — (g$) because of the properties of valuations. Therefore sending a function $f$ to its divisor $(f)$ gives a homomorphism of the multiplicative group  $K^*$ to the additive group $\mathrm{Div}~ \sX$, and the image, which consists of the principal divisors, is a subgroup of $\mathrm{Div}~ \sX$. 
\begin{definition}
...
\begin{definition}
	...
	contractible subcomplexes ?Xi º such that every intersection of the Xi is either
