...
\ee
where $f, g \in \Ga_c\left(\G\left(M, \sF\right),\Om^{1/2}  \right)$ (cf. Appendix \ref{foli_alg_subsec}). The product \ref{foli_a_prod_eqn} ans the involution \ref{groupoid_inv_c_eqn} yields a *-algebra, we denote it by  $\Ga_c\left(\G\left(M, \sF\right),\Om^{1/2} , \sigma \right)$. 
...
\end{itemize}
\end{lemma}
	\begin{empt}\label{ind_ca_lim_empt}\cite{blackadar:oa}		
	One of the simplest and most useful constructions of $C^*$-algebras
	is the inductive limit (sometimes called direct limit). An \textit{inductive system} of $C^*$*-
	algebras is defined just as in the purely algebraic setting: a collection $\left\{f^\mu_\nu : A_\mu \to A_\mu\right\}_{\substack{\mu, \nu \in \La\\ \nu \ge \mu}}
	$, where $\La$ 
	is a directed set, the $A_\la$ are $C^*$-algebras, and $f^\mu_\nu$ is a
	*-homomorphism. Each $f^\mu_\nu$ is
	norm-decreasing, so there is a naturally induced $C^*$-seminorm on the algebraic
	direct limit defined by
	$$
	\forall a \in A_\mu \left\|a  \right\| \bydef \lim_{\la \ge \mu}\left\|f^\mu_\la\left( a \right)  \right\| ;
	$$
	the completion of the algebraic direct limit (with elements of seminorm
	0 divided out) is a $C^*$-algebra called the \textit{inductive limit of the system},
	written $C^*$-$\varinjlim_{\la\in \La} A_\la$ There is a natural
	*-homomorphism $\phi_\la$ from $A_\la$ to the inductive limit.
	If all the connecting maps are injective (and hence isometric), the algebraic
	direct limit may be thought of as the "union" of the $A_\la$, and the inductive limit
	as the completion of this union. In general, the inductive limit of a system
	$\left\{f^\mu_\nu : A_\mu \to A_\mu\right\}_{\substack{\mu, \nu \in \La\\ \nu \ge \mu}}$  may be naturally regarded as a $C^*$-subalgebra 
	$$
	\left( 	\prod_{\la\in \La} A_\la\right) /	\left( \bigoplus_{\la\in \La}  A_\la\right). 
	$$
\end{empt}
