...
\begin{empt}
	...
	\bean
	\Phi_N \bydef L^2\left(\mathbb{F}^n \right)\otimes L^2\left(\mathbb{F}^n \right)\cong L^2\left(\mathbb{F}^n \right),\\
	\xi^{N,m}_j \otimes \xi^{N,m}_k \mapsto \xi^{N,m}_{f\left(j, k \right) }
	\eean.  For any  $a, b \in B\left( L^2\left(\mathbb{F}^n \right)\right)$ we can define 
\bean
\Psi_N\left( a, b\right) 	\in B\left( L^2\left(\mathbb{F}^n \right)\right),\\
\xi_1\otimes  \xi_2 = \Phi^{-1}_N\left(\xi \right) \Rightarrow \Psi_N\left( a, b\right)\left(\xi\right)= \Phi_N \left(\xi_1, \xi_2 \right)  
\eean
	\begin{lemma}
		For any $a, b \in B\left( L^2\left(\mathbb{F}^n \right)\right)$ the sequence $\Psi_N\left( a, b\right)$ is convergent with respect to the strong topology \ref{strong_topology_defn}.
	\end{lemma}
\end{empt}
