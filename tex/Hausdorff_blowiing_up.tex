...
\begin{tikzpicture}[
	...
	child {node[level 2] (ch1) {Hausdorff blowing-up}
		child {node[level 2] (c1) {Hausdorff spectrum}}
		child {node[level 2] (c2) {Other}}{
			child {node[level 2] (c3) {Groipoids}}
		}
	}
	child {node[level 2] (ch2) {Other coverings}
	};
...
\begin{scope}[every node/.style={level 3}]
	\node [below = of  c1, xshift=15pt] (c11) {Commutative $C^*$-algebras};
	\node [below = of  c11] (c12) {Noncommutative $C^*$-algebras};
...
\draw[->] (c1.195) |- (c1\value.west);
\begin{scope}[every node/.style={level 2}]
	\node [below = of  c2, xshift=10pt] (c21) {Coverings of groupoids};
	\node [below = of  c21] (c22) {Coverings of foliations};
\foreach \value in {1,...,2}
