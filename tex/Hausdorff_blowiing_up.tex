...
\eean
\alert{Petr Ivankov} developed  \alert{noncommutative fundamental group} such that there is a isomorphism $\phi :\pi_1\left(\sY \right)\xrightarrow{\cong} \pi_1\left(C_0\left( \sY \right) \right)\xrightarrow{\cong}\pi_1\left(A \right)$  a \alert{noncommutative Hurewicz homomorphism} for noncommutative $C^*$-algebras
$h^A_{\text{nc}}: \pi_1\left(A \right) \to K^1\left( A\right)$
such that
$$
h^A_{\text{nc}}\circ \phi = K_1 
	$$
