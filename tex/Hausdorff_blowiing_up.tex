...
\begin{enumerate}
	\item 	Experimentally proved physics is related to $C^*$-algebras with Hausdorff spectrum.
	\item 		Experimentally proved physics is not self-consistent since it contains divergences.
	\item To avoid divergences one needs application of $C^*$-algebras with non-Hausdorff spectrum.
	\item Hausdorff blowing-up yields a duality between $C^*$-algebras with Hausdorff spectrum.
\end{enumerate}
\end{frame}
...
\begin{frame}
	...
	\end{theorem}
There is a generalization of infinite topological coverings such that an infinite covering is approximated by a set of finite ones.
\end{frame}
...
It is proven that under the above  hypotheses   for all $\la\in\La$ there is a  natural   homomorphism of $A_\la$-$A_\la$-bimodules given by
	...
	The above
	homomorphism   $\desc_{\la}$ of $A_\la$-$A_\la$-bimodules is the $\la$-\alert{descent}.% If $\la_{\min}\in \La$ is the minimal element (cf. then a given by	
%	$$
...
\begin{frame}
	It is not clear whether pre-covering of the algebraical finite covering category is unique.
...
$\mathfrak{S}=\left\{\left\{A_\la\right\}_{\la\in \La}, \left\{\pi^\mu_\nu : A_\mu \hookto A_\nu\right\}_{\substack{\mu, \nu \in \La\\\mu \le \nu}}\right\}$ there is the disconnected infinite noncommutative covering.
\end{theorem}
\end{frame}
\begin{frame}
...
\begin{theorem}\label{blowing_sufficient_covering_inf_thm} % {blowing_inf_cov_thm}
 ...
 such that 	a triple $\left(A, A_0\left(\overline\sY \right), \widehat{G} \right)$  is a {pre}-{covering of algebraical finite covering category}  $\mathfrak{S}_{A_0\left(q \right) }$ (cf. Definition \ref{algebraical_finite_covering_category_defn}), 
		\item[(ii)] if the triple $\left(A, A_0\left(\overline\sY \right), \widehat{G} \right)$   is the  \textit{infinite noncommutative pre-covering} of  $\mathfrak{S}_{A_0\left(q \right) }$.
	\end{enumerate}
\end{theorem}
