...
namespace IBSampleApp.ui
{
    public class HistoricalDataManager : DataManager
    {
        ...
        public void SaveHistoricalData(HistoricalDataEndMessage message)
        {
            if (StaticExtensionIBApi.HistoryWriteState != HistoryWriteState.Active) 
            {
                StaticExtensionIBSample.ResetHisoryWrite.Set();
                return;
            }
            var start = message.StartDate.Split(' ')[0] + ".history";
            start = StaticExtensionIBSample.HistoryPath + start;
            if (File.Exists(start))
            {

                using (var stream = File.OpenRead(start))
                {
                    ...
                    using (var st = new MemoryStream())
                    {
                        ...
                        bf.Serialize(st, historicalData);
                        var bt = st.ToArray();
                        var k = bt.SequenceEqual(b);
                        if (!k) { throw new Exception(); }
                    }
                }
            ...
            {
                using (var stream = File.OpenWrite(start))
                {
                    BinaryFormatter bf = new BinaryFormatter();
                    bf.Serialize(stream, historicalData);
                }
            }
        ...
        private void PaintChart()
        {
            Chart historicalChart = (Chart)uiControl;
            /*
            var dt =  DateTime.Now + ".history";
            dt = dt.Replace('/', '_').Replace(':', '_');
            using (Stream stream = File.OpenWrite(dt)) 
            { 
                BinaryFormatter bf = new BinaryFormatter();
                bf.Serialize(stream, historicalData);
            }*/
