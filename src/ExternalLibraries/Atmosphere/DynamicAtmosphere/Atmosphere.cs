...
-0.067,-0.088,-0.094,-0.088,-0.053,
     ...
     0.17,0.156,0.119,0.073,0.027,-0.023,-0.055,-0.078};
        ...
        public double Density(DateTime time, double[] state)
        {
            int ho = time.Hour;
            int mi = time.Minute;
            int ss = time.Second;
             double days = days1900(time) + 1;
        ...
        /*    date[0] = dt.Day;
            date[1] = dt.Month;
            date[2] = dt.Year;*/
            double rho = atm(state, time, ASoL, DSoL, ref alphastar, ref h, date);
            return rho;
        }
        ...
        double atm(double[] x, DateTime time, ref double s0, ref double h, int[] ifa, DateTime Date)
        {
            date[0] = Date.Day;
            date[1] = Date.Month;
            date[2] = Date.Year;
            double days = days1900(Date);
             double ASoL = 0, DSoL = 0, day, hour;
            sunPosition.GetPosition(time, out DSoL, out ASoL, out day, out hour);
            return atm(x, time, ASoL, DSoL, ref s0, ref h, date);
        }

        double atm(double[] x, DateTime time, ref double s0, ref double h, int[] ifa)
        {
            double ASoL = 0, DSoL = 0, day, hour;
            sunPosition.GetPosition(time, out DSoL, out ASoL, out day, out hour);
            //MyTime=t;
