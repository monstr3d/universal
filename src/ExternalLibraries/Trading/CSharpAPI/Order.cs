        ...
        */
        ...
        public object Clone()
        {
            return new Order()
            {
                ...
                ActiveStopTime = this.ActiveStopTime,
                AdjustableTrailingUnit = this.AdjustableTrailingUnit,
                AdjustedOrderType = this.AdjustedOrderType,
                AdjustedStopLimitPrice = this.AdjustedStopLimitPrice,
                AdjustedStopPrice = this.AdjustedStopPrice,
                AdjustedTrailingAmount = this.AdjustedTrailingAmount,
                AdvancedErrorOverride = this.AdvancedErrorOverride,
                AlgoId = this.AlgoId,
                AlgoParams = this.AlgoParams,
                AlgoStrategy = this.AlgoStrategy,
                AllOrNone = this.AllOrNone,
                AuctionStrategy = this.AuctionStrategy,
                AutoCancelDate = this.AutoCancelDate,
                AutoCancelParent = this.AutoCancelParent,
                AuxPrice = this.AuxPrice,
                BasisPoints = this.BasisPoints,
                BasisPointsType = this.BasisPointsType,
                BlockOrder = this.BlockOrder,
                CashQty = this.CashQty,
                ClearingIntent = this.ClearingIntent,
                ClearingAccount = this.ClearingAccount,
                ClientId = this.ClientId,
                CompeteAgainstBestOffset = this.CompeteAgainstBestOffset,
                Conditions = this.Conditions,
                ConditionsCancelOrder = this.ConditionsCancelOrder,
                ConditionsIgnoreRth = this.ConditionsIgnoreRth,
                ContinuousUpdate = this.ContinuousUpdate,
                Delta = this.Delta,
                DeltaNeutralAuxPrice = this.DeltaNeutralAuxPrice,
                DeltaNeutralClearingAccount = this.DeltaNeutralClearingAccount,
                DeltaNeutralClearingIntent = this.DeltaNeutralClearingIntent,
                DeltaNeutralConId = this.DeltaNeutralConId,
                DeltaNeutralDesignatedLocation = this.DeltaNeutralDesignatedLocation,
                DeltaNeutralOpenClose = this.DeltaNeutralOpenClose,
                DeltaNeutralOrderType = this.DeltaNeutralOrderType,
                DeltaNeutralSettlingFirm = this.DeltaNeutralSettlingFirm,
                DeltaNeutralShortSale = this.DeltaNeutralShortSale,
                DeltaNeutralShortSaleSlot = this.DeltaNeutralShortSaleSlot,
                DesignatedLocation = this.DesignatedLocation,
                DiscretionaryAmt = this.DiscretionaryAmt,
                DiscretionaryUpToLimitPrice = this.DiscretionaryUpToLimitPrice,
                DisplaySize = this.DisplaySize,
                DontUseAutoPriceForHedge = this.DontUseAutoPriceForHedge,
                Duration = this.Duration,
                ExemptCode = this.ExemptCode,
                ExtOperator = this.ExtOperator,
                FaGroup = this.FaGroup,
                FaMethod = this.FaMethod,
                FaPercentage = this.FaPercentage,
                FaProfile = this.FaProfile,
                FilledQuantity = this.FilledQuantity,
                GoodAfterTime = this.GoodAfterTime,
                GoodTillDate = this.GoodTillDate,
                HedgeParam = this.HedgeParam,
                HedgeType = this.HedgeType,
                Hidden = this.Hidden,
                ImbalanceOnly = this.ImbalanceOnly,
                IsOmsContainer = this.IsOmsContainer,
                IsPeggedChangeAmountDecrease = this.IsPeggedChangeAmountDecrease,
                LmtPrice = this.LmtPrice,
                LmtPriceOffset = this.LmtPriceOffset,
                ManualOrderTime = this.ManualOrderTime,
                MidOffsetAtHalf = this.MidOffsetAtHalf,
                MidOffsetAtWhole = this.MidOffsetAtWhole,
                Mifid2DecisionAlgo = this.Mifid2DecisionAlgo,
                Mifid2DecisionMaker = this.Mifid2DecisionMaker,
                Mifid2ExecutionAlgo = this.Mifid2ExecutionAlgo,
                Mifid2ExecutionTrader = this.Mifid2ExecutionTrader,
                MinCompeteSize = this.MinCompeteSize,
                MinQty = this.MinQty,
                MinTradeQty = this.MinTradeQty,
                ModelCode = this.ModelCode,
                NotHeld = this.NotHeld,
                OcaGroup = this.OcaGroup,
                OcaType = this.OcaType,
                OpenClose = this.OpenClose,
                OptOutSmartRouting = this.OptOutSmartRouting,
                OrderComboLegs = this.OrderComboLegs,
                OrderId = this.OrderId,
                OrderMiscOptions = this.OrderMiscOptions,
                OrderRef = this.OrderRef,
                OrderType = this.OrderType,
                Origin = this.Origin,
                OutsideRth = this.OutsideRth,
                OverridePercentageConstraints = this.OverridePercentageConstraints,
                ParentId = this.ParentId,
                ParentPermId = this.ParentPermId,
                PeggedChangeAmount = this.PeggedChangeAmount,
                PercentOffset = this.PercentOffset,
                PermId = this.PermId,
                PostToAts = this.PostToAts,
                ReferenceChangeAmount = this.ReferenceChangeAmount,
                RandomizePrice = this.RandomizePrice,
                RandomizeSize = this.RandomizeSize,
                ReferenceContractId = this.ReferenceContractId,
                ReferenceExchange = this.ReferenceExchange,
                ReferencePriceType = this.ReferencePriceType,
                RefFuturesConId = this.RefFuturesConId,
                RouteMarketableToBbo = this.RouteMarketableToBbo,
                Rule80A = this.Rule80A,
                ScaleAutoReset = this.ScaleAutoReset,
                ScaleInitFillQty = this.ScaleInitFillQty,
                ScaleInitLevelSize = this.ScaleInitLevelSize,
                ScaleInitPosition = this.ScaleInitPosition,
                ScalePriceAdjustInterval = this.ScalePriceAdjustInterval,
                ScalePriceAdjustValue = this.ScalePriceAdjustValue,
                ScalePriceIncrement = this.ScalePriceIncrement,
                ScaleProfitOffset = this.ScaleProfitOffset,
                ScaleRandomPercent = this.ScaleRandomPercent,
                ScaleSubsLevelSize = this.ScaleSubsLevelSize,
                ScaleTable = this.ScaleTable,
                SettlingFirm = this.SettlingFirm,
                Shareholder = this.Shareholder,
                ShortSaleSlot = this.ShortSaleSlot,
                SmartComboRoutingParams = this.SmartComboRoutingParams,
                Solicited = this.Solicited,
                StartingPrice = this.StartingPrice,
                StockRangeLower = this.StockRangeLower,
                StockRangeUpper = this.StockRangeUpper,
                StockRefPrice = this.StockRefPrice,
                SweepToFill = this.SweepToFill,
                Tier = this.Tier,
                Tif = this.Tif,
                TotalQuantity = this.TotalQuantity,
                TrailingPercent = this.TrailingPercent,
                TrailStopPrice = this.TrailStopPrice,
                Transmit = this.Transmit,
                TriggerMethod = this.TriggerMethod,
                TriggerPrice = this.TriggerPrice,
                UsePriceMgmtAlgo = this.UsePriceMgmtAlgo,
                Volatility = this.Volatility,
                VolatilityType = this.VolatilityType,
                WhatIf = this.WhatIf
