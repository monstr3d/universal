...
{
    public class Upload3D1Model : PageModel
    {
        private readonly long _fileSizeLimit;
        private readonly string[] _permittedExtensions = { ".obj", ".ac", ".dae" };

        IBytesSingleton bytesSingleton;
        ...
        public Upload3D1Model(IConfiguration config, IHyperLinkTransient hyperLink, IBytesSingleton bytes)
        {
            bytesSingleton = bytes;
            HyperLink = hyperLink;
...
*/
        ...
        public async Task<IActionResult> OnPostUploadAsync()
        {
            ...
            using (var stream = new MemoryStream(formFileContent))
            {
                ...
                var r = p.CreateString(inex, b, ext.Item1[0], ext.Item2);
                using (var outp = new MemoryStream())
                {
                    using (var outps = new StreamWriter(outp))
                    {
                     ...
                     //   var routeValues = new { Tuple =  new Tuple<byte[], string>(Bytes, FileName) };
                        ...
                        var bt = outp.GetBuffer();
      /*                  using (var z = Zip(bt))
                        {
                            var bb = new byte[z.Length];
                            z.Read(bb);
                            bytesSingleton.Tuple = new Tuple<byte[], string>(bb, FileName);
                            
                        }*/
                        var t = new Tuple<Stream, string>(Zip(bt), FileName);
                        ViewData["Tuple"] = t;
