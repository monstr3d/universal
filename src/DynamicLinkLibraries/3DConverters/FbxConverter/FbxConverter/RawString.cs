...
namespace FbxConverter
{
    ...
    public  class  RawString : IEquatable<RawString>
    {
        ...
        internal RawString(int begin, byte[] data, int byteLen)
        {
            Bytes = new byte[byteLen];
            Array.Copy(data, begin, Bytes, 0, byteLen);
            _byteLength = byteLen;
        }
        ...
        public RawString Slice(int start)
        {
            ...
            return new RawString(start, Bytes, _byteLength - start);
        }
        ...
        public RawString Slice(int start, int length)
        {
            ...
            return new RawString(length, Bytes, _byteLength - start);
                //((IntPtr.Add(_headPointer, start), length);
        }
...
#else
            return Bytes;// new ReadOnlySpan<byte>((void*)_headPointer, _byteLength);
#endif
        }
...
#else
            return new ReadOnlySpan<byte>(Bytes, start, Bytes.Length - start); //new ReadOnlySpan<byte>(((byte*)_headPointer) + start, _byteLength - start);
#endif
        }
...
#else
            return new ReadOnlySpan<byte>(Bytes, start, length);
#endif
        }
        ...
        {
            return xXHash32.ComputeHash(Bytes);
        }
        ...
        {
           // IntPtr ptr = IntPtr.Zer
            throw new ErrorHandler.OwnNotImplemented("Raw String");
      /*      ref

            return  ref ptr ; // return ref Unsafe.AsRef<byte>((void*)_headPointer);*/
        }
        ...
        public static bool operator == (RawString left, string right)
        {
           /* return ReEncodingOperation.Func(right, left, &Compere, &Fallback);
            ...
            var s = BitConverter.ToString(left.Bytes);
            return s == right;
        }
