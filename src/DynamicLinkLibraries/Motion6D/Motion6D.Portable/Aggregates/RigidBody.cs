...
namespace Motion6D.Portable.Aggregates
{
        ...
        INormalizable, IOrientation, IAlias
    {
        ...
        {
            realMatrix.Normalize(state, 6, 4);
            SetConnectionStates();
        }
        ...
        protected virtual void SetProperties(object[] o)
        {
           ...
           realMatrix.Invert(momentOfInertia, invertedMomentOfInertia);
            if (o[1] != null)
            {
        ...
        protected virtual void SetAliasValue(string name, object value)
        {
            ...
            if (i < 9)
            {
                ...
                vp.ToQuaternion(angles, 6, initialState);
            }
            else
            {
                initialState[i + 1] = v;
            ...
            double[][] connections, double[,] momemtOfInertia, double mass, double[] initialState)
        {
            ...
            realMatrix.Invert(momemtOfInertia, invertedMomentOfInertia);
            this.mass = mass;
            invertedMass = 1 / mass;
       ...
       private void SetOrientation()
        {
            ...
            vp.QuaternionToMatrix(quater, orientation, qq);
            realMatrix.Transpose(orientation, transposedOrientation);
        }
        ...
        protected void SetConnectionState(double[] connectionState, int numberOfConnection)
        {
            ...
            realMatrix.Multiply(orientation, v3d, v3d1);
            for (int i = 0; i < 3; i++)
            {
            ...
            vp.QuaternionMultiply(quater, quaterAdd, quaterAddP);
        ...
        private void Transform(double[,] trans, double[] vector, double[] result, int offset)
        {
            ...
            realMatrix.Multiply(trans, v3d, result);
        }
        ...
        private void Transform(double[,] trans, double[] vector, double[] result, int source, int target)
        {
            ...
            realMatrix.Multiply(trans, v3d, v3d1);
        ...
        protected void SetStateByConnection(double[] connectionExternal, int number)
        {
            ...
            vp.QuaternionInvertMultiply(quaterAddP, quaterAdd, quater);
            ...
            realMatrix.Multiply(v3d1, orientation, omega);
        ...
        protected void SetForcesMatrix(int n)
        {
            ...
            vp.QuaternionToMatrix(quater, tm, qq);
            ...
            realMatrix.Multiply(invertedMomentOfInertia, tm, auxiliaryMatrix);
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
            ...
            vp.SO3VectorToSO3Matrix(v3d, auxiliaryMatrix);
            realMatrix.Multiply(auxiliaryMatrix, tm, auxiliaryMatrixAdd);
            realMatrix.Multiply(invertedMomentOfInertia, auxiliaryMatrixAdd, auxiliaryMatrix);
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
        ...
        protected virtual void SetAccelerationMatrix(int n)
        {
            ...
            vp.SO3VectorToSO3Matrix(v3d, auxiliaryMatrix);
            realMatrix.Multiply(auxiliaryMatrix, t, auxiliaryMatrixAdd);
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
        ...
        protected virtual void CalculateInternalAccelerations(int n)
        {
            ...
            realMatrix.Multiply(v3d, orientation, v3d1);
