...
namespace Collada150.Classes.Comlicated
{
    ...
    public class Node : Collada.XmlHolder
    {
        ...
        private Node(XmlElement element, IMeshCreator meshCreator) : base(element)
        {
            ...
            var mesh = Create(geom, mat, name);
            var a = new AbstractMesh(name);
            creator.Meshes[element] = a;
        }
        AbstractMesh Create(InstanceGeomery geom, BindMaterial material, string name)
        {
            if (geom == null)
            {
                return new AbstractMesh(name);
            }
            var g = geom.Geometry;
            var mesh = g.Mesh;
